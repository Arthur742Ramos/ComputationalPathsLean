...
structure FontaineFunctor (K : Type u) (F : PathField K) where
  ...
  /-- Functoriality with morphisms. -/
  morMap : ∀ (V W : CrystallineRepresentation K F),
    (V.toRepr.space → W.toRepr.space) →
    FilteredPhiModule.Morphism (objMap V) (objMap W)

where
  toRepr := PadicRepresentation.mk
