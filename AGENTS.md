...
**The Calculus of Computational Paths** is a Lean 4 formalization of propositional equality via explicit computational paths and rewrite equality. The core idea: `Path a b` wraps Lean's `Eq` with an explicit list of rewrite steps — a trace-carrying equality inside extensional type theory. `RwEq` is the symmetric reflexive-transitive closure generated by the explicit `Path.Step` rules. The current `normalize` function is a sound invariant of `RwEq`, not by itself a complete characterization; a converse requires a separately proved `CompleteStepSystem`.
...
- **`RwEq`**: Equivalence closure generated by primitive `Path.Step` rewrites; equal current normalizations are not, on their own, an `RwEq` certificate
