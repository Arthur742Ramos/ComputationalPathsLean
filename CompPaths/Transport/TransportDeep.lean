...
3. `depPath_over_transport` — Dependent path over transport (CompPaths vs HoTT)
...
/-! ## 3. Dependent path over transport — CompPaths vs HoTT -/

/-- A dependent path over `p : Path a b` in fiber `D`:
    given `x : D a` and `y : D b`, witnesses that `transport p x = y`.

    In HoTT this is a primitive; in CompPaths it's a propositional equation
...
/-- Key HoTT-vs-CompPaths observation: in our framework, `DepPath` is
