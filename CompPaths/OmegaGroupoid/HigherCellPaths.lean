...
def higherCellCoherence (d₁ d₂ : Derivation₂ p q) : Derivation₃ d₁ d₂ :=
  contractibility₃ d₁ d₂

/-- Coherence between two one-step 2-cells with common endpoints. -/
def stepHigherCell (s₁ s₂ : CStep p q) :
...
def whiskerLeftHigherCell {r : CPath a b}
...
def whiskerRightHigherCell {r : CPath a b}
...
theorem level2_requires_rewrite (d : Derivation₂ p q) : CRwEq p q :=
  derivation₂_to_rweq d

/-- Level-3 cells satisfy Batanin-style contractibility for parallel 2-cells. -/
theorem level3_contractible (d₁ d₂ : Derivation₂ p q) : Nonempty (Derivation₃ d₁ d₂) :=
...
def bataninLeinsterWitness (A : Type u) : BataninLeinsterWitness A where
  contract₃ := contractibility₃
  contract₄ := contractibility₄

/-- The witness agrees with the existing weak ω-groupoid packaging at level 3. -/
theorem bataninLeinster_contract₃_eq_weakOmega (A : Type u)
    {a b : A} {p q : CPath a b} (d₁ d₂ : Derivation₂ p q) :
