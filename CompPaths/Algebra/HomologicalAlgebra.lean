...
theorem h2_obstructions_finite_from_squier :
    CompPaths.Rewriting.HasFDT CompPaths.Rewriting.computationalPathMonoid ∧
      ∃ n : Nat, CompPaths.Rewriting.homotopyBasis.length ≤ n := by
  constructor
  · exact CompPaths.Rewriting.computationalPathMonoid_hasFDT
  · exact ⟨CompPaths.Rewriting.homotopyBasisUpperBound,
      CompPaths.Rewriting.homotopyBasis_finite_bound⟩

/-- Concrete bound extracted from the Squier package. -/
theorem h2_obstruction_bound_eval :
    CompPaths.Rewriting.homotopyBasisUpperBound = 66 :=
  CompPaths.Rewriting.homotopyBasis_bound_eval
