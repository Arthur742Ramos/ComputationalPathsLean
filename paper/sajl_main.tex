...
Having established the algebraic foundations and the higher-dimensional structure, we now turn to the homotopy theory built on them: fundamental groups, covering spaces, fibrations, exact sequences, and the computation of $\pi_1$ for standard spaces.
...
\begin{remark}[Formalization status]\label{rem:hurewicz-formalization}
  The results in this section range from fully formalized to partially
  scaffolded in our Lean development. We present them to illustrate the
  scope of the computational-paths framework, but the reader should
  consult the formalization-status table (\cref{tab:formalization-status})
  for the precise verification level of each result.
\end{remark}
