% ============================================================================
% The Calculus of Computational Paths — SAJL Format
% ============================================================================
% Compile: pdflatex sajl_main && pdflatex sajl_main && pdflatex sajl_main
% ============================================================================
\documentclass{sajl}
...
% Tell cleveref about thmcounter-based environments
\crefname{thmcounter}{Theorem}{Theorems}
\Crefname{thmcounter}{Theorem}{Theorems}
...
\Crefname{example}{Example}{Examples}
\crefalias{definition}{thmcounter}
\crefalias{theorem}{thmcounter}
\crefalias{lemma}{thmcounter}
\crefalias{proposition}{thmcounter}
\crefalias{corollary}{thmcounter}
\crefalias{remark}{thmcounter}
\crefalias{example}{thmcounter}
...
% --- Diagrams ---
% \usepackage{tikz-cd}  % not used in this version
...
% ============================================================================
\title{The Calculus of Computational Paths}%
      {The Calculus of Computational Paths: \\
       Rewrite-Trace Equality, Weak $\omega$-Groupoids, \\
       and Higher-Dimensional Structure}

\author{Ramos, de Queiroz, de Oliveira \& de Veras}%
       {Arthur F. Ramos, Ruy J.\,G.\,B.\ de Queiroz, \\
        Anjolina G.\ de Oliveira and Tiago M.\,L.\ de Veras}
...
confluent, terminating rewrite system whose 74~rules\footnote{The count of 74 follows from the distinct constructors in our formalization; the earlier convention of 76 in~\cite{ramos18} counted two context-congruence rules that serve double duty.} are organized into eight
...
the sense of Batanin--Leinster~\cite{batanin98}, with contractibility beginning at
dimension~3---the critical threshold that preserves non-trivial fundamental
groups. The entire development is mechanically verified in a proof assistant.
\end{abstract}

\keywords{computational paths, identity types, weak $\omega$-groupoid,
  rewrite system, type theory, higher-dimensional algebra, formalization}
...
In Martin-L\"of type theory~\cite{mltt84}, the \emph{identity type} $\IdA_A(a,b)$ captures
...
per pair of endpoints. From the perspective of homotopy type theory~\cite{hott13},
...
Following de~Queiroz, de~Oliveira, and Ramos~\cite{dqor16, ramos18, ramos19}, we propose
...
We enumerate all 74 such rules in \cref{ch:rewrite-system}.
...
higher-dimensional algebra~\cite{baez95}.

\paragraph{Homotopy Type Theory.}
The Univalent Foundations program~\cite{hott13} interprets types as spaces,
...
Lumsdaine~\cite{lumsdaine10} and van~den~Berg--Garner~\cite{vdberg11}
...
Bezem, Coquand, and Huber~\cite{bezem14} introduced cubical sets as a
...
variables, computational paths are built from explicit rewrite steps.~\cite{cohen18}
...
cells. Our 74-rule rewrite system on paths (\cref{ch:rewrite-system}) can be
...
\begin{description}[leftmargin=2em,style=nextline]
  ...
  \item[\cref{ch:rewrite-system}: The Rewrite System.]
    We define the single-step rewrite relation $\Step$ with its 74 rules in
...
\begin{definition}[Single-Step Rewrite]\label{def:step-rewrite}
  ...
  the 74 rules organized into eight groups below. We write
...
\begin{proof}
  By induction on the derivation of $\Step(p, q)$. Each of the 74 rules
...
\begin{remark}[RwEq triviality in the UIP setting]\label{rem:rweq-triviality}
  ...
  This may appear to trivialize the rewrite system, but the opposite is
  the case. The value of the $\mathrm{LND}_{\mathrm{EQ}}$-TRS lies not in
  ...
  form. These witnesses are the data that drive the higher-dimensional
  structure: derivation cells (\cref{sec:omega-groupoid}) record
  \emph{which} rewrite steps are applied, and distinct derivations between
  the same pair of paths give rise to non-trivial three-cells, four-cells,
  and beyond. The rich combinatorial content of the rewrite system lives
  in the derivation tower, not in the bare equivalence relation.
\end{remark}
...
\begin{definition}[Rule Precedence]\label{def:rule-precedence}
  The 74 rewrite rules are assigned a numeric rank
...
\begin{proof}
  By case analysis on the 74 rules. Each rule either reduces the symbol
...
\begin{proof}
  ...
  join. The analysis covers all pairs among the 74 rules.
\end{proof}
...
\begin{remark}[Scale and tooling]\label{rem:critical-pair-tooling}
  The scale of the 74-rule system raises maintainability questions. We organize
...
\begin{proof}
  The proof uses a two-level architecture.
  At the first level, the groupoid fragment (associativity, units,
  inverses) is completed with two algebraic cancellation rules
  (\texttt{trans\_cancel\_left/right}) and shown confluent via a
  semantic interpretation into the free group: every expression maps
  to a reduced word, every rewrite step preserves that word, and
  every expression reduces to a canonical form determined by its
  reduced word (\texttt{GroupoidConfluence}).
  %
  At the second level, the full 75-rule system is handled by lifting
  this groupoid confluence through the typeclass \texttt{HasJoinOfRw},
  combined with the strip lemma (\cref{thm:strip-lemma}) and
  termination (\cref{thm:termination}).  No appeal to Newman's lemma
  or proof irrelevance is required: the proof is entirely algebraic.
\end{proof}
...
%% Part II of "The Calculus of Computational Paths"
%% ============================================================================
...
For $n \geq 2$, the group $\pi_n(A, a)$ is abelian.~\cite{eckmann62}
\end{theorem}
...
%%%% Converted from chapters/ch8_fibrations.tex (article (as-is)) %%%%
...
\begin{remark}[Formalization status]
  This section is mixed: path lifting is fully formalized, while the Hopf fibration data, Galois correspondence, and long exact sequence are partially scaffolded, and Mayer--Vietoris is statement-only. See \cref{tab:formalization-status}.
\end{remark}
...
%%%% Converted from chapters/ch9_hurewicz.tex (article (as-is)) %%%%
...
\begin{remark}[Formalization status]\label{rem:hurewicz-formalization}
  This section is largely scaffolded. The dimension-1 Hurewicz map is partially
  formalized, while Nielsen--Schreier and the higher Hurewicz theorem are
  currently statement-only; see \cref{tab:formalization-status}.
\end{remark}

The Hurewicz theorem establishes the fundamental connection between homotopy groups and homology, relating $\pi_1$ to the first homology group $H_1$ via abelianization.~\cite{switzer75} In the computational-paths framework, both sides of this connection are constructively defined: $\pi_1$ via the loop quotient, and $H_1$ as its abelianization.
...
%%%% Converted from chapters/ch10_advanced.tex (article (as-is)) %%%%
...
\begin{remark}[Formalization status]\label{rem:advanced-formalization}
  This section is scaffolded at the statement level: the definitions and results
  are recorded without full proofs in Lean. See \cref{tab:formalization-status}
  for the precise status of each item.
\end{remark}
...
A weak equivalence $f : A \to B$ between CW complexes (or, in the type-theoretic setting, between types with suitable structure) is a homotopy equivalence: there exists $g : B \to A$ with $g \circ f \sim \mathrm{id}_A$ and $f \circ g \sim \mathrm{id}_B$.~\cite{whitehead78}
\end{theorem}
...
\[
  \sigma : \Omega(X, x_0) \to \Omega(\Sigma X, \mathsf{north}), \qquad \sigma(\ell) = \mathsf{merid}(\ell) \cdot \mathsf{merid}(x_0)^{-1},
\]
...
\begin{itemize}
  ...
  \item Each $d_r : E_r^{p,q} \to E_r^{p+r,\, q-r+1}$ is a differential satisfying $d_r \circ d_r = 0$;
...
The Adams spectral sequence is the principal tool for computing stable homotopy groups.~\cite{adams74} Its $E_2$ page is
\[
...
The chromatic filtration of the stable homotopy category---organized by height of formal group laws---is recorded, including Morava K-theories $K(n)$ and the thick subcategory theorem.~\cite{ravenel86}
...
The applications surveyed in Part~II (Sections~\ref{sec:fundamental-groups}--\ref{sec:advanced}) range from fully formalized results (e.g., fundamental groups and path lifting) to statement-only scaffolding (e.g., spectral sequences, K-theory); see \cref{tab:formalization-status}. In each case, the computational-paths framework provides the type-theoretic substrate: definitions are stated in terms of $\mathrm{Path}$, $\mathrm{RwEq}$, and $\pi_n$, and properties are witnessed by computational path constructions. The scaffolded results indicate the framework's extensibility and suggest directions for future complete formalization.
\end{remark}


%%%% Converted from chapters/ch11_metatheory.tex (book→article) %%%%

% ============================================================================
% Chapter 11: The Rewrite System — Metatheory
% Part III of "The Calculus of Computational Paths"
% ============================================================================
...
In Section~\ref{ch:rewrite-system}, we introduced the 74-rule
...
\begin{definition}[Typed Rewriting System]\label{def:typed-trs}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item A \emph{reduction relation} $\rew$ on terms of the same sort,
      given by the 74 rules of Section~\ref{ch:rewrite-system}.
  \end{enumerate}
...
\begin{definition}[The $\mathrm{LND}_{\mathrm{EQ}}$-TRS]\label{def:lndeq}
  ...
  (following the terminology of~\cite{ramos18}). The rules are enumerated
...
$\mathrm{PathExpr}$, with the same 74 rules applied syntactically. The
...
The 74 rules generate a finite (though large) set of critical pairs.
...
\begin{definition}[Path Tactics]\label{def:path-tactics}
  ...
  \begin{itemize}
    ...
    \item \emph{Simplification} ($\mathit{path\_simp}$): applies the simplification
      procedure with the library of $\RwEq$ lemmas (all 74 rules and their
...
\midrule
Step definitions & The 74-rule $\mathrm{Step}$ relation and soundness proof \\
...
The 74 rules of the $\mathrm{LND}_{\mathrm{EQ}}$-TRS
...
and global confluence via a groupoid-algebraic argument
(\cref{thm:confluence}) that interprets path expressions into the
free group and derives canonical forms without appealing to Newman's
lemma or proof irrelevance.
...
\begin{itemize}
  ...
  \item Fibrations, covering spaces, and the long exact sequence
    of homotopy groups (Section~\ref{sec:fibrations}), including the Hopf
    fibration.
  \item The Hurewicz theorem ($H_1 \cong \pi_1^{\mathrm{ab}}$) and
    homological algebra (Section~\ref{sec:hurewicz}).
  \item Advanced structures: Eilenberg--MacLane spaces, Postnikov
    ...
    classes, and operads (Section~\ref{sec:advanced}).
\end{itemize}
...
Path Lifting (Thm.~8.3) & \S\ref{sec:fibrations} & FF \\
Hopf Fibration Data (Thm.~8.6) & \S\ref{sec:fibrations} & PS \\
Galois Correspondence (Thm.~8.11) & \S\ref{sec:fibrations} & PS \\
Long Exact Sequence (Thm.~8.15) & \S\ref{sec:fibrations} & PS \\
Mayer--Vietoris (Thm.~8.18) & \S\ref{sec:fibrations} & SO \\
\midrule
Hurewicz, Dim~1 (Thm.~9.5) & \S\ref{sec:hurewicz} & PS \\
Nielsen--Schreier (Thm.~9.10) & \S\ref{sec:hurewicz} & SO \\
Higher Hurewicz (Thm.~9.12) & \S\ref{sec:hurewicz} & SO \\
Postnikov Tower (Def.~10.4) & \S\ref{sec:advanced} & SO \\
Whitehead (Thm.~10.7) & \S\ref{sec:advanced} & SO \\
Freudenthal Suspension (Thm.~10.9) & \S\ref{sec:advanced} & SO \\
Spectral Seq.\ Convergence (Thm.~10.13) & \S\ref{sec:advanced} & SO \\
K-theory / Char.\ Classes / Operads & \S\ref{sec:advanced} & SO \\
\midrule
Decidability of $\RwEq$ (Thm.~11.8) & \S\ref{ch:metatheory} & FF \\
$J$-Elimination (Thm.~11.11) & \S\ref{ch:metatheory} & FF \\
HoTT Compatibility (Thm.~11.13) & \S\ref{ch:metatheory} & FF \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{Interface-based confluence.} The confluence interface
    \texttt{HasJoinOfRw} is a parameterized typeclass (not globally
    instantiated), allowing different confluence
    strategies (constructive, classical, syntactic) to be plugged in
    transparently. The primary instance is
    \texttt{HasConfluencePropExpr}, proved via groupoid confluence on
...
With all 74 rules and their congruence variants registered as
simplification lemmas, the tactic resolves most $\RwEq$ goals
automatically. For more complex goals, the dedicated tactics
(transitivity, left/right congruence,
associativity) provide fine-grained control.
...
closely with cubical type theory~\cite{bezem14}, while retaining the
...
The current formalization of 217,000 lines covers a substantial portion
...
We list all 74 rewrite rules of the $\mathrm{LND}_{\mathrm{EQ}}$-TRS,
...
\begin{remark}
  The total count of 74 reflects the distinct constructors of the
  $\mathrm{Step}$ relation in the formalization. Context congruence
  (R31) appears in both Group~IV as a context-specific rule and in
  Group~VIII as a structural closure principle; it corresponds to a single
  constructor. The earlier convention of 76 in~\cite{ramos18} listed
  certain rules in multiple groups for expository clarity.
\end{remark}
...
\bibitem{cplean}
A.~F. Ramos.
\newblock \emph{Computational Paths in Lean~4} (GitHub repository).
\newblock \url{https://github.com/Arthur742Ramos/ComputationalPathsLean}.
...
\newblock In {\em 25th EACSL Annual Conference on Computer Science Logic (CSL 2016)},
  Leibniz International Proceedings in Informatics~62, pages 22:1--22:16, 2016.

\bibitem{brunerie16}
G.~Brunerie.
\newblock {\em On the Homotopy Groups of Spheres in Homotopy Type Theory}.
...
\authorname{Tiago Mendon\c{c}a Lucena de Veras}
\address{Department of Mathematics\\
Rural Federal University of Pernambuco (UFRPE)\\
R. Manuel de Medeiros, Dois Irm\~aos, CEP 52171-900, Recife, PE, Brazil}
