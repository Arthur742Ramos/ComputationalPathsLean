...
\item \textbf{Machine verification} in Lean 4: over 130 modules, zero \texttt{sorry} placeholders, no custom axioms beyond the circle HIT.
\end{enumerate}
...
\item Circle HIT axioms (for $\pi_1(S^1) \cong \mathbb{Z}$, if computing fundamental groups)
\end{enumerate}
\end{theorem}
...
\begin{proof}[Proof Outline]
Using the circle HIT with point constructor $\mathsf{base} : S^1$ and loop constructor $\mathsf{loop} : \mathsf{base} = \mathsf{base}$, we define:
\begin{itemize}
...
\end{itemize}
The encode-decode equivalence is established by path induction, with $\mathsf{loop}^n$ corresponding to $n \in \mathbb{Z}$.
\end{proof}
...
\begin{itemize}
\item Free group definition as a HIT quotient
