...
\begin{abstract}
The main result is a collapse.  In a formalized computational-path rewriting
system, packaging a family of pointwise paths into a path between functions
records the \emph{empty} trace; the function-application beta rule therefore
exhibits the trace-free path as a one-step predecessor of an arbitrary path with
the same endpoints.  Rewrite equivalence is total on every fiber, and
\[
 \PRW_A(a,b)\simeq(a=b)
\]
for every carrier.  The rewrite quotient retains no computational-path
information, every invariant on it is constant, and it supports
unrestricted based path induction on every carrier for exactly that reason.  We
state the positive half with its cause attached, because the unqualified form
invites the reading that a proof-relevant path object has been shown to admit
\(J\), which is not what is proved.  A parity invariant shows the collapse is
not intrinsic to path rewriting: the groupoid fragment preserves trace-length
parity and does not collapse, so the degeneracy comes from the function-space
rules rather than from path algebra.

Locating that failure requires a criterion, and the first part of the paper
builds one.  Suppose equality from a fixed \(a:A\) is refined by observable
metadata,
\[
  R_M(a,b)=\sum_{h:a=b}M(b,h),
  \qquad r^M_a=(\refl_a,m_0).
\]
Unrestricted based elimination into arbitrary motives, with a propositional beta
law, holds precisely when \(M(a,\refl_a)\) is contractible: equality induction
collapses the endpoint and equality coordinates but cannot erase the observable
one.  Every ordinary setoid repair is then classified exactly---for a carrier
\(X\), \(\isContr(X/S)\Leftrightarrow(\mathsf{Inh}(X)\wedge\SetTot(S))\)---so a
set-level quotient restores \(J\) only by identifying \emph{all} reflexivity
metadata, and no visible distinction survives there.  Replacing the
sigma-shaped presentation by a reflexive projection \(q_b:R(b)\to(a=b)\) makes
the criterion contractibility of the kernel
\(\Ker(q)=\{r:R(a)\mid q_a(r)=\refl_a\}\); for \(\PRW\) that kernel is the loop
quotient, and pushing the condition back to representatives removes the quotient
altogether, leaving a statement about raw loops that a rule set can be checked
against by inspection.  These stages are individually elementary, and we present
them as instruments rather than as results.

The trade-off they expose is structural, not accidental.  A setoid is total
exactly when every invariant of it is constant, so on an inhabited
carrier---which a pointed fiber always is---contractibility and retained
information are strictly opposed, and any rule making one path a predecessor of
a whole fiber destroys everything in it.

We close by auditing the computational-path examples.  The reflexivity fiber of
the raw trace record is computed exactly---it is \(\List(A)\)---so failure there
is unconditional on every pointed carrier, in sharp contrast with the quotient.
The current ``circle'' is a one-constructor inductive type and the current
...
\(\mathbb Z\) and \(\mathbb Z^2\), and the collapse theorem upgrades the
resulting no-bridge statement from those two carriers to all of them.  A Lean
development verifies every result using only propositional extensionality and
quotient soundness.  The scope throughout is proof-irrelevant ambient equality,
not full Martin-L\"of or homotopy type theory; the pointed-family equivalence
underlying the criterion is the proof-irrelevant, propositional-beta
specialization of the identity-system characterization of
\cite[Thm.~5.8.2]{HoTTBook2013} and is not claimed as a contribution.
\end{abstract}
...
\]
(Theorem~\ref{thm:rweq-total}, Corollary~\ref{cor:unconditional-repair}).  The
rewrite quotient is therefore ambient equality: it retains no
...
reason.  We keep the cause attached to the positive half throughout---the
...
The centrepiece is Part~\ref{part:case-study}, and~\eqref{eq:collapse-intro} is
its principal result.  Around it, Section~\ref{sec:design} shows the trade-off
is structural rather than accidental: a setoid is total exactly when every
invariant of it is constant, so for an inhabited carrier---which a pointed fiber
always is---elimination and retained information are strictly opposed, and any
rule making one path a predecessor of a whole fiber destroys everything there
(Theorem~\ref{thm:design}).  Section~\ref{subsec:sharpness} localises the
present failure by a parity invariant: the groupoid fragment does not collapse,
so the degeneracy comes from the function-space rules and not from path algebra
(Theorem~\ref{thm:sharpness}).  The audit of the circle and torus, and the
universal no-bridge theorem (Theorem~\ref{thm:universal-no-bridge}), follow.

Part~\ref{part:general} supplies the instruments these rest on: the
...
quotient; and a factorization theorem for equality-projection motives.  Each is
an elementary consequence of equality induction and quotient soundness, and we
present them as such.  Their value is not individual: it is that the chain they
form converts an unquantifiable question about all motives into a condition on
raw representatives, which is what makes the case study decidable by
inspection.
...
set/type-theoretic metatheory, matching the formal artifact, and classify
\emph{propositional} beta only.  We present no syntax or model of
Martin-L\"of type theory, assume and prove no univalence or higher
...
Section~\ref{sec:formalization} describes the division.  These boundaries are
stated once here and once, in full, in Section~\ref{subsec:not-said}.

\paragraph{Organization.}
The paper is in two parts.  Part~\ref{part:general} builds the instruments and
...
\part{Instruments: equality with observable metadata}
\label{part:general}

Part~\ref{part:general} builds the instruments used in
Part~\ref{part:case-study}.  Nothing in it mentions a rewrite system, and
nothing in it is difficult; each result is an elementary consequence of
equality induction, quotient soundness, and quotient exactness.  Their interest
is collective.  Taken together they turn the question ``does this decorated
equality admit unrestricted based elimination?''---which as posed quantifies
over all motives and is not something one can inspect---into a question about a
single fiber, then into a question about a quotient, and finally into a
condition on raw representatives that can be settled by reading a rule set.
That last reduction is what makes the case study decidable at all.

The chain has four links:

\begin{itemize}[leftmargin=1.35em]
\item Section~\ref{sec:total-space} records the identity-system equivalence
  between based elimination and total-space contractibility.  It is standard,
  and it is the only stage we do not claim.
\item Section~\ref{sec:metadata} computes the decorated total space, reducing
  contractibility to contractibility of the single reflexivity metadata fiber
  (Theorem~\ref{thm:headline}).  Section~\ref{sec:traces} applies it to the raw
  trace record.
\item Section~\ref{sec:setoid-repair} classifies which ordinary setoid
  quotients repair a failure, exactly and with the empty case handled.  This is
  what makes ``quotient the metadata'' a decidable strategy rather than a hope.
\item Section~\ref{sec:projection-kernel} replaces the sigma-shaped
  presentation by a projection kernel, and
  Section~\ref{sec:pathrwquot-criterion} then removes the quotient from the
  criterion entirely.  This is the link that reaches raw representatives, and
  hence the rule set.
\end{itemize}

Section~\ref{sec:factorization} records what survives a failure, and
Section~\ref{sec:examples} calibrates the criterion against other decorations.
...
if some genuine quotient fails it, so we exhibit one.  The informal instruction
``quotient the administrative metadata'' can easily
...
\section{Motives that factor through the equality projection}
\label{sec:factorization}

Failure of unrestricted elimination does not mean that no useful elimination
principle remains.  The projection~\eqref{eq:projection} supports a large class
of motives uniformly, and this section identifies it.
...
\begin{remark}[What Theorem~\ref{thm:motive-factorization} does and does not
classify]
\label{rem:factorization-scope}
The theorem exhibits a class of motives that is uniformly supported: every
motive equipped with a factorization through equality, strictly or up to
fiberwise equivalence, is eliminable by one construction from one base datum,
and the construction is the same for all of them.  It is not a maximality
theorem.  We do not claim that a motive outside this class admits no eliminator;
a particular metadata-sensitive motive may well have a section for reasons
special to it.  What Theorem~\ref{thm:metadata-fiber} rules out, when the
reflexivity fiber is noncontractible, is a single uniform operator covering
\emph{every} motive from one base datum---not the individual eliminability of
each.
\end{remark}
...
Part~\ref{part:case-study} applies the instruments of Part~\ref{part:general} to
one concrete implementation, and finds that it collapses.  The general theorems
are not tied to that implementation; the strongest concrete result is.
...
relation and met in the extreme form: by~(i) the quotient retains the ambient
equality and nothing else, so \(\PRW\) is equivalent to the indiscrete repair of
Section~\ref{subsec:indiscrete}.  A user of \(\PRW\) may therefore reason by
path induction on every carrier, with no hypothesis and no additional axiom---and
for the same reason no invariant defined on \(\PRW\) can be nonconstant, so any
construction whose interest depends on separating two of its classes is, under
the present rule set, vacuous.  The two readings are inseparable, which is the
point.  Section~\ref{sec:design} shows this is not a coincidence of these rules
but a theorem about ordinary quotients, and the dichotomy with the raw record is
sharp.
...
naming and interpretation in the artifact, its documentation, and the earlier
preprint \cite{ComputationalPathsArxiv2025}; no previously proved theorem is
retracted.
...
\item \emph{The \(\pi_1\) aliases.}  Two names in the development, and
  ...
  \(\PRW\) loop quotient''.
\item \emph{The ``drop-in replacement'' description.}  The abstract of
  \cite{ComputationalPathsArxiv2025} describes the trace-carrying record as a
  drop-in replacement for propositional equality, and presents the circle and
  torus results without the genuine/synthetic distinction of
  Section~\ref{sec:genuine-synthetic}.  Both descriptions are withdrawn.  A
  record whose reflexivity fiber is \(\List(A)\) is not interchangeable with
  ambient equality, because it does not admit the eliminator
  (Proposition~\ref{prop:trace-fiber-computed}); and its rewrite quotient,
  which does admit the eliminator, is interchangeable with ambient equality
  only in the degenerate sense established by Theorem~\ref{thm:rweq-total}.
  The repository documentation now states the distinction, and a corrected
  version of the preprint is being posted alongside the present article.
...
\label{subsec:not-said}

This is the consolidated list of non-claims; the earlier sections state their
scope once and do not restate it.
...
\item They do not refute uniqueness of identity proofs for ambient equality.
...
\item Theorem~\ref{thm:rweq-total} is a theorem about the primitive rule set
  ...
  degenerate.
\item The no-bridge theorems concern invertible bridges (see the discussion
  after Theorem~\ref{thm:no-bridges}).
\end{enumerate}
...
\[
                 R_M(a,b)=\sum_{h:a=b}M(b,h),
\]
unrestricted based elimination with propositional beta is controlled by a single
object, the reflexivity fiber \(M(a,\refl_a)\); the chain~\eqref{eq:new-chain}
separates the stage of that control which is standard identity-system theory
from the stage which is specific to the decoration.  Ordinary setoid repairs are
then classified exactly: a set-level quotient restores the eliminator precisely
when it identifies \emph{all} reflexivity metadata.  The indiscrete relation is
the canonical successful repair, and trace-length parity shows that identifying
a great deal is not the same as identifying enough.  Recasting the criterion
through a projection kernel removes the sigma presentation, and its raw-level
form removes the quotient: an eliminator for the rewrite quotient is exactly as
strong as the assertion that every two raw loops at the base point admit a
rewrite derivation.

That assertion is then decided, and the answer is a collapse.  Because the
implementation packages pointwise paths into a path between functions while
recording the empty trace, the primitive application rule makes the trace-free
path a one-step predecessor of every path with the same endpoints.  Hence
\(\mathsf{RwEq}\) is total and \(\PRW_A(a,b)\Eqv(a=b)\).  The implemented
rewrite quotient is ambient equality: it retains no computational-path
information and admits only constant invariants.  It does support unrestricted
based path induction on every carrier, and the reason should travel with the
fact.  What has path induction here is not a proof-relevant path object that
preserves computational reasons; it is a quotient that has stopped
distinguishing them.  The raw record fails in the opposite and equally
unconditional way, its reflexivity fiber being \(\List(A)\) on every pointed
carrier---and there, at least, the failure is bounded: equality-projection
motives remain eliminable, so the obstruction is a precise boundary rather than
a blanket impossibility.

Neither outcome is accidental.  A setoid is total exactly when every invariant
of it is constant, so on an inhabited carrier elimination and retained
information are strictly opposed for ordinary quotients, and any rule set
admitting a universal predecessor at a fiber retains nothing there.  Parity
localises the present failure to the function-space rules together with the
trace-erasing packaging convention, which leaves a redesign two finite
obligations: exhibit a separating invariant, and verify that no path is a
predecessor of a whole fiber.

The circle and torus audit is the concrete consequence, and it separates
constructions that legacy names had obscured.  The genuine loop quotients of the
one-constructor circle and its product torus are contractible; the synthetic
winding quotients are equivalent to \(\mathbb Z\) and \(\mathbb Z^2\) and are
noncontractible; no equivalence bridges them.  The collapse theorem upgrades
this from a fact about two carriers to a fact about all of them, so no change of
point space can restore the intended reading while the present rules stand.
Histories, costs, certificates, normalization logs, and labels are governed by
the same reflexivity-fiber test.  Anything stronger---a judgmental beta
classification, full MLTT or HoTT, univalence, a new higher-inductive circle, or
a higher-quotient universality theorem---requires theory beyond what is proved
here.
