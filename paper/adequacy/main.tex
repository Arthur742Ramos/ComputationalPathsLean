...
\item It does not refute ordinary equality elimination.  The projection
  \(\proj\) still supports Theorem~\ref{thm:motive-factorization}, so ordinary
  path induction through ambient equality remains available.
\item It does not refute uniqueness of identity proofs for ambient equality.
  In the proof-irrelevant metatheory used here, UIP already holds for that
  equality.  The distinct values are metadata records, not distinct ambient
  equality proofs.
...
\item \path{based_identity_total_space_contractible}, the standard
  contractibility theorem for \(\sum_{b:A}(a=b)\);
...
\end{itemize}
The first, second, and fourth declarations in this list are the stable names
for the three abstract results identified in the earlier combined draft: the
based identity total-space theorem, the
unrestricted-eliminator/contractibility equivalence with explicit
propositional beta, and the general metadata-fiber criterion.
