...
Trace-length parity shows that the criterion is not about how much a quotient
identifies: it collapses infinitely many traces and still repairs nothing.
...
\(J\) nor that it always fails.  Pushing the criterion back to representatives
removes the quotient entirely: it holds exactly when \(\mathsf{RwEq}\) relates
every pair of raw loops at the base point, so an eliminator is precisely as
strong as a rewrite certificate between any two loops.

Finally we audit the computational-path examples.  The reflexivity fiber of the
trace record is computed exactly---it is \(\List(A)\)---so failure there is
unconditional on every pointed carrier.  Empty and singleton reflexive raw paths
are unequal records but are explicitly
...
the quotient exactness, factorization, kernel, raw-level, computed-fiber,
trace, contractibility, and no-bridge results, using only propositional
extensionality and quotient soundness.  The scope remains proof-irrelevant
ambient equality with propositional beta, not full Martin-L\"of or homotopy
type theory.
\end{abstract}
...
\item a raw-level form of that criterion: \(\PRW\) elimination holds exactly
  when \(\mathsf{RwEq}\) connects every pair of raw loops, which exhibits the
  rewrite quotient as an instance of the setoid classification and makes the
  condition checkable without forming any quotient;
...
\item a worked nontrivial quotient---trace-length parity---that collapses
  infinitely many traces and still repairs nothing, showing that the necessity
  direction of the repair criterion has content;
...
equivalent to \(\prod_{a:A}\RwTot(A,a)\).  Whether some carrier violates
\(\RwTot(A,a)\) is a question about the specific primitive rule set of
LND\(_{\mathrm{EQ}}\)-TRS, not about the classification, and we do not settle it
here.  What Theorem~\ref{thm:raw-criterion} contributes is that the question is
now completely elementary: it asks only whether two given step lists are
connected by primitive rewrites.
\end{remark}
...
Proposition~\ref{prop:trace-fiber-computed}).  For \(\PRW\) the answer is
conditional, and the condition is~\eqref{eq:rweq-total}.  Quotienting therefore
changes the problem from ``always fails'' to ``fails exactly when some raw loop
is not rewrite-connected to reflexivity''.
...
\item The \(\PRW\) theorem is conditional on local K; it does not label
  `RwEq` a universal success or a universal failure.  Equivalently, by
  Theorem~\ref{thm:raw-criterion}, it does not decide whether every raw loop of
  every carrier is rewrite-connected to reflexivity.  That is a question about
  the primitive rule set, and we leave it open; the classification is valid
  either way.
...
\item the computed trace fiber \path{stepEquivPoint},
  \path{traceEquivPointList}, \path{loopPathEquivPointList}, and
...
Empty and singleton reflexive paths show why this precision matters: they are
distinct raw records but explicitly `RwEq`-equivalent and hence equal in the
quotient.  The raw record itself is beyond repair without quotienting, since its
reflexivity fiber is exactly \(\List(A)\) for every pointed carrier.
