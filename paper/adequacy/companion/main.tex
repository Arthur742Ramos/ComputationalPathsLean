...
\begin{abstract}
This paper is a machine-checked audit, not a proposal for a new type theory.
Its subject is the raw
...
it fails to prove, and what its traces actually record.  The failures are
reported as theorems rather than as gaps, which is what distinguishes an audit
from an incomplete metatheory.
...
The framing matters because raw calculi invite over-reading.  A syntax with
\(\Pi\)-types, universes, an identity type, and a \(J\)-constructor suggests
that the usual metatheory is present in the background.  Here it is not, and the
paper's task is to say exactly where the resemblance stops.
...
Theorem~\ref{thm:intro-image}, replace that claim with a description of what the
target retains, what it cannot express, and which of its programs are actually
reached.  The
...
The two-binder constructors are the only cases where the argument is not
mechanical, and they are mechanical for a different reason: their recursive
equations must apply \(\lift\) the correct number of times in each position.
For \(\NatElim_\ell(M,z,s,k)\) the motive takes one lift, the successor branch
two, and the zero branch and scrutinee none; for \(\EqJ(M,r,b,p)\) the motive
takes two and the remaining three positions none.  Given that bookkeeping, both
cases close by lift composition exactly as the \(\PiTy\) case does, and the
identity proof is the same induction with
\((\idren^{\Ren})^\lift=\idren^{\Ren}\).  Appendix~\ref{app:grammar} displays
the clauses, and the artifact checks every case.
\end{proof}
...
The two-binder constructors again need explicit bookkeeping.  The four
induction hypotheses for \(\NatElim_\ell\) use, respectively,
\(\sigma^\lift\) at the motive, \(\sigma\) at the zero branch,
\(\sigma^{\lift\lift}\) at the successor branch, and \(\sigma\) at the
scrutinee; one compares those four positions in that order, changing exactly
one argument at each step by the corresponding one-position congruence
constructor, and transitivity chains the four steps.  Raw \(\EqJ\) is the same
pattern, with \(\sigma^{\lift\lift}\) at the motive and \(\sigma\) at its
remaining three arguments.  Thus every constructor, including the two-binder
cases, is covered by the explicit compatible closure of \(\defeq\).
\end{proof}
