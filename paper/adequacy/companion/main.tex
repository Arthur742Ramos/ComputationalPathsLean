...
\hypersetup{
  hidelinks,
  pdftitle={A Scoped Calculus of Equality Traces: Structural Metatheory, Reduction, and Derivation Erasure},
...
\title[A Scoped Calculus of Equality Traces]
...
laws, and typed substitution for the raw typing judgment.

We then develop the calculus' reduction theory.  Ten top-level primitive
redexes preserve typing modulo source definitional equality between the
displayed type expressions.  Beyond these we define the full congruence
closure of the primitive rules and its reflexive-transitive closure, and prove
both sound for definitional equality and stable under simultaneous
substitution.  Contextual subject reduction is not available in the raw
system, and we identify exactly why: rebuilding an elimination around a
reduced subterm requires retyping at a definitionally equal type.  Isolating
that requirement as an explicit hypothesis---conversion together with context
conversion, assumed as a record and never as an axiom---we derive contextual
and multi-step subject reduction for the entire congruence closure.  The
result measures the precise distance between the raw calculus and a
conversion-closed one.
...
That the traces are unlabelled is proved rather than asserted.  We introduce a
label-free program syntax over the quotient, whose atoms carry only
proof-irrelevant equalities of classes and which therefore cannot mention a
source derivation, and show that evaluation \emph{factors} through it.
Erasure loses nothing---the factorization is an equality of paths---and
invents nothing, since quotient exactness recovers a source derivation from
any equality of classes and the round trip is the identity.  Erasure preserves
program size, so the arrangement of equality steps survives; only labels are
discarded.  This separates the present target sharply from label-sensitive
presentations of computational paths.

Finally, trace records themselves admit no unrestricted based eliminator.  For
\(R_M(a,b)=\sum_{h:a=b}M(b,h)\), such an eliminator with propositional beta
exists exactly when the reflexivity metadata fiber \(M(a,\mathsf{refl})\) is
contractible; the quotient trace fiber is computed to be a list of quotient
classes, which is never contractible.  Motives factoring through the equality
projection remain available.  The carriers and motive values lie
...
lie in a larger \(\mathcal V\).  No judgmental beta is claimed.  The results do
not supply a model of Martin-L\"of or homotopy type theory.  A Lean~4
development machine-checks the structural laws, the reduction and subject
reduction theorems, quotient substitution, erasure, rewrite soundness, and the
trace obstruction.
\end{abstract}
...
This paper is a structural study of a deliberately raw calculus of equality
traces, not an interpretation theorem.  Fix \(n\in\mathbb N\).  The
...
Two questions organize the development.  The first is how far the raw calculus
gets on its own, and where exactly it stops.  The second is what its traces
actually record.

\subsection*{Reduction, and the exact cost of the missing conversion rule}
...
Contextual subject reduction, however, genuinely fails in the raw system, and
the reason is identifiable rather than incidental.  Rebuilding an elimination
around a reduced subterm demands that subterm at a definitionally equal type,
and the raw typing family has no rule that retypes a term along \(\defeq\).
Reducing inside a domain annotation additionally changes the context under a
binder.  These are precisely the conversion and context-conversion rules the
calculus omits.  Rather than record this as a limitation, we isolate it.

\begin{theorem}[Contextual and multi-step subject reduction, conditional form]
\label{thm:intro-subject-reduction}
Assume the two omitted structural rules as an explicit hypothesis: a
conversion rule retyping along \(\defeq\), and a context-conversion rule
retyping along pointwise \(\defeq\).  Then for every \(t\ctxred u\) and every
\(\Gamma\vdash t:A\) one has \(\Gamma\vdash u:A\), at the same type, for the
full congruence closure; and the same holds for \(t\ctxredstar u\).
\end{theorem}

The hypothesis is a record of assumed rules, never an axiom, so every theorem
depending on it carries that dependence in its statement.  Theorem
\ref{thm:intro-subject-reduction} therefore measures the exact distance
between this calculus and a conversion-closed one: the two rules are not only
sufficient, they are all that is missing.
...
label-free syntax is sufficient: no target trace can depend on which
derivation an atom supplied.  Size preservation says erasure is not a
collapse---the arrangement of equality steps survives, and only labels are
discarded.  The round trip says erasure invents nothing, because any equality
of classes is supported by some source derivation.  Together they identify the
target exactly.
...
\item A pointwise typed simultaneous substitution preserves the raw typing
  ...
  source and target type expressions.
\item The congruence closure \(\ctxred\) of the primitive rules and its
  reflexive-transitive closure \(\ctxredstar\) are sound for \(\defeq\) and
  stable under simultaneous substitution.  Unconditional contextual subject
  reduction is unavailable, and Theorem~\ref{thm:intro-subject-reduction}
  identifies the two omitted structural rules that are exactly sufficient to
  restore it.
...
\item An independent source family
  ...
  \[
      ...
      \ev(p)\TrEq\ev(q).
  \]
\item Evaluation of source identity programs factors through a label-free
  quotient syntax, which preserves program size and is inverse at atoms to
  the recovery map given by quotient exactness
  (Theorem~\ref{thm:intro-erasure}).
...
The contribution is the combination of sharply bounded pieces:
binder-correct raw scoped syntax; source-derived preservation for ten
primitive redexes; a contextual and multi-step reduction theory together with
an exact identification of the two structural rules whose absence blocks
subject reduction; representative-independent and compositional quotient
substitution; source/target-separated identity programs; a proof that the
resulting traces are unlabelled; and the trace-record elimination obstruction.

Of these, the reduction analysis and the erasure factorization are the
substantive additions.  The former converts a limitation into a measurement:
Theorem~\ref{thm:intro-subject-reduction} shows that conversion and context
conversion are not merely sufficient to restore contextual subject reduction
but are all that is missing, which is a sharper statement than either
asserting the gap or silently adding the rules.  The latter converts a naming
convention into mathematics: calling a trace ``unlabelled'' is otherwise a
claim about intent, whereas the factorization, size-preservation, and atom
round-trip clauses of Theorem~\ref{thm:intro-erasure} pin down what the target
retains and what it cannot express.
...
contribution in Section~\ref{sec:j-boundary} is the reduction to the
reflexivity metadata fiber and its application to trace records.  Source
identity rewrite soundness is a homomorphic simulation, largely forced by the
structural-recursion interpretation once the target operations and their laws
are supplied.  Endpoint exactness is tautological after quotienting.  The
paper claims no absolute priority; its point is the combined interface, the
reduction measurement, and the erasure theorem.
...
model or initial CwF is supplied.  Contextual and multi-step subject reduction
are proved \emph{conditionally}, on the explicitly stated conversion
hypothesis of Theorem~\ref{thm:intro-subject-reduction}; unconditionally we
prove preservation for the ten primitive redexes only, and we do not claim the
raw system enjoys subject reduction.  Source identity
...
\(t\defeq u\).  There is no normalization, confluence, canonicity,
...
These are the only two obstructions.  Rather than leave them as a limitation,
we name them.
...
\frac{\Gamma\vdash t:A\qquad \Gamma\defeq\Delta}
     {\Delta\vdash t:A}
\tag{6.1}\label{eq:conversion-rules}
\]
...
visible in the statement itself.  Two congruence lemmas are needed to apply
it.
...
\begin{remark}[What the conditional form buys]
\label{rem:conditional-value}
Theorem~\ref{thm:contextual-subject-reduction} is not a claim that the raw
calculus enjoys subject reduction; it does not.  Its content is that the
failure is exactly two rules deep.  Every case of the induction goes through
unchanged in a conversion-closed system, and every case that needs repair
needs precisely one of the two rules in \eqref{eq:conversion-rules}.  A reader
extending this calculus therefore knows what to add and what it will cost,
which is more informative than either omitting the theorem or adding the rules
silently at the outset.
\end{remark}
...
\tag{7.3}\label{eq:subst-congruence}
\]
...
\tag{7.4}\label{eq:lift-env-congruence}
\]
...
\end{aligned}
\tag{7.5}\label{eq:quotient-compositionality}
\]
...
\[
                       ...
                       \Q(\sigma)(\den t)=\den{t[\sigma]}.
\tag{7.6}\label{eq:environment-interpretation}
\]
Then
\[
          \den{t[\sigma]}_\tau
            =\den t_{\sigma;_{\Sub}\tau}.
\tag{7.7}\label{eq:interpretation-composition}
\]
...
\subsection{Erasure loses nothing}
...
\subsection{Erasure is not a collapse, and invents nothing}
...
and that erasing that derivation returns the atom one started from.  Erasure
therefore does not over-approximate: the label-free syntax expresses exactly
the atomic equalities the source calculus can justify, no more.
...
\[
 ...
 \Snd(\Pair(a,b)):
 B\{\Fst(\Pair(a,b))\}.
\tag{10.7}\label{eq:snd-source-type}
\]
The primitive redex is
\[
                    \Snd(\Pair(a,b))\red b.
\tag{10.8}\label{eq:snd-redex}
\]
...
\ \Longrightarrow\
B\{a\}\defeq B\{\Fst(\Pair(a,b))\}.
\tag{10.9}\label{eq:snd-type-coherence}
\]
...
\NatElim_\ell(M,z,s,\Succ_\ell(k))
  \red s\{k,R_k\}.
\tag{10.10}\label{eq:nat-succ-example}
\]
...
\item The ten top-level primitive redex constructors satisfy
  Theorem~\ref{thm:redex-preservation} unconditionally.  Contextual and
  multi-step subject reduction hold under the conversion hypothesis of
  Definition~\ref{def:conversion-rules}
  (Theorem~\ref{thm:contextual-subject-reduction},
  Corollary~\ref{cor:multistep-subject-reduction}), and not otherwise.
...
\[
  \operatorname{Nonempty}
    \bigl(\Tr_{\Q_n}(\den t,\den u)\bigr)
       \quad\Longleftrightarrow\quad t\defeq u.
\tag{11.1}\label{eq:exact-endpoint}
\]
\end{proposition}
...
\tag{12.1}\label{eq:based-total-space}
\]
Write
\[
...
\tag{12.2}\label{eq:contractibility}
\]
...
\[
 ...
 \quad\Longleftrightarrow\quad
 \mathsf{isContr}(B_a).
\tag{12.5}\label{eq:j-total-equivalence}
\]
\end{theorem}
...
\tag{12.6}\label{eq:contraction-motive}
\]
...
\tag{12.7}\label{eq:contraction-path}
\]
...
\tag{12.8}\label{eq:contraction-center-beta}
\]
...
\tag{12.9}\label{eq:j-from-contraction}
\]
...
\[
 ...
 =d.
\tag{12.10}\label{eq:contraction-derived-beta}
\]
...
\tag{12.11}\label{eq:metadata-refinement}
\]
and
\[
 B_M\coloneqq
 \sum_{b:A}\sum_{h:a=b}M(b,h).
\tag{12.12}\label{eq:metadata-total-space}
\]
...
\tag{12.13}\label{eq:metadata-equivalence}
\]
...
\[
 ...
 \end{aligned}
\tag{12.14}\label{eq:metadata-equivalence-maps}
\]
...
\tag{12.15}\label{eq:metadata-fiber-criterion}
\]
...
\tag{12.16}\label{eq:equality-projection}
\]
...
\[
 J_q(D,d,b,p):D(b,q(p))
\tag{12.17}\label{eq:general-equality-component-j}
\]
...
\[
             ...
             \qquad \pi(L,e)=e.
\tag{12.18}\label{eq:forget-trace}
\]
...
\[
             J_{=}(C,c,P)=J_{=}(C,c,Q).
\tag{12.21}\label{eq:j-trace-irrelevance}
\]
...
a[\sigma](\delta)&=a(\sigma\delta).
\end{aligned}
\tag{13.1}\label{eq:family-substitution}
\]
...
preservation for ten top-level redexes unconditionally, and contextual and
multi-step subject reduction only under the conversion hypothesis of
Definition~\ref{def:conversion-rules}.  It proves no normalization,
confluence, standardization, canonicity, or decidable type checking.
...
surjective.  Proposition~\ref{prop:erasure-roundtrip} establishes the exact
correspondence at atoms, which is what the unlabelledness claim requires, and
...
\cite{RamosEtAl2026}.  Those are label-sensitive developments.  This article
...
primitive-redex preservation, contextual and multi-step reduction with the
conditional subject-reduction theorems, quotient substitution, erasure and its
factorization, source identity rewrite soundness, endpoint exactness, the
...
The correspondence between the theorems above and the development is as
...
Two points about the formal development are worth stating precisely.  First,
the conversion hypothesis of Definition~\ref{def:conversion-rules} is
formalized as a record of assumed rules that every dependent theorem takes as
an explicit argument.  It is not an axiom: the development declares none, and
the dependence of Theorem~\ref{thm:contextual-subject-reduction} on the two
omitted rules is visible in its statement rather than hidden in the ambient
theory.  Second, the machine-checked theorems rely only on propositional
extensionality and quotient soundness.  In particular no form of choice is
used, so the results are constructive relative to those two principles.
...
substitution, preservation for ten top-level primitive redexes, a contextual
and multi-step reduction theory, compositional substitution on a syntactic
quotient, and sound evaluation of source identity rewrites into an explicitly
presented unlabelled trace relation.  None of these statements requires
calling the quotient a model of MLTT.

Two of the results are measurements rather than constructions, and that is
their point.

The first concerns reduction.  Unconditional contextual subject reduction is
unavailable here, and Section~\ref{sec:reduction} does not paper over the
fact.  It instead shows that the failure is exactly two rules deep: assuming
conversion and context conversion---as an explicit hypothesis record, never as
an axiom---the entire congruence closure preserves typing at the same type,
and so does its reflexive-transitive closure.  Every case of the induction
that needs repair needs precisely one of those two rules.  A reader extending
the calculus therefore learns what to add and what it buys, which is more
informative than either omitting the theorem or adopting the rules silently at
the outset.
...
atoms erasure is inverse to the recovery map supplied by quotient exactness, so
nothing is invented either.  Together these pin down what the target retains
and what it cannot express, and they make the contrast with label-sensitive
computational paths precise: an analogue of
Corollary~\ref{cor:no-derivation-observation} fails there by design, and that
failure is the source of that theory's content.

Endpoint exactness, by contrast, is immediate from quotient soundness and
exactness; its role is bookkeeping, not semantic completeness.

Raw \(\EqJ\)-beta and the external equality-component eliminator remain
separate results.  For \(R_M(a,b)=\sum_{h:a=b}M(b,h)\), an unrestricted based
eliminator satisfying propositional beta exists exactly when the reflexivity
metadata fiber \(M(a,\mathsf{refl})\) is contractible.  The quotient trace
fiber is a list of quotient classes and is therefore never contractible, while
equality-component motives remain available through projection.

Several directions remain open.  We prove no confluence, normalization, or
standardization theorem for the contextual reduction relation, and the
syntactic quotient is built from \(\defeq\) directly rather than from a
normal-form analysis.  A genuine raw-to-CwF interpretation is not attempted.
Each such extension must state which equality layer and which elimination
principle it uses.
