...
Contextual subject reduction then \emph{fails}, and we prove that it does.  We
exhibit a reduction inside a well-typed term whose reduct admits no derivation
at the same type, so the failure is a theorem rather than a stuck induction.  We
also prove that neither of the two rules usually invoked to repair it---the
conversion rule and its context-wise companion---is admissible for the raw
judgment, and hence that the natural hypothesis record packaging them is
\emph{uninhabited}.  A conditional subject-reduction theorem over that record is
therefore vacuously true, and we say so: what it establishes is an analysis of
the induction, namely that repairs of exactly those two shapes suffice to close
every case, not a hypothesis a reader could discharge.  The positive counterpart
is the conversion-closed extension, in which both rules hold by construction; we
state precisely what unconditional subject reduction for it would additionally
require, namely generation lemmas and hence confluence, which we do not prove.
...
source derivation, and show that evaluation \emph{factors} through it.  Three
statements are proved, and we are careful to claim no more: evaluation is
insensitive to source labels; program size and constructor shape are preserved,
so the arrangement of equality steps survives; and every quotient equality
between represented classes has a source witness, which erases back to the same
atom.  Erasure is \emph{not} surjective onto the label-free grammar, whose
congruence constructor accepts arbitrary endofunctions of the term model, so we
characterize the essential image exactly: it is the fragment whose atoms are
quotient soundness applied to a source definitional equality and whose
congruences are induced by source frames.  This separates the present target
sharply from label-sensitive presentations of computational paths.

Finally, we record what the traces are and are not.  A trace is a list of
equality steps over a proof-irrelevant quotient together with an endpoint
equality; its entries need not be adjacent, composable, or related to the outer
endpoints.  These objects are observable list metadata decorating an equality,
not carriers of reasons for it---indeed the erasure theorem says precisely that
the reasons have been discarded.  Applying the metadata-fiber criterion of the
companion article to our own records makes this quantitative: the quotient trace
fiber is computed to be a list of quotient classes, which is never contractible,
so trace records admit no unrestricted based eliminator, while motives factoring
through the equality projection remain available.  No judgmental beta is
claimed, and the results do not supply a model of Martin-L\"of or homotopy type
theory.  A Lean~4 development machine-checks the structural laws, the reduction
theory, the failure and necessity theorems, quotient substitution, erasure and
its essential image, rewrite soundness, and the trace obstruction.
\end{abstract}
...
Theorem~\ref{thm:intro-not-admissible} has a consequence we state plainly
because it is easy to state misleadingly.  Assuming the two rules as a
hypothesis record and deriving contextual and multi-step subject reduction from
it---which we do, in Section~\ref{sec:reduction}---yields a theorem whose
hypothesis is empty, hence a vacuously true one.
...
We therefore do not claim that the two rules are ``all that is missing'' between
this calculus and a conversion-closed one; that phrasing would confuse a fact
...
obstruction arising in the structural subject-reduction proof}, and that they
are \emph{necessary} in the concrete sense of
...
What the three clauses do not say is that the two program languages are
equivalent, and we do not claim it.  The label-free grammar is strictly more
permissive than erasure needs: its congruence constructor accepts an arbitrary
endofunction of the term model, whereas erasure only ever supplies one induced
by a source frame.  Section~\ref{sec:erasure} therefore characterizes the
essential image instead.
...
a proved failure of contextual subject reduction and proved inadmissibility of
the two rules that would repair it; representative-independent and
compositional quotient substitution; source/target-separated identity programs;
a proof that the resulting traces are unlabelled, together with an exact
characterization of the essential image of erasure; and the trace-record
elimination obstruction.

Of these, the negative reduction results and the erasure analysis are the
substantive additions.  The former replace a reported obstruction by theorems:
contextual subject reduction is false
...
(Theorem~\ref{thm:intro-not-admissible}).  That is a sharper outcome than
asserting a gap, and a more honest one than a conditional theorem presented as
a measurement.  The latter converts a naming convention into mathematics:
calling a trace ``unlabelled'' is otherwise a claim about intent, whereas the
factorization, size-preservation, and atom round-trip clauses of
Theorem~\ref{thm:intro-erasure}, together with the image characterization of
Theorem~\ref{thm:intro-image}, pin down what the target retains, what it cannot
express, and which of its programs are actually reached.
...
\[
 ...
 \qquad
 u=\App(\Var_0,\Zero_0).
\tag{6.2}\label{eq:sr-counterexample}
\]
...
The defensible formulation is therefore the following, and it is the one we
assert.  Conversion and context conversion are \emph{sufficient to repair every
obstruction arising in the structural subject-reduction proof}, and each is
\emph{necessary} in the sense of Theorem~\ref{thm:not-admissible}.  We do not
assert that they are ``all that is missing'', that the gap is ``exactly two
rules deep'', or that Theorem~\ref{thm:contextual-subject-reduction} measures a
distance between calculi; those readings confuse a fact about a proof with a
fact about a judgment.
\end{remark}
...
confluence.  We regard supplying it as the natural next step for this calculus,
and note that it is exactly the step at which the present development stops
being elementary.
\end{remark}
...
Three points about the formal development are worth stating precisely.  First,
...
theory.  Second---and this is why that visibility matters---the record is
proved uninhabited, so a reader auditing the artifact should read
\path{contextual_subject_reduction} as an analysis of the induction and not as
a usable subject-reduction lemma.  Third, the machine-checked theorems rely
only on propositional extensionality and quotient soundness; the negative
theorems of Section~\ref{sec:reduction} use propositional extensionality alone.
No form of choice is used, so the results are constructive relative to those
principles.
...
Axiom audit: \texttt{\#print axioms} on any theorem named above.
...
reduction is therefore a different judgment, not two hypotheses.
...
own records gives the deflationary reading we recommend: the quotient trace
fiber is a list of quotient classes, hence never contractible, so trace records
support no unrestricted based eliminator, while equality-component motives
remain available through projection.  These objects are observable list metadata
decorating an equality, not carriers of reasons for it.

The open directions follow from the above.  Confluence for \(\defeq\) would
unlock generation lemmas and hence an unconditional subject-reduction theorem
for \(\vdashC\); that is the single most valuable next step.  We also prove no
