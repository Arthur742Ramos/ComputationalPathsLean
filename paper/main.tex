...
\begin{remark}[The role of $\mathsf{rweq\_eq}$ and proof irrelevance]%
\label{rem:rweq-eq-honesty}
Contractibility at dimension $\ge 3$ relies on the
$\mathsf{MetaStep}_3.\mathsf{rweq\_eq}$ constructor, which connects
two derivations whose $\mathsf{toRwEq}$ projections are
propositionally equal.  In practice, the equality of these projections
is discharged by $\mathsf{rfl}$, which succeeds because both
projections inhabit $\RwEq\;p\;q$ and their underlying $\Eq$-valued
components (which live in $\Prop$) are identified by UIP.

This means that contractibility at level $\ge 3$ is ultimately a
\emph{consequence} of Lean's proof irrelevance on $\Eq$, mediated
through confluence: Church--Rosser guarantees that all derivations
between the same endpoints reduce to the same canonical form, so their
$\RwEq$ projections agree at the $\Prop$ level.  A fully explicit
confluence-only proof of contractibility---avoiding any appeal to
proof irrelevance on $\Eq$---is sketched in the formalization but
remains incomplete; we regard it as important future work.

The resulting structure is thus a \emph{weak 2-groupoid with
contractible higher cells}: genuinely non-degenerate at levels
0--2 (where the TRS provides real structure), and trivially
contractible above.  This contrasts with the Lumsdaine and van den
...
Third, contractibility at dimension~$\ge 3$ is derived via the
$\mathsf{rweq\_eq}$ constructor, whose soundness rests on
Church--Rosser confluence combined with proof irrelevance on the
underlying $\Eq$ (see Remark~\ref{rem:rweq-eq-honesty}).
...
(Remark~\ref{rem:rweq-eq-honesty}); the resulting structure is
...
\begin{enumerate}[label=(\roman*),nosep]
  ...
  \item A \textbf{weak $\omega$-groupoid structure} in the
    ...
    Church--Rosser confluence combined with proof irrelevance on the
    underlying $\Eq$ (Remark~\ref{rem:rweq-eq-honesty})
    (\S\ref{sec:omega}).
