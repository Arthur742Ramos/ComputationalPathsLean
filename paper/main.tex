...
Contractibility at dimension~\(\ge 3\) is achieved via the full
Squier program: a real normalization pipeline
(\(\mathsf{IsReduced}\), \(\mathsf{normalize}\),
\(\mathsf{to\_normal\_form}_3\)) reduces every 2-cell to a canonical
reduced flat chain using only groupoid-law \(\mathsf{MetaStep}_3\)
constructors, and Squier-style \(\mathsf{diamond\_filler}\) 3-cells
connect the canonical forms across confluence diamonds.
The \(\mathsf{rweq\_transport}\) constructor participates in connecting
normalized forms, but its proof obligation
\(\mathsf{derivation_2\_toEq\_eq}\) is discharged by \(\mathsf{rfl}\)
(definitional equality); no invocation of
\(\mathsf{Subsingleton.elim}\) appears in the contractibility proof.
...
The contractibility proof via $\mathsf{contractibility}_3$ uses:
\begin{enumerate}[nosep]
  ...
  \item \textbf{Definitional transport} ($\mathsf{rweq\_transport}$):
        ...
        Crucially, the proof obligation $\mathsf{derivation_2\_toEq\_eq}$
...
and the groupoid laws.  The $\mathsf{step\_eq}$ constructor
...
Squier-style diamond fillers, groupoid-law normalization
($\mathsf{IsReduced}$, $\mathsf{normalize}$,
$\mathsf{to\_normal\_form}_3$), and the $\mathsf{rweq\_transport}$
constructor whose proof obligation is definitional ($\mathsf{rfl}$).
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Contractibility mechanism.} Higher-dimensional
        ...
        $\mathsf{diamond\_filler}$ constructors for confluence diamonds,
        and the $\mathsf{rweq\_transport}$ constructor whose proof
        obligation is definitional ($\mathsf{rfl}$).
        The 3-cells are
        built from $\mathsf{step\_eq}$, $\mathsf{diamond\_filler}$,
        $\mathsf{rweq\_transport}$,
...
\begin{enumerate}[label=(\roman*),nosep]
  ...
  \item A \textbf{weak $\omega$-groupoid structure} adapted from the
    ...
    cells.  Contractibility at dimensions $\ge 3$ is achieved via the
    full Squier program: a real normalization pipeline
    ($\mathsf{IsReduced}$, $\mathsf{normalize}$,
    $\mathsf{to\_normal\_form}_3$) reduces 2-cells to canonical form,
    Squier-style $\mathsf{diamond\_filler}$ 3-cells connect across
    confluence diamonds, and the $\mathsf{rweq\_transport}$ constructor's
    proof obligation is discharged by $\mathsf{rfl}$ (definitional equality),
    eliminating all uses of $\mathsf{Subsingleton.elim}$ from the
