...
constructors.
At level~3 (connecting parallel 2-cells), genuine confluence diamonds
(diverging steps with \(\mathsf{StepStar}\) joining chains) are filled
by Squier-style \(\mathsf{diamond\_filler}\) 3-cells;
\(\mathsf{rweq\_transport}\) serves as a fallback for non-confluence
patterns whose proof obligation is discharged by \(\mathsf{rfl}\).
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Squier-style diamond fillers} ($\mathsf{diamond\_filler}$):
        used in $\mathsf{connect\_normalized}$ for genuine confluence
        diamonds---where both normalized 2-cells are
        $\mathsf{vcomp}(\mathsf{step}\;s_i, \mathsf{derivation\_2\_of\_stepstar}\;j_i)$,
        the filler $\mathsf{diamond\_filler}(s_1, s_2, j_1, j_2)$
        connects them directly.
  \item \textbf{Definitional transport} ($\mathsf{rweq\_transport}$, fallback):
        for non-confluence patterns (mixed constructors, inverse-of-inverse,
        etc.), connects normalized forms whose $\mathsf{toEq}$ projections agree.
...
($\mathsf{IsReduced}$, $\mathsf{normalize}$,
$\mathsf{to\_normal\_form}_3$), Squier-style $\mathsf{diamond\_filler}$
cells for genuine confluence diamonds at level~3 and above, and
$\mathsf{rweq\_transport}$ as a fallback for remaining non-confluence
patterns (with proof obligation discharged by $\mathsf{rfl}$).
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Contractibility mechanism.} Higher-dimensional
        ...
        diamonds (used at level~3 in $\mathsf{connect\_normalized}$
        and at levels~4+), and $\mathsf{rweq\_transport}$ as a fallback
        for non-confluence patterns (proof obligation discharged by
        $\mathsf{rfl}$).
        The 3-cells are
...
At the 3-cell level, Squier-style $\mathsf{diamond\_filler}$
constructors connect genuine confluence diamonds (diverging steps with
$\mathsf{StepStar}$ joining chains), while $\mathsf{rweq\_transport}$
serves as a fallback for non-confluence patterns
(proof obligation discharged by $\mathsf{rfl}$).
...
\begin{enumerate}[label=(\roman*),nosep]
  ...
  \item A \textbf{weak $\omega$-groupoid structure} adapted from the
    ...
    At level~3, genuine confluence diamonds are filled by
    $\mathsf{diamond\_filler}$; $\mathsf{rweq\_transport}$ is a fallback
    for non-confluence patterns, with proof obligations discharged by
    $\mathsf{rfl}$ (definitional equality).
