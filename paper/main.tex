...
\(\mathsf{rweq\_transport}\) constructor mediated by proof irrelevance
on the underlying \(\Eq\).
...
However, $\mathsf{Step}$ is $\Prop$-valued, so its proof irrelevance is
...
\smallskip
\noindent\textbf{Key achievement: structured contractibility.}
The $\mathsf{MetaStep}_3.\mathsf{rweq\_transport}$ constructor remains available
in the formalization as a fallback (and \emph{does} rely on proof irrelevance
of the underlying $\Eq$ via $\mathsf{rweq\_toEq}$), but the primary contractibility proof
via $\mathsf{contractibility}_3$ uses only:
\begin{enumerate}[nosep]
  ...
  \item \textbf{Confluence diamonds} ($\mathsf{diamond\_filler}$):
        constrained 3-cells connecting only derivations arising from
        actual diverging-step / rejoin configurations in the TRS.
\end{enumerate}

This achieves the Squier program explicitly: contractibility at
dimension $\ge 3$ is a \emph{structured consequence} of confluence
and the groupoid laws.  Note, however, that the $\mathsf{step\_eq}$ constructor
relies on $\mathsf{Step}$ being $\Prop$-valued (hence proof-irrelevant),
and the $\mathsf{rweq\_transport}$ fallback appeals to proof irrelevance
on the underlying $\Eq$ via $\mathsf{rweq\_toEq}$.
...
Squier-style diamond fillers together with proof irrelevance
on $\Eq$ (through the $\mathsf{step\_eq}$ and $\mathsf{rweq\_transport}$ constructors).
\end{remark}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Contractibility mechanism.} Higher-dimensional
        contractibility is derived via Squier-style diamond
        fillers and groupoid-law normalization; the
        $\mathsf{step\_eq}$ constructor relies on $\mathsf{Step}$ being
        $\Prop$-valued (proof-irrelevant), while higher levels use
        blanket $\mathsf{diamond\_filler}$ constructors.  The 3-cells are
...
contractible (via $\mathsf{rweq\_transport}$ and $\mathsf{diamond\_filler}$)
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Contractibility} at level~$\ge 3$:
    $\mathsf{MetaStep}_3.\mathsf{rweq\_transport}\;\mathsf{rfl}$
    at level~3 (using proof irrelevance of $\Eq$),
...
The formalization declares \textbf{no custom \texttt{axiom}} commands
...
development).
However, the formalization relies on the following \emph{Lean kernel
...
while the $\mathsf{rweq\_transport}$ constructor (at level~3) and
blanket $\mathsf{diamond\_filler}$ constructors (at levels $\ge 4$) complete
contractibility, ultimately relying on proof irrelevance of $\Eq$.
...
\begin{enumerate}[label=(\roman*),nosep]
  ...
  \item A \textbf{weak $\omega$-groupoid structure} adapted from the
    ...
    cells; contractibility at dimensions $\ge 3$ relies on
    Church--Rosser confluence via Squier-style diamond fillers
    (from critical-pair joins), together with the
    $\mathsf{rweq\_transport}$ constructor which appeals to proof irrelevance
