...
\begin{remark}[The Squier program: $\mathsf{step\_eq}$, $\mathsf{diamond\_filler}$, and no proof irrelevance]%
...
\smallskip
\noindent\textbf{Key achievement: no blanket axioms.}
The $\mathsf{MetaStep}_3.\mathsf{rweq\_eq}$ constructor remains available
in the formalization as a fallback, but the primary contractibility proof
...
dimension $\ge 3$ is a \emph{constructive consequence} of confluence
and the groupoid laws, not an appeal to proof irrelevance on $\Eq$.
...
Unlike the blanket $\mathsf{rweq\_eq}$, the $\mathsf{diamond\_filler}$
...
UIP foundation, but the higher-dimensional collapse is achieved via
explicit Squier-style diamond fillers rather than by appealing to
proof irrelevance on $\Eq$.
\end{remark}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Contractibility mechanism.} Higher-dimensional
        contractibility is derived via explicit Squier-style diamond
        fillers and groupoid-law normalization, \emph{not} via
        appeal to proof irrelevance on $\Eq$.  The 3-cells are
