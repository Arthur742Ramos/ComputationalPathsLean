...
constructors.
At level~3, the function \(\mathsf{connect\_strict}\) handles all
\(25 = 5 \times 5\) case combinations of \(\mathsf{StrictNormalForm}\)
pairs using purely structural 3-cell constructors:
\(\mathsf{step\_eq}\), \(\mathsf{diamond\_filler}\) (with constructive
\(\mathsf{StepStar}\) witnesses),
\(\mathsf{whisker\_inv}_3\), and the groupoid laws.
No \(\mathsf{rweq\_transport}\) is used at level~3.
At levels~4 and above, the same \(\mathsf{diamond\_filler}\) strategy
connects normalized higher cells.
No invocation of \(\mathsf{Subsingleton.elim}\) appears in the
contractibility proof.
To our knowledge, this is the first mechanized formalization of the
complete Squier program: higher-dimensional contractibility is a
genuine structural consequence of confluence, not UIP in disguise.
...
The function $\mathsf{connect\_strict}$ handles all $25 = 5 \times 5$
case combinations, producing a 3-cell connecting any two strict
normal forms with the same endpoints.
It uses only three $\mathsf{MetaStep}_3$ constructors:
\begin{itemize}[nosep]
  ...
  \item $\mathsf{whisker\_inv}_3$: inverts a step equality under
        inversion, connecting $\mathsf{inv}(\mathsf{step}\;s_1)$
        to $\mathsf{inv}(\mathsf{step}\;s_2)$ via
        $\mathsf{step\_eq}(s_1, s_2)$.
\end{itemize}
These are composed with groupoid-law constructors
($\mathsf{vcomp\_refl\_right}$, $\mathsf{vcomp\_congr}_{3,\mathrm{left}}$,
$\mathsf{vcomp\_congr}_{3,\mathrm{right}}$) to handle the full
case matrix.  No $\mathsf{rweq\_transport}$ appears at level~3.
...
\begin{remark}[The complete Squier program: purely structural 3-cells]%
...
\smallskip
\noindent\textbf{Key achievement: complete Squier program with purely structural 3-cells.}
The level-3 contractibility proof uses \emph{no} $\mathsf{rweq\_transport}$
and \emph{no} $\mathsf{Subsingleton.elim}$.%
\footnote{The constructor $\mathsf{rweq\_transport}$ exists in $\mathsf{MetaStep}_3$
as a fallback mechanism, but it is \emph{unused} in $\mathsf{contractibility}_3$.
The function $\mathsf{connect\_strict\_structural}$ handles all cases purely
via $\mathsf{diamond\_filler}$ (with constructive $\mathsf{StepStar}$ witnesses),
$\mathsf{step\_eq}$, $\mathsf{whisker\_inv}_3$, and the groupoid laws.}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Squier-style diamond fillers} ($\mathsf{diamond\_filler}$):
        used in $\mathsf{connect\_strict}$ for genuine confluence
        diamonds---where both normalized 2-cells begin with
        $\mathsf{vcomp}(\mathsf{step}\;s_i, \ldots)$,
        the filler connects them directly.
  \item \textbf{Inverse whiskering} ($\mathsf{whisker\_inv}_3$):
        lifts $\mathsf{step\_eq}$ through inversion, connecting
        $\mathsf{inv}(\mathsf{step}\;s_1)$ to $\mathsf{inv}(\mathsf{step}\;s_2)$.
\end{enumerate}
The function $\mathsf{connect\_strict}$ dispatches on all
$25 = 5 \times 5$ combinations of $\mathsf{StrictNormalForm}$
constructors ($\mathsf{refl}$, $\mathsf{single\_step}$,
$\mathsf{single\_inv}$, $\mathsf{cons\_step}$, $\mathsf{cons\_inv}$),
building each 3-cell from the constructors above.
This is a genuine structural result, not UIP in disguise:
the 3-cells witness specific rewriting confluences and groupoid
identities, making each connection inspectable and meaningful.
...
uses no $\mathsf{Subsingleton.elim}$ and no $\mathsf{rweq\_transport}$
at level~3: the $\mathsf{connect\_strict}$ function handles all 25
$\mathsf{StrictNormalForm}$ case combinations using purely structural
constructors ($\mathsf{step\_eq}$, $\mathsf{diamond\_filler}$,
$\mathsf{whisker\_inv}_3$, and groupoid laws).
...
At the 3-cell level, $\mathsf{connect\_strict}$ handles all 25
$\mathsf{StrictNormalForm}$ case combinations using purely structural
constructors: $\mathsf{step\_eq}$, $\mathsf{diamond\_filler}$,
$\mathsf{whisker\_inv}_3$, and groupoid laws.
...
Guiraud--Malbos.
The function $\mathsf{connect\_strict}$ handles all 25
$\mathsf{StrictNormalForm}$ case combinations using only
$\mathsf{step\_eq}$, $\mathsf{diamond\_filler}$,
$\mathsf{whisker\_inv}_3$, and groupoid laws---completing the
Squier program with purely structural 3-cells
(Remark~\ref{rem:rweq-eq-honesty}).
...
\begin{enumerate}[label=(\roman*),nosep]
  ...
  \item A \textbf{weak $\omega$-groupoid structure} adapted from the
    ...
    At level~3, $\mathsf{connect\_strict}$ handles all $25$
    $\mathsf{StrictNormalForm}$ case combinations using purely
    structural constructors ($\mathsf{step\_eq}$,
    $\mathsf{diamond\_filler}$, $\mathsf{whisker\_inv}_3$, and
    groupoid laws), with no $\mathsf{rweq\_transport}$.
    At levels~4+, Squier-style $\mathsf{diamond\_filler}$ cells connect
    normalized higher cells.
    This completes the Squier program with purely structural 3-cells,
    achieving contractibility as a genuine structural consequence of
    confluence (Remark~\ref{rem:rweq-eq-honesty})
    (\S\ref{sec:omega}).
