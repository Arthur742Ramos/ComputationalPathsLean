...
The central technical choice is to place the primitive step relation
\(\Step\) and rewrite equivalence \(\RwEq\) in \(\Ty\,(u{+}1)\) rather than
\(\Prop\), preventing higher coherences from collapsing under proof
irrelevance.  When a \(\Prop\)-valued relation is required, we use
\(\RwEqProp\;p\;q := \mathsf{Nonempty}(\RwEq\;p\;q)\).
We define steps, paths, and rewrite equivalence over the step relation in
\texttt{Path/Rewrite/Step.lean}; prove soundness and congruence properties; and
establish type-valued Church--Rosser confluence for the completed groupoid
fragment (\texttt{Path/Rewrite/ConfluenceDeepType.lean}).
...
\(\mathsf{StrictNormalForm}\) using groupoid-law \(\mathsf{MetaStep}_3\)
constructors.
At level~3, the function \(\mathsf{connect\_strict}\) connects
\(\mathsf{StrictNormalForm}\) pairs via a three-tier approach:
(1)~\emph{same-shape} cases (both refl, both single-step, both single-inv)
use purely structural constructors (\(\mathsf{step\_eq}\),
\(\mathsf{whisker\_inv}_3\), groupoid laws);
(2)~\emph{cross-shape} cases use propositional transport
(\(\mathsf{rweq\_transport}\)), which requires an explicit proof that
the underlying path equalities coincide---not a blanket axiom;
(3)~\emph{eliminated}: blanket axioms like \(\mathsf{rweq\_eq}\) (asserting
any two 2-cells equal) that would trivialize the type-valued coherence.
At levels~4 and above, the \(\mathsf{diamond\_filler}\) strategy
connects normalized higher cells.
No invocation of \(\mathsf{Subsingleton.elim}\) appears in the
contractibility proof.
This achieves partial structural coherence: same-shape derivations
are connected purely structurally, while cross-shape cases require
propositional witnesses---a significant advance over blanket UIP.
...
Lean~4 proof assistant~\cite{Moura2021}.  The formalization encompasses 1,297
Lean source files and contains no uses of \texttt{sorry} or \texttt{admit}---every
...
Our primitive step relation and rewrite equivalence are defined as:
\[
\mathsf{Step} : \mathsf{Path}\,a\,b \to \mathsf{Path}\,a\,b \to \mathsf{Type}(u{+}1),
\qquad
\mathsf{RwEq} : \mathsf{Path}\,a\,b \to \mathsf{Path}\,a\,b \to \mathsf{Type}(u{+}1)
\]
...
wrapper, we provide
\[
\mathsf{RwEqProp}\;p\;q := \mathsf{Nonempty}(\mathsf{RwEq}\;p\;q).
\]
This two-tier design---$\mathsf{Type}$-valued for structure,
...
\item \textbf{The Path/Step/RwEq framework.}  We define computational paths as
  ...
  the step relation in \texttt{Path/Rewrite/Step.lean}, targeting
  $\mathsf{Type}(u{+}1)$ rather than $\mathsf{Prop}$.

\item \textbf{Type-valued rewriting combinators.}
  \texttt{Path/Rewrite/RewritingDeep.lean} provides type-valued rewriting
  combinators, conversions, and normal-form witnesses.

\item \textbf{Type-valued confluence.}
  \texttt{Path/Rewrite/ConfluenceDeepType.lean} establishes Church--Rosser
  confluence with $\Sigma$-witnesses of joinability.

\item \textbf{A lightweight weak 2-groupoid interface (towards a weak $\omega$-groupoid).}
  The module \texttt{Path/OmegaGroupoid/OmegaWeakGroupoid.lean} packages the
  0/1/2-cell structure carried by computational paths, taking 2-cells to be the
  type-valued rewrite equivalences $\RwEq$.  It records the standard operations
  (vertical composition, whiskering, horizontal composition) together with
  pentagon/triangle/interchange/Eckmann--Hilton coherence statements after
  truncation via \texttt{rweq\_toEq}.  The higher-dimensional development and the
  contractibility-based Batanin--Leinster conditions are treated in the
  \texttt{Path/OmegaGroupoid} development (see in particular
  \texttt{Path/OmegaGroupoid/TruncationProof.lean}).

\item \textbf{Explicit pentagon and Eckmann--Hilton.}  Genuine 2-cell coherence
  proofs for the pentagon and Eckmann--Hilton arguments are constructed in
  \texttt{Path/OmegaGroupoid/PentagonProof.lean} and
  \texttt{Path/OmegaGroupoid/EckmannHiltonProof.lean}.

\item \textbf{Deformation theory over $\RwEq$.}  The module
  \texttt{Path/Algebra/DeformationDeep.lean} develops deformation theory built on
  type-valued rewrite equivalence.
...
\begin{tikzcd}[column sep=2.5em, row sep=2.8em]
  ...
  & \text{Eq} \;:\; \Prop
    \ar[r, "\text{UIP}"']
  & \text{trivial above } n \ge 3
\end{tikzcd}
\caption{Architecture of the computational paths framework.
  The top row carries proof-relevant structure: elementary
  $\Step$ rules lift to $\RwEq$ witnesses in $\Ty$, which
  iterate into the cell tower.
  The bottom row is the proof-irrelevant shadow: paths project
  to propositional equality in $\Prop$, which is trivial by UIP.
  The key insight is that genuine higher structure lives in the
  top row at levels 0--2, while contractibility at level $\ge 3$
  is mediated by the projection to the bottom row.}
...
\begin{theorem}[Definitional groupoid laws]\label{thm:weak-groupoid-laws}
...
\begin{table}[htbp]
\centering
\caption{Basic path algebra step constructors (Rules 1--8).}
...
\begin{table}[htbp]
\centering
\caption{Product type step constructors (Rules 9--16).}
\label{tab:product-rules}
\renewcommand{\arraystretch}{1.2}
\resizebox{\textwidth}{!}{%
...
\begin{table}[htbp]
\centering
\caption{Structural closure and Knuth--Bendix completion rules (Rules 73--77).}
\label{tab:closure-rules}
\renewcommand{\arraystretch}{1.2}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}clp{9cm}@{}}
\toprule
...
\begin{remark}[The cancellation rules]\label{rem:cancellation}
...
By targeting $\mathsf{Type}(u{+}1)$, each level of the tower carries
...
\mathsf{RwEqProp}(p, q) := \mathsf{Nonempty}(\mathsf{RwEq}(p, q))
\]
In Lean this is just \texttt{Nonempty} (\texttt{RwEq p q}).
...
the semantic equality (proved by case analysis on all Step constructors).  The
...
In the Lean development, the type-valued closure is made explicit in
\texttt{Path/Rewrite/ConfluenceDeepType.lean}, where the type-valued multi-step
closure \texttt{RwT} and the abstract closure \texttt{ARTC} support confluence
and Church--Rosser statements producing $\Sigma$-witnesses of joinability.
Complementing this, \texttt{Path/Rewrite/RewritingDeep.lean} supplies
combinators on multi-step reductions, conversion (zigzag) paths, and the
\texttt{NormalFormWitness} package for normal forms.
...
\begin{tikzcd}[column sep=3em, row sep=2.5em]
  & a \ar[dl, twoheadrightarrow] \ar[dr, twoheadrightarrow] & \\
  b \ar[dr, twoheadrightarrow]
    \ar[rr, dashed, "\llbracket b \rrbracket = \llbracket c \rrbracket"
      description]
  & & c \ar[dl, twoheadrightarrow] \\
  & d = \mathsf{canon}(b) &
\end{tikzcd}
\caption{Confluence via the free-group interpretation.
  Any two reducts $b$, $c$ of a common ancestor $a$ share the
  same free-group image (Theorem~\ref{thm:invariance}), hence
  reduce to the same canonical form $d$
  (Theorem~\ref{thm:reachability}).}
...
$\mathsf{Subsingleton.elim}$ at the $\RwEq$ level.  Since $\Step$ and $\RwEq$
are type-valued, these coherence witnesses are genuine data; when a
propositional coherence statement is desired we apply the truncation
map $\mathsf{rweq\_toEq}$.

These constructions are realized in
\texttt{Path/OmegaGroupoid/PentagonProof.lean} and
\texttt{Path/OmegaGroupoid/EckmannHiltonProof.lean}.  The lightweight
\texttt{Path/OmegaGroupoid/OmegaWeakGroupoid.lean} interface packages a weak
2-groupoid/$\omega$-groupoid structure whose pentagon, triangle, interchange,
and Eckmann--Hilton coherences are stated after truncation via
\texttt{rweq\_toEq}.
...
\begin{align}
  e_1 &: \RwEq\;\big(((p \cmpA q) \cmpA r) \cmpA s\big)
    ...
    \notag \\
    &\quad \triangleq\; \RwEq.\mathsf{step}(\Step.\mathsf{trans\_assoc}\;
  ...
  e_3 &: \RwEq\;\big(((p \cmpA q) \cmpA r) \cmpA s\big)
                \;\big((p \cmpA (q \cmpA r)) \cmpA s\big)
    \notag \\
    &\quad \triangleq\; \RwEq.\mathsf{step}(\Step.\mathsf{trans\_congr\_left}\;
  ...
  e_5 &: \RwEq\;\big(p \cmpA ((q \cmpA r) \cmpA s)\big)
                \;\big(p \cmpA (q \cmpA (r \cmpA s))\big)
    \notag \\
    &\quad \triangleq\; \RwEq.\mathsf{step}(\Step.\mathsf{trans\_congr\_right}\;
...
\begin{figure}[htbp]
\centering
\begin{tikzcd}[column sep=1.2em, row sep=3.5em]
  & (p \cmpA q) \cmpA (r \cmpA s)
    \ar[dr, "{e_2}"] & \\
  ((p \cmpA q) \cmpA r) \cmpA s
    \ar[ur, "{e_1}"]
  ...
  p \cmpA ((q \cmpA r) \cmpA s)
    \ar[u, "{e_5}"']
\end{tikzcd}
\caption{The Mac\,Lane pentagon for path composition.
  The right route $R = e_1 \vcomp e_2$ (top) and the left route
  $L = e_3 \vcomp e_4 \vcomp e_5$ (bottom) yield the same
  underlying equality.}
\label{fig:pentagon}
\end{figure}
...
\begin{tikzcd}[column sep=4em, row sep=2.5em]
  p_1 \cmpA q_1
    \ar[r, Rightarrow, "{\alpha_1 \hcomp \beta_1}"]
    \ar[d, Rightarrow, "{\alpha_1 \vcomp \alpha_2}"']
  & p_2 \cmpA q_2
    \ar[d, Rightarrow, "{\alpha_2 \hcomp \beta_2}"] \\
  p_3 \cmpA q_1
    \ar[r, Rightarrow]
  & p_3 \cmpA q_3
\end{tikzcd}
\caption{The interchange law: horizontal-then-vertical equals
  vertical-then-horizontal composition of a $2 \times 2$ grid
  of 2-cells.}
...
The proof proceeds in three explicit steps (see Figure~\ref{fig:eckmann-hilton}):
\begin{enumerate}
...
\begin{gather*}
  v(\alpha, \beta)
  \xrightarrow[\text{R9}]{\mathsf{map2\_subst}}
  h(\alpha, \beta)
  \xrightarrow[\text{R3, R4}]{\text{unit laws}}
  \alpha \cmpA \beta \\
  \xrightarrow[\text{swap}]{\text{interchange}}
  \beta \cmpA \alpha
  \xrightarrow[\text{R3, R4}]{\text{unit laws}}
  h(\beta, \alpha)
  \xrightarrow[\text{R9}^{-1}]{\mathsf{map2\_subst}}
  v(\beta, \alpha)
\end{gather*}
\caption{The Eckmann--Hilton argument as a chain of $\Step$/$\RwEq$
  applications.  Each arrow is a concrete rewrite step (or its inverse)
  from the 77-constructor step relation.}
\label{fig:eckmann-hilton}
\end{figure}
...
\begin{tikzcd}[column sep=4em, row sep=3em]
  (p \cmpA \refl_b) \cmpA q
    \ar[rr, "{\alpha_{p,\,\refl_b,\,q}}"]
    \ar[dr, "{\rho_p \wr q}"'] & &
  p \cmpA (\refl_b \cmpA q)
    \ar[dl, "{p \wl \lambda_q}"] \\
  & p \cmpA q &
\end{tikzcd}
\caption{The triangle identity.
  Route $T_1$ (top, then right leg) uses associativity followed
  by the left unitor; route $T_2$ (left leg) uses the right unitor
  under congruence.}
\label{fig:triangle}
\end{figure}
Explicitly, defining:
...
\begin{tikzcd}[column sep=3.5em, row sep=0.8em]
  A
    \ar[r, shift left=0.7ex, "p"{name=U1}]
    \ar[r, shift right=0.7ex, "q"'{name=D1}]
  & A
  \ar[from=U1, to=D1, Rightarrow, "\alpha"{description}]
\end{tikzcd}
...
$\underbrace{%
  \mathsf{Cell}_0 = A,\;
  \mathsf{Cell}_1 = \Path,\;
  \mathsf{Cell}_2 = \mathsf{Deriv}_2,\;
  \mathsf{Cell}_3 = \mathsf{Deriv}_3,\;
  \ldots
}_{\text{globular tower: genuine at levels 0--2, contractible at level } \ge 3}$%
\caption{The cell tower.
  0-cells are elements of $A$; 1-cells are paths $p, q : \Path\;a\;b$;
  2-cells are derivations $\alpha : \mathsf{Derivation}_2\;p\;q$;
  higher cells witness coherences between lower cells.
  From level 3 onward the tower is contractible
...
\begin{itemize}[nosep]
  ...
  \item $\mathsf{rweq\_transport}$ (two derivations whose $\RwEq$
    ...
    available in the metatheory but \emph{not used} in the level-3
    contractibility proof).
\end{itemize}
...
\medskip
\noindent\textbf{Step 2: Connecting strict normal forms ($\mathsf{connect\_strict}$).}
After normalization, each 2-cell is in $\mathsf{StrictNormalForm}$---a
right-associated chain of signed single steps.
The type $\mathsf{StrictNormalForm}$ has five constructors:
$\mathsf{refl}$, $\mathsf{single\_step}$, $\mathsf{single\_inv}$,
$\mathsf{cons\_step}$, and $\mathsf{cons\_inv}$.
The function $\mathsf{connect\_strict}$ connects strict normal forms
via a \textbf{three-tier approach}:

\noindent\emph{Tier 1: Same-shape (purely structural).}
When both derivations have the same outermost shape, we use purely
structural constructors:
\begin{itemize}[nosep]
  \item $\mathsf{refl}$/$\mathsf{refl}$: connected by $\mathsf{Derivation}_3.\mathsf{refl}$
  \item $\mathsf{step}$/$\mathsf{step}$: connected by $\mathsf{step\_eq}(s_1, s_2)$
  \item $\mathsf{inv}(\mathsf{step})$/$\mathsf{inv}(\mathsf{step})$:
        connected by $\mathsf{whisker\_inv}_3(\mathsf{step\_eq}(s_1, s_2))$
\end{itemize}
These cases use no propositional reasoning beyond the fact that
$\mathsf{Step}$ is proof-irrelevant (Prop-valued), which allows
$\mathsf{step\_eq}$ to identify any two steps with the same endpoints.

\noindent\emph{Tier 2: Cross-shape (propositional transport).}
When derivations have different shapes (e.g., $\mathsf{refl}$ vs.\
$\mathsf{step}$, or $\mathsf{vcomp}$ vs.\ anything), we use:
\begin{itemize}[nosep]
  \item $\mathsf{rweq\_transport}(h)$: given a proof
        $h : \mathsf{rweq\_toEq}(d_1.\mathsf{toRwEq}) = \mathsf{rweq\_toEq}(d_2.\mathsf{toRwEq})$
        that the underlying path equalities coincide, this constructor
        produces a 3-cell connecting $d_1$ and $d_2$.
\end{itemize}
Crucially, $\mathsf{rweq\_transport}$ is \emph{not} a blanket axiom.
It requires an explicit proof that the two derivations project to
the same propositional equality.  For parallel derivations with
matching endpoints, this proof is $\mathsf{rfl}$ (definitionally equal),
but the requirement is real.

\noindent\emph{Tier 3: Eliminated (blanket axioms).}
The following were removed from the formalization:
\begin{itemize}[nosep]
  \item $\mathsf{rweq\_eq}$: a blanket axiom asserting any two 2-cells
        with the same endpoints are equal---this would trivialize
        type-valued coherence.
  \item $\mathsf{strict\_shape\_bridge}$: a catch-all axiom connecting
        any two strict normal forms regardless of shape.
\end{itemize}

The $\mathsf{diamond\_filler}$ constructor (used in more complex cases
via composition) is also structural: given diverging steps
$s_1 : p \to q$ and $s_2 : p \to r$ that rejoin via
$j_1 : q \twoheadrightarrow m$ and $j_2 : r \twoheadrightarrow m$,
it provides a 3-cell witnessing commutativity of the diamond.
This is \emph{not} a blanket axiom; it only connects the specific
...
\begin{remark}[Partial structural coherence at level 3]%
\label{rem:rweq-eq-honesty}
The contractibility theorem realizes a \emph{partial} Squier program for
...
\smallskip
\noindent\textbf{Key achievement: partial structural coherence.}
The level-3 contractibility proof achieves \emph{structural coherence}
for same-shape derivations, and uses \emph{propositional transport}
only for cross-shape cases:

\begin{itemize}[nosep]
  \item \textbf{Same-shape cases} (purely structural): When both
        normalized derivations have matching outermost shape
        ($\mathsf{refl}$/$\mathsf{refl}$, $\mathsf{step}$/$\mathsf{step}$,
        or $\mathsf{inv}(\mathsf{step})$/$\mathsf{inv}(\mathsf{step})$),
        the connection is purely structural via $\mathsf{step\_eq}$,
        $\mathsf{whisker\_inv}_3$, or $\mathsf{Derivation}_3.\mathsf{refl}$.
  \item \textbf{Cross-shape cases} (propositional transport): When
        shapes differ (e.g., $\mathsf{refl}$ vs.\ $\mathsf{step}$),
        we use $\mathsf{rweq\_transport}$, which requires an explicit
        proof that $\mathsf{rweq\_toEq}(d_1.\mathsf{toRwEq}) =
        \mathsf{rweq\_toEq}(d_2.\mathsf{toRwEq})$.  This is \emph{not}
        a blanket axiom: it requires a propositional witness that the
        underlying path equalities coincide.
  \item \textbf{Eliminated axioms}: No $\mathsf{rweq\_eq}$ (blanket
        equality of 2-cells) or $\mathsf{strict\_shape\_bridge}$
        (catch-all connection) appears.  No $\mathsf{Subsingleton.elim}$
        is invoked.
\end{itemize}

\smallskip\noindent\textit{Level~3 ($\mathsf{contractibility}_3$, connecting parallel 2-cells):}
\begin{enumerate}[nosep]
  \item \textbf{Groupoid-law rewrites} (normalization phase):
        $\mathsf{vcomp\_refl\_left}$,
  ...
  \item \textbf{Squier-style diamond fillers} ($\mathsf{diamond\_filler}$):
        connect genuine confluence diamonds with constructive
        $\mathsf{StepStar}$ witnesses.
  \item \textbf{Inverse whiskering} ($\mathsf{whisker\_inv}_3$):
        lifts $\mathsf{step\_eq}$ through inversion.
  \item \textbf{Propositional transport} ($\mathsf{rweq\_transport}$):
        used for cross-shape cases where structural constructors
        do not apply, requiring proof that underlying equalities match.
\end{enumerate}
This is a significant advance over blanket UIP: the structural cases
give inspectable, meaningful 3-cells witnessing specific rewriting
confluences and groupoid identities.  The propositional cases are
limited and require explicit proof obligations.
...
\emph{structured consequence} of normalization and the groupoid laws.
The $\mathsf{step\_eq}$ constructor
relies on $\mathsf{Step}$ being $\Prop$-valued (hence proof-irrelevant),
but the overall contractibility proof is not a disguised invocation of
UIP: each 3-cell records the specific groupoid identity or confluence
diamond that justifies the connection.
No invocation of $\mathsf{Subsingleton.elim}$ or $\mathsf{rweq\_transport}$
appears in the level-3 contractibility proof.
The formalization validates this claim: the $\mathsf{connect\_strict}$
function builds explicit 3-cells from the constructors above for
every case combination.

\smallskip
\noindent\textbf{The constrained $\mathsf{diamond\_filler}$.}
The $\mathsf{diamond\_filler}$
...
($\mathsf{IsReduced}$, $\mathsf{normalize}$,
$\mathsf{to\_normal\_form}_3$) and Squier-style
$\mathsf{diamond\_filler}$, $\mathsf{step\_eq}$, and
$\mathsf{whisker\_inv}_3$ cells at level~3 and above.
No use of $\mathsf{Subsingleton.elim}$ or $\mathsf{rweq\_transport}$
is required at level~3.
\end{remark}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Contractibility mechanism.} Higher-dimensional
        ...
        ($\mathsf{IsReduced}$, $\mathsf{normalize}$,
        $\mathsf{to\_normal\_form}_3$), Squier-style
        $\mathsf{diamond\_filler}$ constructors for genuine confluence
        diamonds, $\mathsf{step\_eq}$ for step coherence, and
        $\mathsf{whisker\_inv}_3$ for inverse whiskering---all
        composed with groupoid-law constructors.
        At level~3, $\mathsf{connect\_strict}$ handles all
        $25$ case combinations of $\mathsf{StrictNormalForm}$
        using these constructors alone, with no appeal to
        $\mathsf{rweq\_transport}$ or $\mathsf{Subsingleton.elim}$.
        The 3-cells are
        built from $\mathsf{step\_eq}$, $\mathsf{diamond\_filler}$,
        $\mathsf{whisker\_inv}_3$,
        and the groupoid-law $\mathsf{MetaStep}_3$ constructors
        ($\mathsf{vcomp\_refl\_right}$,
        $\mathsf{vcomp\_congr}_{3,\mathrm{left}}$,
        $\mathsf{vcomp\_congr}_{3,\mathrm{right}}$).
\end{enumerate}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Contractibility} at level~$\ge 3$:
    ...
    and $\mathsf{connect\_strict}$ bridges strict normal forms using
    $\mathsf{step\_eq}$, $\mathsf{diamond\_filler}$, and
    $\mathsf{whisker\_inv}_3$ (all 25 case combinations handled
    without $\mathsf{rweq\_transport}$);
...
% ======================================================================
\section{Homotopy-Theoretic Applications}\label{sec:applications}
% ======================================================================
...
Beyond the homotopy computations, \texttt{Path/Algebra/DeformationDeep.lean}
develops deformation theory for computational paths: formal deformations and
their equivalences, obstruction and unobstructedness criteria for extensions,
and rigidity/formal rigidity results.  All equalities in this development are
tracked by Type-valued $\RwEq$ witnesses.
...
\begin{tikzcd}[column sep=5em, row sep=2.5em]
  \pi_1(S^1, \mathsf{base})
    \ar[r, shift left=1ex, "\overline{\mathsf{wind}}"]
  & \mathbb{Z}
    \ar[l, shift left=1ex, "\overline{\mathsf{decode}}"]
\end{tikzcd}
\caption{The encode--decode isomorphism
  $\pi_1(S^1) \cong \mathbb{Z}$.
  The winding number $\overline{\mathsf{wind}}$ counts
  loop traversals; $\overline{\mathsf{decode}}$ constructs
  the $n$-fold loop $\mathsf{loop}^n$.
  Both directions are verified as $\RwEq$-compatible.}
\label{fig:pi1-circle}
\end{figure}
\end{theorem}
...
\subsection{Seifert--van Kampen theorem}\label{sec:svk}
...
\subsection{Partial univalence}\label{sec:univalence}
...
\quad Critical pair witnesses & $\FF$ & 3 & Rem.~\ref{rem:cancellation} \\
...
\quad Partial univalence & $\PS$ & 3 & Thms.~\ref{thm:partial-ua}, \ref{thm:no-ua-full} \\
...
uses no $\mathsf{Subsingleton.elim}$.
At level~3, the $\mathsf{connect\_strict}$ function achieves
\emph{partial structural coherence}: same-shape cases (both
$\mathsf{refl}$, both $\mathsf{step}$, or both
$\mathsf{inv}(\mathsf{step})$) use purely structural constructors
($\mathsf{step\_eq}$, $\mathsf{whisker\_inv}_3$, groupoid laws);
cross-shape cases use $\mathsf{rweq\_transport}$ with an explicit
...
At the 3-cell level, $\mathsf{connect\_strict}$ achieves \emph{partial
structural coherence}: same-shape cases use purely structural constructors
($\mathsf{step\_eq}$, $\mathsf{whisker\_inv}_3$, groupoid laws), while
cross-shape cases require propositional transport with an explicit witness.
At levels $\ge 4$, the same $\mathsf{diamond\_filler}$ strategy
connects normalized higher cells.
...
Our partial univalence result (Theorem~\ref{thm:partial-ua})
...
Guiraud--Malbos.
The function $\mathsf{connect\_strict}$ achieves partial structural
coherence: same-shape cases use purely structural constructors
($\mathsf{step\_eq}$, $\mathsf{whisker\_inv}_3$, groupoid laws),
while cross-shape cases require propositional transport
(Remark~\ref{rem:rweq-eq-honesty}).
...
\begin{enumerate}[label=(\roman*),nosep]
  ...
  \item A \textbf{weak $\omega$-groupoid structure} adapted from the
    ...
    At level~3, $\mathsf{connect\_strict}$ achieves \emph{partial
    structural coherence}: same-shape derivations (both $\mathsf{refl}$,
    both $\mathsf{step}$, or both $\mathsf{inv}(\mathsf{step})$) are
    connected via purely structural constructors ($\mathsf{step\_eq}$,
    $\mathsf{whisker\_inv}_3$, groupoid laws); cross-shape cases use
    propositional transport ($\mathsf{rweq\_transport}$) requiring an
    explicit proof that underlying path equalities coincide.
    Blanket axioms ($\mathsf{rweq\_eq}$, $\mathsf{strict\_shape\_bridge}$)
    are eliminated; no $\mathsf{Subsingleton.elim}$ is invoked.
    At levels~4+, Squier-style $\mathsf{diamond\_filler}$ cells connect
    normalized higher cells.
    This achieves significant structural coherence beyond blanket UIP
    (Remark~\ref{rem:rweq-eq-honesty})
    (\S\ref{sec:omega}).
  ...
  \item \textbf{Extensions to 72 mathematical modules}
    (\S\ref{app:modules}), of which ${\sim}26$ are FF or PS level,
...
The HoTT modules (\S\ref{app:modules}) include higher inductive
