...
\(\omega\)-groupoid structure in the sense of Batanin--Leinster, with
genuine non-degeneracy at levels~0--2 and contractible higher cells.
Contractibility at dimension~\(\ge 3\) uses both a
Squier-style \(\mathsf{diamond\_filler}\) constructor---whose instances
arise from the critical pairs of the groupoid TRS---and a blanket
\(\mathsf{rweq\_eq}\) axiom mediated by proof irrelevance.
...
This 3-cell is constructed using \emph{only} the groupoid-law
constructors of $\mathsf{MetaStep}_3$:
\begin{itemize}[nosep]
  \item $\mathsf{vcomp\_refl\_left}$, $\mathsf{vcomp\_refl\_right}$
...
\end{itemize}
No axioms or proof-irrelevance principles are used.  The construction
is purely structural: it normalizes the free-groupoid word represented
...
\begin{remark}[The role of $\mathsf{rweq\_eq}$, $\mathsf{diamond\_filler}$, and proof irrelevance]%
\label{rem:rweq-eq-honesty}
Contractibility at dimension $\ge 3$ currently uses two mechanisms.

\smallskip
\noindent\textbf{(a) The blanket axiom $\mathsf{rweq\_eq}$.}
The $\mathsf{MetaStep}_3.\mathsf{rweq\_eq}$ constructor connects
\emph{any} two parallel derivations $d_1, d_2 : \mathsf{Derivation}_2\;p\;q$
unconditionally.  In practice, the equality of their $\mathsf{toRwEq}$
projections is discharged by $\mathsf{rfl}$, which succeeds because
those projections inhabit $\RwEq\;p\;q$ and the underlying $\Eq$-valued
components (in $\Prop$) are identified by UIP.
This is the constructor that ultimately makes contractibility hold, but
it is a blanket assertion that does not record \emph{why} the two
derivations are related.

\smallskip
\noindent\textbf{(b) The constrained $\mathsf{diamond\_filler}$.}
The $\mathsf{MetaStep}_3.\mathsf{diamond\_filler}$ constructor
provides Squier-style finite derivation type (FDT) structure.
Unlike $\mathsf{rweq\_eq}$, it is \emph{constrained}: it requires
explicit diverging steps $s_1 : \Step\;p\;q$, $s_2 : \Step\;p\;r$
together with explicit joining chains $j_1 : \mathsf{StepStar}\;q\;m$,
$j_2 : \mathsf{StepStar}\;r\;m$, and it only connects the two
derivations that arise from a genuine confluence diamond.
This constructor directly implements Squier's program: each of the
33 critical pairs of the completed groupoid TRS generates a
diamond-filler 3-cell.

Two technical points deserve emphasis.
First, $\Step$ itself remains $\Prop$-valued (a mere relation),
but $\mathsf{StepStar}$---the reflexive-transitive closure used for
critical-pair joins---inhabits $\Ty\;(u{+}1)$, so the joining chains
carry explicit computational content.
Second, $\mathsf{diamond\_filler}$ connects only the specific
composites $\mathsf{vcomp}(\mathsf{step}\;s_1, \mathsf{derivation\_2\_of\_stepstar}\;j_1)$
and $\mathsf{vcomp}(\mathsf{step}\;s_2, \mathsf{derivation\_2\_of\_stepstar}\;j_2)$---it
is not a blanket identification of arbitrary parallel 2-cells.

\smallskip
\noindent\textbf{Status.}
The $\mathsf{rweq\_eq}$ constructor is still present and is the
mechanism that the contractibility proof actually uses.
We have not yet eliminated it.
The significance of $\mathsf{diamond\_filler}$ is that it provides
a \emph{structured} alternative whose generators come from
Knuth--Bendix critical pairs rather than from UIP collapse.
A future goal is to derive contractibility purely from
$\mathsf{diamond\_filler}$ instances and thereby eliminate the
blanket $\mathsf{rweq\_eq}$ axiom.

The resulting structure is thus a \emph{weak 2-groupoid with
contractible higher cells}: genuinely non-degenerate at levels
0--2 (where the TRS provides real structure), and contractible
above.  This contrasts with the Lumsdaine and van den
Berg--Garner constructions~\cite{Lumsdaine2010,vdBG2011}, which
produce genuinely non-degenerate structure at all levels in
intensional MLTT (where UIP is not available).  Our construction is
necessarily different because we build \emph{on top of} a UIP
foundation, obtaining genuine structure at levels 0--2 via the TRS
while the higher levels collapse via proof irrelevance on the
underlying $\Eq$.
\end{remark}
...
Third, contractibility at dimension~$\ge 3$ is derived via two
mechanisms: a constrained $\mathsf{diamond\_filler}$ constructor
that implements Squier-style finite derivation type structure from
critical-pair diamonds, and a blanket $\mathsf{rweq\_eq}$ constructor
whose soundness rests on proof irrelevance on the underlying $\Eq$
(see Remark~\ref{rem:rweq-eq-honesty}).
...
The computational-path framework extends naturally to higher-categorical
and algebro-geometric settings.
...
infrastructure for future development rather than substantive
...
At the 3-cell level, a constrained $\mathsf{diamond\_filler}$
constructor provides Squier-style structure from critical-pair joins,
while the blanket $\mathsf{rweq\_eq}$ constructor completes
contractibility via proof irrelevance.
...
Guiraud--Malbos.
(The blanket $\mathsf{rweq\_eq}$ constructor provides an alternative
...
\begin{enumerate}[label=(\roman*),nosep]
  ...
  \item A \textbf{weak $\omega$-groupoid structure} in the
    Batanin--Leinster sense~\cite{Batanin1998,Leinster2004}, with
    genuine non-degeneracy at levels 0--2 and contractible higher
    cells; contractibility at dimensions $\ge 3$ is derived from
    Church--Rosser confluence via Squier-style diamond fillers
    (from critical-pair joins), together with a blanket
    $\mathsf{rweq\_eq}$ constructor mediated by proof irrelevance
    on the underlying $\Eq$ (Remark~\ref{rem:rweq-eq-honesty})
    (\S\ref{sec:omega}).
