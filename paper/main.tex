...
paths}, initiated by de~Queiroz~\cite{deQueiroz1994} in the context of labelled
...
(HoTT)~\cite{HoTTBook}, where the identity type is endowed with a rich
...
deductive systems~\cite{deQueiroz1994}, which assigned explicit labels to proof
...
papers~\cite{deQueiroz2011,RamosQueiroz2016,RamosQueiroz2018} that progressively
...
Lean~4 proof assistant~\cite{Lean4}.  The formalization encompasses approximately
...
\item \emph{Constructivity.}  The univalence axiom in HoTT is not
  ...
  type theory~\cite{CCHM} provides a computational interpretation).  The
...
\begin{remark}[Comparison with the HoTT proof]\label{rem:hott-comparison}
...
\begin{enumerate}[label=(\roman*),nosep]
  ...
  \item \textbf{Extensions to 72+ mathematical domains}
    (\S\ref{sec:extensions}), with honest formalization status reporting
...
\small
\begin{tabular}{p{4.8cm}rp{5.2cm}c}
\toprule
