% ============================================================================
% The Calculus of Computational Paths — BSL Format
% ============================================================================
...
\Title[The Calculus of Computational Paths]{The Calculus of Computational Paths: \\
...
system is confluent and terminating (77 rules in eight groups), supports path
calculus and type-former $\beta/\eta$ laws, and yields a weak $\omega$-groupoid
...
We enumerate all 77 such rules in \cref{ch:rewrite-system}.
...
cells. Our 77-rule rewrite system on paths (\cref{ch:rewrite-system}) can be
...
\begin{description}[leftmargin=2em,style=nextline]
  \item[\cref{sec:basic-constructions}: Basic Constructions.]
    ...
    develop the path calculus for products, sums, dependent pairs, and function
  ...
  \item[\cref{ch:rewrite-system}: The Rewrite System.]
    We define the single-step rewrite relation $\Step$ with its 77 rules in
...
Context substitution supports an extensive calculus of identities
...
\begin{definition}[Single-Step Rewrite]\label{def:step-rewrite}
  ...
  the 77 rules organized into eight groups below. We write
  $\Step(p, q)$ or $p \rew q$ to denote that $p$ rewrites to $q$ in one step.
\end{definition}

\subsubsection{Group I: Path Calculus (8 rules)}
\label{sec:group-i}
...
\begin{proof}
  By induction on the derivation of $\Step(p, q)$. Each of the 77 rules
...
\begin{remark}[RwEq triviality in the UIP setting]\label{rem:rweq-triviality}
  ...
  form. The path calculus and groupoid operations at dimension~1 are
...
\begin{definition}[Rule Precedence]\label{def:rule-precedence}
  The 77 rewrite rules are assigned a numeric rank
...
\begin{proof}
  By case analysis on the 77 rules. Each rule either reduces the symbol
...
\begin{proof}
  ...
  join. The analysis covers all pairs among the 77 rules.
\end{proof}
...
\begin{remark}[Scale and tooling]\label{rem:critical-pair-tooling}
  The scale of the 77-rule system raises maintainability questions. We organize
...
\begin{proof}
  ...
  (the left and right transitivity cancellation rules) and shown confluent via a
  ...
  reduced word (the \emph{groupoid confluence} construction).
  %
  At the second level, the full 77-rule system is handled by lifting
  this groupoid confluence through a parameterized join-of-rewrite interface,
...
\begin{enumerate}[label=(\roman*)]
  \item Group~I: path calculus (unit, associativity, inverse, cancellation).
...
We now ascend from the one-dimensional calculus of paths to the
...
\begin{definition}[Anti-Involution]\label{def:anti-involution}
  The symmetry operation $\mathrm{inv}$ on the monoidal path calculus is an
...
Throughout this section, we refer extensively to the accompanying Lean~4
formalization, which is publicly available at:
\begin{center}
\fbox{\url{https://github.com/Arthur742Ramos/ComputationalPathsLean}}
\end{center}
The repository contains over 909 files and 217{,}000 lines of verified Lean
code. Readers are encouraged to consult the source for complete proof details;
formalization status for each component is summarized in
\cref{tab:formalization-status}.

\begin{remark}[Formalization status]
  This section condenses advanced applications from the Lean development.
  All results described here are fully formalized with zero \textup{sorry}
  declarations in the codebase. See
  \cref{tab:formalization-status}.
\end{remark}

The path-calculus and fundamental-group computations above are only the first
...
the fibration module of the formalization. One defines $\tilde{\gamma}$ by induction on the
...
the covering space module of the formalization) and the explicit action of $\pi_1(X,x_0)$ on the
...
path expressions. The classification is formalized in a dedicated module.
\end{proof}
...
The long exact sequence construction is formalized in the accompanying library, and the Hopf fibration example appears in
a dedicated module.
\end{proof}
...
quotients. The Hurewicz theorem and its prerequisites are formalized in the accompanying library.
\end{proof}
...
required fillers. The formalization appears in the excision module of the library.
\end{proof}
...
See the Mayer--Vietoris module of the formalization.
\end{proof}
...
computational-path formulation are formalized in the cobordism theory module.
\end{theorem}
...
The ring computation is formalized in the cobordism ring module.
\end{theorem}
...
the Bott periodicity and $K$-theory modules of the library.
\end{theorem}
...
framework. The construction is formalized in the algebraic $K$-theory module.
\end{theorem}
...
The construction appears in the spectral sequence module of the formalization.
\end{theorem}
...
Ext groups of the Steenrod algebra; the construction is
formalized in the Adams spectral sequence module.
\end{theorem}
...
decomposition, and the resulting Morse complex computes homology. The
Morse theory, Morse homology, and handle decomposition constructions
are formalized in the accompanying library.
\end{theorem}
...
expected long exact sequences. The sheafification, sheaf cohomology,
and derived sheaf modules are formalized in the library.
\end{theorem}
...
stable stems. The spectrum theory, stable homotopy, and stable stems
modules are formalized in the library.
\end{theorem}
...
groupoid-like models. The model category and Quillen adjunction
constructions are formalized in the library.
\end{theorem}
...
classified by maps into classifying spaces. The vector bundle,
principal bundle, and characteristic class constructions are
formalized in the library.
\end{theorem}
...
In Section~\ref{ch:rewrite-system}, we introduced the 77-rule
...
\begin{definition}[Typed Rewriting System]\label{def:typed-trs}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item A \emph{reduction relation} $\rew$ on terms of the same sort,
      given by the 77 rules of Section~\ref{ch:rewrite-system}.
  \end{enumerate}
...
$\mathrm{PathExpr}$, with the same 77 rules applied syntactically. The
...
The 77 rules generate a finite (though large) set of critical pairs.
...
\begin{definition}[Path Tactics]\label{def:path-tactics}
  ...
  \begin{itemize}
    ...
    \item \emph{Simplification} ($\mathit{path\_simp}$): applies the simplification
      procedure with the library of $\RwEq$ lemmas (all 77 rules and their
...
\midrule
Step definitions & The 77-rule $\mathrm{Step}$ relation and soundness proof \\
...
The 77 rules of the $\mathrm{LND}_{\mathrm{EQ}}$-TRS
(Section~\ref{ch:rewrite-system}), organized into eight groups---path
calculus, type-former $\beta$/$\eta$-rules, transport, contexts,
...
\midrule
Source files & 909 \\
Total lines of code & 217,668 \\
Definitions & 9,102 \\
Theorems and lemmas & 11,200 \\
Structures & 4,814 \\
Inductive types & 418 \\
\bottomrule
\end{tabular}
...
\caption{Formalization status of principal results. \emph{FF}: Fully
  Formalized (statement and proof checked; the codebase contains zero
  \textup{sorry} declarations).}
...
$\pi_1(S^1 \vee S^1) \cong \ZZ * \ZZ$ (Thm.~7.8) & \S\ref{sec:spaces} & FF \\
Seifert--van Kampen (Thm.~7.14) & \S\ref{sec:spaces} & FF \\
$\pi_1(\Sigma X) = 0$ (Thm.~7.16) & \S\ref{sec:spaces} & FF \\
\midrule
...
Hurewicz Theorem (Thm.~8.4) & \S\ref{sec:applications} & FF \\
Excision / Blakers--Massey (Thm.~8.5) & \S\ref{sec:applications} & FF \\
Mayer--Vietoris (Thm.~8.6) & \S\ref{sec:applications} & FF \\
\midrule
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{Interface-based confluence.} The confluence interface
    is a parameterized join-of-rewrite structure (not globally
    ...
    proved via groupoid confluence on
...
The current formalization of 217,668 lines covers a substantial portion
...
calculus of equality: by recording \emph{how} an equality was derived
...
We list all 77 rewrite rules of the $\mathrm{LND}_{\mathrm{EQ}}$-TRS,
...
\subsection{Group I: Path Calculus (8 rules)}
...
\begin{remark}
  The formalization defines 77 distinct constructors for the
  $\mathrm{Step}$ relation. The listing above enumerates rules R1--R74,
  where R74 ($\mathit{context\_congr}$) duplicates R31 for expository
  convenience. The remaining three constructors are the two algebraic
  cancellation rules ($\mathit{trans\_cancel\_left}$ and
  $\mathit{trans\_cancel\_right}$, listed as R75--R76 below) and the
  dependent sigma transport rule
  ($\mathit{subst\_sigmaMk\_dep\_beta}$, R77). All 77 rules are
  present as constructors in the formalization.
\end{remark}
