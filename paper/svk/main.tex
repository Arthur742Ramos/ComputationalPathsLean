...
\(\mathsf{Ho}(N\Pi(\mathcal P))\cong\Pi(\mathcal P)\).  Mathlib does not yet
provide the remaining general comparison between the topological fundamental
groupoid of a geometric realization and this simplicial homotopy category.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        names, but does not assume, the remaining general topological bridge.
    \end{itemize}
...
\]
requires a theorem comparing the topological fundamental groupoid of
\(\mathsf{SSet.toTop}(K)\) with the homotopy category of a Kan simplicial set.
That bridge is not available in Mathlib~v4.24.  We name its exact specialization
as \path{Presented.Realization.TopologicalComparisonStatement}, without adding
it as an axiom or typeclass hypothesis.  Theorem~\ref{thm:circle-realization}
proves the fundamental-group comparison directly for the circle.
