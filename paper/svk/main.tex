...
%% Long Lean identifiers must be allowed to wrap inside narrow table columns and
%% at the end of prose lines, but only at their natural boundaries.  Breaking is
%% permitted after `.', `_' and `-', and before `/'; never inside a word, so a
%% declaration name is either kept whole or split where a reader expects it.
%% In this paper \path is used exclusively for Lean names and module paths, so
%% it is given the same breaking policy.
...
\DeclareUrlCommand\leanname{\LeanBreakSetup}
\DeclareUrlCommand\path{\LeanBreakSetup}

%% Ragged-right paragraph columns keep prose in tables free of the very loose
%% interword spacing that justification forces in narrow measures.  Numeric
%% columns use LaTeX's fully stretchable \raggedleft so that short entries do
%% not register as underfull boxes.
\newcolumntype{L}[1]{>{\RaggedRight\arraybackslash}p{#1}}
\newcolumntype{R}[1]{>{\raggedleft\arraybackslash}p{#1}}

%% An inventory entry: the Lean declaration, with its module subordinated on a
%% second line.  Keeping both in one cell lets the declaration name occupy a
%% column wide enough to hold it without breaking mid-identifier.
\newcommand{\leanentry}[2]{%
  \leanname{#1}\newline
  {\scriptsize\itshape module\/}\enspace{\scriptsize\leanname{#2}}}
...
Our unconditional theorem concerns a proof-relevant presented path groupoid;
the corresponding theorem for the library's global rewrite quotient is stated
separately and is degenerate under the current rule system.
...
We additionally prove the space-level statement
\(\mathsf{CircleNerveRealization}\simeq_h\mathsf{TopologicalCircle}\):
the nerve realization of the presented circle is homotopy equivalent to the
ordinary additive circle.  The Lean construction uses explicit continuous maps,
an exact right inverse, and a globally compatible simplexwise homotopy for the
left inverse descended through the realization colimit.
...
Finally, the endpoint-varying quotient of geometric computational paths is
packaged as an unconditional quotient-compatible topological groupoid model.
Source, target, identity, reversal, and composition are continuous, and the
endpoint, unit, inverse, and associativity laws are proved.  Composition is
continuous on the final topology induced from composable representatives; the
ordinary inherited product/subspace topology has a separate theorem with an
explicit product-quotient compatibility hypothesis.

The distinction between this presented path groupoid and the library's global
\texttt{PathRwQuot} is explicit.  We isolate the exact mechanism by which
function-space rules make every loop fiber of the latter contractible, and read
it as a constraint on the design space: a trace-carrying equality cannot both
admit function-path constructors that erase traces and retain non-trivial loop
quotients.  Consequently the global-rule SVK theorem is degenerate unless
...
The thirty-one Lean modules in the formalization core described here comprise
\textbf{21,025 lines}.
The formalization contains no \texttt{sorry}, \texttt{admit}, or custom
\texttt{axiom} declaration.  The matching Lean artifact, build instructions,
and theorem manifest are archived as Zenodo version~0.4.2 at
\url{https://doi.org/10.5281/zenodo.21779801}; the corresponding source
release is
\url{https://github.com/Arthur742Ramos/ComputationalPathsLean/releases/tag/v0.4.2}.
\end{abstract}
...
\begin{enumerate}
    \item \textbf{Explicit witnesses}: Every computational path witness records an explicit rewrite trace showing how its endpoints are related.
...
\paragraph{Relation to Classical SVK} The classical theorem requires $X = U \cup V$ with $U, V, U \cap V$ open and path-connected. The HIT version generalizes this: the pushout $\Pushout(A, B, C, f, g)$ corresponds to the union where $f$ and $g$ specify how the ``intersection'' $C$ embeds into both spaces. Path-connectedness is used to state the single-basepoint amalgamated-product formula and to compare the relevant basepoints; in the HIT setting, this becomes the assumption that all types are path-connected.

\paragraph{Scope of the headline theorem.} The unconditional equivalence in
this paper is the theorem for the proof-relevant presented path groupoid
$\mathcal P(G_1,G_2;H)$. The global trace-carrying quotient
$\texttt{PathRwQuot}$ is treated separately: its nondegenerate SVK interface
remains conditional, while its unconditional full-target quotient collapses.
...
\begin{enumerate}
    \item Pushouts implemented via Lean's quotient types (\emph{no custom axiom declarations}), with \emph{modular typeclass assumptions} for computation rules:
    \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \path{FullAmalgamatedFreeProduct}: the \emph{full} amalgamated
        free product, which also reduces via free-group laws
    ...
    \begin{itemize}
        \item \path{isCoveringMap_circleCover} proves that
        ...
        \item \path{circlePresented_topologicalRealization_homotopyEquiv} proves
        the stronger ambient homotopy equivalence between the presented circle's
        nerve realization and \(\mathsf{AddCircle}\,1\).
    \end{itemize}

    \item A general simplicial and geometric realization:
    \begin{itemize}
        \item \path{Presented.Realization.nerve} constructs the nerve, and
        \path{Presented.Realization.topologicalRealization} its Mathlib
        ...
        comparison;
        \item \path{topologicalFundamentalGroupoidEquivalence} packages
        ...
        \path{exists_mem_realizationStar}, and
        \path{preimage_realizationStar_eq_iUnion_nerveCoreSheet} establish
        the quotient atlas, global open-star cover, and exact sheet
        decomposition;
        \item \path{isCoveringMap_nerveCoverMap} establishes the realized
        covering theorem.
    \end{itemize}

    \item An unconditional quotient-compatible topological groupoid model for
    endpoint-varying geometric computational paths:
    \begin{itemize}
        \item \path{unconditionalTopologicalGroupoidCertificate} packages
        continuity of source, target, identity, inverse, and composition;
        \item the same certificate proves both endpoint laws, both unit laws,
        both inverse laws, and associativity on quotient classes;
        \item \texttt{continuousPresentedTopological}\allowbreak
        \texttt{GroupoidCertificate}
        specializes the construction to every presented realization, while
        \texttt{continuousAmbientTopological}\allowbreak
        \texttt{GroupoidCertificate} transports it
        along any supplied ambient homotopy equivalence.
    \end{itemize}
    ...
    \begin{itemize}
        \item \path{HasPushoutSVKEncodeQuot}: encode map (assumed)
        \item \path{HasPushoutSVKDecodeEncode}: decode-encode round-trip (assumed)
        \item \path{HasPushoutSVKEncodeDecodeFull}: encode-decode round-trip
        modulo amalgamation and free-group reduction
        \item \path{HasPushoutSVKDecodeFullAmalgBijective}: Prop-level
        bijectivity alternative
        \item Decode map and both relation-respecting properties:
        \textbf{proved}
    \end{itemize}

    \item A proved normal-form equivalence
    \path{scopedSeifertVanKampenEquiv} for scoped expression completions,
    together with its nontrivial figure-eight instance.
    ...
    \begin{itemize}
        \item 21,025 lines across the 31 modules used by this paper
...
The Lean~4 formalization artifact matching this revision is archived as
Zenodo version~0.4.2:
\url{https://doi.org/10.5281/zenodo.21779801}.
It contains the 31 paper entry modules and their 61 transitive local
dependencies (92 Lean files total), the pinned Lean/Lake configuration, MIT
license, theorem manifest, invariant checker, and the manuscript and response
sources.  Its archive SHA-256 is
\texttt{80bba4a3e0d2853c}\allowbreak
\texttt{997fd43d5d909be2}\allowbreak
\texttt{426ce65ff6b0f9b4}\allowbreak
\texttt{14ed2b82181e62c}.
The standalone commands are
\texttt{lake update},
\texttt{lake build}, and \texttt{python3 scripts/check\_invariants.py}.
The corresponding source release is
\url{https://github.com/Arthur742Ramos/ComputationalPathsLean/releases/tag/v0.4.2}.
The Zenodo concept DOI for the archival series is
\url{https://doi.org/10.5281/zenodo.21696874}; the repository is
...
\begin{lstlisting}
inductive Step : {A : Type u} -> {a b : A} -> Path a b -> Path a b -> Type (u + 1)
...
\subsection{Metatheory of the \texorpdfstring{$\mathrm{LND}_{\mathrm{EQ}}$}{LND\_EQ}-TRS}
...
\begin{tabular}{@{}L{0.22\textwidth}L{0.15\textwidth}L{0.55\textwidth}@{}}
\toprule
...
\leanname{instHasTerminationProp}, via a lexicographic weight on
\leanname{GroupoidTRS.Expr} \\
Expression confluence & Proved & Library instance
\leanname{instHasConfluencePropExpr}, via reduced-word interpretation \\
Path-level confluence & Not globally instantiated &
\leanname{HasJoinOfRw}/\leanname{HasConfluenceProp}; a caller must supply join
...
\leanname{CompleteStepSystem}; then \leanname{CompleteStepSystem.decideRwEq}
is proved correct \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{remark}[Why the global quotient collapses, and what it rules out]
\label{rem:collapse}
The collapse is not an artifact of a particular carrier or of a stray rule; it
is forced by two features acting together, and the mechanism is short enough to
state.  First, \path{Path.lamCongr}, which packages a family of pointwise paths
into a single path between functions, records the empty trace.  Second, the
primitive application rule \path{Step.fun_app_beta}, instantiated at the unit
domain, makes that trace-free path a one-step predecessor of an \emph{arbitrary}
path with the same endpoints.  Because the ambient equality carried inside a
path is proof-irrelevant, the trace-free path depends only on its endpoints, so
the two facts collapse each fiber to a single $\RwEq$ class
(\path{rweq_total}).
We record this as a constraint on the design space rather than as a defect of
one implementation: \emph{a trace-carrying equality cannot simultaneously admit
function-path constructors that erase traces and retain non-trivial loop
quotients}.  The obstruction is independent of the space being studied, so no
choice of carrier repairs it.  This is what motivates
Section~\ref{sec:presented-spaces}, where generating edges are proof-relevant
and homotopy is generated only from named relators, and it is why the
conditional global-rule schema of Section~\ref{sec:svk} is presented separately
from the unconditional presented theorem.
\end{remark}
...
\begin{remark}[This is the standard combinatorial model]
\label{rem:presented-standard}
Definition~\ref{def:presented} is not an object introduced to make a theorem
come out right.  Items (1)--(3) are precisely the classical data of a
\emph{presentation of a groupoid}: a graph of generators, the free groupoid on
that graph, and a set of relators between parallel words
\cite{Brown2006,Higgins1971}.  The vertex group
\(\operatorname{Aut}_{\Pi(\mathcal P)}(a)\) is then the group presented by those
generators and relators, which for a presentation arising from a CW pair is the
fundamental group of the associated presentation complex.  Brown states the
Seifert--van Kampen theorem for fundamental \emph{groupoids} in exactly this
form, and Theorem~\ref{thm:svk} is its expected statement, not a weakened
substitute.  What is new here is that the raw paths carry explicit rewrite
traces, that homotopy is generated rather than assumed, and that both round
trips and the group homomorphism laws are machine-checked.  The circle
instance is pinned to Mathlib's topological fundamental group in
Theorem~\ref{thm:circle-realization}, which is an external check that the
presented notion is the intended one.
\end{remark}
...
\]
These are the declarations \path{Presented.Realization.nerve} and
\path{Presented.Realization.topologicalRealization}.
Full faithfulness of Mathlib's nerve functor gives the checked isomorphism
\[
...
The instance \path{topologicalComparisonFunctor_full} establishes the full
part of the topological edge-path theorem.  The instance
\path{topologicalComparisonFunctor_faithful} establishes faithfulness, and
...
calculation for the circle.  In addition, the concrete circle instance has a
space-level comparison with the ordinary additive circle, stated below.
...
\subsection{A quotient-compatible topological groupoid of computational paths}

Let \(\mathcal T_S\) be the total space of finite geometric computational
paths for a continuous step system \(S\), with varying source and target, and
let \(\mathcal H_S\) be its quotient by endpoint-fixed path homotopy.  We equip
\(\mathcal T_S\) with the topology generated jointly by source, target, trace
length, trace realization, and the underlying compact--open path.  The quotient
topology on \(\mathcal H_S\) makes source, target, identity, and reversal
continuous.

For composition, the unconditional domain is the final topology on composable
homotopy classes induced by the quotient map from composable representatives.
The declaration
\path{unconditionalTopologicalGroupoidCertificate} proves, without any
additional topological hypothesis, continuity of composition on this domain,
the source and target formulas, the left and right unit laws, the left and
right inverse laws, and associativity.  In particular, the result packages the
entire quotient-level groupoid algebra rather than only a representative-level
composition operation.  The declarations
\path{continuousPresentedTopologicalGroupoidCertificate} and
\path{continuousAmbientTopologicalGroupoidCertificate} instantiate it for the
canonical realization of every presentation and for every ambient space
equipped with a supplied homotopy equivalence from that realization.

There is an unavoidable distinction between this quotient-compatible domain
and the ordinary subspace topology inherited from
\(\mathcal H_S\times\mathcal H_S\).  Products of quotient maps need not be
quotient maps in arbitrary topological spaces.  Accordingly,
\path{continuous_quotientTransOnProduct} retains the explicit hypothesis
\path{ProductQuotientCompatibility}; the unconditional theorem does not claim
that generally false strengthening.  The continuous comparison map from the
quotient-compatible domain to the ordinary composable-pair space is part of
the certificate.
...
\path{CirclePresented.encode_mul}, \path{CirclePresented.encode_id}, and
\path{CirclePresented.encode_inv} prove that it preserves the group
operations.  Moreover,
...
\begin{proof}
Lean theorem \path{isCoveringMap_circleCover} constructs an evenly covered
...
\begin{theorem}[Ambient realization of the presented circle]
\label{thm:circle-ambient-realization}
Let \(\mathsf{CircleNerveRealization}\) be the Mathlib geometric realization of
the nerve of the presented circle groupoid.  Then
\[
\begin{gathered}
\mathsf{CircleNerveRealization}\simeq_h\mathsf{TopologicalCircle},\\
\mathsf{CircleNerveRealization}\simeq_h\mathbb R/\mathbb Z.
\end{gathered}
\]
\end{theorem}

\begin{proof}
The Lean declarations \path{circleNerveToAmbient} and
\path{circleAmbientToNerve} define the two continuous maps.  Their composite
from the ambient circle to the nerve and back is proved exactly equal to the
identity.  For the other composite, \path{circleLocalJoin} gives a
simplexwise join homotopy between the chosen generator edge and the original
simplex point; face and degeneracy naturality makes these homotopies compatible
on overlaps.  The compatible cocone descends through the Mathlib realization
colimit as \path{circleAmbientClassifyingSpaceHomotopy}, yielding the
unconditional definition \path{circlePresented_topologicalRealization_homotopyEquiv}.
No encode/decode parameter, ambient-space hypothesis, or custom axiom is used.
\end{proof}
...
In Lean, pushouts are implemented using quotient types (\textbf{no custom axiom declarations}):
\begin{lstlisting}
...
Assume \path{HasGlueNaturalLoopRwEq} at \(c_1\).  For any path
...
\path{glue_natural_square_rweq} in
...
\subsubsection{Bouquet of \texorpdfstring{$n$}{n} Circles}
...
A \emph{word} in the free product $G_1 \FreeProduct G_2$ is a tagged sequence
of elements from $G_1$ and $G_2$:
\begin{lstlisting}
...
The inductive constructors permit adjacent tags from the same side; reduced
normal forms are obtained only after the group operations and their reduction
relations are applied. Thus ``alternating'' describes the usual reduced-word
normal form, not the raw inductive word type.
...
-- Inversion
noncomputable def inverse [Neg G1] [Neg G2] :
    FreeProductWord G1 G2 -> FreeProductWord G1 G2
  | nil => nil
...
When $G_1$ and $G_2$ receive homomorphisms from a common group $H$, written $i_1 : H \to G_1$ and $i_2 : H \to G_2$, the \emph{amalgamated free product} $G_1 \AmalgProduct_H G_2$ identifies $i_1(h)$ with $i_2(h)$ for all $h : H$.
...
For the group-valued target, we define the \emph{full amalgamated free product}
by quotienting simultaneously by amalgamation and the free-product word
reductions implemented in the Lean source:

\begin{lstlisting}
inductive FreeGroupStep [Add G1] [Add G2] [Zero G1] [Zero G2] :
    FreeProductWord G1 G2 -> FreeProductWord G1 G2 -> Type (u + 1) where
  | combineLeft (x y : G1) (rest : FreeProductWord G1 G2) :
      FreeGroupStep (consLeft x (consLeft y rest))
                    (consLeft (x + y) rest)
  | combineRight (x y : G2) (rest : FreeProductWord G1 G2) :
      FreeGroupStep (consRight x (consRight y rest))
                    (consRight (x + y) rest)
  | removeLeftZero (rest : FreeProductWord G1 G2) :
      FreeGroupStep (consLeft 0 rest) rest
  | removeRightZero (rest : FreeProductWord G1 G2) :
      FreeGroupStep (consRight 0 rest) rest
  | congrLeft (x : G1) {w w' : FreeProductWord G1 G2} :
      FreeGroupStep w w' ->
      FreeGroupStep (consLeft x w) (consLeft x w')
  | congrRight (y : G2) {w w' : FreeProductWord G1 G2} :
      FreeGroupStep w w' ->
      FreeGroupStep (consRight y w) (consRight y w')

inductive FullAmalgEquiv {G1 G2 H : Type u}
    [Add G1] [Add G2] [Zero G1] [Zero G2]
    (i1 : H -> G1) (i2 : H -> G2) :
    FreeProductWord G1 G2 -> FreeProductWord G1 G2 -> Prop where
  | refl (w : FreeProductWord G1 G2) : FullAmalgEquiv i1 i2 w w
  | amalg {w1 w2 : FreeProductWord G1 G2} :
      AmalgRelation i1 i2 w1 w2 ->
      FullAmalgEquiv i1 i2 w1 w2
  | freeGroup {w1 w2 : FreeProductWord G1 G2} :
      FreeGroupStep w1 w2 ->
      FullAmalgEquiv i1 i2 w1 w2
  | symm {w1 w2 : FreeProductWord G1 G2} :
      FullAmalgEquiv i1 i2 w1 w2 ->
      FullAmalgEquiv i1 i2 w2 w1
  | trans {w1 w2 w3 : FreeProductWord G1 G2} :
      FullAmalgEquiv i1 i2 w1 w2 ->
      FullAmalgEquiv i1 i2 w2 w3 -> FullAmalgEquiv i1 i2 w1 w3

noncomputable def FullAmalgamatedFreeProduct
    (G1 G2 H : Type u) [Add G1] [Add G2] [Zero G1] [Zero G2]
    (i1 : H -> G1) (i2 : H -> G2) : Type u :=
...
\path{HasPushoutSVKEncodeQuot}; it is not a custom
...
\begin{itemize}
    ...
    \path{HasPushoutSVKDecodeEncode})
    \item $\encode(\decode(w)) \sim w$ up to amalgamation and free-group
    reduction (via
    \path{HasPushoutSVKEncodeDecodeFull})
\end{itemize}
...
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}L{0.15\textwidth}L{0.30\textwidth}L{0.21\textwidth}L{0.23\textwidth}@{}}
\toprule
...
\path{figureEightPresented_nontrivial} separates the left unit generator from
...
equivalences.
For the global-rule loop quotient, the separate declaration
\path{figureEightPiOneEquiv} is conditional on
...
\begin{proof}
The Lean theorem \path{sphereN_piOne_equiv_unit} is a specialization of
the universal collapse theorem \path{pathRwQuotLoopEquivUnit}, so it carries
no SVK encode/decode dependency.
\end{proof}
...
not prove a presented or topological
...
\leavevmode
\begin{itemize}
    \item Projective-plane expressions satisfy
      \(\mathsf{RP2ScopedQuotient}\simeq\langle a\mid a^2=1\rangle\cong\ZZ_2\).
    \item Klein-bottle expressions satisfy
      \(\mathsf{KleinScopedQuotient}\simeq
       \langle a,b\mid aba^{-1}b=1\rangle\cong\ZZ\rtimes\ZZ\).
\end{itemize}
\end{example}
...
\subsection{Bouquet of \texorpdfstring{$n$}{n} Circles}
...
\begin{definition}[Formal higher-homotopy interface]
The intended $n$-th homotopy group is obtained from an $n$-fold iterated
loop-space construction. In the present formalization, the nontrivial higher
construction is the explicitly defined second group
\[
\piTwo(A,a)=\mathsf{Loop2Space}(A,a)/\mathsf{Loop2Eq}.
\]
The uniform Lean interface \texttt{HigherHomotopy.PiN} exposes the index $n$;
its branches for $n\geq 3$ currently return a unit placeholder and require
the missing higher-cell infrastructure. No general $\pi_n$ theorem is claimed
here beyond the proved $\pi_2$ result below.
\end{definition}

\begin{theorem}[Eckmann--Hilton for $\pi_2$]
The formally defined second homotopy group $\piTwo(A,a)$ is abelian.
\end{theorem}

\begin{proof}
The Eckmann--Hilton argument shows that when two binary operations share a
common unit and satisfy an interchange law, they must coincide and be
commutative. In the explicitly constructed $\Omega^2(A,a)$, horizontal and
vertical composition of 2-loops satisfy these conditions.
\end{proof}

In Lean:
\begin{lstlisting}
-- The explicitly formalized nontrivial higher level is pi_2.
...
\subsection{Weak groupoid structure through level 3}

\begin{proposition}[Truncated weak groupoid structure]
Via computational paths, the current development provides the following
weak-groupoid data through level 3:
\begin{itemize}
...
\end{itemize}
The uniform higher-cell tower beyond level 3 is not part of the present
formalization and remains future work.
\end{proposition}
...
\begin{definition}[Low-degree fibration sequence]
For a type-family fibration $p : E \to B$ with fiber $F$, the development
constructs the maps in the low-degree sequence
\[
\cdots \to \piTwo(F) \to \piTwo(E) \to \piTwo(B)
\to \piOne(F) \to \piOne(E) \to \piOne(B)
\to \pi_0(F) \to \pi_0(E) \to \pi_0(B).
\]
It proves the relevant pointed adjacent-composition laws. A complete
image-equals-kernel exactness theorem for this sequence is not claimed and
remains future work.
\end{definition}

\texttt{Pi0} is defined as the quotient by path-relatedness. The Lean
development packages the low-degree maps and their pointed
adjacent-composition laws in
...
terminal \(1\)'s may be used as shorthand. The separate
\texttt{LongExactSequencePi1} structure likewise records the relevant
composition-trivial consequences at the \(\pi_1\) level; its name does not
assert full image-kernel exactness for the displayed tower.
...
comment-stripped lexical count.  None of the 20,389 declarations introduced with
\texttt{theorem}/\texttt{lemma} directly invokes a \texttt{path\_*} tactic.
Across all 54,249 declarations counted (including \texttt{def},
\texttt{instance}, and \texttt{example}), 11 invoke one directly
(approximately \(0.020\%\)).  In the thirty-one modules used by this paper,
one of 1,465 declarations does so (\(0.068\%\)).  Thus the tactics are
...
\begin{enumerate}
    \item \textbf{Naturality}: \path{HasGlueNaturalLoopRwEq} (glue commutes with paths)
    \item \textbf{Nondegenerate encode/decode data}:
    \begin{itemize}
        \item \path{HasPushoutSVKEncodeQuot}
        \item \path{HasPushoutSVKDecodeEncode}
        \item \path{HasPushoutSVKEncodeDecodeFull}
    \end{itemize}
\end{enumerate}
\end{definition}
...
\midrule
\leanname{HasGlueNaturalLoopRwEq} & Required by the general decode lift;
proved for wedges and other named cases.  Under the global \(\RwEq\) relation,
\leanname{hasGlueNaturalLoopRwEq_collapsed} discharges it generally. \\
\leanname{HasPushoutSVKEncodeQuot} & When you have an explicit encode function. \\
\leanname{HasPushoutSVKDecodeEncode} & Standard encode-decode round-trip. \\
\leanname{HasPushoutSVKEncodeDecodeFull} & Round-trip modulo amalgamation and
free-group reduction. \\
\leanname{HasPushoutSVKDecodeFullAmalgBijective} & Choice-based full-target
...
The Lean 4 formalization is organized as follows.  Every module listed below
is relative to \path{ComputationalPaths/Path/} and carries the \path{.lean}
extension.
...
\caption{Formalization architecture.}\label{tab:architecture}\\
\toprule
...
\endhead
\midrule
\multicolumn{2}{r@{}}{\footnotesize\emph{continued on the next page}} \\
\endfoot
\bottomrule
\endlastfoot
\leanname{Basic/Core} & Path type, refl, symm, trans \\
\leanname{Basic/Congruence} & congrArg, transport \\
\leanname{Rewrite/Step} & Single-step rewrites (50+ rules) \\
\leanname{Rewrite/RwEq} & Rewrite equality \\
\leanname{Rewrite/ScopedCompletion} & Presentation-specific completion relations \\
\leanname{Rewrite/PathTactic} & Automated tactics \\
\leanname{Homotopy/Loops} & Loop spaces \\
\leanname{Homotopy/FundamentalGroup} & $\piOne$ definition \\
\leanname{Homotopy/PresentedFundamentalGroup} &
Raw presented paths, generated homotopy, and \(\piOne^{\mathrm{pres}}\) \\
\leanname{Homotopy/PresentedGroupoidRealization} &
Nerve, geometric realization, and homotopy-category recovery \\
\leanname{Homotopy/TopologicalNerve} &
Realized vertices and edges, functor laws, and essential surjectivity \\
\leanname{Homotopy/TopologicalNerveContractible} &
Quotient simplex atlas and contractions of nerves with an initial object \\
\leanname{Homotopy/TopologicalRealizationOpen} &
Descent of compatible open simplex families to realization \\
\leanname{Homotopy/TopologicalSimplexStar} &
Degeneracy-aware open core-face stars for realized-cover charts \\
\leanname{Homotopy/TopologicalNerveCover} &
Global lifted sheets, homeomorphisms, and the realized covering map \\
\leanname{Homotopy/TopologicalNerveComparison} &
Path/homotopy lifting, full faithfulness, and the edge-path equivalence \\
\leanname{Topology/TopologicalCompPathGroupoidLaws} &
Unconditional quotient-compatible continuity and all groupoid laws \\
\leanname{Homotopy/PresentedContinuousGeometricAdapter} &
Presented-realization specialization of the topological groupoid certificate \\
{\scriptsize\leanname{Homotopy/PresentedAmbientContinuousGeometricAdapter}} &
Ambient-homotopy-equivalence specialization of the certificate \\
\leanname{Homotopy/HigherHomotopy} & Explicit $\piTwo$; indexed $\piN$ interface with unit placeholders for $n \geq 3$ \\
\leanname{CompPath/PushoutCompPath} & Quotient pushout and glue naturality \\
\leanname{CompPath/PushoutPaths} & Word quotients, decode, conditional full SVK \\
\leanname{CompPath/PushoutSVKInstances} & Obstructions and collapsed full SVK \\
\leanname{CompPath/ScopedSeifertVanKampen} & Scoped SVK encode/decode equivalence \\
\leanname{CompPath/PresentedSeifertVanKampen} &
Presented SVK and figure-eight theorem \\
\leanname{CompPath/CirclePresented} &
Presented circle fundamental group \(\simeq\ZZ\) \\
\leanname{CompPath/CircleTopologicalRealization} &
Covering map, topological winding, and circle comparison \\
\leanname{CompPath/CircleNerveAmbient} &
Explicit ambient circle maps and realization homotopy equivalence \\
\leanname{CompPath/CircleScoped} & Circle and torus scoped completions \\
\leanname{CompPath/ClassicalPresentationsScoped} & Klein, projective, and lens completions \\
\leanname{CompPath/FigureEight} & Conditional and provenance interfaces \\
\leanname{CompPath/SuspensionDeep} & Iterated spheres under the global relation \\
\leanname{Homotopy/Fibration} & $\pi_1$ sequence and $\pi_0$ tail \\
\leanname{TypeTheory/QuotientPathInduction} & Universal global-loop collapse \\
\leanname{TypeTheory/MetadataRepair} & Global/scoped no-bridge results \\
\leanname{OmegaGroupoid} & Weak-groupoid data through level 3 \\
\end{longtable}
}
...
\begin{center}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}L{0.58\textwidth}R{0.34\textwidth}@{}}
\toprule
\textbf{Metric} & \textbf{Value} \\
\midrule
Lean lines in the 31-module formalization core & 21,025 \\
Modules in the formalization core & 31 \\
Custom \texttt{axiom} declarations & 0 \\
\texttt{sorry}/\texttt{admit} occurrences after comment stripping & 0/0 \\
Reproduction script & \leanname{paper/svk/check_stats.py} \\
\bottomrule
\end{tabular}
\end{center}
...
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}L{0.42\textwidth}L{0.20\textwidth}L{0.30\textwidth}@{}}
\toprule
\textbf{Typeclass} & \textbf{Role} & \textbf{Status} \\
\midrule
\leanname{HasGlueNaturalLoopRwEq} & Glue naturality & Proved for named
cases; generally derivable in the collapsed system \\
\leanname{HasPushoutSVKEncodeQuot} & Nondegenerate encode map & Open parameter \\
\leanname{HasPushoutSVKDecodeEncode} & Decode--encode & Open parameter \\
\leanname{HasPushoutSVKEncodeDecodeFull} & Full encode--decode & Open parameter \\
\leanname{HasPushoutSVKDecodeFullAmalgBijective} & Alternative full-decode
...
The library exposes a uniform $\piN$ interface, but its substantive
higher-homotopy construction in this paper is the explicitly defined $\piTwo$.
For $n \geq 3$, the current \texttt{PiN} branch is a unit placeholder pending
the missing higher-cell infrastructure.  Key results include:
\[
\piTwo(A,a) = \mathsf{Loop2Space}(A,a)/\mathsf{Loop2Eq}
\]
\begin{itemize}
    \item Abelianness of $\piTwo$ via Eckmann-Hilton (\texttt{HigherHomotopy.lean})
    \item Low-degree fibration maps and pointed adjacent-composition laws
    (\texttt{Fibration.lean})
\end{itemize}

\subsection{Weak groupoid structure through level 3}

The current development records weak-groupoid data through level 3 via the
derivation tower:
\begin{itemize}
    \item 1-cells: paths $\Path(a, b)$
    \item 2-cells: derivations $\Der_2(p, q)$ witnessing $p \RwEq q$
    \item 3-cells: derivations $\Der_3(d_1, d_2)$ between derivations
    \item Higher uniform cells beyond level 3 are not claimed
    (\texttt{OmegaGroupoid.lean})
\end{itemize}
...
\begin{itemize}
    ...
    classical benchmark statements (\path{EilenbergMacLane.lean}); no
    carrier-level \(S^1=K(\ZZ,1)\) theorem is claimed here
    \item \textbf{Mayer-Vietoris}: Abelianized SVK sequence
    (\path{MayerVietoris.lean})
    \item \textbf{Path tactics}: \path{path_simp}, \path{path_auto}, and
    \path{path_normalize} (\path{PathTactic.lean})
\end{itemize}
...
\begin{enumerate}
    ...
    \item The thirty-one modules used by the paper comprise 21,025 lines, and the
...
Weak groupoid data & Higher identity types & Derivation tower through level 3 \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
    \item \textbf{Scope of the realization theorem}: The general comparison
    identifies a presented path groupoid with the topological fundamental
    groupoid of the genuine Mathlib realization of its own simplicial nerve.
    It does not identify the library's global \texttt{PathRwQuot} with that
    topological groupoid, nor does it assert that an arbitrary ambient space
    supplied with the same presentation is homotopy equivalent to the nerve
    realization.  The concrete presented-circle case is now closed by
    Theorem~\ref{thm:circle-ambient-realization}; the general ambient-space
    comparison remains open.

    \item \textbf{Topology on composable quotient paths}: The unconditional
    computational-path groupoid uses the quotient-compatible final topology
    induced from composable representatives.  We do not identify it in
    arbitrary \texttt{Top} with the subspace topology inherited from the
    product of two quotient arrow spaces.  The latter comparison requires the
    explicit \path{ProductQuotientCompatibility} hypothesis because products
    of quotient maps are not quotient maps in general.
...
\begin{enumerate}
    ...
    \item \textbf{From presentations to ambient spaces}: Beyond the verified
    presented-circle case, compare canonical nerve realizations for presentations
    extracted from concrete CW decompositions with their original ambient spaces,
    and extend the present fundamental-groupoid result to higher homotopy
    invariants.

    \item \textbf{Uniform higher homotopy and exactness}: Replace the
    $n\geq3$ placeholder branches of \texttt{HigherHomotopy.PiN} with genuine
    higher-cell quotients, and strengthen the low-degree fibration maps to a
    full image-kernel exactness theorem.
...
\newblock Ergebnisse der Mathematik und ihrer Grenzgebiete 35. Springer, 1967.
...
\bibitem{Higgins1971}
Philip J. Higgins.
\newblock {\em Categories and Groupoids}.
\newblock Van Nostrand, 1971.
\newblock Reprinted in {\em Reprints in Theory and Applications of Categories},
7:1--195, 2005.
...
\newblock Lean 4 formalization artifact for the Seifert--van Kampen theorem via computational paths, version 0.4.2.
\newblock Zenodo, 2026. \url{https://doi.org/10.5281/zenodo.21779801}.
...
listed as distinct objects.  There are no opt-in axiom imports in the source
tree.
...
\setlength{\LTpre}{0.6\baselineskip}
\setlength{\LTpost}{0.6\baselineskip}
\begin{longtable}{@{}L{0.23\textwidth}L{0.46\textwidth}L{0.20\textwidth}@{}}
\toprule
\textbf{Object/result} & \textbf{Lean declaration and module} &
\textbf{Status} \\
\midrule
\endfirsthead
\toprule
...
\endlastfoot
Presented circle fundamental group \(\simeq\ZZ\) &
\leanentry{CirclePresented.piOneEquivInt}{CirclePresented} &
Proved nontrivial \\
Presented/topological circle group comparison &
\leanentry{presentedCircleTopologicalPiOneGroupEquiv}{CircleTopologicalRealization} &
Proved group equivalence \\
Circle nerve realization / ambient circle &
\leanentry{circlePresented_topologicalRealization_homotopyEquiv}{CircleNerveAmbient} &
Proved unconditional homotopy equivalence \\
General nerve recovery &
\leanentry{Presented.Realization.hoNerveIso}{PresentedGroupoidRealization} &
Proved category isomorphism \\
General topological fundamental-groupoid comparison &
\leanentry{Presented.Realization.topologicalFundamentalGroupoidEquivalence}
  {PresentedGroupoidRealization} &
Proved category equivalence \\
Topological computational-path groupoid &
{\scriptsize\leanentry{unconditionalTopologicalGroupoidCertificate}
  {TopologicalCompPathGroupoidLaws}} &
Proved unconditionally for the quotient-compatible composition topology \\
General presented SVK group equivalence &
\leanentry{presentedSeifertVanKampenGroupEquiv}{PresentedSeifertVanKampen} &
Proved \\
Presented figure-eight fundamental group &
\leanentry{figureEightPresentedPiOneGroupEquiv}{PresentedSeifertVanKampen} &
Proved nontrivial \\
Every global-rule loop quotient \(\simeq 1\) &
\leanentry{pathRwQuotLoopEquivUnit}{QuotientPathInduction} &
Proved \\
Iterated sphere global-rule loop quotient \(\simeq 1\) &
\leanentry{sphereN_piOne_equiv_unit}{SuspensionDeep} &
Proved; no SVK \\
Scoped circle expression quotient \(\simeq\ZZ\) &
\leanentry{circleScopedPiOneEquivInt}{CircleScoped} &
Proved nontrivial \\
Scoped torus expression quotient \(\simeq\ZZ^2\) &
\leanentry{torusScopedPiOneEquivIntProd}{CircleScoped} &
Proved nontrivial \\
Scoped figure-eight SVK equivalence &
\leanentry{figureEightScopedSVKEquiv}{ScopedSeifertVanKampen} &
Proved nontrivial \\
General scoped SVK equivalence &
\leanentry{scopedSeifertVanKampenEquiv}{ScopedSeifertVanKampen} &
Proved \\
Nondegenerate full SVK schema &
\leanentry{seifertVanKampenFullEquiv}{PushoutPaths} &
Four displayed classes \\
Collapsed full SVK schema &
\leanentry{seifertVanKampenFullEquiv_collapsed}{PushoutSVKInstances} &
Proved, degenerate \\
Amalgamation-only target obstruction &
\leanentry{hasPushoutSVKEncodeDecode_impossible}{PushoutSVKInstances} &
Proved \\
Scoped Klein presentation \(\simeq\ZZ\rtimes\ZZ\) &
\leanentry{kleinBottleScopedPiOneEquivIntProd}{ClassicalPresentationsScoped} &
Proved nontrivial \\
Scoped \(\RP^2\) presentation \(\simeq\ZZ_2\) &
\leanentry{realProjective2ScopedPiOneEquivZ2}{ClassicalPresentationsScoped} &
Proved nontrivial \\
Scoped lens presentation \(\simeq\mathrm{Fin}\,p\) &
\leanentry{lensScopedPiOneEquivZp}{ClassicalPresentationsScoped} &
Proved for \(p>0\) \\
Bouquet candidate decode &
\leanentry{BouquetN.decode_def}{BouquetN} &
Equivalence open \\
\(\pi_0\) fibration tail &
\leanentry{lowDegreeFibrationSequence}{Fibration} &
Proved maps/laws \\
\end{longtable}
}
