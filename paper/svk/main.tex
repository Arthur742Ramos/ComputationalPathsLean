...
\(\mathsf{Ho}(N\Pi(\mathcal P))\cong\Pi(\mathcal P)\).  The remaining comparison
with the topological fundamental groupoid of that realization is classical, but
is not yet available in Mathlib; no result in this paper assumes it.
...
The twenty-one Lean modules in the formalization core described here comprise
\textbf{12,653 lines}.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        names, but does not assume, the remaining topological bridge, which is
        classical~\cite{GabrielZisman1967} but not yet formalized in Mathlib.
    \end{itemize}
    ...
    \begin{itemize}
        \item 12,653 lines across the 21 modules used by this paper
...
\]
is classically true and standard.  Gabriel and Zisman construct a fundamental
groupoid functor \(\pi:\mathbf{sSet}\to\mathbf{Grpd}\), left adjoint to the
nerve, and prove \(\Pi_1^{\mathrm{top}}(\lvert K\rvert)\cong\pi(K)\) for every
simplicial set \(K\)~\cite{GabrielZisman1967}.  Since the nerve is fully
faithful, the counit \(\pi N\Rightarrow\mathrm{id}\) is an isomorphism, so
\(\pi(N(\mathcal P))\cong\Pi(\mathcal P)\); composing the two gives the display.
The obstruction is therefore one of formalization rather than of mathematics:
Mathlib~v4.24 supplies the nerve, the geometric realization, and the topological
fundamental groupoid, but not the comparison between them.  We name the exact
specialization that would be needed as
\path{Presented.Realization.TopologicalComparisonStatement} and deliberately do
not assume it as an axiom or as a typeclass hypothesis, so no result in this
paper depends on it.  Theorem~\ref{thm:circle-realization} discharges the
fundamental-group comparison for the circle directly, without that bridge.
...
comment-stripped lexical count.  None of the 19,967 declarations introduced with
\texttt{theorem}/\texttt{lemma} directly invokes a \texttt{path\_*} tactic.
Across all 53,451 declarations counted (including \texttt{def},
\texttt{instance}, and \texttt{example}), 11 invoke one directly
(approximately \(0.021\%\)).  In the twenty-one modules used by this paper,
one of 1,018 declarations does so (\(0.098\%\)).  Thus the tactics are
...
\midrule
Lean lines in the 21-module formalization core & 12,653 \\
Modules in the formalization core & 21 \\
...
\begin{enumerate}
    ...
    \item The twenty-one modules used by the paper comprise 12,653 lines, and the
...
\begin{enumerate}
    ...
    We do not yet formalize the identification of the topological fundamental
    groupoid of that realization, including the presented figure-eight case.
    This is a formalization gap rather than a mathematical one: the comparison
    is classical~\cite{GabrielZisman1967}, but the supporting simplicial
    homotopy theory is not yet in Mathlib~v4.24.  No result in this paper
    assumes it.
