...
terminating normalization plus semantic separation.  As validation,
finite-generator based-loop presentations for the circle and torus recover
$\mathbb Z$ and $\mathbb Z^2$.  A Lean 4.24.0 development checks the theorem
package; the mathematical presentation is independent of the implementation.
\end{abstract}
...
\item We prove functoriality, the realization comparison, the universal
  presentation theorem, and an effective normal-form criterion for geometric
...
\item We apply the normal-form criterion on the based-loop fibers of the
  ...
  fundamental groups.  A Lean 4.24.0 artifact checks the theorem package while
  the mathematical arguments remain independent of the implementation.
\end{enumerate}
...
the corresponding factor by the constant path at its endpoint; the stated
...
For reversal, reverse the homotopy parameter.  Unit, associativity,
...
Earlier work develops the calculus, its higher coherence, and particular
...
topological application, Lean development, and Seifert--van Kampen preprint
provide geometric and formal case studies
\cite{DeVerasRamosDeQueirozDeOliveira2025topological,
RamosEtAl2025LeanPreprint,RamosEtAl2025SVKPreprint}.  The precise distinction
is that this paper supplies the general continuous quotient semantics and the
comparison between the final and ordinary composable topologies, together
with the universal-property and completeness interfaces needed to host those
calculations.
...
The Lean artifact now checks the transfer argument in
\LeanName{ScopedGeometricRewriteHawaiianEarring}:
...
Hawaiian-earring transfer &
\LeanName{QuotientObstructionTransfer},
\LeanName{QuotientObstructionTransfer.external_isQuotientMap_of_source_isQuotientMap},
\LeanName{QuotientObstructionTransfer.not_isQuotientMap_quotientMap},
\LeanName{MultiplicationComparison},
\LeanName{MultiplicationComparison.not_source_continuous_of_not_external_continuous},
\LeanName{ScopedGeometricRewrite.not_productQuotientCompatibility_of_transfer},
\LeanName{ScopedGeometricRewrite.not_final_topology_agreement_of_transfer}\\
...
\href{https://doi.org/10.5281/zenodo.21797011}
{\nolinkurl{doi:10.5281/zenodo.21797011}} and corresponds to the immutable tag
\texttt{topological-paper-v2} in the
...
final/ordinary distinction can be strict.  The effective normal-form criterion
