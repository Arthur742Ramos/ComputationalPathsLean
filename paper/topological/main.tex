...
The realization map to geometric homotopy classes is always continuous and is
faithful exactly under geometric completeness.  A continuous section identifies the
...
The novelty claim is deliberately architectural.  Quotient factorization,
continuous-bijection criteria, and the classical circle and torus winding
classifications are standard ingredients in isolation.  The contribution here
is their integration with scoped computational syntax: the same coherent
carrier supports a trace-sensitive and an observable topology, the quotient of
explicitly composable representatives supplies an unconditional multiplication
domain, and the exact comparison with the ordinary pullback topology records
when that domain can be replaced.  The semantic factorization and normal-form
theorems then separate what is proved by the rewrite calculus from what must be
shown geometrically.  Thus the circle and torus sections validate the general
interface; they are not presented as new computations of classical
fundamental groups.
...
\begin{example}[A genuine quotient-topology separation]
\label{ex:quotient-topology-separation}
Let $X=\{*\}$ have its unique topology, let
$E=\{e,f\}$ be discrete, and let both primitive steps have source and target
$*$ and realization the constant path $c_*$.  Declare no named rewrite
relating the one-letter traces $e$ and $f$; only the structural groupoid rules
are present.  Write
\[
  u_e=(e,c_*),\qquad u_f=(f,c_*),
\]
for the corresponding coherent points.  The trace-sensitive carrier is
discrete in this example: every finite word stratum is finite discrete, and
the path and coherence coordinates are singletons.  Hence the distinct
scoped classes $q_{\mathrm{tr}}(u_e)$ and $q_{\mathrm{tr}}(u_f)$ can be
separated.
...
therefore topologically indistinguishable in $T_{\mathrm{obs}}$.  To make the
remaining scoped distinction explicit, let
\[
  \Phi:\{e^{\pm1},f^{\pm1}\}^{*}\longrightarrow F(e,f)
\]
be the word-evaluation map into the free group on $e$ and $f$.  The structural
unit, associativity, double-inverse, and cancellation coherences preserve
$\Phi$, whereas $\Phi(e)=e\neq f=\Phi(f)$ in $F(e,f)$.  Since there is no
named rewrite relating $e$ and $f$, their scoped classes remain distinct, but
the singleton $\{q_{\mathrm{obs}}(u_e)\}$ is not open.  Consequently the
continuous bijection
\[
  J:G_{\mathcal P}^{\mathrm{tr}}\longrightarrow
    G_{\mathcal P}^{\mathrm{obs}}
\]
from Proposition~\ref{prop:trace-sensitive-refinement} is not a
homeomorphism.  Thus the distinction between the two representative
topologies survives the scoped quotient; it disappears only under additional
hypotheses, such as the universal section of Section~7.
\end{example}
...
\end{aligned}
\]
Thus every zero-weight factor has coincident endpoints and is endpoint-fixed
homotopic to the constant path at that endpoint.  For arbitrary
...
Let $G\rightrightarrows X$ be a groupoid whose arrow space and structure maps
are topological, and let $C_{\mathrm{fin}}$ be a specified topology on the set
of composable arrow pairs $G\times_XG$.  We call this data a
\emph{final-domain topological groupoid} when multiplication
\[
  m:C_{\mathrm{fin}}\longrightarrow G
\]
is continuous and the remaining groupoid laws hold.  The designated topology
is part of the structure; it need not be the subspace topology inherited from
the ordinary product $G\times G$.
\end{definition}
...
Accordingly, the phrase ``topological groupoid'' without qualification is
 ...
 proved.  The unconditional object of Theorem~3.2 is instead called a
...
based-endpoint coherent carrier
\[
  T_{\mathcal U_X}^{*}(a,b)=
...
\]
is a bijective homeomorphism.  In particular the scoped presentation is
...
For the actual topological torus, the quotient $L_{\mathbb T^2}$ satisfies
\[
...
derivation, so the normal-form criterion proves geometric completeness.  The
fiberwise quotient construction identifies $L_{\mathbb T^2}$ with the based
geometric quotient, and the quotient-factorization argument in
 Theorem~\ref{thm:faithfulness-homeomorphism} restricts to the fixed endpoint
 fiber.  It
therefore supplies the continuous inverse $S_{\mathbb T^2}$, proving the
asserted bijective homeomorphism.
\end{proof}
...
computational traces at the presented level.
...
The finite two-generator separation mechanism from
Example~\ref{ex:quotient-topology-separation} is checked independently in
\LeanName{TraceSensitiveSeparation}: its certificate proves that the one-letter
trace codes differ, that the observable codes agree, and that the identity from
the discrete trace topology to the indiscrete observable topology is continuous
while the reverse identity is not.  This is a finite code-level certificate
for the example's topological mechanism; the mathematical statement that the
scoped quotient retains two reduced one-letter classes uses the presentation's
reduced-word invariant.
...
The Lean core artifact, version \texttt{0.6.1+lean-only-v3}, is archived on Zenodo
under the version DOI
\href{https://doi.org/10.5281/zenodo.21938980}
{\nolinkurl{doi:10.5281/zenodo.21938980}}; its permanent concept DOI is
