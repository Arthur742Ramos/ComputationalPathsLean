...
A representative pairs a trace with a coherent interval path.  An initial
topology records endpoints, word length, trace realization, and the chosen
path.  Named rewrites alone generate syntactic identifications; arbitrary
endpoint-fixed homotopies enter through a comparison map.
...
fundamental groupoid.  Normal-form certificates reduce completeness to
normalization plus semantic separation.  As validation,
...
semantic framework and finite-generator presentation data; the mathematical
winding reductions are stated independently of the implementation.
\end{abstract}
...
flat word carrier, $T$ adds a coherent ambient representative and its
observable topology, and $G_{\mathcal P}$ is the scoped quotient arrow space.
...
\item We define continuous geometric rewrite presentations, their scoped
  derivation relation, and realization soundness.  This supplies the syntax
  and the semantic induction principle for the rest of the paper.
...
\item We prove functoriality, the realization comparison, the universal
  ...
  completeness.  The comparison preserves the final composable-domain
  operation, not only a set-level map on arrows.
\item We apply the normal-form criterion on the based-loop fibers of the
  ...
  interfaces and finite-generator presentation data; the winding arguments
  remain independent of the implementation.
\end{enumerate}
...
influences the representative topology and defines the raw realization map,
...
$\kappa$ continuous.  Operationally, convergence in $T$ is detected through
these observables: the endpoints control the groupoid base, the discrete
length records the word stratum, $|p|$ observes the movement of the
computational realization, and $\gamma$ observes the movement of the chosen
geometric representative.  The initial topology is exactly the coarsest
topology with these observations, while the scoped quotient relation below
controls which computational traces are identified.

The word ``coarsest'' is the relevant canonicity claim.  It is relative to the
chosen semantic observables, not a claim that no other useful topology exists.
For example, the subspace topology inherited from
$\mathsf{Tr}_{\mathcal P}\times X^I$ would additionally make the complete word
$p$ observable.  The topology used here deliberately retains only the
endpoints, length, realization, and coherent representative needed by the
semantic operations.  Among topologies on the same carrier making this code
continuous, the initial topology is minimal.
...
\]
Then $(p,\gamma)$ and $(p,\delta)$ are both points of $T$.  The initial
...
% Alt text: A commutative square compares the quotient of explicitly composable
% representatives with the ordinary pullback of quotient arrows.  The upper
% quotient maps to the lower pullback through the canonical map j.
...
topology.  The realization comparison and its composable-domain map are
\[
...
inverse coherences in Definition~2.6 instantiate to
...
The Lean artifact now checks the transfer argument in the dedicated
...
integer-pair trace codes, packages the corresponding finite-carrier completion
certificates, and proves soundness of the torus commuting square.  The
...
The preceding Lean artifact snapshot is archived as
\href{https://doi.org/10.5281/zenodo.21816790}
{\nolinkurl{doi:10.5281/zenodo.21816790}} and corresponds to the immutable tag
\texttt{topological-paper-v3} in the
\href{https://github.com/Arthur742Ramos/ComputationalPathsLean}{GitHub repository}.
This revision adds the finite-generator module and the torus
simultaneous/sequential bridge, so those changes require a new immutable tag
and archive before submission.  The literal root reproducibility command is
\texttt{lake build}; focused module commands are listed in the artifact README
...
revision, reference auditing, and software and documentation support.  The
authors reviewed the resulting text and take responsibility for the final
content.
