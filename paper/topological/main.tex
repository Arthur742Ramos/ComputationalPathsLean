...
fundamental groupoid.  We then give finite-generator circle and genuine torus
...
\item We construct the quotient arrow space and its canonical final-domain
  topological groupoid, then isolate the ordinary-pullback issue.  The exact
  four-way criterion, together with the compact-Hausdorff and discrete
  sufficient conditions, identifies precisely when the two composable-pair
  topologies agree.
...
\item We apply the criterion to the additive circle and the genuine
  topological torus, obtaining $\Z$ and $\Z^2$ classifications from winding
  normal forms.  A Lean 4.24.0 artifact checks the theorem package while the
  mathematical arguments remain independent of the implementation.
\end{enumerate}
...
\LeanName{weightedConcatenationInvariant\_balanced}\\
...
\LeanName{WeightedSlotReparam}, \LeanName{balancedSlotReparam},
\LeanName{weightedConcatenationInvariant}, and
\LeanName{weightedConcatenationInvariant\_balanced} declarations formalize
the endpoint-preserving reparametrization bridge, including the endpoint-fixed
null-homotopy hypotheses for zero slots.  Thus the artifact checks the
corresponding soundness and quotient interfaces while the manuscript presents
the flat-word strictification at the mathematical level.
...
compact-Hausdorff and discrete finite-presentation theorems give a positive
ordinary-pullback class, while the effective normal-form criterion turns
concrete reductions into geometric completeness.  The finite-generator circle
and genuine torus calculations show that the framework retains nontrivial
topological information without silently identifying scoped syntax with
ambient equality.
