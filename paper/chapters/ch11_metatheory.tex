...
In Part~I (Chapter~\ref{ch:rewrite-system}), we introduced the 76-rule
...
\begin{definition}[Typed Rewriting System]\label{def:typed-trs}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item A \emph{reduction relation} $\rew$ on terms of the same sort,
      given by the 76 rules of Chapter~\ref{ch:rewrite-system}.
  \end{enumerate}
...
$\mathrm{PathExpr}$, with the same 76 rules applied syntactically. The
...
The 76 rules generate a finite (though large) set of critical pairs.
...
\begin{definition}[Path Tactics]\label{def:path-tactics}
  ...
  \begin{itemize}
    ...
    \item \texttt{path\_simp}: applies the $\mathtt{simp}$ tactic with the
      ...
      attributes on all 76 rules and their derived consequences).
...
\midrule
\texttt{Step.lean} & The 76-rule \texttt{Step} inductive type and soundness proof \\
