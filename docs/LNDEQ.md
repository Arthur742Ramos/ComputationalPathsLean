...
| 29 | `symm (pair r s)` ↔ `pair (symm r) (symm s)` (sxss) | `Step.symm_congr` + `Step.map2_subst` | Symmetry commutes with pair constructors. |
...
| 34 | `symm (Σ-elim r …)` ↔ … | `Step.symm_congr` + `Step.sigma_eta`/`beta` rules | Dependent eliminations enjoy symmetry via the generic congruence rule. |
...
| 40 | `trans (C[u]) (trans (C[symm u]) v)` etc. (ttsv/tstu) | Needs dedicated lemma | These context-sensitive `tt` completions are expected to follow from `Step.trans_assoc` + `context_congr`, but no explicit constructor exists yet; requires verification. |

## Outstanding items

- **Rule 40 (`ttsv`/`tstu`)** still lacks a named `Step` constructor.  While it should be
  derivable from the existing associativity and congruence rules, adding an explicit lemma
  would make the correspondence with the paper immediate.
...
1. **Enumerate rules:** create `ComputationalPaths/Path/Rewrite/LNDEQ.lean` defining
  `Rule` with one constructor per row of the coverage table.  The only ambiguity is the
  pair of context-sensitive completions (`ttsv`/`tstu`), which can share a constructor
  once we add the missing `Step` lemma.
...
To finish aligning the code with §3.3 of the paper we need three workstreams:

1. **Fill the rule gap:** add an explicit constructor (or lemma) to `Step` for the
  context-sensitive `ttsv`/`tstu` completions.  Once present we can reduce `Rule.ttsv`
  and `Rule.tstu` to first-class `Step`s and complete the coverage table above.
2. **Termination proof:** replicate Dershowitz’s recursive path ordering in Lean by:
...
3. **Confluence and correspondence:** re-run the Knuth–Bendix superposition inside Lean.
...
- Missing `Step` lemma for rule 40 and the helper tactic that produces instantiations.
