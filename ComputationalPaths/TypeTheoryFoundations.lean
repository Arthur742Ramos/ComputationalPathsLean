...
This module formalizes type theory foundations — Martin-Löf type theory,
identity types, dependent products/sums, W-types, universes à la Tarski,
the univalence axiom (statement), function extensionality, and propositional
truncation — all with `Path` coherence witnesses.

## Mathematical Background

Martin-Löf type theory (MLTT) is an intensional dependent type theory
that serves as a foundation for constructive mathematics:

1. **Martin-Löf Type Theory**: A dependent type theory with Π-types,
   Σ-types, identity types, natural numbers, and universes. The judgment
   forms are: Γ ctx, Γ ⊢ A type, Γ ⊢ a : A, Γ ⊢ a ≡ b : A.
2. **Identity Types**: Id_A(a,b) — the type of proofs that a = b in A.
...
3. **Dependent Products (Π-types)**: Π(x:A).B(x) — the type of
   dependent functions. With introduction (λ) and elimination (app)
   satisfying β and η rules.
4. **Dependent Sums (Σ-types)**: Σ(x:A).B(x) — the type of
   dependent pairs. With introduction (pair) and elimination (pr₁, pr₂).
5. **W-types**: W(x:A).B(x) — well-founded trees with nodes labeled
   by a : A and branching type B(a). The type of well-founded trees
   is inductively generated by the constructor sup.
6. **Universes à la Tarski**: A universe U is a type together with a
   decoding function El : U → Type. Codes in U represent types,
   and El maps codes to their types.
7. **Univalence Axiom**: (A ≃ B) ≃ Id_U(A,B) — equivalence of types
...
9. **Propositional Truncation**: ‖A‖ — the (-1)-truncation of A,
...
|-------------------|-------------|
| `MLTTData` | Martin-Löf type theory data |
...
| `UniverseData` | Universe à la Tarski |
...
| `pi_beta_path` | Π β-reduction |
| `sigma_eta_path` | Σ η-expansion |
...
@[simp] theorem foundational_multistep_path_rweq
  ...
  · exact
      Path.rweq_trans_congr_right
        (Path.trans (Path.trans (Path.refl a) core))
        (Path.rweq_of_step (Path.Step.symm_trans (Path.refl b)))
  · apply Path.rweq_trans
    · exact
        Path.rweq_of_step
          (Path.Step.trans_assoc (Path.refl a) core (Path.refl b))
    · apply Path.rweq_trans
      · exact
          Path.rweq_trans_congr_right
            (Path.refl a)
            (Path.rweq_of_step (Path.Step.trans_refl_right core))
      · exact Path.rweq_of_step (Path.Step.trans_refl_left core)

/-! ## Martin-Löf Type Theory Data -/

/-- Data representing the structure of Martin-Löf type theory.
...
structure MLTTData where
  ...
  /-- Number of type formers (Π, Σ, Id, ℕ, 𝟎, 𝟏, 𝟐, W, U). -/
  numTypeFormers : Nat
  /-- numTypeFormers ≥ 4 (at minimum: Π, Σ, Id, ℕ). -/
  typeFormers_ge : numTypeFormers ≥ 4
...
/-- Π-type: Π(x:A).B(x). The type of dependent functions.
We track the β-rule (computation) and η-rule (uniqueness). -/
structure PiTypeData where
  ...
  resultSize : Nat
  /-- β-reduction obstruction (0 = (λx.t)(a) reduces to t[a/x]). -/
  betaObstruction : Nat
  /-- β-reduction holds. -/
  beta_zero : betaObstruction = 0
  /-- η-expansion obstruction (0 = f ≡ λx.f(x)). -/
  etaObstruction : Nat
  /-- η-expansion holds. -/
  eta_zero : etaObstruction = 0
  /-- Substitution obstruction (0 = Π commutes with substitution). -/
...
/-- Σ-type: Σ(x:A).B(x). The type of dependent pairs.
...
and B(a)-many branches. The constructor is sup : (a : A) → (B(a) → W) → W.
...
/-- Natural numbers as W-type: A = {zero, succ}, B(zero) = ∅, B(succ) = {*}. -/
...
/-- Binary trees: A = {leaf, node}, B(leaf) = ∅, B(node) = {left, right}. -/
...
/-- Universe à la Tarski: a type U with a decoding function El : U → Type.
Elements of U are "codes" for types. The universe hierarchy
U₀ : U₁ : U₂ : ⋯ with lifting maps. -/
structure UniverseData where
  /-- Universe level. -/
  level : Nat
  /-- Number of type formers closed under this universe. -/
  numClosed : Nat
  /-- numClosed ≥ 4 (Π, Σ, Id, ℕ at minimum). -/
  numClosed_ge : numClosed ≥ 4
  /-- Cumulativity obstruction (0 = U_i ⊆ U_{i+1}). -/
...
/-- The univalence axiom: (A ≃ B) ≃ Id_U(A,B). This identifies
...
/-- Propositional truncation ‖A‖: the (-1)-truncation of A.
‖A‖ is a proposition (all elements are equal) such that
A → ‖A‖ (surjection) and ‖A‖ → P for any proposition P with A → P. -/
structure PropTruncData where
  ...
  truncated_le : truncatedSize ≤ 1
  /-- Surjection obstruction (0 = |_| : A → ‖A‖ exists). -/
  ...
  /-- Propositional obstruction (0 = ‖A‖ is a proposition). -/
  propObstruction : Nat
  /-- ‖A‖ is a proposition. -/
...
and path constructors. The circle S¹ is the prototypical example:
S¹ has a point base and a path loop : base = base. -/
structure HITData where
  /-- Number of point constructors. -/
  numPointConstructors : Nat
  /-- numPointConstructors ≥ 1. -/
  numPoint_pos : numPointConstructors ≥ 1
...
/-- Circle S¹: one point (base), one path (loop). -/
...
structure TruncationLevel where
  ...
  numElements : Nat
  /-- At level ≤ 1 (propositional), numElements ≤ 1. -/
  prop_bound : level ≤ 1 → numElements ≤ 1
  /-- Closure under Π-types obstruction (0 = closed). -/
  piClosureObstruction : Nat
  /-- Π-closure holds. -/
  piClosure_zero : piClosureObstruction = 0
  /-- Closure under Σ-types obstruction. -/
...
/-- Master: truncation level Π-closure. -/
