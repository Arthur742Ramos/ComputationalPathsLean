...
noncomputable def ShStep.toStep {A : Type u} {a b : A} {p q : Path a b}
...
noncomputable def restrict_id (x : A) : Path (F.restrict x x) (F.sections x) :=
  F.restrictId x

/-- Theorem 2: restriction composition. -/
noncomputable def restrict_comp (x y z : A) :
    Path (F.restrict x z) (F.restrict (F.restrict x y) z) :=
  F.restrictComp x y z

/-- Theorem 3: restriction of zero. -/
noncomputable def restrict_zero (x y : A) : Path (F.restrict F.zero y) F.zero :=
  F.restrictZero x y
...
noncomputable def coboundary_sq (x : A) : Path (C.coboundary (C.coboundary x)) C.zero :=
  C.coboundarySquare x

/-- Theorem 14: coboundary of zero is zero. -/
noncomputable def coboundary_zero : Path (C.coboundary C.zero) C.zero :=
  C.coboundaryZero

/-- Theorem 15: coboundary distributes over addition. -/
noncomputable def coboundary_add (x y : A) :
    Path (C.coboundary (C.add x y)) (C.add (C.coboundary x) (C.coboundary y)) :=
  C.coboundaryAdd x y

/-- Theorem 16: coboundary commutes with negation. -/
noncomputable def coboundary_neg (x : A) :
...
noncomputable def coboundary_double_congr {x₁ x₂ : A} (p : Path x₁ x₂) :
...
noncomputable def coboundary_triple_zero (x : A) :
...
noncomputable def neg_congr {x₁ x₂ : A} (p : Path x₁ x₂) :
...
noncomputable def gf_exact (x : A) : Path (E.g (E.f x)) E.zero :=
  E.fgExact x

/-- Theorem 27: δ ∘ g = 0 exactness. -/
noncomputable def dg_exact (x : A) : Path (E.delta (E.g x)) E.zero :=
  E.gdExact x

/-- Theorem 28: f ∘ δ = 0 exactness. -/
noncomputable def fd_exact (x : A) : Path (E.f (E.delta x)) E.zero :=
  E.dfExact x

/-- Theorem 29: f preserves zero. -/
noncomputable def f_zero : Path (E.f E.zero) E.zero :=
  E.fZero

/-- Theorem 30: g preserves zero. -/
noncomputable def g_zero : Path (E.g E.zero) E.zero :=
  E.gZero

/-- Theorem 31: delta preserves zero. -/
noncomputable def delta_zero : Path (E.delta E.zero) E.zero :=
  E.deltaZero
...
noncomputable def f_congr {x₁ x₂ : A} (p : Path x₁ x₂) :
    Path (E.f x₁) (E.f x₂) :=
  Path.congrArg E.f p

/-- Theorem 36: g congruence. -/
noncomputable def g_congr {x₁ x₂ : A} (p : Path x₁ x₂) :
    Path (E.g x₁) (E.g x₂) :=
  Path.congrArg E.g p

/-- Theorem 37: delta congruence. -/
noncomputable def delta_congr {x₁ x₂ : A} (p : Path x₁ x₂) :
    Path (E.delta x₁) (E.delta x₂) :=
  Path.congrArg E.delta p

/-- Theorem 38: g(f(0)) = 0 composite path. -/
noncomputable def gf_zero_path : Path (E.g (E.f E.zero)) E.zero :=
  E.fgExact E.zero

/-- Theorem 39: δ(g(0)) = 0 composite path. -/
noncomputable def dg_zero_path : Path (E.delta (E.g E.zero)) E.zero :=
  E.gdExact E.zero

/-- Theorem 40: f(δ(0)) = 0 composite path. -/
noncomputable def fd_zero_path : Path (E.f (E.delta E.zero)) E.zero :=
  E.dfExact E.zero
...
noncomputable def connect_zero : Path (D.connect D.zero) D.zero :=
  D.connectZero

/-- Theorem 42: connecting distributes over addition. -/
noncomputable def connect_add (x y : A) :
    Path (D.connect (D.add x y)) (D.add (D.connect x) (D.connect y)) :=
  D.connectNatural x y

/-- Theorem 43: connecting commutes with negation. -/
noncomputable def connect_neg (x : A) :
    Path (D.connect (D.neg x)) (D.neg (D.connect x)) :=
  D.connectNeg x

/-- Theorem 44: f ∘ connect = 0. -/
noncomputable def f_connect_exact (x : A) : Path (D.f (D.connect x)) D.zero :=
  D.fConnectExact x

/-- Theorem 45: connect ∘ g = 0. -/
noncomputable def connect_g_exact (x : A) : Path (D.connect (D.g x)) D.zero :=
  D.connectGExact x
...
noncomputable def f_connect_zero_path : Path (D.f (D.connect D.zero)) D.zero :=
  D.fConnectExact D.zero

/-- Theorem 53: connect(g(0)) = 0 composite alternative. -/
noncomputable def connect_g_zero_path : Path (D.connect (D.g D.zero)) D.zero :=
...
noncomputable def ruv_iu (x : A) : Path (M.rUV (M.iU x)) x :=
  M.mvExact1 x

/-- Theorem 57: restriction after inclusion V. -/
noncomputable def ruv_iv (x : A) : Path (M.rUV (M.iV x)) x :=
  M.mvExact2 x

/-- Theorem 58: MV delta exactness. -/
noncomputable def mv_delta_exact (x : A) :
    Path (M.add (M.iU (M.mv_delta x)) (M.neg (M.iV (M.mv_delta x)))) M.zero :=
  M.mvDeltaExact x

/-- Theorem 59: iU preserves zero. -/
noncomputable def iu_zero : Path (M.iU M.zero) M.zero :=
  M.iUZero

/-- Theorem 60: iV preserves zero. -/
noncomputable def iv_zero : Path (M.iV M.zero) M.zero :=
  M.iVZero

/-- Theorem 61: rUV preserves zero. -/
noncomputable def ruv_zero : Path (M.rUV M.zero) M.zero :=
  M.rUVZero

/-- Theorem 62: iU congruence. -/
noncomputable def iu_congr {x₁ x₂ : A} (p : Path x₁ x₂) :
    Path (M.iU x₁) (M.iU x₂) :=
  Path.congrArg M.iU p

/-- Theorem 63: iV congruence. -/
noncomputable def iv_congr {x₁ x₂ : A} (p : Path x₁ x₂) :
    Path (M.iV x₁) (M.iV x₂) :=
  Path.congrArg M.iV p

/-- Theorem 64: rUV congruence. -/
noncomputable def ruv_congr {x₁ x₂ : A} (p : Path x₁ x₂) :
    Path (M.rUV x₁) (M.rUV x₂) :=
  Path.congrArg M.rUV p

/-- Theorem 65: mv_delta congruence. -/
noncomputable def mv_delta_congr {x₁ x₂ : A} (p : Path x₁ x₂) :
    Path (M.mv_delta x₁) (M.mv_delta x₂) :=
  Path.congrArg M.mv_delta p

/-- Theorem 66: rUV(iU(0)) = 0 composite. -/
noncomputable def ruv_iu_zero : Path (M.rUV (M.iU M.zero)) M.zero :=
  M.mvExact1 M.zero

/-- Theorem 67: rUV(iV(0)) = 0 composite. -/
noncomputable def ruv_iv_zero : Path (M.rUV (M.iV M.zero)) M.zero :=
  M.mvExact2 M.zero
...
noncomputable def derF_double_congr (n m : Nat) {x₁ x₂ : A} (p : Path x₁ x₂) :
