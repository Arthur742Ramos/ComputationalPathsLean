...
def associatorPath (f : Hom a b) (g : Hom b c) (h : Hom c d) :
    TwoCell (Path.trans (Path.trans f g) h) (Path.trans f (Path.trans g h)) :=
  associator f g h

/-- Left unitor as an explicit 2-cell. -/
def leftUnitorPath (f : Hom a b) : TwoCell (Path.trans (Path.refl a) f) f :=
  leftUnitor f

/-- Right unitor as an explicit 2-cell. -/
def rightUnitorPath (f : Hom a b) : TwoCell (Path.trans f (Path.refl b)) f :=
  rightUnitor f
