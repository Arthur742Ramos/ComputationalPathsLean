...
noncomputable def LieStep.toStep {A : Type u} {a b : A} {p q : Path a b}
...
noncomputable def bracket_zero_left (x : A) : Path (L.bracket L.zero x) L.zero :=
  L.bracketZeroLeft x

/-- Theorem 2: bracket with zero on the right. -/
noncomputable def bracket_zero_right (x : A) : Path (L.bracket x L.zero) L.zero :=
  L.bracketZeroRight x

/-- Theorem 3: antisymmetry of the bracket. -/
noncomputable def bracket_antisym (x y : A) : Path (L.bracket x y) (L.neg (L.bracket y x)) :=
  L.antisymPath x y

/-- Theorem 4: self-bracket vanishes via antisymmetry. -/
noncomputable def bracket_self_zero (x : A)
...
noncomputable def jacobi_identity (x y z : A) :
...
noncomputable def bracket_right_unit_rweq (x y : A) :
...
noncomputable def bracket_left_unit_rweq (x y : A) :
...
noncomputable def antisym_cancel_rweq (x y : A) :
...
noncomputable def jacobi_left_unit_rweq (x y z : A) :
    RwEq (Path.trans (Path.refl _) (L.jacobiPath x y z))
         (L.jacobiPath x y z) :=
  rweq_of_lie_step (LieStep.left_unit _)

/-- Theorem 10: Jacobi right unit. -/
noncomputable def jacobi_right_unit_rweq (x y z : A) :
...
noncomputable def kill_symm (x y : A) : Path (K.kill x y) (K.kill y x) :=
  K.killSymm x y

/-- Theorem 22: Killing form ad-invariance. -/
noncomputable def kill_ad_inv (x y z : A) :
...
noncomputable def ideal_has_zero : I.isIdeal L.zero := I.idealZero

/-- Theorem 32: bracket of ideal element stays in ideal. -/
noncomputable def ideal_bracket_closed (x y : A) (hx : I.isIdeal x) :
    I.isIdeal (L.bracket x y) :=
  I.idealBracket x y hx

/-- Theorem 33: negation preserves ideal membership. -/
noncomputable def ideal_neg_closed (x : A) (hx : I.isIdeal x) :
    I.isIdeal (L.neg x) :=
  I.idealNeg x hx

/-- Theorem 34: addition preserves ideal membership. -/
noncomputable def ideal_add_closed (x y : A) (hx : I.isIdeal x) (hy : I.isIdeal y) :
    I.isIdeal (L.add x y) :=
  I.idealClosed x y hx hy

/-- Theorem 35: right bracket with ideal element via antisymmetry. -/
noncomputable def ideal_bracket_right (x y : A) (hy : I.isIdeal y)
...
noncomputable def derived_zero_id (x : A) : Path (S.derived 0 x) x :=
  S.derivedZero x
...
noncomputable def derived_zero_inverse_cancel (x : A) :
...
noncomputable def derived_congr (k : Nat) {x₁ x₂ : A} (p : Path x₁ x₂) :
...
noncomputable def reflect_roundtrip (x : A) :
...
noncomputable def reflect_zero_roundtrip :
...
noncomputable def weight_zero_right_unit :
...
noncomputable def uea_assoc (x y z : A) : Path (U.mul (U.mul x y) z) (U.mul x (U.mul y z)) :=
  U.mulAssoc x y z

/-- Theorem 54: UEA left unit. -/
noncomputable def uea_one_left (x : A) : Path (U.mul U.one x) x :=
  U.mulOneLeft x

/-- Theorem 55: UEA right unit. -/
noncomputable def uea_one_right (x : A) : Path (U.mul x U.one) x :=
  U.mulOneRight x

/-- Theorem 56: UEA embed bracket relation. -/
noncomputable def uea_embed_bracket (x y : A) :
...
noncomputable def cartan_comm (x y : A) (hx : C.isCartan x) (hy : C.isCartan y) :
    Path (L.bracket x y) L.zero :=
  C.cartanComm x y hx hy

/-- Theorem 66: zero is Cartan. -/
noncomputable def cartan_has_zero : C.isCartan L.zero := C.cartanZero

/-- Theorem 67: weight decomposition of zero. -/
noncomputable def weight_decomp_zero (h : A) (hh : C.isCartan h) :
    Path (C.weightDecomp h L.zero) L.zero :=
  C.weightDecompZero h hh

/-- Theorem 68: Cartan comm right unit. -/
noncomputable def cartan_comm_right_unit (x y : A) (hx : C.isCartan x) (hy : C.isCartan y) :
    RwEq (Path.trans (C.cartanComm x y hx hy) (Path.refl _))
         (C.cartanComm x y hx hy) :=
  rweq_of_lie_step (LieStep.right_unit _)

/-- Theorem 69: Cartan comm inverse cancel. -/
noncomputable def cartan_comm_inverse_cancel (x y : A) (hx : C.isCartan x) (hy : C.isCartan y) :
    RwEq (Path.trans (C.cartanComm x y hx hy) (Path.symm (C.cartanComm x y hx hy)))
         (Path.refl _) :=
  rweq_of_lie_step (LieStep.inverse_cancel _)

/-- Theorem 70: weight decomposition congruence. -/
noncomputable def weight_decomp_congr (h : A) {x₁ x₂ : A} (p : Path x₁ x₂) :
