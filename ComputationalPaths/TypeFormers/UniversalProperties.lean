...
noncomputable def prod_pairing_unique
    ...
    (hp : Path.RwEq (Path.fst r) p)
    (hq : Path.RwEq (Path.snd r) q) :
    Path.RwEq r (Path.prodMk p q) := by
  have hη : Path.RwEq r (Path.prodMk (Path.fst r) (Path.snd r)) :=
    prod_factor_through_pairing (A := A) (B := B) r
  have hpair :
      Path.RwEq (Path.prodMk (Path.fst r) (Path.snd r)) (Path.prodMk p q) :=
...
@[simp] noncomputable def caseInlStep (f : A → C) (g : B → C) (p : Path a₁ a₂) :
    Path.Step (Path.congrArg (copair f g) (inlPath p)) (Path.congrArg f p) := by
  simpa [copair, inlPath] using
    (Path.Step.sum_rec_inl_beta (A := C) (α := A) (β := B)
      (f := f) (g := g) (p := p))

@[simp] noncomputable def caseInrStep (f : A → C) (g : B → C) (p : Path b₁ b₂) :
    Path.Step (Path.congrArg (copair f g) (inrPath p)) (Path.congrArg g p) := by
  simpa [copair, inrPath] using
    (Path.Step.sum_rec_inr_beta (A := C) (α := A) (β := B)
      (f := f) (g := g) (p := p))

@[simp] noncomputable def caseInlRwEq (f : A → C) (g : B → C) (p : Path a₁ a₂) :
    Path.RwEq (Path.congrArg (copair f g) (inlPath p)) (Path.congrArg f p) :=
  rweq_of_step (caseInlStep f g p)

@[simp] noncomputable def caseInrRwEq (f : A → C) (g : B → C) (p : Path b₁ b₂) :
    Path.RwEq (Path.congrArg (copair f g) (inrPath p)) (Path.congrArg g p) :=
...
noncomputable def copairFactor_unique
    ...
    (p : Path h (copair f g))
    (hp : ∀ s : Sum A B,
      Path.RwEq (Path.app p s)
        (match s with
        | Sum.inl a => hl a
        | Sum.inr b => hr b)) :
    Path.RwEq p (copairFactorPath (A := A) (B := B) (C := C) hl hr) := by
  have hη : Path.RwEq p (Path.lamCongr (fun s => Path.app p s)) :=
    rweq_symm (rweq_fun_eta p)
  have hlam :
      Path.RwEq (Path.lamCongr (fun s => Path.app p s))
        (Path.lamCongr (fun s =>
          match s with
          | Sum.inl a => hl a
          | Sum.inr b => hr b)) :=
...
@[simp] noncomputable def lamAppStep
    ...
    Path.Step (Path.app (Path.lamCongr p) a) (p a) := by
...
@[simp] noncomputable def lamAppRwEq
    (p : ∀ x : A, Path (f x) (g x)) (a : A) :
    Path.RwEq (Path.app (Path.lamCongr p) a) (p a) :=
