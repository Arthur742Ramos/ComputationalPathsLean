...
noncomputable def IsELocal (E : HomologyTheory'.{u}) (X : E.carrier) : Prop :=
  ∀ W : E.carrier, IsEAcyclic E W → E.evaluate W X = E.evaluate X X
...
structure SmashingLocalization (E : HomologyTheory'.{u}) extends BousfieldLocalization E where
  smashing : Prop :=
    ∀ (X Y : E.carrier), localize (E.evaluate X Y) = E.evaluate (localize X) (localize Y)
...
structure FiniteLocalization (E : HomologyTheory'.{u}) extends SmashingLocalization E where
  finite_type : Prop :=
    ∃ _S : E.carrier → Prop, ∀ x : E.carrier, localize x = localize x
