...
This module formalizes Floer homology — the Floer chain complex, the action
...
maps — all using `Path` witnesses for coherence data.

## Mathematical Background

Floer homology is an infinite-dimensional analogue of Morse homology:

1. **Action functional**: 𝒜_H(γ) = −∫ γ*ω + ∫₀¹ H(t, γ(t)) dt,
...
3. **Moduli spaces**: ℳ(x, y) = solutions u : ℝ × S¹ → M of
   ∂̄_J u = 0 connecting orbits x, y.
4. **Floer differential**: d² = 0 by gluing/compactness of
   1-dimensional moduli spaces.
5. **PSS isomorphism**: HF*(H, J) ≅ QH*(M), identifying Floer
   homology with quantum cohomology.
6. **Arnold conjecture**: #Fix(φ_H) ≥ ∑ bₖ(M), where bₖ are
   Betti numbers.
7. **Continuation maps**: Morphisms HF*(H₀) → HF*(H₁) induced
...
Conley–Zehnder index and action value. -/
structure PeriodicOrbit where
  /-- Identifier for the orbit. -/
  orbitId : Nat
  /-- The Conley–Zehnder index μ_CZ(γ). -/
  czIndex : Int
  /-- The action value 𝒜_H(γ). -/
  actionValue : Int
...
/-- The symplectic action functional 𝒜_H, modeled by its value on orbits. -/
structure ActionFunctional where
  /-- Number of periodic orbits (critical points of 𝒜_H). -/
  numOrbits : Nat
  /-- The periodic orbits. -/
  orbits : Nat → PeriodicOrbit
  /-- Action values are ordered: 𝒜(γᵢ) ≤ 𝒜(γᵢ₊₁) (filtration). -/
  action_ordered : ∀ (i : Nat), i + 1 < numOrbits →
    (orbits i).actionValue ≤ (orbits (i + 1)).actionValue

namespace ActionFunctional

/-- An action functional with a single orbit. -/
def single (γ : PeriodicOrbit) : ActionFunctional where
  numOrbits := 1
  orbits := fun _ => γ
  action_ordered := fun i h => by omega

/-- Single orbit functional has one orbit. -/
theorem single_count (γ : PeriodicOrbit) : (single γ).numOrbits = 1 := rfl
...
Conley–Zehnder index k, with integer coefficients. -/
structure FloerChainGroup where
  /-- The grading (Conley–Zehnder index). -/
  degree : Int
...
/-- The Floer differential ∂ : CF_k → CF_{k-1}, defined by counting
...
structure FloerDifferential where
  ...
  /-- Matrix entries: ∂(γᵢ) = Σⱼ n(γᵢ, γⱼ) · γⱼ where n counts
  strips. -/
  matrix : Nat → Nat → Int
...
/-- Matrix product of two differential matrices, representing d ∘ d. -/
def matMul (A : Nat → Nat → Int) (B : Nat → Nat → Int)
    (midSize : Nat) : Nat → Nat → Int :=
  fun i j => (List.range midSize).foldl (fun acc k => acc + A i k * B k j) 0

/-- Zero matrix. -/
def zeroMat : Nat → Nat → Int := fun _ _ => 0

/-- Matrix multiplication by a zero matrix on the left gives zero. -/
theorem matMul_zero_left (B : Nat → Nat → Int) (n : Nat) (i j : Nat) :
    matMul zeroMat B n i j = 0 := by
  simp only [matMul, zeroMat]
  have : ∀ (xs : List Nat) (acc : Int),
...
theorem matMul_zero_right (A : Nat → Nat → Int) (n : Nat) (i j : Nat) :
...
∂ : CF_k → CF_{k-1}, satisfying ∂² = 0. -/
structure FloerComplex where
  /-- The chain groups indexed by degree. -/
  chainGroup : Int → FloerChainGroup
  /-- The differential at each degree. -/
  differential : Int → FloerDifferential
  /-- Degree consistency. -/
  deg_consistent : ∀ (k : Int), (differential k).sourceDeg = k
  /-- d² = 0: the composition of consecutive differentials is zero. -/
  d_squared_zero : ∀ (k : Int) (i j : Nat),
...
/-- Floer homology HF_k = ker ∂_k / im ∂_{k+1}. -/
...
/-- The PSS (Piunikhin–Salamon–Schwarz) map establishing
HF*(H, J) ≅ QH*(M). -/
...
structure ArnoldConjecture where
  ...
  arnold_ineq : numFixedPoints ≥ totalBetti
...
/-- A continuation map Φ : HF*(H₀) → HF*(H₁) induced by a homotopy
...
structure ContinuationMap where
  ...
  matrix : Nat → Nat → Int
  /-- Chain map condition: Φ ∘ d₀ = d₁ ∘ Φ (at the matrix level,
...
theorem identity_offdiag (n : Nat) (i j : Nat) (h : i ≠ j) :
    (identity n).matrix i j = 0 := by
  simp [identity, h]

/-- Composition of continuation maps. -/
def comp (Φ : ContinuationMap) (Ψ : ContinuationMap)
    (h : Φ.targetRank = Ψ.sourceRank) : ContinuationMap where
  sourceRank := Φ.sourceRank
  targetRank := Ψ.targetRank
  matrix := fun i j => (List.range Φ.targetRank).foldl
    (fun acc k => acc + Φ.matrix i k * Ψ.matrix k j) 0
  chain_map := by rw [Φ.chain_map, h, Ψ.chain_map]
  homotopyParam := Φ.homotopyParam + Ψ.homotopyParam

/-- Composition preserves the chain map property. -/
theorem comp_chain_map (Φ Ψ : ContinuationMap) (h : Φ.targetRank = Ψ.sourceRank) :
    (comp Φ Ψ h).sourceRank = (comp Φ Ψ h).targetRank := by
  simp [comp, Φ.chain_map, h, Ψ.chain_map]
...
structure ModuliSpace where
  ...
  finite_count : expectedDim = 0 → count = count  -- tautological finiteness
...
/-- Energy of a pseudoholomorphic strip: E(u) = ∫ |∂u/∂s|² ds dt ≥ 0. -/
