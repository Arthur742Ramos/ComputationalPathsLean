...
theorem leftTriangle_def
    (adj : AdjunctionData C D F G) (x : C) :
    adj.leftTriangle x =
      Path.trans (Path.congrArg F (adj.unit x)) (adj.counit (F x)) := by
  sorry

theorem rightTriangle_def
    (adj : AdjunctionData C D F G) (y : D) :
    adj.rightTriangle y =
      Path.trans (adj.unit (G y)) (Path.congrArg G (adj.counit y)) := by
  sorry

theorem leftCounitInverse_apply
    (adj : AdjunctionData C D F G)
    (hε : LeftCounitInverse adj)
    (x : C) :
    RwEq (adj.counit (F x)) (Path.symm (Path.congrArg F (adj.unit x))) := by
  sorry

theorem rightUnitInverse_apply
    (adj : AdjunctionData C D F G)
    (hη : RightUnitInverse adj)
    (y : D) :
    RwEq (adj.unit (G y)) (Path.symm (Path.congrArg G (adj.counit y))) := by
  sorry

theorem zigzag_left_apply
    (adj : AdjunctionData C D F G)
    (hz : Zigzag adj)
    (x : C) :
    RwEq (adj.leftTriangle x) (Path.refl (F x)) := by
  sorry

theorem zigzag_right_apply
    (adj : AdjunctionData C D F G)
    (hz : Zigzag adj)
    (y : D) :
    RwEq (adj.rightTriangle y) (Path.refl (G y)) := by
  sorry

theorem leftTriangle_reduces_of_inverse
    (adj : AdjunctionData C D F G)
    (hε : LeftCounitInverse adj)
    (x : C) :
    RwEq
...
theorem rightTriangle_reduces_of_inverse
    (adj : AdjunctionData C D F G)
    (hη : RightUnitInverse adj)
    (y : D) :
    RwEq
...
theorem zigzag_of_inverse_left
    (adj : AdjunctionData C D F G)
    (hε : LeftCounitInverse adj)
    (hη : RightUnitInverse adj)
    (x : C) :
...
theorem zigzag_of_inverse_right
    (adj : AdjunctionData C D F G)
    (hε : LeftCounitInverse adj)
    (hη : RightUnitInverse adj)
    (y : D) :
...
theorem mapped_unit_component_rw_refl
    (adj : AdjunctionData C D F G)
    (x : C) :
    RwEq (Path.congrArg F (adj.unit x)) (Path.congrArg F (adj.unit x)) := by
  sorry

theorem mapped_counit_component_rw_refl
    (adj : AdjunctionData C D F G)
    (y : D) :
    RwEq (Path.congrArg G (adj.counit y)) (Path.congrArg G (adj.counit y)) := by
  sorry
