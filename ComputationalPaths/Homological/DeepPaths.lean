...
noncomputable def shift_equiv (x : A) : Path (Tr.shift (Tr.shiftInv x)) x :=
  Tr.shiftInvPath x

/-- Double shift roundtrip. -/
noncomputable def double_shift_roundtrip (x : A) :
...
noncomputable def heart_fixed_leq (x : A) (hx : T.heart x) :
    Path (T.tauLeq x) x :=
  T.tauLeqIdPath x ((T.heartPath x).mp hx).1

/-- Heart objects are fixed by τ≥0. -/
noncomputable def heart_fixed_geq (x : A) (hx : T.heart x) :
    Path (T.tauGeq x) x :=
  T.tauGeqIdPath x ((T.heartPath x).mp hx).2

/-- Heart truncation is idempotent on heart objects. -/
noncomputable def heart_idemp (x : A) (hx : T.heart x)
...
noncomputable def serre_bottom (f g : A) :
...
noncomputable def quotient_kills_thick (x : A) (hx : V.inThick x) :
...
noncomputable def tilting_roundtrip (x : A) :
    Path (T.derivedEquiv (T.derivedEquivInv x)) x :=
  T.equivRoundtrip1 x

/-- Inverse roundtrip. -/
noncomputable def tilting_inv_roundtrip (x : A) :
...
noncomputable def tilting_double_roundtrip (x : A) :
...
/-- SES relation in K_0. -/
noncomputable def k0_ses (a b c : A) :
    Path (K.classOf b) (K.add (K.classOf a) (K.classOf c)) :=
  K.sesRelPath a b c

/-- K_0 commutativity. -/
noncomputable def k0_comm (x y : A) : Path (K.add x y) (K.add y x) :=
  K.addCommPath x y

/-- K_0 associativity. -/
noncomputable def k0_assoc (x y z : A) :
    Path (K.add (K.add x y) z) (K.add x (K.add y z)) :=
  K.addAssocPath x y z

/-- K_0 zero identity. -/
noncomputable def k0_zero (x : A) : Path (K.add x K.zero) x :=
  K.addZeroPath x
...
/-- HH^0 is the center. -/
noncomputable def hh_zero_center : Path (H.hh 0) H.alg := H.hh0Path

/-- HH^1 is outer derivations. -/
noncomputable def hh_one_deriv : Path (H.hh 1) H.bimod := H.hh1Path

/-- Cup product associativity. -/
noncomputable def cup_assoc (p q r : Nat) (x y z : A) :
    Path (H.cup p (q + r) x (H.cup q r y z))
         (H.cup (p + q) r (H.cup p q x y) z) :=
  H.cupAssocPath p q r x y z

/-- Cup product commutativity. -/
noncomputable def cup_comm (p q : Nat) (x y : A) :
...
/-- Associativity. -/
noncomputable def derived_tensor_assoc (n : Nat) (x y z : A) :
    Path (D.tensorL n x (D.tensorL n y z)) (D.tensorL n (D.tensorL n x y) z) :=
  D.assocPath n x y z

/-- Commutativity. -/
noncomputable def derived_tensor_comm (n : Nat) (x y : A) :
    Path (D.tensorL n x y) (D.tensorL n y x) :=
  D.commPath n x y

/-- Left unit. -/
noncomputable def derived_tensor_unit (n : Nat) (x : A) :
    Path (D.tensorL n D.unitObj x) x :=
  D.unitPath n x

/-- Right unit via commutativity. -/
noncomputable def derived_tensor_unit_right (n : Nat) (x : A) :
...
noncomputable def tensor_hom_adj (n : Nat) (x y z : A) (tensorL : Nat → A → A → A) :
...
/-- Duality involution. -/
noncomputable def duality_involution (x : A) :
    Path (D.dualFunctor (D.dualFunctor x)) x :=
  D.involutionPath x

/-- Triple duality reduces to single. -/
noncomputable def triple_duality (x : A) :
...
noncomputable def euler_additive (a b c : A) :
...
/-- Associativity. -/
noncomputable def homotopy_comp_assoc (x y z : A) :
    Path (H.comp (H.comp x y) z) (H.comp x (H.comp y z)) :=
  H.compAssocPath x y z

/-- Right identity. -/
noncomputable def homotopy_comp_id (x : A) : Path (H.comp x (H.idMor x)) x :=
  H.compIdPath x

/-- Left identity. -/
noncomputable def homotopy_id_comp (x : A) : Path (H.comp (H.idMor x) x) x :=
  H.idCompPath x
...
noncomputable def homotopy_comp_assoc4 (w x y z : A) :
...
noncomputable def perverse_tau_idemp (n : Nat) (x : A) :
...
noncomputable def graded_iso (p : Nat) : Path (F.srcFilt.graded p) (F.tgtFilt.graded p) :=
  F.gradedIsoPath p
