...
This module formalizes quantum groups — Hopf algebras, quantum groups
U_q(𝔤), R-matrices, the Yang-Baxter equation, quantum enveloping
algebras, comodule algebras, the Drinfeld double, and braided
categories — all with `Path` coherence witnesses.
...
1. **Hopf algebras**: An algebra (A, m, η) equipped with a coalgebra
   structure (Δ, ε) and an antipode S : A → A satisfying
   m(S ⊗ id)Δ = η ∘ ε = m(id ⊗ S)Δ. The antipode is an
   anti-homomorphism: S(ab) = S(b)S(a).
2. **Quantum groups U_q(𝔤)**: Deformations of the universal enveloping
   algebra U(𝔤) with parameter q. Generated by E_i, F_i, K_i^{±1}
   satisfying q-deformed Serre relations.
3. **R-matrices**: Solutions R ∈ End(V ⊗ V) of the Yang-Baxter
   equation R₁₂R₁₃R₂₃ = R₂₃R₁₃R₁₂. Give rise to braided tensor
   categories.
4. **Yang-Baxter equation**: The fundamental equation of quantum
   integrability. In matrix form: (R ⊗ 1)(1 ⊗ R)(R ⊗ 1) =
   (1 ⊗ R)(R ⊗ 1)(1 ⊗ R).
5. **Quantum enveloping algebras**: U_q(𝔤) for semisimple 𝔤,
...
7. **Drinfeld double**: D(H) = H ⊗ H^{*op} with a canonical
   quasitriangular structure. dim D(H) = (dim H)².
8. **Braided categories**: Monoidal categories with a braiding
   σ_{V,W} : V ⊗ W → W ⊗ V satisfying the hexagon axioms.
...
| `QuantumEnvelopingData` | U_q(𝔤) data |
...
structure HopfAlgebraData where
  ...
  /-- algDim is positive. -/
  algDim_pos : algDim > 0
  /-- Comultiplication target dimension = algDim². -/
  comultDim : Nat
  /-- Comultiplication dimension formula. -/
  comultDim_eq : comultDim = algDim ^ 2
  /-- Counit value (on the identity: ε(1) = 1). -/
  counitValue : Int
...
structure QuantumParameter where
  ...
  rootOrder : Nat
  /-- If root of unity, order is ≥ 2. -/
  rootOrder_valid : isRootOfUnity = true → rootOrder ≥ 2
  /-- If not root of unity, rootOrder = 0. -/
  rootOrder_zero : isRootOfUnity = false → rootOrder = 0
...
def rootOfUnity (n : Nat) (hn : n ≥ 2) : QuantumParameter where
  qValue := Int.ofNat n
  isRootOfUnity := true
  rootOrder := n
...
def root_order_path (qp : QuantumParameter) (h : qp.isRootOfUnity = true) :
  ...
  Path.ofEq (Nat.min_eq_left (qp.rootOrder_valid h))
...
End(V ⊗ V) where V has dimension n. -/
...
/-- Trivial R-matrix (identity ⊗ identity = flip). -/
...
/-- Data for a quantum enveloping algebra U_q(𝔤) where 𝔤 is a
...
structure QuantumEnvelopingData where
  ...
  cartanMatrix : Nat → Nat → Int
  /-- Diagonal entries are 2: a_{ii} = 2. -/
  cartan_diag : ∀ i, i < rank → cartanMatrix i i = 2
  /-- Off-diagonal entries are ≤ 0: a_{ij} ≤ 0 for i ≠ j. -/
  cartan_offdiag : ∀ i j, i < rank → j < rank → i ≠ j →
    cartanMatrix i j ≤ 0
...
/-- U_q(𝔤) for a rank-r algebra with trivial Cartan matrix. -/
...
/-- Category of representations of U_q(sl₂). -/
...
def symmetric_order_path (bc : BraidedCategoryData) (h : bc.isSymmetric = true) :
  ...
  Path.ofEq (bc.symmetric_order h)
...
def totalDim (bc : BraidedCategoryData) : Nat :=
  List.range bc.numObjects |>.map bc.objectDim |>.foldl (· + ·) 0

end BraidedCategoryData

/-! ## Quantum Casimir Element -/

/-- The quantum Casimir element C_q: a central element in U_q(𝔤).
Centrality means [C_q, x] = 0 for all x ∈ U_q(𝔤). -/
structure QuantumCasimir where
  /-- Rank of the Lie algebra. -/
  rank : Nat
  /-- rank is positive. -/
  rank_pos : rank > 0
  /-- Casimir value on the representation V_λ (eigenvalue). -/
  eigenvalue : Nat → Int
...
(balancing) θ_V : V → V satisfying naturality and compatibility
with duality. -/
structure RibbonCategoryData where
  ...
  twistEigenvalue : Nat → Int
  /-- Twist squared eigenvalue. -/
  twistSquared : Nat → Int
  /-- Twist compatibility: θ_{V⊗W} relates to θ_V ⊗ θ_W via braiding. -/
...
dim_q(V) = Tr_V(K_{2ρ}) where ρ is the Weyl vector. -/
structure QuantumDimension where
  ...
  /-- In the classical limit (q → 1), quantum dim → classical dim. -/
  classicalLimit : Nat
...
/-- Quantum dimension of the fundamental representation of U_q(sl₂). -/
...
def classical_limit_path (qd : QuantumDimension) :
  ...
  Path.ofEq qd.classicalLimit_eq
