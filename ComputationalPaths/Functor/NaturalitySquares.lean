...
private noncomputable def naturalityCore {a b : A} (p : Path a b) (x : F.obj a) :
...
noncomputable def naturalityWitness {a b : A} (p : Path a b) (x : F.obj a) :
    Path (topThenRight (η := η) p x) (leftThenBottom (η := η) p x) :=
  Path.trans
...
noncomputable def naturalityWitness_comp {a b c : A}
...
noncomputable def naturalityWitness_comp_via_mapComp {a b c : A}
    (p : Path a b) (q : Path b c) (x : F.obj a) :
    Path (G.map q (G.map p (η.app a x))) (η.app c (F.map q (F.map p x))) :=
  Path.trans
...
noncomputable def induced_commutative_diagram {a b : A} (p : Path a b) (x : F.obj a) :
    Path (topThenRight (η := η) p x) (leftThenBottom (η := η) p x) :=
