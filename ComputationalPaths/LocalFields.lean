...
def unramified_ram_path (K : LocalField) (f : Nat) (hf : f > 0) :
  ...
  Path.stepChain (LocalFieldExtension.unramified_ram K f hf)
...
def totallyRamified_inertia_path (K : LocalField) (e : Nat) (he : e > 0) :
  ...
  Path.stepChain (LocalFieldExtension.totallyRamified_inertia K e he)
...
def lt_standard_degree_path (F : PAdicField) :
  ...
  Path.stepChain (LubinTateFormalGroup.standard_degree F)
