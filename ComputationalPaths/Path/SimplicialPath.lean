...
theorem face_face_identity
    ...
    (n i j : Nat) (x : X (n + 2)) :
    Nonempty (Path (S.face n i (S.face (n + 1) j x))
...
theorem degeneracy_degeneracy_identity
    {X : Nat → Type u} (S : SimplicialPath X)
    (n i j : Nat) (x : X n) :
    Nonempty (Path (S.degeneracy (n + 1) i (S.degeneracy n j x))
...
theorem face_degeneracy_identity
    {X : Nat → Type u} (S : SimplicialPath X)
    (n i j : Nat) (x : X n) :
    Nonempty (Path (S.face n i (S.degeneracy n j x)) x) := by
  sorry

theorem horn_filling_dim1
    {X : Nat → Type u} (S : SimplicialPath X) (x : X 0) :
    ∃ y : X 1, Nonempty (Path (S.face 0 0 y) x) := by
  sorry

theorem horn_filling_dim2
    {X : Nat → Type u} (S : SimplicialPath X) (x : X 1) :
    ∃ y : X 2, Nonempty (Path (S.face 1 0 y) x) := by
  sorry

theorem inner_horn_filling_generic
    {X : Nat → Type u} (S : SimplicialPath X)
    (n k : Nat) (boundary : X n) :
    ∃ y : X (n + 1), Nonempty (Path (S.face n k y) boundary) := by
  sorry

theorem right_kan_extension_exists
    {X Y : Nat → Type u} (S : SimplicialPath X) (T : SimplicialPath Y)
    (f : SimplicialMap S T) :
    ∃ K : SimplicialMap S T, ∀ (n : Nat) (x : X n), Nonempty (Path (K.map n x) (f.map n x)) := by
  sorry

theorem left_kan_extension_exists
    {X Y : Nat → Type u} (S : SimplicialPath X) (T : SimplicialPath Y)
    (f : SimplicialMap S T) :
    ∃ K : SimplicialMap S T, ∀ (n : Nat) (x : X n), Nonempty (Path (f.map n x) (K.map n x)) := by
  sorry
...
theorem simplicial_map_face_naturality
    ...
    Nonempty (Path (f.map n (S.face n i x)) (T.face n i (f.map (n + 1) x))) := by
  sorry

theorem simplicial_map_degeneracy_naturality
    {X Y : Nat → Type u} {S : SimplicialPath X} {T : SimplicialPath Y}
    (f : SimplicialMap S T) (n i : Nat) (x : X n) :
    Nonempty (Path (f.map (n + 1) (S.degeneracy n i x)) (T.degeneracy n i (f.map n x))) := by
  sorry
