...
noncomputable def OrbitRel (x y : X) : Prop :=
  ∃ g : G, A.act g x = y
...
noncomputable def orbitMap : X → OrbitSpace A :=
  fun x => Quot.mk _ x
...
noncomputable def equivariantPath_of_path {x y : X} (p : Path x y) :
...
noncomputable def EquivariantHomotopy (A : GroupAction G S X) (B : GroupAction G S Y)
...
@[simp] noncomputable def equivariantHomotopy_refl (A : GroupAction G S X) (B : GroupAction G S Y)
    (f : X → Y) :
    EquivariantHomotopy A B f f :=
  fun _ => Path.refl _

/-- Symmetry of equivariant homotopy. -/
@[simp] noncomputable def equivariantHomotopy_symm (A : GroupAction G S X) (B : GroupAction G S Y)
    {f g : X → Y} (H : EquivariantHomotopy A B f g) :
    EquivariantHomotopy A B g f :=
  fun x => Path.symm (H x)

/-- Transitivity of equivariant homotopy. -/
@[simp] noncomputable def equivariantHomotopy_trans (A : GroupAction G S X) (B : GroupAction G S Y)
    {f g h : X → Y}
...
noncomputable def orbitSpacePiOneMap (x : X) :
