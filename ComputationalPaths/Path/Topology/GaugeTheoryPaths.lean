...
/-- Bianchi identity certificate at a concrete base point. -/
...
/-- Holonomy conjugation certificate for a concrete gauge element. -/
...
/-- Ad-invariance certificate for a sampled adjoint orbit. -/
structure InvariantPolynomialCertificate (G : GaugeGroup)
    (eval : G.lieAlg → Int) where
  groupElement : G.carrier
  sample : G.lieAlg
  adjointPath : Path (eval (adjointRep G groupElement sample)) (eval sample)
  adjointTrace :
    PathLawCertificate (eval (adjointRep G groupElement sample)) (eval sample)
...
/-- Closed characteristic-form certificate at a concrete base point. -/
structure ClosedCharacteristicCertificate (G : GaugeGroup) (P : PrincipalBundle G)
    (charForm : P.base → Int) where
  sample : P.base
  closedPath : Path (charForm sample) (charForm sample)
  closedTrace : PathLawCertificate (charForm sample) (charForm sample)

/-- Connection-independence certificate comparing two sampled connections. -/
structure ConnectionIndependenceCertificate (G : GaugeGroup) (P : PrincipalBundle G)
    (charForm : P.base → Int) where
  source : Connection G P
  target : Connection G P
  sample : P.base
  comparisonPath : Path (charForm sample) (charForm sample)
  comparisonTrace : PathLawCertificate (charForm sample) (charForm sample)
...
theorem cs_cobordism (G : GaugeGroup) (P : PrincipalBundle G)
    (_CS : ChernSimonsInvariant G P) :
    True := _CS.transgression
...
theorem volume_conjecture (G : GaugeGroup) (ae : AsymptoticExpansion G) :
    True := ae.leading_term
...
theorem floer_exact_triangle_exists (_G : GaugeGroup) (tri : FloerExactTriangle _G) :
    True := tri.exact
