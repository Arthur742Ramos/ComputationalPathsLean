...
noncomputable def InnerProductSpace.normSq (H : InnerProductSpace) (v : H.carrier) : Int :=
  H.inner v v

/-- 1. Norm squared of zero -/
noncomputable def normSq_zero (H : InnerProductSpace) :
    Path (H.normSq H.zero) 0 :=
  H.inner_zero_left H.zero

/-- 2. Inner product with zero on the right -/
noncomputable def inner_right_zero (H : InnerProductSpace) (v : H.carrier) :
    Path (H.inner v H.zero) 0 :=
  H.inner_zero_right v

/-- 3. Inner product with zero on the left -/
noncomputable def inner_left_zero (H : InnerProductSpace) (v : H.carrier) :
    Path (H.inner H.zero v) 0 :=
  H.inner_zero_left v

/-- 4. Inner product commutativity -/
noncomputable def inner_comm_path (H : InnerProductSpace) (v w : H.carrier) :
    Path (H.inner v w) (H.inner w v) :=
  H.inner_comm v w

/-- 5. Double commutativity returns to start -/
noncomputable def inner_comm_double (H : InnerProductSpace) (v w : H.carrier) :
    Path (H.inner v w) (H.inner v w) :=
  trans (H.inner_comm v w) (H.inner_comm w v)

/-- 6. Inner product with neg on left -/
noncomputable def inner_neg_left_path (H : InnerProductSpace) (v w : H.carrier) :
    Path (H.inner (H.neg v) w) (- H.inner v w) :=
  H.inner_neg_left v w

/-- 7. Inner product with neg on right via commutativity -/
noncomputable def inner_neg_right_path (H : InnerProductSpace) (v w : H.carrier) :
...
noncomputable def normSq_neg (H : InnerProductSpace) (v : H.carrier) :
...
noncomputable def orthogonal_symm (H : InnerProductSpace) (v w : H.carrier)
    (h : Orthogonal H v w) : Orthogonal H w v :=
  ⟨trans (H.inner_comm w v) h.witness⟩

/-- 10. Zero is orthogonal to everything -/
noncomputable def zero_orthogonal (H : InnerProductSpace) (v : H.carrier) :
    Orthogonal H H.zero v :=
  ⟨H.inner_zero_left v⟩

/-- 11. Everything is orthogonal to zero -/
noncomputable def orthogonal_zero (H : InnerProductSpace) (v : H.carrier) :
    Orthogonal H v H.zero :=
  ⟨H.inner_zero_right v⟩

/-- 12. Zero is orthogonal to itself -/
noncomputable def zero_self_orthogonal (H : InnerProductSpace) :
...
noncomputable def proj_triple (H : InnerProductSpace) (P : Projection H) (v : H.carrier) :
    Path (P.proj (P.proj (P.proj v))) (P.proj v) :=
  trans (congrArg P.proj (P.idempotent v)) (P.idempotent v)

/-- 14. Projection applied four times -/
noncomputable def proj_quad (H : InnerProductSpace) (P : Projection H) (v : H.carrier) :
    Path (P.proj (P.proj (P.proj (P.proj v)))) (P.proj v) :=
  trans (congrArg P.proj (proj_triple H P v)) (P.idempotent v)

/-- 15. Projection at zero -/
noncomputable def proj_zero_path (H : InnerProductSpace) (P : Projection H) :
    Path (P.proj H.zero) H.zero :=
  P.proj_zero

/-- 16. Double projection at zero -/
noncomputable def proj_zero_double (H : InnerProductSpace) (P : Projection H) :
    Path (P.proj (P.proj H.zero)) H.zero :=
  trans (congrArg P.proj P.proj_zero) P.proj_zero

/-- The identity projection. -/
noncomputable def Projection.identity (H : InnerProductSpace) : Projection H where
  proj := fun v => v
  idempotent := fun _ => Path.refl _
  proj_zero := Path.refl _

/-- The zero projection. -/
noncomputable def Projection.zeroProj (H : InnerProductSpace) : Projection H where
  proj := fun _ => H.zero
  idempotent := fun _ => Path.refl _
  proj_zero := Path.refl _

/-- 17. Identity projection is transparent -/
noncomputable def id_proj_transparent (H : InnerProductSpace) (v : H.carrier) :
    Path ((Projection.identity H).proj v) v :=
  Path.refl _

/-- 18. Zero projection sends everything to zero -/
noncomputable def zero_proj_value (H : InnerProductSpace) (v : H.carrier) :
...
noncomputable def riesz_at_zero (H : InnerProductSpace) (r : RieszRep H) :
    Path (r.func.eval H.zero) 0 :=
  r.func.eval_zero

/-- 20. Riesz rep at zero via inner product -/
noncomputable def riesz_at_zero_inner (H : InnerProductSpace) (r : RieszRep H) :
    Path (H.inner r.representer H.zero) 0 :=
  H.inner_zero_right r.representer

/-- 21. Riesz rep consistency at zero -/
noncomputable def riesz_zero_consistent (H : InnerProductSpace) (r : RieszRep H) :
...
noncomputable def zero_decomp (H : InnerProductSpace) :
...
noncomputable def decomp_inner_proj (H : InnerProductSpace) (v : H.carrier)
    (d : OrthoDecomp H v) :
    Path (H.inner d.proj_part d.comp_part) 0 :=
  d.ortho.witness

/-- 24. Symmetry of orthogonality in decomposition -/
noncomputable def decomp_ortho_symm (H : InnerProductSpace) (v : H.carrier)
    (d : OrthoDecomp H v) :
...
noncomputable def gs_orthogonal (H : InnerProductSpace) (gs : GramSchmidtPair H) :
    Path (H.inner gs.e₁ gs.e₂) 0 :=
  gs.ortho.witness

/-- 26. Gram-Schmidt pair reverse orthogonality -/
noncomputable def gs_orthogonal_symm (H : InnerProductSpace) (gs : GramSchmidtPair H) :
    Path (H.inner gs.e₂ gs.e₁) 0 :=
...
noncomputable def ons_ortho (H : InnerProductSpace) (sys : OrthonormalSystem H)
    (i j : Nat) (h : i ≠ j) :
    Path (H.inner (sys.basis i) (sys.basis j)) 0 :=
  (sys.ortho i j h).witness

/-- 28. Orthonormal system: basis elements are normalized -/
noncomputable def ons_normalized (H : InnerProductSpace) (sys : OrthonormalSystem H)
    (i : Nat) :
    Path (H.inner (sys.basis i) (sys.basis i)) 1 :=
  sys.normalized i

/-- 29. Reverse orthogonality in orthonormal system -/
noncomputable def ons_ortho_symm (H : InnerProductSpace) (sys : OrthonormalSystem H)
    (i j : Nat) (h : i ≠ j) :
...
/-- 30. Adjoint property -/
noncomputable def adjoint_inner (H : InnerProductSpace) (ap : AdjointPair H)
    (v w : H.carrier) :
    Path (H.inner (ap.op v) w) (H.inner v (ap.adj w)) :=
  ap.adjoint_prop v w

/-- 31. Adjoint op at zero -/
noncomputable def adjoint_op_zero (H : InnerProductSpace) (ap : AdjointPair H) :
    Path (ap.op H.zero) H.zero :=
  ap.op_zero

/-- 32. Adjoint adj at zero -/
noncomputable def adjoint_adj_zero (H : InnerProductSpace) (ap : AdjointPair H) :
    Path (ap.adj H.zero) H.zero :=
  ap.adj_zero

/-- 33. Inner product of op(0) with anything -/
noncomputable def adjoint_op_zero_inner (H : InnerProductSpace) (ap : AdjointPair H)
    (v : H.carrier) :
    Path (H.inner (ap.op H.zero) v) 0 :=
  trans (congrArg (H.inner · v) ap.op_zero) (H.inner_zero_left v)

/-- 34. Inner product with adj(0) -/
noncomputable def adjoint_adj_zero_inner (H : InnerProductSpace) (ap : AdjointPair H)
    (v : H.carrier) :
...
noncomputable def sa_at_zero (H : InnerProductSpace) (sa : SelfAdjoint H) :
    Path (sa.op H.zero) H.zero :=
  sa.op_zero

/-- 36. Self-adjoint inner with zero -/
noncomputable def sa_inner_zero (H : InnerProductSpace) (sa : SelfAdjoint H)
    (v : H.carrier) :
...
noncomputable def sa_double_adj (H : InnerProductSpace) (sa : SelfAdjoint H)
    (v w : H.carrier) :
    Path (H.inner (sa.op v) w) (H.inner v (sa.op w)) :=
  sa.self_adj v w

/-- 38. Self-adjoint is its own adjoint -/
noncomputable def sa_to_adjoint_pair (H : InnerProductSpace) (sa : SelfAdjoint H) :
    AdjointPair H where
  op := sa.op
  adj := sa.op
...
noncomputable def topologyHilbertPathsAssoc (a b c : Nat) : Path ((a + b) + c) (a + (b + c)) :=
  Path.ofEq (Nat.add_assoc a b c)

/-- Commutativity rewrite `a + b ⤳ b + a`: one genuine step. -/
noncomputable def topologyHilbertPathsComm (a b : Nat) : Path (a + b) (b + a) :=
  Path.ofEq (Nat.add_comm a b)

/-- Inner commutativity `a + (b + c) ⤳ a + (c + b)` via congruence in the
    right argument (`_root_.congrArg`, since `congrArg` here is `Path.congrArg`). -/
noncomputable def topologyHilbertPathsInner (a b c : Nat) : Path (a + (b + c)) (a + (c + b)) :=
  Path.ofEq (_root_.congrArg (fun t => a + t) (Nat.add_comm b c))

/-- A genuine **two-step** path: reassociate, then commute the inner pair.
    Its trace has length two — this is not a reflexive path. -/
noncomputable def topologyHilbertPathsTwoStep (a b c : Nat) : Path ((a + b) + c) (a + (c + b)) :=
  Path.trans (topologyHilbertPathsAssoc a b c) (topologyHilbertPathsInner a b c)

/-- The two-step path composed with its inverse cancels to the reflexive path —
    a non-decorative `RwEq` (the `trans_symm` rule on a length-two trace). -/
noncomputable def topologyHilbertPathsCancel (a b c : Nat) :
    Path.RwEq (Path.trans (topologyHilbertPathsTwoStep a b c) (Path.symm (topologyHilbertPathsTwoStep a b c)))
      (Path.refl ((a + b) + c)) :=
  Path.rweq_cmpA_inv_right (topologyHilbertPathsTwoStep a b c)

/-- Associativity-of-composition (`trans_assoc`, the `tt` rewrite) on any three
    composable paths — a genuine `RwEq` between distinct bracketings. -/
noncomputable def topologyHilbertPathsAssocCoh {α : Type} {a b c d : α}
