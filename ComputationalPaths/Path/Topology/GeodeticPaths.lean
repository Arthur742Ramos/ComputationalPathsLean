...
noncomputable def pathLength {A : Type u} {a b : A} (p : Path a b) : Nat :=
  p.steps.length
...
noncomputable def distSelf_step {A : Type u} (M : PathMetric A) (a : A) :
    Path (M.dist a a) 0 :=
  Path.stepChain (M.dist_self a)

/-- Single-step path from dist_symm. -/
noncomputable def distSymm_step {A : Type u} (M : PathMetric A) (a b : A) :
...
/-- Reflexive geodesic. -/
noncomputable def geodesicRefl {A : Type u} (M : PathMetric A) (a : A) :
    Geodesic M a a where
  path := Path.refl a
  optimal := by simp [pathLength, M.dist_self]

/-- Step: length(g.path) = dist(a,b). -/
noncomputable def geodesic_optimal_step {A : Type u} {M : PathMetric A} {a b : A}
    (g : Geodesic M a b) :
...
noncomputable def geodesic_length_path {A : Type u} {M : PathMetric A} {a b : A}
    (g₁ g₂ : Geodesic M a b) :
...
noncomputable def geodesic_optimal_succ {A : Type u} {M : PathMetric A} {a b : A}
    (g : Geodesic M a b) :
...
noncomputable def trivialMidpoint {A : Type u} (M : PathMetric A) (a : A) :
    Midpoint M a a where
  mid := a
  equidist := by simp [M.dist_self]

/-- Step: dist(a, a) = 0 for trivial midpoint. -/
noncomputable def trivialMidpoint_dist_step {A : Type u} (M : PathMetric A) (a : A) :
    Path (M.dist a (trivialMidpoint M a).mid) 0 :=
  distSelf_step M a

/-- 2-step: dist(a, trivialMid.mid) → 0 → dist(a, trivialMid.mid). -/
noncomputable def trivialMidpoint_roundtrip {A : Type u} (M : PathMetric A) (a : A) :
...
noncomputable def compTriangle {A : Type u} (M : PathMetric A) (a b c : A) :
...
noncomputable def perimeter (t : ComparisonTriangle) : Nat :=
  t.side_a + t.side_b + t.side_c

/-- A degenerate triangle has a zero side. -/
noncomputable def isDegenerate (t : ComparisonTriangle) : Prop :=
...
noncomputable def compTriangle_side_step {A : Type u} (M : PathMetric A) (a b c : A) :
    Path (compTriangle M a b c).side_c (M.dist a b) :=
  Path.refl _

/-- Step: side_a = dist(b,c). -/
noncomputable def compTriangle_sideA_step {A : Type u} (M : PathMetric A) (a b c : A) :
    Path (compTriangle M a b c).side_a (M.dist b c) :=
  Path.refl _

/-- 2-step: side_a of a triangle → dist(b,c) → dist(c,b) via symm. -/
noncomputable def compTriangle_sideA_symm {A : Type u} (M : PathMetric A) (a b c : A) :
...
noncomputable def perimeter_succ (t : ComparisonTriangle) :
...
noncomputable def cat_zero_self_step {A : Type u} {M : PathMetric A}
    (hcat : CATZeroSpace M) (a : A) :
    Path (M.dist a (trivialMidpoint M a).mid) 0 :=
  Path.stepChain (cat_zero_self_midpoint hcat a)

/-- 2-step: dist(a, trivMid) = 0, then Nat.succ over it. -/
noncomputable def cat_zero_self_succ {A : Type u} {M : PathMetric A}
    (hcat : CATZeroSpace M) (a : A) :
...
noncomputable def cat_zero_self_symm {A : Type u} {M : PathMetric A}
    (hcat : CATZeroSpace M) (a : A) :
...
noncomputable def geodesic_concat_step {A : Type u} {M : PathMetric A}
    {a b c : A} (g₁ : Geodesic M a b) (g₂ : Geodesic M b c) :
...
noncomputable def geodesic_concat_symm_left {A : Type u} {M : PathMetric A}
    {a b c : A} (g₁ : Geodesic M a b) (g₂ : Geodesic M b c) :
...
noncomputable def geodesic_concat_succ {A : Type u} {M : PathMetric A}
    {a b c : A} (g₁ : Geodesic M a b) (g₂ : Geodesic M b c) :
...
noncomputable def geodesicSymm {A : Type u} {M : PathMetric A} {a b : A}
...
noncomputable def geodesic_symm_length_step {A : Type u} {M : PathMetric A} {a b : A}
    (g : Geodesic M a b) :
    Path (pathLength (geodesicSymm g).path) (M.dist b a) :=
  Path.stepChain (geodesicSymm g).optimal

/-- 2-step: len(symm(g)) → dist(b,a) → dist(a,b). -/
noncomputable def geodesic_symm_length_dist {A : Type u} {M : PathMetric A} {a b : A}
    (g : Geodesic M a b) :
...
noncomputable def geodesic_symm_length_roundtrip {A : Type u} {M : PathMetric A} {a b : A}
    (g : Geodesic M a b) :
...
noncomputable def thin_transport {A : Type u} {M : PathMetric A} {δ₁ δ₂ : Nat}
    (h : δ₁ = δ₂) (t : ThinTriangle M δ₁) : ThinTriangle M δ₂ :=
  h ▸ t

/-- Step: δ + 0 = δ for thin-delta parameters. -/
noncomputable def thin_delta_step (δ : Nat) : Path (δ + 0) δ :=
  Path.stepChain (Nat.add_zero δ)

/-- symm: δ = δ + 0. -/
noncomputable def thin_delta_symm (δ : Nat) : Path δ (δ + 0) :=
  Path.symm (thin_delta_step δ)

/-- Round-trip: (δ + 0) → δ → (δ + 0). -/
noncomputable def thin_delta_roundtrip (δ : Nat) :
    Path (δ + 0) (δ + 0) :=
  Path.trans (thin_delta_step δ) (thin_delta_symm δ)

/-- congrArg: Nat.succ(δ + 0) = Nat.succ(δ). -/
noncomputable def thin_delta_succ (δ : Nat) :
...
noncomputable def geodesic_concat_full_symm {A : Type u} {M : PathMetric A}
    {a b c : A} (g₁ : Geodesic M a b) (g₂ : Geodesic M b c) :
    Path (pathLength (Path.trans g₁.path g₂.path))
...
noncomputable def midpoint_dist_double {A : Type u} {M : PathMetric A} {a b : A}
    (m : Midpoint M a b) :
    Path (M.dist a m.mid) (M.dist b m.mid) :=
  Path.trans (midpoint_step m) (distSymm_step M m.mid b)

/-- Nested congrArg: f(g(dist(a,a))) = f(g(0)). -/
noncomputable def distSelf_nested_congrArg {A : Type u} (M : PathMetric A)
    (f g : Nat → Nat) (a : A) :
