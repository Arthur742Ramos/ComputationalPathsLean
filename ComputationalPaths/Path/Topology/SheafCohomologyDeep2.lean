...
noncomputable def trans {A : Type u} {a b c : A} : SheafPath a b → SheafPath b c → SheafPath a c
  | .nil _, q => q
  | .cons s p, q => .cons s (trans p q)

noncomputable def symm {A : Type u} {a b : A} : SheafPath a b → SheafPath b a
  | .nil _ => .nil _
  | .cons s p => trans (symm p) (.cons (.symm s) (.nil _))

noncomputable def congrArg {A B : Type u} (f : A → B) {a b : A} : SheafPath a b → SheafPath (f a) (f b)
  | .nil _ => .nil _
  | .cons s p => .cons (.congrArg f s) (congrArg f p)

noncomputable def length {A : Type u} {a b : A} : SheafPath a b → Nat
  | .nil _ => 0
  | .cons _ p => 1 + p.length
...
noncomputable def cech_derived_path {A : Type u} (cech derived : A → A) (F : A) :
    SheafPath (cech F) (derived F) :=
  .cons (.cechDerived cech derived F) (.nil _)

-- 2. Godement resolution step
noncomputable def godement_step {A : Type u} (G : Nat → A → A) (n : Nat) (F : A) :
    SheafPath (G n F) (G (n + 1) F) :=
  .cons (.godementResolution G n F) (.nil _)

-- 3. Godement two-step
noncomputable def godement_two_step {A : Type u} (G : Nat → A → A) (n : Nat) (F : A) :
...
noncomputable def godement_three_step {A : Type u} (G : Nat → A → A) (n : Nat) (F : A) :
...
noncomputable def de_rham_serre {A : Type u} (HdR Hsing Hdual : A → A) (X : A) :
...
noncomputable def leray_vanishing {A : Type u} (E₂ : A → A → A) (Htotal zero : A → A) (p q : A) :
...
noncomputable def acyclic_resolution_path {A : Type u} (F resolvedF : A) :
    SheafPath F resolvedF :=
  .cons (.acyclicResolution F resolvedF) (.nil _)

-- 20. Long exact connecting path
noncomputable def long_exact_connecting_path {A : Type u} (δ : A → A) (x : A) :
    SheafPath x (δ x) :=
  .cons (.longExactConnecting δ x) (.nil _)

-- 21. Long exact sequence: two connecting morphisms
noncomputable def long_exact_double_connecting {A : Type u} (δ₁ δ₂ : A → A) (x : A) :
    SheafPath x (δ₂ (δ₁ x)) :=
  SheafPath.trans
...
noncomputable def long_exact_triple_connecting {A : Type u} (δ₁ δ₂ δ₃ : A → A) (x : A) :
    SheafPath x (δ₃ (δ₂ (δ₁ x))) :=
  SheafPath.trans
...
noncomputable def mayer_vietoris_three {A : Type u} (H₀ H₁ H₂ H₃ : A → A) (UV : A) :
    SheafPath (H₀ UV) (H₃ UV) :=
  SheafPath.trans
...
noncomputable def godement_cech {A : Type u} (G : Nat → A → A) (cech derived : A → A)
    (n : Nat) (F : A)
...
noncomputable def leray_serre_chain {A : Type u} (E₂ : A → A → A) (Htotal Hdual : A → A) (p q : A) :
...
noncomputable def de_rham_vanishing {A : Type u} (HdR Hsing zero : A → A) (X : A) :
...
noncomputable def acyclic_serre {A : Type u} (Hi Hdual : A → A) (F : A) (resolvedF : A)
...
noncomputable def congr_base_change {A : Type u} (f lhs rhs : A → A) (F : A) :
    SheafPath (f (lhs F)) (f (rhs F)) :=
  (base_change_path lhs rhs F).congrArg f

-- 33. Double congrArg over Godement
noncomputable def double_congr_godement {A : Type u} (f g : A → A) (G : Nat → A → A) (n : Nat) (F : A) :
    SheafPath (f (g (G n F))) (f (g (G (n + 1) F))) :=
  ((godement_step G n F).congrArg g).congrArg f

-- 34. Serre duality roundtrip
noncomputable def serre_roundtrip {A : Type u} (Hi Hdual : A → A) (F : A) :
    SheafPath (Hi F) (Hi F) :=
  SheafPath.trans
    (serre_duality_path Hi Hdual F)
...
noncomputable def full_cohomology_pipeline {A : Type u} (cech derived Hdual : A → A) (F : A) :
    SheafPath (cech F) (Hdual F) :=
  SheafPath.trans
    (cech_derived_path cech derived F)
...
noncomputable def mayer_vietoris_de_rham {A : Type u} (Hn HnPlus HdR Hsing : A → A) (UV : A)
...
noncomputable def projection_formula_congr {A : Type u} (f lhs rhs : A → A) (FG : A) :
    SheafPath (f (lhs FG)) (f (rhs FG)) :=
  (projection_formula_path lhs rhs FG).congrArg f

-- 38. Godement four-step
noncomputable def godement_four_step {A : Type u} (G : Nat → A → A) (n : Nat) (F : A) :
...
noncomputable def long_exact_mayer_vietoris {A : Type u} (δ Hn HnPlus : A → A) (x : A)
