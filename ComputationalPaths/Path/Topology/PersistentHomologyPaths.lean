...
noncomputable def PersistenceInterval.lifetime (iv : PersistenceInterval) : Nat :=
  if iv.death == 0 then 0 else iv.death - iv.birth

/-- Whether an interval is alive at time t. -/
noncomputable def PersistenceInterval.aliveAt (iv : PersistenceInterval) (t : Nat) : Bool :=
  iv.birth ≤ t && (iv.death == 0 || t < iv.death)

/-- Persistence module dimension at time t (simplified). -/
noncomputable def moduleDim (intervals : List PersistenceInterval) (t : Nat) : Nat :=
...
noncomputable def barcodeConcat (b1 b2 : List PersistenceInterval) : List PersistenceInterval := b1 ++ b2

/-- Empty barcode. -/
noncomputable def barcodeEmpty : List PersistenceInterval := []
...
noncomputable def barcode_assoc_unit (b1 b2 : List PersistenceInterval) :
...
noncomputable def barcode_nested_empty (b1 b2 : List PersistenceInterval) :
...
noncomputable def barcode_assoc_four (b1 b2 b3 b4 : List PersistenceInterval) :
...
noncomputable def barcode_assoc_symm (b1 b2 b3 : List PersistenceInterval) :
...
noncomputable def barcode_congrArg_unitR (b1 b2 : List PersistenceInterval) :
...
noncomputable def barcode_double_congrArg (b1 b2 b3 : List PersistenceInterval) :
...
noncomputable def bettiNumber (ivs : List PersistenceInterval) (t : Nat) : Nat :=
  moduleDim ivs t

/-- Step: bettiNumber [] t = 0. -/
noncomputable def betti_empty_step (t : Nat) : Path (bettiNumber [] t) 0 :=
  Path.stepChain (moduleDim_empty t)

/-- symm: 0 = bettiNumber [] t. -/
noncomputable def betti_empty_symm (t : Nat) : Path 0 (bettiNumber [] t) :=
  Path.symm (betti_empty_step t)

/-- Round-trip: betti([] ,t) → 0 → betti([], t). -/
noncomputable def betti_empty_roundtrip (t : Nat) :
...
noncomputable def betti_empty_concat (b : List PersistenceInterval) (t : Nat) :
...
noncomputable def betti_concat_empty (b : List PersistenceInterval) (t : Nat) :
...
noncomputable def betti_nested_empty (b1 b2 : List PersistenceInterval) (t : Nat) :
...
noncomputable def filtrationMap (f : Nat → Nat) (n : Nat) : Nat := f n
...
noncomputable def filtrationMap_id_path (n : Nat) : Path (filtrationMap id n) n :=
  Path.refl n

/-- Path for composition. -/
noncomputable def filtrationMap_comp_path (f g : Nat → Nat) (n : Nat) :
    Path (filtrationMap f (filtrationMap g n)) (filtrationMap (f ∘ g) n) :=
  Path.refl _

/-- congrArg: mapping a function h over filtrationMap_id. -/
noncomputable def filtrationMap_congrArg (h : Nat → Nat) (n : Nat) :
    Path (h (filtrationMap id n)) (h n) :=
  Path.congrArg h (filtrationMap_id_path n)

/-- 2-step functoriality: map id after map g = map g. -/
noncomputable def filtrationMap_functor_2step (g : Nat → Nat) (n : Nat) :
...
noncomputable def eulerChar (b0 b1 b2 : Nat) : Int := (b0 : Int) - (b1 : Int) + (b2 : Int)
...
noncomputable def diagramSize (ivs : List PersistenceInterval) : Nat := ivs.length
...
noncomputable def diagram_size_cons_step (iv : PersistenceInterval) (ivs : List PersistenceInterval) :
...
noncomputable def diagram_size_cons_symm (iv : PersistenceInterval) (ivs : List PersistenceInterval) :
    Path (diagramSize ivs + 1) (diagramSize (iv :: ivs)) :=
  Path.symm (diagram_size_cons_step iv ivs)

/-- Round-trip on diagram size. -/
noncomputable def diagram_size_roundtrip (iv : PersistenceInterval) (ivs : List PersistenceInterval) :
...
noncomputable def diagram_size_succ_congrArg (iv : PersistenceInterval) (ivs : List PersistenceInterval) :
...
noncomputable def diagram_size_double_cons (iv1 iv2 : PersistenceInterval)
...
noncomputable def intervalDist (i1 i2 : PersistenceInterval) : Nat :=
...
/-- Step: dist(iv, iv) = 0. -/
noncomputable def intervalDist_self_step (iv : PersistenceInterval) :
    Path (intervalDist iv iv) 0 :=
  Path.stepChain (intervalDist_self_eq iv)

/-- symm: 0 = dist(iv, iv). -/
noncomputable def intervalDist_self_symm (iv : PersistenceInterval) :
    Path 0 (intervalDist iv iv) :=
  Path.symm (intervalDist_self_step iv)

/-- Round-trip on dist reflexivity. -/
noncomputable def intervalDist_self_roundtrip (iv : PersistenceInterval) :
...
noncomputable def intervalDist_self_succ (iv : PersistenceInterval) :
...
noncomputable def persistenceMap_congr_step (f : Nat → Nat)
    (a : Nat) : Path (f a) (f a) :=
  Path.refl (f a)

/-- congrArg: mapping a function g over a persistence map application. -/
noncomputable def persistenceMap_congrArg (f g : Nat → Nat)
    (a b : Nat) (p : Path a b) :
    Path (f (g a)) (f (g b)) :=
  Path.congrArg f (Path.congrArg g p)

/-- Double congrArg composition. -/
noncomputable def persistenceMap_double_congrArg (f g h : Nat → Nat)
    (a b : Nat) (p : Path a b) :
...
noncomputable def betti_assoc_path (b1 b2 b3 : List PersistenceInterval) (t : Nat) :
...
noncomputable def diagramSize_nested_empty (b1 b2 : List PersistenceInterval) :
...
noncomputable def euler_add_symm (a0 a1 a2 b0 b1 b2 : Nat) :
...
noncomputable def euler_two_point :
...
noncomputable def diagramSize_functor (b : List PersistenceInterval) :
