...
-/
noncomputable def toPath {a b : A} : MorseStep (M := M) a b → Path a b
  | .flow_crit_fixed p =>
...
noncomputable def attach_assoc_path (p q r : A) : Path (M.attach (M.attach p q) r) (M.attach p (M.attach q r)) :=
  ⟦MorseStep.attach_assoc (M := M) p q r⟧

-- 11. Attachment unit

noncomputable def attach_unit_path (p : A) : Path (M.attach p M.base) p :=
  ⟦MorseStep.attach_unit (M := M) p⟧

-- 12. Cancellation-like 3-step: attach (attach p base) base -> p

noncomputable def attach_unit_twice (p : A) : Path (M.attach (M.attach p M.base) M.base) p :=
  Path.trans
...
noncomputable def cell_attach_path (p q : A) : Path (M.cell (M.attach p q)) (M.attach (M.cell p) (M.cell q)) :=
  ⟦MorseStep.cell_attach (M := M) p q⟧

-- 14. Cell idempotent

noncomputable def cell_idem_path (p : A) : Path (M.cell (M.cell p)) (M.cell p) :=
  ⟦MorseStep.cell_idem (M := M) p⟧

-- 15. Cell of cell(attach p q) reduces (3 steps)

noncomputable def cell_cell_attach_3step (p q : A) :
...
noncomputable def cobord_assoc_path (p q r : A) : Path (M.cobord p (M.cobord q r)) (M.cobord (M.cobord p q) r) :=
  ⟦MorseStep.cobord_assoc (M := M) p q r⟧

-- 17. Cobordism identity

noncomputable def cobord_id_path (p : A) : Path (M.cobord p M.base) p :=
...
noncomputable def cell_of_bdry_sq (p : A) : Path (M.cell (M.bdry (M.bdry p))) (M.cell M.base) :=
  Path.congrArg M.cell (bdry_squared_path (M := M) p)

-- 26. ∂² = 0, then apply crit (congrArg)

noncomputable def crit_of_bdry_sq (p : A) : Path (M.crit (M.bdry (M.bdry p))) (M.crit M.base) :=
  Path.congrArg M.crit (bdry_squared_path (M := M) p)

-- 27. Smale-style cancellation toy: attach(attach p q) base -> attach p q

noncomputable def smale_cancel_toy (p q : A) : Path (M.attach (M.attach p q) M.base) (M.attach p q) :=
...
noncomputable def handle_cancel_3step (p q : A) : Path (M.attach (M.attach p q) M.base) (M.attach p q) :=
  Path.trans
...
noncomputable def cerf_loop_3step (p : A) : Path (M.crit (M.crit p)) (M.crit p) :=
  Path.trans
...
noncomputable def bdry_nested_congrArg (p : A) :
...
noncomputable def bdry_nested_to_base (p : A) :
...
noncomputable def cobord_bdry_congrArg (p : A) : Path (M.bdry (M.cobord p M.base)) (M.bdry p) :=
  Path.congrArg M.bdry (cobord_id_path (M := M) p)

-- 33. Euler invariance under cobordism identity (2 steps)

noncomputable def euler_cobord_id (p : A) : Path (M.euler (M.cobord p M.base)) (M.crit p) :=
  Path.trans
...
noncomputable def cell_cobord_id (p : A) : Path (M.cell (M.cobord p M.base)) (M.cell p) :=
  Path.congrArg M.cell (cobord_id_path (M := M) p)

-- 35. A final coherence loop: attach_unit then symm gives a loop

noncomputable def attach_unit_loop (p : A) : Path (M.attach p M.base) (M.attach p M.base) :=
...
noncomputable def trivial : MorseAlg PUnit where
  f := fun _ => PUnit.unit
