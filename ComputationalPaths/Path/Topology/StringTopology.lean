...
theorem string_topology_path_refl {α : Type u} (x : α) : Path.refl x = Path.refl x :=
  rfl

theorem string_topology_path_symm {α : Type u} {x y : α} (h : Path x y) : Path.symm h = Path.symm h :=
  rfl

theorem string_topology_path_trans {α : Type u} {x y z : α}
...
theorem string_topology_path_toEq_ofEq {α : Type u} {x y : α} (h : x = y) :
