...
noncomputable def SmoothSpace.zero_mul (M : SmoothSpace) (a : M.scalar) :
...
noncomputable def zeroTangent (M : SmoothSpace) (p : M.carrier) : TangentVector M where
  basePoint := p
  deriv := M.scZero

/-- Scalar multiplication of a tangent vector. -/
noncomputable def tangentSmul (M : SmoothSpace) (c : M.scalar) (v : TangentVector M) :
    TangentVector M where
  basePoint := v.basePoint
  deriv := M.scMul c v.deriv

/-- Tangent vector addition at the same base point. -/
noncomputable def tangentAdd (M : SmoothSpace) (v w : TangentVector M)
    (_ : Path v.basePoint w.basePoint) : TangentVector M where
  basePoint := v.basePoint
  deriv := M.scAdd v.deriv w.deriv

/-- Adding zero tangent on the right. -/
noncomputable def tangentAdd_zero (M : SmoothSpace) (v : TangentVector M) (h : Path v.basePoint v.basePoint) :
...
noncomputable def zero_tangentAdd (M : SmoothSpace) (v : TangentVector M) (h : Path v.basePoint v.basePoint) :
...
noncomputable def tangentAdd_comm (M : SmoothSpace) (v w : TangentVector M)
...
noncomputable def smoothId (M : SmoothSpace) : SmoothEndo M where
  toFun := fun x => x
  scalarMap := fun s => s
...
noncomputable def smoothComp {M : SmoothSpace} (g f : SmoothEndo M) : SmoothEndo M where
...
noncomputable def differential {M : SmoothSpace} (f : SmoothEndo M) (v : TangentVector M) :
    TangentVector M where
...
noncomputable def differential_id_path (M : SmoothSpace) (v : TangentVector M) :
...
/-- Path: chain rule. -/
noncomputable def chain_rule_path {M : SmoothSpace} (g f : SmoothEndo M) (v : TangentVector M) :
...
noncomputable def differential_zero {M : SmoothSpace} (f : SmoothEndo M) (p : M.carrier) :
...
noncomputable def pair (M : SmoothSpace) (ω : Covector M) (v : TangentVector M) : M.scalar :=
  M.scMul ω.coeff v.deriv

/-- Zero covector. -/
noncomputable def zeroCovector (M : SmoothSpace) (p : M.carrier) : Covector M where
  point := p
  coeff := M.scZero

/-- Zero covector pairs to zero. -/
noncomputable def zeroCovector_pair (M : SmoothSpace) (p : M.carrier) (v : TangentVector M) :
    Path (pair M (zeroCovector M p) v) M.scZero :=
  M.zero_mul v.deriv

/-- Pairing with zero tangent gives zero. -/
noncomputable def pair_zeroTangent (M : SmoothSpace) (ω : Covector M) (p : M.carrier) :
    Path (pair M ω (zeroTangent M p)) M.scZero :=
  M.mul_zero ω.coeff

/-- Pairing is commutative. -/
noncomputable def pair_comm (M : SmoothSpace) (ω : Covector M) (v : TangentVector M) :
...
noncomputable def scalarFieldMul (M : SmoothSpace) (f g : ScalarField M) : ScalarField M where
  toFun := fun x => M.scMul (f.toFun x) (g.toFun x)

/-- Sum of scalar fields. -/
noncomputable def scalarFieldAdd (M : SmoothSpace) (f g : ScalarField M) : ScalarField M where
  toFun := fun x => M.scAdd (f.toFun x) (g.toFun x)

/-- Zero scalar field. -/
noncomputable def zeroScalarField (M : SmoothSpace) : ScalarField M where
...
noncomputable def zeroDerivation (M : SmoothSpace) (p : M.carrier) : Derivation M where
  point := p
...
noncomputable def zeroDerivation_zero (M : SmoothSpace) (p : M.carrier) :
...
noncomputable def pullbackField (M : SmoothSpace) (f : M.carrier → M.carrier) (φ : ScalarField M) :
    ScalarField M where
...
noncomputable def pullbackField_comp_path (M : SmoothSpace) (g f : M.carrier → M.carrier) (φ : ScalarField M) :
...
noncomputable def exteriorD (_ : SmoothSpace) (ω : GradedForm M) : GradedForm M where
  degree := ω.degree + 1
  value := ω.value
...
/-- Path: d² degree. -/
noncomputable def exteriorD_squared_path (M : SmoothSpace) (ω : GradedForm M) :
...
/-- Zero graded form. -/
noncomputable def zeroGradedForm (M : SmoothSpace) (p : Nat) : GradedForm M where
  degree := p
  value := M.scZero
...
/-- Path: d(0) = 0. -/
noncomputable def exteriorD_zero_path (M : SmoothSpace) (p : Nat) :
...
noncomputable def transportTangent (M : SmoothSpace) {p q : M.carrier}
...
noncomputable def transportTangent_basePoint_path (M : SmoothSpace) {p q : M.carrier}
    (γ : Path p q) (v : TangentVector M) (h : Path v.basePoint p) :
