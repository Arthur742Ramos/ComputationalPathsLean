...
@[simp] noncomputable def bordismAdd (a b : BordismClass n) : BordismClass n := ⟨a.rep + b.rep⟩
@[simp] noncomputable def bordismZero (n : Nat) : BordismClass n := ⟨0⟩
...
noncomputable def BordismStep.toPath : BordismStep n a b → Path a b
...
noncomputable def bordismDoubleZeroPath (a : BordismClass n) :
...
noncomputable def bordismCommAssocChain (a b c : BordismClass n) :
...
noncomputable def bordismAssocSymmPath (a b c : BordismClass n) :
...
noncomputable def bordismAdd_congrArg_left (a₁ a₂ b : BordismClass n)
    (p : Path a₁ a₂) : Path (bordismAdd a₁ b) (bordismAdd a₂ b) :=
  Path.congrArg (fun x => bordismAdd x b) p

noncomputable def bordismAdd_congrArg_right (a : BordismClass n) (b₁ b₂ : BordismClass n)
    (p : Path b₁ b₂) : Path (bordismAdd a b₁) (bordismAdd a b₂) :=
  Path.congrArg (bordismAdd a) p

-- 11. Zero from both sides chain
noncomputable def bordismZeroBothPath (a : BordismClass n) :
...
noncomputable def bordismRoundtripPath (a b : BordismClass n) :
    Path (bordismAdd a b) (bordismAdd a b) :=
  Path.trans (bordismCommPath a b) (bordismCommSymmPath a b)

/-! ## Unoriented Cobordism Ring Ω_N -/

@[simp] noncomputable def cobordismMul (a : BordismClass n) (b : BordismClass m) : BordismClass (n + m) :=
  ⟨a.rep * b.rep⟩

@[simp] noncomputable def cobordismOne : BordismClass 0 := ⟨1⟩
...
@[simp] noncomputable def thomShift (baseDim fiberDim : Nat) : Nat := baseDim + fiberDim
...
noncomputable def thomShiftAssocPath (b f₁ f₂ : Nat) :
...
noncomputable def thomShiftCommPath (b f₁ f₂ : Nat) :
...
@[simp] noncomputable def thomIsoDim (n k : Nat) : Nat := n + k
...
@[simp] noncomputable def ptCollapseDim (n k : Nat) : Nat := n + k
...
noncomputable def ptStablePath (n k j : Nat) :
...
noncomputable def suspensionPath (n k : Nat) :
...
@[simp] noncomputable def swAdd (a b : SWNumber) : SWNumber := ⟨xor a.val b.val⟩
@[simp] noncomputable def swZero : SWNumber := ⟨false⟩
...
noncomputable def SWStep.toPath : SWStep a b → Path a b
...
noncomputable def swCommPath (a b : SWNumber) : Path (swAdd a b) (swAdd b a) :=
  (SWStep.add_comm a b).toPath

-- 24. SW 2-torsion path
noncomputable def swSelfPath (a : SWNumber) : Path (swAdd a a) swZero :=
  (SWStep.add_self a).toPath

-- 25. SW zero identity path
noncomputable def swZeroPath (a : SWNumber) : Path (swAdd a swZero) a :=
  (SWStep.add_zero a).toPath

-- 26. SW associativity path
noncomputable def swAssocPath (a b c : SWNumber) :
...
noncomputable def swTripleSelfPath (a : SWNumber) :
...
noncomputable def swDoubleAddChain (a b : SWNumber) :
...
noncomputable def swRoundtripPath (a b : SWNumber) : Path (swAdd a b) (swAdd a b) :=
...
@[simp] noncomputable def pnAdd (a b : PontryaginNumber) : PontryaginNumber := ⟨a.val + b.val⟩
@[simp] noncomputable def pnZero : PontryaginNumber := ⟨0⟩

-- 30. Pontryagin number commutativity
theorem pn_add_comm (a b : PontryaginNumber) : pnAdd a b = pnAdd b a := by
  ext; simp [Int.add_comm]

noncomputable def pnCommPath (a b : PontryaginNumber) : Path (pnAdd a b) (pnAdd b a) :=
...
noncomputable def pnAssocPath (a b c : PontryaginNumber) :
...
noncomputable def pnZeroPath (a : PontryaginNumber) : Path (pnAdd a pnZero) a :=
  Path.mk [⟨_, _, pn_add_zero a⟩] (pn_add_zero a)

/-! ## Formal Group Law from MU -/

@[simp] noncomputable def fglLeading (x y : Nat) : Nat := x + y
...
noncomputable def FGLStep.toPath : FGLStep a b → Path a b
...
noncomputable def fglCommPath (x y : Nat) : Path (fglLeading x y) (fglLeading y x) :=
  (FGLStep.comm x y).toPath

-- 34. FGL associativity
noncomputable def fglAssocPath (x y z : Nat) :
    Path (fglLeading (fglLeading x y) z) (fglLeading x (fglLeading y z)) :=
  (FGLStep.assoc x y z).toPath

-- 35. FGL unit
noncomputable def fglUnitPath (x : Nat) : Path (fglLeading x 0) x :=
  (FGLStep.unit x).toPath

-- 36. FGL chain: F(F(x,0),y) → F(x,y) via unit
noncomputable def fglChainPath (x y : Nat) :
...
noncomputable def fglUnitCommPath (x y : Nat) :
...
@[simp] noncomputable def sigAdd (a b : Signature) : Signature := ⟨a.val + b.val⟩

-- 38. Signature commutativity
theorem sig_add_comm (a b : Signature) : sigAdd a b = sigAdd b a := by
  ext; simp [Int.add_comm]

noncomputable def sigCommPath (a b : Signature) : Path (sigAdd a b) (sigAdd b a) :=
...
noncomputable def sigCobPath (S : SigCobInvariant) : Path S.sig1 S.sig2 :=
  Path.mk [⟨S.sig1, S.sig2, S.cobordant_eq⟩] S.cobordant_eq

-- 40. Bordism transitivity chain: (a+b)+c → (b+a)+c → b+(a+c)
noncomputable def bordismTransChain (a b c : BordismClass n) :
    Path (bordismAdd (bordismAdd a b) c) (bordismAdd b (bordismAdd a c)) :=
  Path.trans
    (Path.congrArg (fun x => bordismAdd x c) (bordismCommPath a b))
    (bordismAssocPath b a c)
