...
structure GeodesicExistence (M : RiemannianManifold)
  ...
  /-- Speed `|startVelocity|` of the prescribed initial velocity. -/
  startSpeed : Int
  /-- Speed `|γ'(0)|` of the geodesic at time `0`. -/
  geodesicSpeedAtZero : Int
  /-- The geodesic's initial speed matches the prescribed velocity — a genuine
      path `|γ'(0)| ⤳ |startVelocity|`. -/
  initial_velocity : Path geodesicSpeedAtZero startSpeed
...
structure GeodesicUniqueness (M : RiemannianManifold)
  ...
  /-- Initial speed of `γ₂`. -/
  speed₂ : Int
  /-- Same initial velocity, recorded as equal initial speeds — a genuine path
      `|γ₁'(0)| ⤳ |γ₂'(0)|`. -/
  same_velocity : Path speed₁ speed₂
...
structure ExponentialMap (M : RiemannianManifold)
  ...
  /-- Jacobian determinant of `exp_p` at the origin. -/
  jacobianAtZero : Int
  /-- `exp_p` is a local diffeomorphism near `0`: its derivative there is the
      identity `d(exp_p)|₀ = id`, so the Jacobian is `1` — a genuine path
      `det d(exp_p)|₀ ⤳ 1`. -/
  local_diffeo : Path jacobianAtZero 1
...
structure AmbroseSinger (M : RiemannianManifold)
  ...
  /-- Dimension of the holonomy Lie algebra `𝔥𝔬𝔩`. -/
  holonomyAlgebraDim : Nat
  /-- Dimension of the curvature algebra generated by the `R(X,Y)`. -/
  curvatureAlgebraDim : Nat
  /-- Ambrose–Singer: the holonomy algebra equals the curvature algebra, recorded
      as equal dimensions — a genuine path `dim 𝔥𝔬𝔩 ⤳ dim ⟨R(X,Y)⟩`. -/
  algebra_eq : Path holonomyAlgebraDim curvatureAlgebraDim
...
structure BonnetMyers (M : RiemannianManifold)
  ...
  /-- Diameter bound. -/
  diameterBound : Nat
  /-- Number of boundary components of the manifold. -/
  boundaryComponents : Nat
  /-- Bonnet–Myers compactness: the manifold is *closed* — compact without
      boundary — a genuine path `#∂ ⤳ 0`. -/
  compact : Path boundaryComponents 0
  /-- Order `|π₁|` of the (finite) fundamental group. -/
  pi1Order : Nat
  /-- Number of sheets of the universal cover. -/
  universalCoverSheets : Nat
  /-- Finiteness of `π₁`: its order equals the finite number of sheets of the
      universal cover — a genuine path `|π₁| ⤳ #sheets`. -/
  finite_pi1 : Path pi1Order universalCoverSheets
...
structure HadamardCartan (M : RiemannianManifold)
  ...
  /-- Order `|π₁|` of the fundamental group. -/
  pi1Order : Nat
  /-- Simple connectivity: `π₁` is trivial, `|π₁| = 1` — a genuine path
      `|π₁| ⤳ 1`. -/
  simply_connected : Path pi1Order 1
  /-- Incompleteness defect: the number of geodesics that fail to extend to all of
      `ℝ`. -/
  incompletenessDefect : Nat
  /-- Geodesic completeness (Hopf–Rinow): every geodesic extends, so the defect
      vanishes — a genuine path `defect ⤳ 0`. -/
  complete : Path incompletenessDefect 0
  /-- Number of conjugate points along a geodesic from `p`. -/
  conjugatePoints : Nat
  /-- Cartan–Hadamard: non-positive curvature yields no conjugate points, so
      `exp_p` is a global diffeomorphism — a genuine path `#conjugate ⤳ 0`. -/
  exp_diffeo : Path conjugatePoints 0
