...
structure BraidClosure (n : Nat) (B : ArtinBraidGroup n) where
  ...
  link : Type u
  /-- Number of connected components of the closure link. -/
  linkComponents : Nat
  /-- Number of cycles of the underlying strand permutation of the braid. -/
  permutationCycles : Nat
  /-- Closure is well-defined up to isotopy: the number of link components of the
      closure equals the number of cycles of the braid's strand permutation — a
      genuine `Nat` computational path (replacing the `True` placeholder). -/
  closure_well_defined : Path linkComponents permutationCycles
...
structure AlexanderTheorem where
  ...
  /-- Number of connected components of the target link. -/
  linkComponentCount : Nat
  /-- Surjectivity: every link is realised as a braid closure, so the closure's
      component count matches the link's — a genuine `Nat` computational path
      (replacing the `True` placeholder). -/
  covers : Path closure.linkComponents linkComponentCount
...
structure MarkovMove (n : Nat) (B : ArtinBraidGroup n) where
  ...
  /-- Target braid. -/
  braid2 : B.data.Braid
  /-- Exponent sum (writhe) of the source braid word. -/
  writhe1 : Int
  /-- Exponent sum (writhe) of the target braid word. -/
  writhe2 : Int
  /-- The move data: a Markov move of the second kind (conjugation) preserves the
      exponent sum of the braid word, so the two writhes agree — a genuine `Int`
      computational path (replacing the `True` placeholder). -/
  move : Path writhe1 writhe2
...
/-- Alexander witness: the component count of its closure matches the cycle count
    of the braid's strand permutation. -/
def alexander_closure_components (A : AlexanderTheorem) :
    Path A.closure.linkComponents A.closure.permutationCycles :=
  A.closure.closure_well_defined
...
noncomputable def topologyBraidGroupAssoc (a b c : Nat) : Path ((a + b) + c) (a + (b + c)) :=
  Path.ofEq (Nat.add_assoc a b c)

/-- Commutativity rewrite `a + b ⤳ b + a`: one genuine step. -/
noncomputable def topologyBraidGroupComm (a b : Nat) : Path (a + b) (b + a) :=
  Path.ofEq (Nat.add_comm a b)

/-- Inner commutativity `a + (b + c) ⤳ a + (c + b)` via congruence in the
    right argument (`_root_.congrArg`, since `congrArg` here is `Path.congrArg`). -/
noncomputable def topologyBraidGroupInner (a b c : Nat) : Path (a + (b + c)) (a + (c + b)) :=
  Path.ofEq (_root_.congrArg (fun t => a + t) (Nat.add_comm b c))

/-- A genuine **two-step** path: reassociate, then commute the inner pair.
    Its trace has length two — this is not a reflexive path. -/
noncomputable def topologyBraidGroupTwoStep (a b c : Nat) : Path ((a + b) + c) (a + (c + b)) :=
  Path.trans (topologyBraidGroupAssoc a b c) (topologyBraidGroupInner a b c)

/-- The two-step path composed with its inverse cancels to the reflexive path —
    a non-decorative `RwEq` (the `trans_symm` rule on a length-two trace). -/
noncomputable def topologyBraidGroupCancel (a b c : Nat) :
    Path.RwEq (Path.trans (topologyBraidGroupTwoStep a b c) (Path.symm (topologyBraidGroupTwoStep a b c)))
      (Path.refl ((a + b) + c)) :=
  Path.rweq_cmpA_inv_right (topologyBraidGroupTwoStep a b c)

/-- Associativity-of-composition (`trans_assoc`, the `tt` rewrite) on any three
    composable paths — a genuine `RwEq` between distinct bracketings. -/
noncomputable def topologyBraidGroupAssocCoh {α : Type} {a b c d : α}
