...
@[simp] def bUnion (M N : Boundary) : Boundary :=
  ⟨M.dim, M.idx + N.idx⟩

/-- Empty boundary. -/
@[simp] def bEmpty (n : Nat) : Boundary := ⟨n, 0⟩

/-- Reverse orientation of a boundary. -/
@[simp] def bReverse (M : Boundary) : Boundary := ⟨M.dim, M.idx⟩
...
def compose_id_left (W : Cobordism) :
...
def compose_id_right (W : Cobordism) :
...
def compose_assoc (W₁ W₂ W₃ : Cobordism) :
...
def reverse_reverse (W : Cobordism) :
...
def reverse_compose (W₁ W₂ : Cobordism) :
...
def compose_assoc4 (W₁ W₂ W₃ W₄ : Cobordism) :
...
def reverse_self_compose (W : Cobordism) :
...
def disjoint_weight_comm (W₁ W₂ : Cobordism) :
...
def disjoint_empty_left_weight (W : Cobordism) :
...
def disjoint_assoc (W₁ W₂ W₃ : Cobordism) :
...
def disjoint_assoc4 (W₁ W₂ W₃ W₄ : Cobordism) :
...
@[simp] def stateSpaceDim (M : Boundary) : Nat := M.idx + 1

/-- TQFT value of a cobordism: maps state space dims. -/
@[simp] def tqftValue (W : Cobordism) : Nat :=
...
def tqft_disjoint (W₁ W₂ : Cobordism) :
...
def tqft_reverse_comm (W : Cobordism) :
...
def morse_empty_euler : Path (morseEuler morseEmpty) 0 :=
...
def morse_cat_empty_left (md : MorseData) :
...
def morse_cat_empty_right (md : MorseData) :
...
def morse_cat_assoc (m₁ m₂ m₃ : MorseData) :
...
def handle_cat_nil_right (hs : List Handle) :
    Path (handleCat hs []) hs :=
...
def handle_cat_assoc (h₁ h₂ h₃ : List Handle) :
...
def handle_cat_assoc4 (h₁ h₂ h₃ h₄ : List Handle) :
...
@[simp] def cobAdd (x y : CobRingElem) : CobRingElem :=
  ⟨x.dim, x.rep + y.rep⟩

/-- Zero element. -/
@[simp] def cobZero (n : Nat) : CobRingElem := ⟨n, 0⟩

/-- Multiplication in cobordism ring (Cartesian product). -/
@[simp] def cobMul (x y : CobRingElem) : CobRingElem :=
...
def cob_add_comm (x y : CobRingElem) :
...
def cob_add_assoc (x y z : CobRingElem) :
...
def cob_zero_add (x : CobRingElem) :
...
def cob_mul_comm (x y : CobRingElem) :
...
def cob_mul_assoc (x y z : CobRingElem) :
...
def cob_distrib_left (x y z : CobRingElem) :
...
def cob_distrib_comm (x y z : CobRingElem) :
    Path (cobMul x (cobAdd y z)).rep
...
def cob_mul_zero (x : CobRingElem) (n : Nat) :
...
def connected_sum_comm (S₁ S₂ : Surface) :
...
def connected_sum_assoc (S₁ S₂ S₃ : Surface) :
...
@[simp] def trivialHCob (M : Boundary) : HCobordism :=
  ⟨idCob M, 0⟩

/-- Compose h-cobordisms. -/
@[simp] def composeHCob (h₁ h₂ : HCobordism) : HCobordism :=
...
def hcob_trivial_left (h : HCobordism) :
...
def hcob_trivial_right (h : HCobordism) :
...
def hcob_assoc (h₁ h₂ h₃ : HCobordism) :
...
def compose_reverse_chain (W₁ W₂ : Cobordism) :
    Path (reverseCob (composeCob W₁ W₂))
         (composeCob (reverseCob W₂) (reverseCob W₁)) :=
  reverse_compose W₁ W₂

-- 45. **Multi-step** (W₁∘W₂)ᵒᵖ ∘ᵒᵖ = W₁∘W₂ via trans chain
def double_reverse_compose (W₁ W₂ : Cobordism) :
...
def cylinder_reverse_refl (M : Boundary) :
    Path (reverseCob (idCob M)) (idCob M) :=
  reverse_id M

-- 47. congrArg path: composeCob preserves equality in first arg
def compose_congrArg_left {W₁ W₁' : Cobordism} (p : Path W₁ W₁') (W₂ : Cobordism) :
...
def compose_congrArg_right (W₁ : Cobordism) {W₂ W₂' : Cobordism} (p : Path W₂ W₂') :
    Path (composeCob W₁ W₂) (composeCob W₁ W₂') :=
  Path.congrArg (composeCob W₁ ·) p
...
@[simp] def spectrumDim (n : Nat) : Nat := n + 1

-- 49. Spectrum dimension is positive
theorem spectrum_pos_thm (n : Nat) : spectrumDim n ≥ 1 := by
  simp [spectrumDim]

def spectrum_pos_nat (n : Nat) : Path (spectrumDim n) (n + 1) :=
  stepPath (by simp)
...
def transportCob {W₁ W₂ : Cobordism} (p : Path W₁ W₂)
...
def transport_refl_path (W : Cobordism) (F : Cobordism → Type _) (x : F W) :
...
def weight_transport_path (W : Cobordism) :
...
def disjoint_compose_pentagon (W₁ W₂ W₃ W₄ : Cobordism) :
...
def connected_sum_genus (S₁ S₂ : Surface) :
...
def connected_sum_genus_comm (S₁ S₂ : Surface) :
...
def connected_sum_assoc4 (S₁ S₂ S₃ S₄ : Surface) :
