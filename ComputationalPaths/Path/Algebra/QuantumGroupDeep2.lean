...
noncomputable def trans {α : Type} {a b c : α} (p : QGPath α a b) (q : QGPath α b c) :
    QGPath α a c :=
  match p with
  | .nil _ => q
  | .cons s rest => .cons s (rest.trans q)

noncomputable def symm {α : Type} {a b : α} (p : QGPath α a b) : QGPath α b a :=
  match p with
  | .nil _ => .nil _
  | .cons s rest => rest.symm.trans (.cons (.symm s) (.nil _))

noncomputable def congrArg {α β : Type} {a b : α} (f : α → β) (p : QGPath α a b) :
    QGPath β (f a) (f b) :=
  match p with
  | .nil _ => .nil _
  | .cons s rest => .cons (.congrArg f s) (congrArg f rest)

noncomputable def length {α : Type} {a b : α} (p : QGPath α a b) : Nat :=
  match p with
  | .nil _ => 0
...
noncomputable def kk_inv_one (i : Nat) :
    QGPath QAlgElem
      (QAlgElem.mul (QAlgElem.gen (UqGen.K i)) (QAlgElem.gen (UqGen.Kinv i)))
      QAlgElem.one :=
  .cons .kkInv (.nil _)

noncomputable def kinv_k_one (i : Nat) :
    QGPath QAlgElem
      (QAlgElem.mul (QAlgElem.gen (UqGen.Kinv i)) (QAlgElem.gen (UqGen.K i)))
      QAlgElem.one :=
  .cons .kInvK (.nil _)

noncomputable def k_both_inv (i : Nat) :
    QGPath QAlgElem
      (QAlgElem.mul (QAlgElem.gen (UqGen.K i)) (QAlgElem.gen (UqGen.Kinv i)))
      (QAlgElem.mul (QAlgElem.gen (UqGen.Kinv i)) (QAlgElem.gen (UqGen.K i))) :=
  (kk_inv_one i).trans (kinv_k_one i).symm

noncomputable def q_serre_relation (i j : Nat) :
    QGPath QAlgElem
      (QAlgElem.mul (QAlgElem.gen (UqGen.E i)) (QAlgElem.gen (UqGen.F j)))
      (QAlgElem.mul (QAlgElem.gen (UqGen.F j)) (QAlgElem.gen (UqGen.E i))) :=
  .cons .qSerre (.nil _)

noncomputable def unit_laws (a : QAlgElem) :
    QGPath QAlgElem (QAlgElem.mul QAlgElem.one a) a :=
  .cons .unitLeft (.nil _)

noncomputable def zero_absorb (a : QAlgElem) :
    QGPath QAlgElem (QAlgElem.mul QAlgElem.zero a) QAlgElem.zero :=
  .cons .zeroLeft (.nil _)

noncomputable def qadd_comm (a b : QAlgElem) :
    QGPath QAlgElem (QAlgElem.add a b) (QAlgElem.add b a) :=
  .cons .addComm (.nil _)

noncomputable def qadd_double_comm (a b : QAlgElem) :
...
noncomputable def r_identity : QGPath RMatrix (RMatrix.quasitriangular RMatrix.identity) RMatrix.identity :=
  .cons .rIdentity (.nil _)

noncomputable def yang_baxter (r : RMatrix) :
    QGPath RMatrix
      (RMatrix.tensorFlip (RMatrix.quasitriangular r))
      (RMatrix.quasitriangular (RMatrix.tensorFlip r)) :=
  .cons .yangBaxter (.nil _)

noncomputable def r_inv_quasi (r : RMatrix) :
    QGPath RMatrix (RMatrix.quasitriangular (RMatrix.inverse r))
      (RMatrix.inverse (RMatrix.quasitriangular r)) :=
  .cons .rMatrixInv (.nil _)

noncomputable def r_inv_cancel (r : RMatrix) :
    QGPath RMatrix
      (RMatrix.quasitriangular (RMatrix.inverse (RMatrix.quasitriangular r)))
      (RMatrix.inverse r) :=
  .cons .rInvCancel (.nil _)

noncomputable def yang_baxter_symm (r : RMatrix) :
    QGPath RMatrix
      (RMatrix.quasitriangular (RMatrix.tensorFlip r))
      (RMatrix.tensorFlip (RMatrix.quasitriangular r)) :=
  (yang_baxter r).symm

noncomputable def r_matrix_functorial (f : RMatrix → RMatrix) (r : RMatrix) :
    QGPath RMatrix
...
noncomputable def ribbon_twist_sq :
    QGPath RibbonData
      (RibbonData.compose RibbonData.twist RibbonData.twist)
      (RibbonData.compose RibbonData.braiding RibbonData.braiding) :=
  .cons .ribbonTwistSquare (.nil _)

noncomputable def ribbon_braid_dual :
    QGPath RibbonData
      (RibbonData.compose RibbonData.braiding RibbonData.duality)
      (RibbonData.compose RibbonData.duality RibbonData.braiding) :=
  .cons .ribbonBraidDual (.nil _)

noncomputable def ribbon_naturality (a b : RibbonData) :
    QGPath RibbonData
      (RibbonData.compose (RibbonData.tensor a b) RibbonData.braiding)
      (RibbonData.compose RibbonData.braiding (RibbonData.tensor b a)) :=
  .cons .ribbonNaturality (.nil _)

noncomputable def ribbon_twist_braid_symm :
    QGPath RibbonData
      (RibbonData.compose RibbonData.braiding RibbonData.braiding)
      (RibbonData.compose RibbonData.twist RibbonData.twist) :=
...
noncomputable def crystal_assoc (a b c : CrystalBase) :
    QGPath CrystalBase
      (CrystalBase.tensor (CrystalBase.tensor a b) c)
      (CrystalBase.tensor a (CrystalBase.tensor b c)) :=
  .cons .crystalTensorAssoc (.nil _)

noncomputable def crystal_lower (i n : Nat) :
    QGPath CrystalBase
      (CrystalBase.lowering i (CrystalBase.highest n))
      (CrystalBase.highest (n - 1)) :=
  .cons .crystalHighestWeight (.nil _)

noncomputable def crystal_connected (n : Nat) :
    QGPath CrystalBase
      (CrystalBase.tensor (CrystalBase.highest n) CrystalBase.connected)
      (CrystalBase.highest n) :=
  .cons .crystalConnected (.nil _)

noncomputable def crystal_lower_symm (i n : Nat) :
    QGPath CrystalBase (CrystalBase.highest (n - 1))
      (CrystalBase.lowering i (CrystalBase.highest n)) :=
  (crystal_lower i n).symm

noncomputable def crystal_functorial (f : CrystalBase → CrystalBase) (a b c : CrystalBase) :
    QGPath CrystalBase
...
noncomputable def canon_bar_roundtrip (b : CanonBase) :
...
noncomputable def rt_jones_full (n : Nat) :
    QGPath RTInvariant (RTInvariant.jones n) (RTInvariant.colored n RTInvariant.homfly) :=
  rt_jones_homfly n

noncomputable def rt_functorial (f : RTInvariant → RTInvariant) (inv : RTInvariant) :
...
noncomputable def cat_decat (c : CatData) :
    QGPath CatData (CatData.decategorify (CatData.grothendieck c)) c :=
  .cons .catDecatId (.nil _)

noncomputable def cat_functor_assoc (a b c : CatData) :
    QGPath CatData
      (CatData.functor a (CatData.functor b c))
      (CatData.functor (CatData.functor a b) c) :=
  .cons .catFunctorCompose (.nil _)

noncomputable def kl_equivalence :
    QGPath CatData
      (CatData.grothendieck CatData.module)
      (CatData.decategorify CatData.module) :=
  .cons .klEquivalence (.nil _)

noncomputable def cat_decat_roundtrip (c : CatData) :
    QGPath CatData c (CatData.decategorify (CatData.grothendieck c)) :=
  (cat_decat c).symm

noncomputable def cat_functorial (f : CatData → CatData) (c : CatData) :
    QGPath CatData
      (f (CatData.decategorify (CatData.grothendieck c))) (f c) :=
  (cat_decat c).congrArg f

noncomputable def kl_with_functor (f : CatData → CatData) :
    QGPath CatData
...
noncomputable def classical_limit : QGPath QParam QParam.generic QParam.classical :=
  .cons .classicalLimit (.nil _)

noncomputable def uq_crystal_connection (f : QAlgElem → CrystalBase) (a b : QAlgElem) :
    QGPath CrystalBase (f (QAlgElem.add a b)) (f (QAlgElem.add b a)) :=
  (qadd_comm a b).congrArg f

noncomputable def r_to_ribbon (f : RMatrix → RibbonData) (r : RMatrix) :
    QGPath RibbonData
      (f (RMatrix.quasitriangular (RMatrix.inverse r)))
      (f (RMatrix.inverse (RMatrix.quasitriangular r))) :=
  (r_inv_quasi r).congrArg f

noncomputable def crystal_to_canon (f : CrystalBase → CanonBase) (a b c : CrystalBase) :
    QGPath CanonBase
      (f (CrystalBase.tensor (CrystalBase.tensor a b) c))
      (f (CrystalBase.tensor a (CrystalBase.tensor b c))) :=
  (crystal_assoc a b c).congrArg f

noncomputable def rt_qdim_connection (f : RTInvariant → QDim) (inv : RTInvariant) :
    QGPath QDim (f (RTInvariant.colored 1 inv)) (f inv) :=
  (rt_colored_one inv).congrArg f

noncomputable def full_quantum_chain (g : CrystalBase → CatData) (f : QAlgElem → CrystalBase)
    (a b : QAlgElem) :
    QGPath CatData (g (f (QAlgElem.add a b))) (g (f (QAlgElem.add b a))) :=
  (uq_crystal_connection f a b).congrArg g

noncomputable def qdim_classical_rt (f : QDim → RTInvariant) (n : Nat) :
