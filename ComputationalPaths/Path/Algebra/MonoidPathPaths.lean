...
noncomputable def FreeMonoid (A : Type u) : PathMonoid (List A) where
  e := []
  mul := List.append
...
noncomputable def pentagon_left (PM : PathMonoid A) (w x y z : A) :
...
noncomputable def pentagon_right (PM : PathMonoid A) (w x y z : A) :
    Path (PM.mul (PM.mul (PM.mul w x) y) z)
         (PM.mul w (PM.mul x (PM.mul y z))) :=
  Path.trans
...
noncomputable def hom_mul {PM₁ : PathMonoid A} {PM₂ : PathMonoid B}
    (f : PathMonoidHom PM₁ PM₂) (x y : A) :
    Path (f.toFun (PM₁.mul x y)) (PM₂.mul (f.toFun x) (f.toFun y)) :=
  f.map_mul_path x y

/-- Homomorphism preserves identity. -/
noncomputable def hom_one {PM₁ : PathMonoid A} {PM₂ : PathMonoid B}
    (f : PathMonoidHom PM₁ PM₂) :
    Path (f.toFun PM₁.e) PM₂.e :=
  f.map_one_path

/-- Identity homomorphism. -/
noncomputable def idHom (PM : PathMonoid A) : PathMonoidHom PM PM where
  toFun := id
...
noncomputable def compHom {A B C : Type u} {PM₁ : PathMonoid A} {PM₂ : PathMonoid B} {PM₃ : PathMonoid C}
...
noncomputable def WordEquiv (_P : Presentation) (w₁ w₂ : Word) : Prop :=
  ∃ _ : Path w₁ w₂, True
...
noncomputable def InRightIdeal (PM : PathMonoid A) (a b : A) : Prop :=
  ∃ s : A, a = PM.mul b s

/-- Left ideal membership (propositional). -/
noncomputable def InLeftIdeal (PM : PathMonoid A) (a b : A) : Prop :=
  ∃ s : A, a = PM.mul s b

/-- Two-sided ideal membership. -/
noncomputable def InIdeal (PM : PathMonoid A) (a b : A) : Prop :=
  ∃ s t : A, a = PM.mul s (PM.mul b t)

/-- Green's R-relation. -/
noncomputable def GreenR (PM : PathMonoid A) (a b : A) : Prop :=
  InRightIdeal PM a b ∧ InRightIdeal PM b a

/-- Green's L-relation. -/
noncomputable def GreenL (PM : PathMonoid A) (a b : A) : Prop :=
  InLeftIdeal PM a b ∧ InLeftIdeal PM b a

/-- Green's J-relation. -/
noncomputable def GreenJ (PM : PathMonoid A) (a b : A) : Prop :=
  InIdeal PM a b ∧ InIdeal PM b a

/-- Green's H-relation: intersection of R and L. -/
noncomputable def GreenH (PM : PathMonoid A) (a b : A) : Prop :=
  GreenR PM a b ∧ GreenL PM a b

/-- Green's D-relation: composition of L and R. -/
noncomputable def GreenD (PM : PathMonoid A) (a b : A) : Prop :=
...
noncomputable def greenR_refl_path (PM : PathMonoid A) (a : A) :
...
noncomputable def identity_idempotent (PM : PathMonoid A) :
...
noncomputable def right_id_naturality (PM : PathMonoid A) (x y : A) :
