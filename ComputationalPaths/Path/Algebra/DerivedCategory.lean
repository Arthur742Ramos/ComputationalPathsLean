...
/-- Combined derived cross-module dependency, composing Category and Homology
path witnesses. -/
def derived_cross_module_dependencies
    {C : Type u} (L : LeftExactLocalization C)
    {a b c : C} (p : Path a b) (q : Path b c)
    (E : GeneralizedHomologyTheory.{u, v})
    {X Y Z : PtdType.{u}} (g : PtdMap Y Z) (f : PtdMap X Y) (n : Nat) :
  ...
  ⟨
    rweq_trans (derived_localization_comp_path L p q) (RwEq.refl _),
