...
noncomputable def IdealStep.sound : IdealStep I J → I = J
...
noncomputable def IdealStep.toPath (s : IdealStep I J) : Path I J :=
...
noncomputable def IdealPath.toPath : IdealPath I J → Path I J
  | .refl I => Path.refl I
  | .step s => s.toPath
...
noncomputable def pideal_sum_comm (I J : PIdeal) : Path (I.sum J) (J.sum I) :=
  (IdealPath.step (.sumComm I J)).toPath

-- 2. Sum is associative
noncomputable def pideal_sum_assoc (I J K : PIdeal) :
    Path ((I.sum J).sum K) (I.sum (J.sum K)) :=
  (IdealPath.step (.sumAssoc I J K)).toPath

-- 3. Sum with zero (right)
noncomputable def pideal_sum_zero_r (I : PIdeal) : Path (I.sum ⟨0⟩) I :=
  (IdealPath.step (.sumZeroR I)).toPath

-- 4. Sum with zero (left)
noncomputable def pideal_sum_zero_l (I : PIdeal) : Path (PIdeal.sum ⟨0⟩ I) I :=
  (IdealPath.step (.sumZeroL I)).toPath

-- 5. Sum idempotent
noncomputable def pideal_sum_idem (I : PIdeal) : Path (I.sum I) I :=
  (IdealPath.step (.sumIdem I)).toPath

-- 6. Product is commutative
noncomputable def pideal_prod_comm (I J : PIdeal) : Path (I.prod J) (J.prod I) :=
  (IdealPath.step (.prodComm I J)).toPath

-- 7. Product is associative
noncomputable def pideal_prod_assoc (I J K : PIdeal) :
    Path ((I.prod J).prod K) (I.prod (J.prod K)) :=
  (IdealPath.step (.prodAssoc I J K)).toPath

-- 8. Product with unit (right)
noncomputable def pideal_prod_one_r (I : PIdeal) : Path (I.prod ⟨1⟩) I :=
  (IdealPath.step (.prodOneR I)).toPath

-- 9. Product with unit (left)
noncomputable def pideal_prod_one_l (I : PIdeal) : Path (PIdeal.prod ⟨1⟩ I) I :=
  (IdealPath.step (.prodOneL I)).toPath

-- 10. Product with zero (right)
noncomputable def pideal_prod_zero_r (I : PIdeal) : Path (I.prod ⟨0⟩) ⟨0⟩ :=
  (IdealPath.step (.prodZeroR I)).toPath

-- 11. Product with zero (left)
noncomputable def pideal_prod_zero_l (I : PIdeal) : Path (PIdeal.prod ⟨0⟩ I) ⟨0⟩ :=
  (IdealPath.step (.prodZeroL I)).toPath

-- 12. Intersection is commutative
noncomputable def pideal_inter_comm (I J : PIdeal) : Path (I.inter J) (J.inter I) :=
  (IdealPath.step (.interComm I J)).toPath

-- 13. Intersection is associative
noncomputable def pideal_inter_assoc (I J K : PIdeal) :
...
noncomputable def prod_one_roundtrip (I : PIdeal) : Path (I.prod ⟨1⟩) I :=
  pideal_prod_one_r I

-- 21. Three-step: (I+0)+J → I+J via congrArg
noncomputable def sum_zero_left_context (I J : PIdeal) :
...
noncomputable def sum_zero_right_context (I J : PIdeal) :
...
noncomputable def inter_comm_roundtrip (I J : PIdeal) : Path (I.inter J) (I.inter J) :=
  Path.trans (pideal_inter_comm I J) (pideal_inter_comm J I)

-- 28. Assoc + comm chain: (I+J)+K → I+(J+K) → (J+K)+I (2 steps)
noncomputable def sum_assoc_then_comm (I J K : PIdeal) :
...
@[simp] noncomputable def FGMod.directSum (M N : FGMod) : FGMod := ⟨M.rank + N.rank⟩
@[simp] noncomputable def FGMod.tensorZ (M : FGMod) (n : Nat) : FGMod := ⟨M.rank * n⟩
...
noncomputable def ModStep.sound : ModStep M N → M = N
...
noncomputable def tensor_zero_sum (M N : FGMod) :
...
@[simp] noncomputable def krullDim (n : Nat) : Nat := if n = 0 then 1 else 0
@[simp] noncomputable def idealHeight (n : Nat) : Nat := if n = 0 then 0 else 1

-- 40. ℤ has Krull dimension 1
noncomputable def krull_dim_Z_path : Path (krullDim 0) 1 := Path.refl 1

-- 41. Height of zero ideal is 0
noncomputable def height_zero_path : Path (idealHeight 0) 0 := Path.refl 0
...
noncomputable def coprime_sum_unit (m n : Nat) (h : Nat.gcd m n = 1) :
...
noncomputable def sum_double_assoc (I J K L : PIdeal) :
...
noncomputable def prod_double_assoc (I J K L : PIdeal) :
...
noncomputable def sum_zero_l_via_comm (I : PIdeal) : Path (PIdeal.sum ⟨0⟩ I) I :=
  Path.trans (pideal_sum_comm ⟨0⟩ I) (pideal_sum_zero_r I)

-- 50. Prod unit via comm: 1*I → I*1 → I (2 steps)
noncomputable def prod_one_l_via_comm (I : PIdeal) : Path (PIdeal.prod ⟨1⟩ I) I :=
  Path.trans (pideal_prod_comm ⟨1⟩ I) (pideal_prod_one_r I)

-- 51. Three-step mod chain: ((M ⊕ N) ⊕ K) → (M ⊕ (N ⊕ K)) → ((N ⊕ K) ⊕ M)
noncomputable def fgmod_assoc_then_comm (M N K : FGMod) :
...
noncomputable def inter_comm_sym (I J : PIdeal) : Path (J.inter I) (I.inter J) :=
  Path.symm (pideal_inter_comm I J)

-- 53. Product assoc then comm in right arg
noncomputable def prod_assoc_comm_right (I J K : PIdeal) :
...
noncomputable def idem_then_unit (I : PIdeal) :
...
noncomputable def fgmod_double_zero_elim (M N : FGMod) :
