...
noncomputable def Virtual (M : Type u) : Type u := M × M

/-- Stabilize a virtual object by a monoid element. -/
noncomputable def stabilize {M : Type u} (S : StrictMonoid M) (x : Virtual M) (c : M) : Virtual M :=
  (S.mul x.1 c, S.mul x.2 c)

/-! ## K0 relation and quotient -/

/-- Stable equivalence using a Path witness after stabilization. -/
noncomputable def K0Rel {M : Type u} (S : StrictMonoid M) (x y : Virtual M) : Prop :=
  ∃ c : M, Nonempty (Path (stabilize S x c) (stabilize S y c))

/-- K0 of a strict monoid as a quotient by stable Path equivalence. -/
noncomputable def K0 {M : Type u} (S : StrictMonoid M) : Type u :=
  Quot (K0Rel (S := S))

/-- Embed a monoid element into K0 via the formal pair `(x, 1)`. -/
noncomputable def toK0 {M : Type u} (S : StrictMonoid M) (x : M) : K0 S :=
  Quot.mk _ (x, S.one)

/-- The basepoint of K0. -/
noncomputable def zero {M : Type u} (S : StrictMonoid M) : K0 S :=
  toK0 S S.one
...
noncomputable def K0PointedSet {M : Type u} (S : StrictMonoid M) : PointedSet.{u} where
  carrier := K0 S
  zero := zero S
