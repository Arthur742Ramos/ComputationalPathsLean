...
noncomputable def SMObj.flatten : SMObj → List Nat
  | .gen n      => [n]
  | .unit       => []
...
noncomputable def SMPath.trans : SMPath a b → SMPath b c → SMPath a c
  | .nil _, q     => q
  | .cons s p, q  => .cons s (p.trans q)

noncomputable def SMStep.symm : SMStep a b → SMStep b a
...
noncomputable def SMPath.symm : SMPath a b → SMPath b a
  | .nil _    => .nil _
  | .cons s p => p.symm.trans (.cons s.symm (.nil _))

/-- Theorem 3: congrArg — tensor on the left side preserves paths. -/
noncomputable def SMPath.congrArgL {a a' : SMObj} (b : SMObj) : SMPath a a' → SMPath (SMObj.tensor a b) (SMObj.tensor a' b)
  | .nil _    => .nil _
  | .cons s p => .cons (.tensorL b s) (p.congrArgL b)

/-- Theorem 4: congrArg — tensor on the right side preserves paths. -/
noncomputable def SMPath.congrArgR (a : SMObj) {b b' : SMObj} : SMPath b b' → SMPath (SMObj.tensor a b) (SMObj.tensor a b')
  | .nil _    => .nil _
  | .cons s p => .cons (.tensorR a s) (p.congrArgR a)

/-- Theorem 5: Bifunctoriality. -/
noncomputable def SMPath.congrArgTensor {a a' b b' : SMObj}
    (p : SMPath a a') (q : SMPath b b') : SMPath (SMObj.tensor a b) (SMObj.tensor a' b') :=
  (p.congrArgL b).trans (q.congrArgR a')

noncomputable def SMPath.length : SMPath a b → Nat
  | .nil _    => 0
  | .cons _ p => 1 + p.length

noncomputable def smSingle (s : SMStep a b) : SMPath a b := .cons s (.nil _)
...
/-- Theorem 6 -/
noncomputable def assocR_path (a b c : SMObj) : SMPath (SMObj.tensor (SMObj.tensor a b) c) (SMObj.tensor a (SMObj.tensor b c)) :=
  smSingle (.assocR a b c)

/-- Theorem 7 -/
noncomputable def assocL_path (a b c : SMObj) : SMPath (SMObj.tensor a (SMObj.tensor b c)) (SMObj.tensor (SMObj.tensor a b) c) :=
  smSingle (.assocL a b c)

/-- Theorem 8 -/
noncomputable def unitL_path (a : SMObj) : SMPath (SMObj.tensor .unit a) a := smSingle (.unitL a)

/-- Theorem 9 -/
noncomputable def unitR_path (a : SMObj) : SMPath (SMObj.tensor a .unit) a := smSingle (.unitR a)

/-- Theorem 10 -/
noncomputable def braid_path (a b : SMObj) : SMPath (SMObj.tensor a b) (SMObj.tensor b a) := smSingle (.braid a b)
...
noncomputable def braid_squared (a b : SMObj) : SMPath (SMObj.tensor a b) (SMObj.tensor a b) :=
...
noncomputable def pentagon_top (a b c d : SMObj)
...
noncomputable def pentagon_bot (a b c d : SMObj)
    : SMPath (SMObj.tensor (SMObj.tensor (SMObj.tensor a b) c) d)
             (SMObj.tensor a (SMObj.tensor b (SMObj.tensor c d))) :=
...
noncomputable def triangle_right (a b : SMObj) : SMPath (SMObj.tensor (SMObj.tensor a .unit) b) (SMObj.tensor a b) :=
  (assocR_path a .unit b).trans ((unitL_path b).congrArgR a)

/-- Theorem 18: Triangle — right unitor on left factor. -/
noncomputable def triangle_left (a b : SMObj) : SMPath (SMObj.tensor (SMObj.tensor a .unit) b) (SMObj.tensor a b) :=
...
noncomputable def hexagon1 (a b c : SMObj)
...
/-- Theorem 20: Hexagon path 2 —
    a⊗(b⊗c) →σ⊗id via α⁻¹ ... -/
noncomputable def hexagon2 (a b c : SMObj)
...
noncomputable def hex_braid_full (a b c : SMObj)
...
noncomputable def braid_natural_left {a a' : SMObj} (b : SMObj)
...
noncomputable def unit_absorb_left (a : SMObj) : SMPath (SMObj.tensor .unit a) a := unitL_path a

/-- Theorem 28: Unit right absorption. -/
noncomputable def unit_absorb_right (a : SMObj) : SMPath (SMObj.tensor a .unit) a := unitR_path a

/-- Theorem 29: Double unit elimination. -/
noncomputable def double_unit_elim : SMPath (SMObj.tensor .unit .unit) .unit := unitL_path .unit

/-- Theorem 30: Quadruple right-association. -/
noncomputable def quad_assoc (a b c d : SMObj)
    : SMPath (SMObj.tensor (SMObj.tensor (SMObj.tensor a b) c) d)
             (SMObj.tensor a (SMObj.tensor b (SMObj.tensor c d))) :=
  (assocR_path (SMObj.tensor a b) c d).trans (assocR_path a b (SMObj.tensor c d))

/-- Theorem 31: Coherence — compose inverse gives endo-path. -/
noncomputable def coherence_compose_inverse {a b : SMObj}
    (p q : SMPath a b) : SMPath a a :=
  p.trans q.symm

/-- Theorem 32: Five-step with braiding. -/
noncomputable def five_step_coherence (a b c d : SMObj)
    : SMPath (SMObj.tensor (SMObj.tensor (SMObj.tensor a b) c) d)
...
noncomputable def bubble_sort_two (n m : Nat) : SMPath (SMObj.tensor (.gen n) (.gen m)) (SMObj.tensor (.gen m) (.gen n)) :=
  braid_path (.gen n) (.gen m)

/-- Theorem 34: Swap second component. -/
noncomputable def sort_three_step (a b c : SMObj) : SMPath (SMObj.tensor a (SMObj.tensor b c)) (SMObj.tensor a (SMObj.tensor c b)) :=
  (braid_path b c).congrArgR a

/-- Theorem 35: Coherence — braid + association. -/
noncomputable def coherence_braid_assoc (a b c : SMObj)
...
noncomputable def TracePath.trans : TracePath a b → TracePath b c → TracePath a c
  | .nil _, q    => q
  | .cons s p, q => .cons s (p.trans q)

/-- Theorem 36: Full trace execution. -/
noncomputable def trace_full (d : TraceData) : TracePath (.start d) (.done d) :=
  .cons (.feedLoop d) (.cons (.closeLoop d) (.nil _))

/-- Theorem 37: Yanking axiom. -/
noncomputable def trace_yanking (d : TraceData) : TracePath (.start d) (.done d) :=
  TracePath.trans
    (.cons (.feedLoop d) (.nil _))
    (.cons (.closeLoop d) (.nil _))

/-- Theorem 38: Superposing. -/
noncomputable def trace_superpose (d : TraceData) : TracePath (.start d) (.done d) :=
  trace_full d
...
noncomputable def CCPath.trans : CCPath a b → CCPath b c → CCPath a c
  | .nil _, q    => q
  | .cons s p, q => .cons s (p.trans q)

noncomputable def ccSingle (s : CCStep a b) : CCPath a b := .cons s (.nil _)

/-- Theorem 39: Snake identity. -/
noncomputable def snake_identity (a : SMObj) : CCPath .un .un :=
  (ccSingle (.coeval a)).trans
    ((ccSingle (.braidCC (.dual a) (.obj a))).trans
      (ccSingle (.eval a)))

/-- Theorem 40: CC trace. -/
noncomputable def cc_trace (a : SMObj) : CCPath .un .un :=
  (ccSingle (.coeval a)).trans (ccSingle (.coeval' a))

/-- Theorem 41: Double dual roundtrip. -/
noncomputable def double_dual_roundtrip (a : SMObj) : CCPath (.obj a) (.obj a) :=
  CCPath.nil _
...
noncomputable def IntPath.trans : IntPath a b → IntPath b c → IntPath a c
  | .nil _, q    => q
  | .cons s p, q => .cons s (p.trans q)

/-- Theorem 42: Int unit preserved. -/
noncomputable def int_unit_preserved : IntPath ⟨.unit, .unit⟩ ⟨.unit, .unit⟩ :=
  .cons .unitInt (.nil _)

/-- Theorem 43: Int duality involutive. -/
noncomputable def int_double_dual (a : IntObj) : IntPath a a :=
...
noncomputable def triple_assoc_chain (a b c d : SMObj)
    : SMPath (SMObj.tensor (SMObj.tensor (SMObj.tensor a b) c) d)
             (SMObj.tensor a (SMObj.tensor b (SMObj.tensor c d))) :=
  quad_assoc a b c d
