...
All constructions use explicit Path/Step/RwEq from ComputationalPaths.Core.
-/
...
open ComputationalPaths.Path.Rewriting (StepRule allStepRules allStepRules_count allStepRules_complete)

universe u

noncomputable section

-- Local alias matching PathAlgebra.Loop but with implicit A
private abbrev Loop' {A : Type u} (a : A) := Path (A := A) a a
