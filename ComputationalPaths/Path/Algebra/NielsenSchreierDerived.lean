...
noncomputable def mul_encoding_path (x y : BouquetFreeGroup 1) :
...
noncomputable def mul_one_left_path (x : BouquetFreeGroup 1) :
...
noncomputable def mul_one_right_path (x : BouquetFreeGroup 1) :
...
noncomputable def pow_zero_path (x : BouquetFreeGroup 1) :
...
noncomputable def pow_one_path (x : BouquetFreeGroup 1) :
...
noncomputable def pow_encoding_path (x : BouquetFreeGroup 1) (k : Nat) :
...
noncomputable def pow_succ_path (x : BouquetFreeGroup 1) (k : Nat) :
...
noncomputable def pow_add_path (x : BouquetFreeGroup 1) (m k : Nat) :
...
noncomputable def mul_assoc_path (x y z : BouquetFreeGroup 1) :
...
noncomputable def inv_inv_path (x : BouquetFreeGroup 1) :
...
noncomputable def inv_one_path :
...
noncomputable def inv_mul_path (x y : BouquetFreeGroup 1) :
...
noncomputable def equiv_natural_add (m n : Int) :
...
noncomputable def encoding_identity_via_roundtrip :
    Path (freeGroupOneToInt (intToFreeGroupOne 0)) 0 :=
  int_roundtrip_path 0
...
noncomputable def transport_predicate_to_F1 (P : Int → Type) (k : Int) (px : P k) :
...
noncomputable def transport_predicate_to_Int (P : BouquetFreeGroup 1 → Type)
...
noncomputable def genPow_zero_path :
...
noncomputable def f1_roundtrip (x : BouquetFreeGroup 1) :
    Path (intToFreeGroupOne (freeGroupOneToInt x)) x :=
