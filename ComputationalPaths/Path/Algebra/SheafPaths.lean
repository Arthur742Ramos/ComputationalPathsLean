...
@[simp] noncomputable def constPresheaf (v : Sect) : Presheaf :=
  ⟨fun _ => v, fun _ _ s => s⟩

/-- The zero presheaf. -/
@[simp] noncomputable def zeroPresheaf : Presheaf :=
  ⟨fun _ => 0, fun _ _ _ => 0⟩
...
@[simp] noncomputable def idPresheafMor (F : Presheaf) : PresheafMor F F :=
  ⟨fun _ s => s⟩

/-- Composition of presheaf morphisms. -/
@[simp] noncomputable def compPresheafMor {F G H : Presheaf}
    (φ : PresheafMor F G) (ψ : PresheafMor G H) : PresheafMor F H :=
  ⟨fun i s => ψ.map i (φ.map i s)⟩

/-- Stalk at a point (evaluate at the index). -/
@[simp] noncomputable def stalk (F : Presheaf) (p : OpenIdx) : Sect :=
  F.sections p

/-- Germ of a section at a point. -/
@[simp] noncomputable def germ (F : Presheaf) (_ : OpenIdx) (p : OpenIdx) : Sect :=
  F.sections p

/-- Sheafification (identity on sections, same restriction). -/
@[simp] noncomputable def sheafify (F : Presheaf) : Presheaf :=
  ⟨F.sections, fun u v s => F.restrict u v s⟩

/-- Sheaf cohomology H^n: H^0 = global sections, H^n = 0 for n > 0. -/
@[simp] noncomputable def sheafCohomology (F : Presheaf) (n : Nat) : Sect :=
  if n = 0 then F.sections 0 else 0

/-- Global sections functor Γ. -/
@[simp] noncomputable def globalSections (F : Presheaf) : Sect := F.sections 0

/-- Direct image presheaf. -/
@[simp] noncomputable def directImage (F : Presheaf) (_ : OpenIdx → OpenIdx) : Presheaf :=
  ⟨F.sections, F.restrict⟩

/-- Čech 0-cochain. -/
@[simp] noncomputable def cech0 (F : Presheaf) : OpenIdx → Sect := F.sections
...
noncomputable def id_mor_act_path (F : Presheaf) (i : OpenIdx) (s : Sect) :
...
noncomputable def comp_id_left_path {F G : Presheaf} (φ : PresheafMor F G) (i : OpenIdx) (s : Sect) :
...
noncomputable def comp_id_right_path {F G : Presheaf} (φ : PresheafMor F G) (i : OpenIdx) (s : Sect) :
...
noncomputable def comp_assoc_path {F G H K : Presheaf}
    (φ : PresheafMor F G) (ψ : PresheafMor G H) (χ : PresheafMor H K)
    (i : OpenIdx) (s : Sect) :
...
noncomputable def stalk_zero_path (p : OpenIdx) : Path (stalk zeroPresheaf p) 0 :=
...
noncomputable def stalk_const_path (v : Sect) (p : OpenIdx) :
...
noncomputable def germ_eq_stalk_path (F : Presheaf) (u p : OpenIdx) :
...
noncomputable def sheafify_sections_path (F : Presheaf) (i : OpenIdx) :
...
noncomputable def sheafify_idem_path (F : Presheaf) (i : OpenIdx) :
...
noncomputable def cohomology_zero_path (F : Presheaf) :
...
noncomputable def cohomology_higher_path (F : Presheaf) (n : Nat) :
...
noncomputable def cohomology_zero_presheaf_path (n : Nat) :
...
noncomputable def global_sections_zero_path : Path (globalSections zeroPresheaf) 0 :=
...
noncomputable def global_sections_const_path (v : Sect) :
...
noncomputable def direct_image_sections_path (F : Presheaf) (f : OpenIdx → OpenIdx) (i : OpenIdx) :
...
noncomputable def cech0_eq_path (F : Presheaf) (i : OpenIdx) :
...
noncomputable def zero_restrict_path (u v : OpenIdx) (s : Sect) :
...
noncomputable def const_restrict_path (val : Sect) (u w : OpenIdx) (s : Sect) :
...
noncomputable def sheafify_cohom_path (F : Presheaf) :
...
noncomputable def stalk_symm_path (v : Sect) (p : OpenIdx) :
    Path v (stalk (constPresheaf v) p) :=
  Path.symm (stalk_const_path v p)

-- 21. Congruence: morphism applied to sections
noncomputable def mor_congr {F G : Presheaf} (φ : PresheafMor F G) (i : OpenIdx)
    (s t : Sect) (p : Path s t) :
...
noncomputable def stalk_direct_image_path (F : Presheaf) (f : OpenIdx → OpenIdx) (p : OpenIdx) :
...
noncomputable def global_sections_natural_path (F : Presheaf) :
...
noncomputable def stalk_sheafify_direct_path (F : Presheaf) (f : OpenIdx → OpenIdx) (p : OpenIdx) :
...
noncomputable def global_sections_double_sheafify_path (F : Presheaf) :
...
noncomputable def comp_chain_symm_path {F G H : Presheaf}
    (φ : PresheafMor F G) (ψ : PresheafMor G H) (i : OpenIdx) (s : Sect) :
...
noncomputable def congrArg_globalSections (F G : Presheaf) (h : F = G) :
