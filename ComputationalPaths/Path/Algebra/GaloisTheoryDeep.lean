...
/-- Local symmetric rewrite equality for audit traces, generated by the project
    path-rewrite `Step` relation without importing the heavyweight normalization
    stack. -/
inductive RwEq {A : Type u} {a b : A} : Path a b → Path a b → Type (u + 1) where
  | refl (p : Path a b) : RwEq p p
  | step {p q : Path a b} : ComputationalPaths.Path.Step p q → RwEq p q
...
noncomputable def degree_tower_path (e1 e2 : FieldExt) :
  ...
  Path.stepChain (towerLaw_degree e1 e2)
...
noncomputable def alg_element_degree_path (n : Nat) :
  ...
  Path.stepChain (by simp [elementDegree, simpleAlgExt])
...
noncomputable def normal_iff_split (d e : Nat) :
  ...
  Path.stepChain (by simp [mkSplittingField])
...
noncomputable def charZero_sep_path (deg ext : Nat) :
  ...
  Path.stepChain (by simp [charZeroSep, mkSepData])
...
noncomputable def cyclicGal_order_path (n : Nat) :
  ...
  Path.stepChain (by simp [cyclicGalGroup])
...
noncomputable def fieldToGroup_invariant (f : IntermediateField) :
  ...
  Path.stepChain (by simp [fieldToGroup, latticeInvariant, galGroupOf])
...
noncomputable def groupToField_invariant (g : GalSubgroup) (totalDeg : Nat) :
  ...
  Path.stepChain (by simp [groupToField, latticeInvariant, fixedField])
...
noncomputable def roundtrip_field_path (f : IntermediateField) (totalDeg : Nat) :
    Path (latticeInvariant (groupToField totalDeg (fieldToGroup (LatticeElem.field f))))
         (latticeInvariant (LatticeElem.field f)) :=
  Path.stepChain (by simp [groupToField, fieldToGroup, latticeInvariant, galGroupOf, fixedField])
...
noncomputable def galois_degree_order (deg : Nat) (a s : Bool) :
  ...
  Path.stepChain (by simp [mkGaloisExt])
...
noncomputable def complete_conditions (deg : Nat) :
  ...
  Path.stepChain (by simp [completePackage])
...
-- ============================================================================
-- Part XVII: Deep-audit trace comparisons and rewrite evidence
-- ============================================================================

/-- A Galois-theoretic equality with both direct and factored computational traces. -/
structure GaloisTrace (source target : Nat) where
  direct : Path source target
  factored : Path source target
  normalized : RwEq (Path.trans factored (Path.refl target)) factored

-- The tower-law degree trace factors through the explicit product formula.
noncomputable def tower_degree_trace_comparison (e1 e2 : FieldExt) :
    GaloisTrace (towerLaw e1 e2).degree (composeDegree e1 e2) :=
  let productPath : Path (towerLaw e1 e2).degree (e1.degree * e2.degree) :=
    Path.stepChain (towerLaw_degree e1 e2)
  let productIsCompose : Path (composeDegree e1 e2) (e1.degree * e2.degree) :=
    Path.stepChain (by simp [composeDegree])
  let factored : Path (towerLaw e1 e2).degree (composeDegree e1 e2) :=
    Path.trans productPath (Path.symm productIsCompose)
  { direct := degree_tower_path e1 e2
    factored := factored
    normalized :=
      RwEq.step
        (ComputationalPaths.Path.Step.trans_refl_right factored) }

theorem tower_degree_trace_factored_steps (e1 e2 : FieldExt) :
    (tower_degree_trace_comparison e1 e2).factored.steps.length = 2 := by
  simp [tower_degree_trace_comparison, degree_tower_path, Path.trans, Path.symm,
    Path.stepChain]

-- The correspondence round-trip is factored through both sides of the invariant.
noncomputable def correspondence_roundtrip_trace
    (f : IntermediateField) (totalDeg : Nat) :
    GaloisTrace (latticeInvariant (LatticeElem.field f))
      (latticeInvariant (groupToField totalDeg (fieldToGroup (LatticeElem.field f)))) :=
  let fieldToGroupTrace :
      Path (latticeInvariant (LatticeElem.field f))
        (latticeInvariant (fieldToGroup (LatticeElem.field f))) :=
    Path.symm (fieldToGroup_invariant f)
  let groupToFieldTrace :
      Path (latticeInvariant (fieldToGroup (LatticeElem.field f)))
        (latticeInvariant (groupToField totalDeg (fieldToGroup (LatticeElem.field f)))) :=
    Path.symm (groupToField_invariant (galGroupOf f) totalDeg)
  let factored : Path (latticeInvariant (LatticeElem.field f))
      (latticeInvariant (groupToField totalDeg (fieldToGroup (LatticeElem.field f)))) :=
    Path.trans fieldToGroupTrace groupToFieldTrace
  { direct := correspondence_trans f totalDeg
    factored := factored
    normalized :=
      RwEq.step
        (ComputationalPaths.Path.Step.trans_refl_right factored) }

theorem correspondence_roundtrip_factored_steps
    (f : IntermediateField) (totalDeg : Nat) :
    (correspondence_roundtrip_trace f totalDeg).factored.steps.length = 2 := by
