...
def addMod (n : Nat) (a b : Nat) : Nat := (a + b) % n
...
def addMod_comm_path (n a b : Nat) :
...
def addMod_zero_path (n a : Nat) (ha : a < n) :
    Path (addMod n a 0) a :=
...
def zero_d_path (n x : Nat) : Path (zeroComplex.d n x) 0 := Path.refl _

-- Theorem 8: Zero dd path
def zero_dd_path (n x : Nat) :
...
def mod2Complex : ChainComplex where
...
def mod2_d0_even_path (k : Nat) : Path (mod2Complex.d 0 (2 * k)) 0 :=
...
/-- d_0(x) = 2*x, d_{n+1} = 0. -/
def doublingComplex : ChainComplex where
  d := fun n x => match n with
    | 0     => 2 * x
    | _ + 1 => 0
  d_zero := fun n => by cases n <;> simp
...
def doubling_d0_path (x : Nat) : Path (doublingComplex.d 0 x) (2 * x) := Path.refl _

-- Theorem 15: dd = 0 in doubling
def doubling_dd_path (n x : Nat) :
...
/-- Cycle: d_n(x) = 0. -/
def IsCycle (C : ChainComplex) (n : Nat) (x : Nat) : Prop := C.d n x = 0

/-- Boundary: ∃ y, d_{n+1}(y) = x. -/
def IsBoundary (C : ChainComplex) (n : Nat) (x : Nat) : Prop :=
  ∃ y, C.d (n + 1) y = x
...
def boundary_cycle_path (C : ChainComplex) (n x : Nat)
    (hx : IsBoundary C n x) :
    Path (C.d n x) 0 :=
...
def zero_cycle_path (C : ChainComplex) (n : Nat) :
...
def IsExact (C : ChainComplex) (n : Nat) : Prop :=
...
def cycle_d_path (C : ChainComplex) (n x : Nat) (hcycle : IsCycle C n x) :
...
def chainMap_comm_path {C D : ChainComplex} (φ : ChainMap C D) (n x : Nat) :
    Path (φ.f n (C.d (n + 1) x)) (D.d (n + 1) (φ.f (n + 1) x)) :=
  Path.mk [⟨_, _, φ.comm n x⟩] (φ.comm n x)

-- Theorem 27: f(0) = 0 path
def chainMap_zero_path {C D : ChainComplex} (φ : ChainMap C D) (n : Nat) :
...
def idChainMap (C : ChainComplex) : ChainMap C C where
  f := fun _ x => x
  f_zero := fun _ => rfl
  comm := fun _ _ => rfl

-- Theorem 28: Id path
def id_map_path (C : ChainComplex) (n x : Nat) :
...
def zeroMap (C D : ChainComplex) : ChainMap C D where
  f := fun _ _ => 0
  f_zero := fun _ => rfl
  comm := fun n _ => by simp [D.d_zero (n + 1)]

-- Theorem 29: Zero map path
def zero_map_path (C D : ChainComplex) (n x : Nat) :
...
def compChainMap {C D E : ChainComplex} (φ : ChainMap C D) (ψ : ChainMap D E) :
    ChainMap C E where
...
def chainmap_cycle_path {C D : ChainComplex} (φ : ChainMap C D)
...
def functorial_cycle_path {C D E : ChainComplex}
    (φ : ChainMap C D) (ψ : ChainMap D E)
    (n x : Nat) (hx : IsCycle C (n + 1) x) :
...
def zeroToMod2 : ChainMap zeroComplex mod2Complex := zeroMap zeroComplex mod2Complex

-- Theorem 46: Zero-to-mod2 path
def zeroToMod2_path (n x : Nat) :
    Path (zeroToMod2.f n x) 0 := Path.refl _

-- Theorem 47: Mod2-to-zero
def mod2ToZero : ChainMap mod2Complex zeroComplex := zeroMap mod2Complex zeroComplex

-- Theorem 48: Mod2-to-zero path
def mod2ToZero_path (n x : Nat) :
...
def pi_zero_path {A B C_ : ChainComplex} (ses : ShortExact A B C_)
    (n x : Nat) :
    Path (ses.p.f n (ses.i.f n x)) 0 :=
  Path.mk [⟨_, _, ses.pi_zero n x⟩] (ses.pi_zero n x)

-- Theorem 50: Composition path
def pi_comp_path {A B C_ : ChainComplex} (ses : ShortExact A B C_)
    (n x : Nat) :
...
def zero_hom_class (C : ChainComplex) (n : Nat) : HomologyClass C n :=
  ⟨0, zero_is_cycle C (n + 1)⟩

-- Theorem 52: Zero class rep path
def zero_class_path (C : ChainComplex) (n : Nat) :
    Path (zero_hom_class C n).rep 0 := Path.refl _

-- Theorem 53: Induced map on homology
def induced_hom {C D : ChainComplex} (φ : ChainMap C D) (n : Nat)
...
def dd_congrArg (C : ChainComplex) (n : Nat) {x y : Nat} (p : Path x y) :
    Path (C.d n (C.d (n + 1) x)) (C.d n (C.d (n + 1) y)) :=
  Path.congrArg (fun z => C.d n (C.d (n + 1) z)) p

-- Theorem 58: dd congrArg + dd=0
def dd_to_zero (C : ChainComplex) (n : Nat) {x y : Nat} (p : Path x y) :
    Path (C.d n (C.d (n + 1) x)) 0 :=
  Path.trans (dd_congrArg C n p) (dd_zero_path C n y)

-- Theorem 59: Symm dd
def dd_symm (C : ChainComplex) (n : Nat) {x y : Nat} (p : Path x y) :
...
def mod2_cls0 : HomologyClass mod2Complex 0 := ⟨0, mod2_zero_cycle⟩  -- IsCycle mod2Complex 1 0

-- Theorem 63: Homology class 2
def mod2_cls2 : HomologyClass mod2Complex 0 := ⟨2, mod2_two_cycle⟩  -- IsCycle mod2Complex 1 2

-- Theorem 64: d(cls0.rep) = 0 path
def mod2_cls0_d_path : Path (mod2Complex.d 1 mod2_cls0.rep) 0 :=
  Path.mk [⟨_, _, mod2_zero_cycle⟩] mod2_zero_cycle

-- Theorem 65: d(cls2.rep) = 0 path
def mod2_cls2_d_path : Path (mod2Complex.d 1 mod2_cls2.rep) 0 :=
...
def ChainHomotopic (C D : ChainComplex) (φ ψ : ChainMap C D) : Prop :=
...
def homotopic_symm_path {C D : ChainComplex} {φ ψ : ChainMap C D}
...
def homotopic_trans_path {C D : ChainComplex} {φ ψ χ : ChainMap C D}
    (h1 : ChainHomotopic C D φ ψ) (h2 : ChainHomotopic C D ψ χ)
...
def PreservesExactness {C D : ChainComplex} (_φ : ChainMap C D) : Prop :=
...
def dd_natural (D : ChainComplex) (n : Nat) (y : Nat) :
    Path (D.d n (D.d (n + 1) y)) 0 :=
  dd_zero_path D n y

-- Theorem 73: dd through chain map
def dd_cm_path {C D : ChainComplex} (φ : ChainMap C D) (n : Nat) (x : Nat) :
...
def dd_cm_full_trans {C D : ChainComplex} (φ : ChainMap C D) (n x : Nat) :
...
def mappingConeZero : ChainComplex where
  d := fun _ _ => 0
  d_zero := fun _ => rfl
  dd_zero := fun _ _ => rfl

-- Theorem 75: Cone path
def cone_d_path (n x : Nat) : Path (mappingConeZero.d n x) 0 := Path.refl _
...
-- Theorem 79: Path
def zero_all_cycles_path (n x : Nat) : Path (zeroComplex.d n x) 0 := Path.refl _
...
def mod2_one_d3_path : Path (mod2Complex.d 3 1) 0 :=
