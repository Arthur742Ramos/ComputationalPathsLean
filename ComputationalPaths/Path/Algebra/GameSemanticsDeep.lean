...
noncomputable def Polarity.swap : Polarity → Polarity
  | .O => .P
  | .P => .O
...
noncomputable def Arena.dual (A : Arena) : Arena where
  moves := A.moves
...
noncomputable def Arena.tensor (A B : Arena) : Arena where
...
noncomputable def Arena.lolli (A B : Arena) : Arena :=
...
noncomputable def comp_assoc_weight (f g h : MorphRepr) :
...
noncomputable def comp_assoc_weight_symm (f g h : MorphRepr) :
...
noncomputable def triple_comp_weight (f g h : MorphRepr) :
    Path (MorphRepr.comp f (MorphRepr.comp g h)).weight
...
noncomputable def MorphRepr.depth : MorphRepr → Nat
  | .ident _ => 0
  | .strat _ => 0
  | .comp f g => max f.depth g.depth + 1

-- Theorem 14: Identity depth is zero
noncomputable def ident_depth (n : Nat) : Path (MorphRepr.ident n).depth 0 :=
  Path.refl 0

-- Theorem 15: Composition depth formula
noncomputable def comp_depth (f g : MorphRepr) :
...
noncomputable def MorphRepr.norm : MorphRepr → MorphRepr
  | .ident _ => .ident 0
  | .strat n => .strat n
...
noncomputable def TyCode.gameSize : TyCode → Nat
  | .base _ => 1
...
noncomputable def unit_arrowCount : Path TyCode.unit.arrowCount 0 := Path.refl 0

-- Theorem 33: Base has no arrows
noncomputable def base_arrowCount (n : Nat) : Path (TyCode.base n).arrowCount 0 :=
  Path.refl 0
...
noncomputable def TermRepr.complexity : TermRepr → Nat
  | .var _ => 1
...
noncomputable def unit_complexity : Path TermRepr.unit.complexity 0 := Path.refl 0

-- Theorem 35: Variable complexity is 1
noncomputable def var_complexity (n : Nat) : Path (TermRepr.var n).complexity 1 :=
  Path.refl 1

-- Theorem 36: Pair complexity is sum
noncomputable def pair_complexity (a b : TermRepr) :
    Path (TermRepr.pair a b).complexity (a.complexity + b.complexity) :=
  Path.refl _

-- Theorem 37: Projection preserves complexity
noncomputable def fst_complexity (t : TermRepr) :
    Path (TermRepr.fst t).complexity t.complexity :=
  Path.refl _

-- Theorem 38: Snd projection preserves complexity
noncomputable def snd_complexity (t : TermRepr) :
    Path (TermRepr.snd t).complexity t.complexity :=
  Path.refl _

/-! ## 14. Compact Closed Structure -/

noncomputable def dualSize : Nat → Nat := id

-- Theorem 39: Dual preserves size
noncomputable def dual_preserves_size (n : Nat) : Path (dualSize n) n :=
  Path.refl n

noncomputable def tensorSize (a b : Nat) : Nat := a + b

noncomputable def lolliSize (a b : Nat) : Nat := dualSize a + b

-- Theorem 40: Lolli size equals tensor with dual
noncomputable def lolli_as_tensor (a b : Nat) :
    Path (lolliSize a b) (tensorSize (dualSize a) b) :=
  Path.refl _

-- Theorem 41: Compact closed coherence for unit
noncomputable def compact_unit : Path (lolliSize 0 0) 0 := Path.refl 0

-- Theorem 42: Lolli size with zero domain
noncomputable def lolli_zero_domain (b : Nat) : Path (lolliSize 0 b) b :=
  Path.mk [Step.mk _ _ (Nat.zero_add b)] (Nat.zero_add b)

-- Evaluation map size
noncomputable def evalSize (a b : Nat) : Nat := lolliSize a b + a

-- Theorem 43: Evaluation unfolds
noncomputable def eval_unfolds (a b : Nat) :
...
noncomputable def tensor_size_comm (a b : Nat) :
...
noncomputable def tensor_size_assoc (a b c : Nat) :
...
noncomputable def triangle_left (a b : Nat) :
...
noncomputable def triangle_right (a b : Nat) :
...
noncomputable def triangle_coherence (a b : Nat) :
...
noncomputable def completenessRank : TyCode → Nat
  | .base _ => 1
...
noncomputable def unit_completeness : Path (completenessRank .unit) 1 :=
  Path.refl 1

-- Theorem 52: Arrow completeness rank
noncomputable def arrow_completeness (a b : TyCode) :
    Path (completenessRank (.arrow a b))
         (completenessRank a + completenessRank b) :=
  Path.refl _

-- Theorem 53: Product completeness rank
noncomputable def prod_completeness (a b : TyCode) :
...
noncomputable def abstractionLevel : TyCode → Nat
  | .base _ => 0
...
noncomputable def unit_abstraction : Path (abstractionLevel .unit) 0 :=
  Path.refl 0

-- Theorem 55: Base abstraction level
noncomputable def base_abstraction (n : Nat) :
    Path (abstractionLevel (.base n)) 0 :=
  Path.refl 0

-- Theorem 56: Arrow increases abstraction
noncomputable def arrow_abstraction (a b : TyCode) :
...
noncomputable def interactionLen (a b c : Nat) : Nat := a + b + b + c

noncomputable def hiddenLen (a c : Nat) : Nat := a + c

-- Theorem 57: Hiding with zero middle removes nothing
noncomputable def hiding_reduces (a c : Nat) :
    Path (interactionLen a 0 c) (hiddenLen a c) :=
  Path.refl _

-- Theorem 58: Zero interaction
noncomputable def zero_interaction : Path (interactionLen 0 0 0) 0 := Path.refl 0

noncomputable def threadCount (strategies : List Nat) : Nat :=
  strategies.foldl (· + ·) 0

-- Theorem 59: Empty thread list
noncomputable def empty_threads : Path (threadCount []) 0 := Path.refl 0

-- Theorem 60: Singleton thread
noncomputable def single_thread (n : Nat) : Path (threadCount [n]) n :=
  Path.mk [Step.mk _ _ (Nat.zero_add n)] (Nat.zero_add n)

/-! ## 20. congrArg Coherence -/

-- Theorem 61: congrArg on weight through equality
noncomputable def weight_congrArg (f g : MorphRepr) (h : f = g) :
    Path f.weight g.weight :=
  Path.congrArg MorphRepr.weight (Path.mk [Step.mk _ _ h] h)

-- Theorem 62: congrArg on interactCount
noncomputable def interactCount : MorphRepr → Nat
  | .ident n => n
  | .strat n => n
  | .comp f g => interactCount f + interactCount g

noncomputable def interact_congrArg (f g : MorphRepr) (h : f = g) :
...
noncomputable def double_ident_interact (n : Nat) :
...
noncomputable def chained_coherence (a b c : Nat) (p : Path a b) (q : Path b c) :
    Path a c :=
  Path.trans p q

-- Theorem 65: Path symmetry for weight equality
noncomputable def weight_symm (f g : MorphRepr) (p : Path f.weight g.weight) :
    Path g.weight f.weight :=
  Path.symm p

-- Theorem 66: Path transitivity for weight
noncomputable def weight_trans (f g h : MorphRepr)
    (p : Path f.weight g.weight) (q : Path g.weight h.weight) :
    Path f.weight h.weight :=
  Path.trans p q

-- Theorem 67: Path refl as identity of trans
noncomputable def path_refl_trans_id (n : Nat) :
    Path n n :=
...
noncomputable def pipelineSize (stages : List Nat) : Nat :=
  stages.foldl (· + ·) 0

-- Theorem 71: Empty pipeline
noncomputable def empty_pipeline : Path (pipelineSize []) 0 := Path.refl 0

-- Theorem 72: Single stage pipeline
noncomputable def single_pipeline (n : Nat) : Path (pipelineSize [n]) n :=
  Path.mk [Step.mk _ _ (Nat.zero_add n)] (Nat.zero_add n)

/-! ## 24. Bracket Depth -/

noncomputable def bracketDepth (open_ close : Nat) : Nat := open_ - close

-- Theorem 73: Equal opens and closes give zero depth
noncomputable def bracket_balanced (n : Nat) :
    Path (bracketDepth n n) 0 :=
  Path.mk [Step.mk _ _ (Nat.sub_self n)] (Nat.sub_self n)

/-! ## 25. View Bounds for Innocence -/

noncomputable def viewBound : Nat → Nat
  | 0 => 0
  | n + 1 => n + 1

-- Theorem 74: View bound is identity on positive
noncomputable def viewBound_succ (n : Nat) : Path (viewBound (n + 1)) (n + 1) :=
  Path.refl (n + 1)

-- Theorem 75: View bound zero
noncomputable def viewBound_zero : Path (viewBound 0) 0 := Path.refl 0

/-! ## 26. Path congrArg on gameSize -/

-- Theorem 76: congrArg through gameSize for arrow
noncomputable def gameSize_arrow_congrArg (a1 a2 b : TyCode) (h : a1 = a2) :
...
noncomputable def complexity_lam_congrArg (t1 t2 : TermRepr) (h : t1 = t2) :
...
noncomputable def semSize_left_id_collapse (n : Nat) (s : MorphRepr) :
...
noncomputable def tensor_comm_roundtrip_chain (a b : Nat) :
    Path (tensorSize a b) (tensorSize a b) :=
  Path.trans (tensor_size_comm a b) (Path.symm (tensor_size_comm a b))

noncomputable def weight_left_id_roundtrip (n : Nat) (f : MorphRepr) :
...
noncomputable def arrow_gameSize_comm_chain (a b : TyCode) :
