...
/-- A typed path certificate between two model-categorical expressions. -/
structure PathCertificate {α : Type u} (lhs rhs : α) where
  /-- Computational path comparing the expressions. -/
  path : Path lhs rhs
  /-- Rewrite evidence for the displayed path. -/
  rewrite : RwEq path path

/-- Reflexive path certificate. -/
noncomputable def PathCertificate.refl {α : Type u} (x : α) :
    PathCertificate x x where
  path := Path.refl x
  rewrite := RwEq.refl _

/-- Certificate that two morphism-class predicates agree on a path, carrying
the path itself instead of a shallow self-iff placeholder. -/
structure ClassAgreementCertificate {A : Type u} {a b : A}
    (p : Path a b) (P Q : Prop) where
  /-- Forward preservation of the class predicate. -/
  forward : P → Q
  /-- Backward preservation of the class predicate. -/
  backward : Q → P
  /-- The compared path as computational domain data. -/
  path_trace : Path p p
  /-- Rewrite evidence for the path trace. -/
  path_rewrite : RwEq path_trace path_trace

/-- Certificate that a cofibration is generated by the chosen generators. -/
structure CofibrantGenerationCertificate {A : Type u}
    (genCof : (Σ (a : A) (b : A), Path a b) → Prop)
    {a b : A} (p : Path a b) where
  /-- The displayed generator used by the certificate. -/
  generator : Σ (x : A) (y : A), Path x y
  /-- The generator lies in the chosen generating class. -/
  generator_mem : genCof generator
  /-- The target cofibration carries explicit path evidence. -/
  target_path : Path p p
  /-- Rewrite evidence for the target path. -/
  target_rewrite : RwEq target_path target_path

/-- Locally presentable data as objectwise computational traces. -/
structure PresentabilityCertificate (A : Type u) where
  /-- Every object has a displayed identity path. -/
  object_path : ∀ a : A, Path a a
  /-- The displayed object paths are rewrite-stable. -/
  object_rewrite : ∀ a : A, RwEq (object_path a) (object_path a)

/-- Accessibility side condition for Smith recognition. -/
structure AccessibilityCertificate (A : Type u)
    (weq : ∀ {a b : A}, Path a b → Prop) where
  /-- Weak-equivalence witnesses carry computational path data. -/
  weak_equivalence_path :
    ∀ {a b : A} (p : Path a b), weq p → Path p p
  /-- Rewrite evidence for the weak-equivalence paths. -/
  weak_equivalence_rewrite :
    ∀ {a b : A} (p : Path a b) (hp : weq p),
      RwEq (weak_equivalence_path p hp) (weak_equivalence_path p hp)

/-- Solution-set side condition for Smith recognition. -/
structure SolutionSetCertificate {A : Type u}
    (I : (Σ (a : A) (b : A), Path a b) → Prop) where
  /-- Chosen generating maps carry computational path data. -/
  generator_path : ∀ g : Σ (a : A) (b : A), Path a b, I g → Path g.2.2 g.2.2
  /-- Rewrite evidence for generator paths. -/
  generator_rewrite :
    ∀ (g : Σ (a : A) (b : A), Path a b) (hg : I g),
      RwEq (generator_path g hg) (generator_path g hg)

/-- Structured output of Smith recognition, replacing description strings. -/
structure SmithRecognitionCertificate (A : Type u)
    (weq : ∀ {a b : A}, Path a b → Prop)
    (I : (Σ (a : A) (b : A), Path a b) → Prop) where
  /-- Accessibility certificate. -/
  accessibility : AccessibilityCertificate A weq
  /-- Solution-set certificate. -/
  solutionSet : SolutionSetCertificate I
...
structure QuillenSymmetryCertificate {A : Type u} {B : Type v}
    {M : MCat A} {N : MCat B} (Q : QuillenEquivalence M N) where
  /-- Swapped left adjoint object map. -/
  swappedLeftObj : B → A := Q.rightObj
  /-- Swapped right adjoint object map. -/
  swappedRightObj : A → B := Q.leftObj
  /-- Unit candidate obtained from the original counit. -/
  swapped_unit : ∀ b : B, Path b (Q.leftObj (Q.rightObj b))
  /-- Counit candidate obtained from the original unit. -/
  swapped_counit : ∀ a : A, Path (Q.rightObj (Q.leftObj a)) a
  /-- Rewrite evidence for the swapped unit. -/
  swapped_unit_rewrite : ∀ b : B, RwEq (swapped_unit b) (swapped_unit b)
  /-- Rewrite evidence for the swapped counit. -/
  swapped_counit_rewrite : ∀ a : A, RwEq (swapped_counit a) (swapped_counit a)

/-- Quillen equivalences are symmetric. -/
noncomputable def quillen_equiv_symm {A : Type u} {B : Type v}
    {M : MCat A} {N : MCat B} (Q : QuillenEquivalence M N) :
    QuillenSymmetryCertificate Q where
  swapped_unit := fun b => Path.symm (Q.counit b)
  swapped_counit := fun a => Path.symm (Q.unit a)
  swapped_unit_rewrite := fun _ => RwEq.refl _
  swapped_counit_rewrite := fun _ => RwEq.refl _
...
noncomputable def reedy_model_structure_exists (R : ReedyCategory.{u}) (A : Type v)
    (M : MCat A) : ReedyModelStructure R A where
  base := M
  reedyCof := fun p => M.cof p
  reedyFib := fun p => M.fib p
  reedyWeq := fun p => M.twoOfThree.isWeq p
...
structure CombinatorialModelCategory (A : Type u) extends MCat A where
  ...
  cof_generated : ∀ {a b : A} (p : Path a b), cof p →
    CofibrantGenerationCertificate genCof p
  /-- Locally presentable. -/
  locally_presentable : PresentabilityCertificate A
...
noncomputable def smith_recognition (A : Type u) (weq : ∀ {a b : A}, Path a b → Prop)
    (I : (Σ (a : A) (b : A), Path a b) → Prop)
    (h_acc : AccessibilityCertificate A weq) (h_sol : SolutionSetCertificate I) :
    SmithRecognitionCertificate A weq I where
  accessibility := h_acc
  solutionSet := h_sol
...
structure BousfieldLocalization (A : Type u) where
  ...
  same_cof : ∀ {a b : A} (p : Path a b),
    ClassAgreementCertificate p (original.cof p) (original.cof p)
...
noncomputable def isLocalObject {A : Type u} (L : BousfieldLocalization A) (_X : A) : Prop :=
  ∀ {a b : A} (f : Path a b), L.localWeq f →
    Nonempty (PathCertificate f f)
...
structure RightBousfieldLocalization (A : Type u) where
  ...
  same_fib : ∀ {a b : A} (p : Path a b),
    ClassAgreementCertificate p (original.fib p) (original.fib p)
...
structure CofibrantlyGenerated (A : Type u) extends MCat A where
  ...
  fib_iff_J_inj : ∀ {a b : A} (p : Path a b),
    ClassAgreementCertificate p (fib p) (fib p)
  /-- Trivial fibrations = I-injectives. -/
  trivFib_iff_I_inj : ∀ {a b : A} (p : Path a b),
    ClassAgreementCertificate p (fib p ∧ weq p) (fib p ∧ weq p)
...
noncomputable def isLeftProper (A : Type u) (M : MCat A) : Prop :=
  ∀ {a b c : A} (f : Path a b) (g : Path a c),
    M.twoOfThree.isWeq f → M.cof g → Nonempty (PathCertificate f f)
...
structure SimplicialModelCategory (A : Type u) extends MCat A where
  ...
  cotensor : A → Nat → A
  /-- Pushout-product axiom as computational certificate data. -/
  pushout_product : ∀ {a b : A} (i : Path a b), cof i → PathCertificate i i

/-- The SM7 axiom: cofibrations tensored with cofibrations of simplicial
    sets yield cofibrations. -/
noncomputable def sm7_axiom {A : Type u} (SM : SimplicialModelCategory A)
    {a b : A} (i : Path a b) (_hi : SM.cof i) :
    PathCertificate i i :=
  SM.pushout_product i _hi
