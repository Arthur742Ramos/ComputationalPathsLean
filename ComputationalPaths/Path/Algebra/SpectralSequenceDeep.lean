...
-- ============================================================================
-- § 16. Deep-audit trace comparisons with rewrite evidence
-- ============================================================================

/-- A direct equality proof together with a factored computational trace and
    the rewrite step normalizing its trailing identity. -/
structure TraceComparison (source target : Nat) where
...
noncomputable def leibniz_zero_trace_payload (p1 q1 p2 q2 : Nat) :
  ...
  let sourceToZero : Path (MultPage.zero_page.diff (p1 + p2) (q1 + q2)) 0 :=
    mp_zero_diff_path (p1 + p2) (q1 + q2)
  let targetToZero :
      Path (MultPage.zero_page.diff p1 q1 + MultPage.zero_page.diff p2 q2) 0 :=
    Path.stepChain (by simp [MultPage.zero_page])
...
noncomputable def ssm_identity_trace_payload
  ...
  let targetToEntry : Path ((SSMorphism.identity entry).targetEntry p q) (entry p q) :=
    ssm_id_target_path entry p q
...
noncomputable def ssm_identity_trace_assoc_rweq
    ...
    RwEq
      (Path.trans
        (Path.trans (ssm_id_source_path entry p q) (Path.symm (ssm_id_target_path entry p q)))
        (Path.refl ((SSMorphism.identity entry).targetEntry p q)))
      (Path.trans
        (ssm_id_source_path entry p q)
        (Path.trans (Path.symm (ssm_id_target_path entry p q))
          (Path.refl ((SSMorphism.identity entry).targetEntry p q)))) :=
  RwEq.step
    (ComputationalPaths.Path.Step.trans_assoc
      (ssm_id_source_path entry p q)
      (Path.symm (ssm_id_target_path entry p q))
      (Path.refl ((SSMorphism.identity entry).targetEntry p q)))
