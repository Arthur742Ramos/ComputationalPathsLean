...
/-- Inverse of a Rule. -/
def Rule.inv (r : Rule) : Rule := ⟨r.rhs, r.lhs⟩
...
def SRS.symClose (R : SRS) : SRS := R ++ R.map Rule.inv
...
def Joinable (R : SRS) (a b : Word) : Prop :=
...
def Rule.lengthReducing (r : Rule) : Prop := r.rhs.length < r.lhs.length

/-- An SRS is length‑reducing. -/
def SRS.lengthReducing (R : SRS) : Prop := ∀ r ∈ R, Rule.lengthReducing r
...
def NormalForm (R : SRS) (w : Word) : Prop :=
  ∀ v, ¬ Step R w v
...
def ThueEquiv (T : ThueSystem) : Word → Word → Prop := SymPath T.rules
...
def WordProblem (T : ThueSystem) (u v : Word) : Prop := ThueEquiv T u v
...
def RewriteInvariant (R : SRS) (P : Word → Prop) : Prop :=
...
def ParityPreserving (R : SRS) : Prop :=
...
def commRule : Rule := ⟨[symA, symB], [symB, symA]⟩
def commSRS : SRS := [commRule]
...
def wordHom (f : Sym → Word) : Word → Word
  | [] => []
  | s :: w => f s ++ wordHom f w
...
def Rule.map (f : Sym → Word) (r : Rule) : Rule :=
