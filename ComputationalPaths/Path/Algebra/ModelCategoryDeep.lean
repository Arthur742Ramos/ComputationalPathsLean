...
noncomputable def mkLiftWitness (d : LiftData) : LiftWitness :=
  { data := d
...
noncomputable def ModelCat.isTrivFib (mc : ModelCat) (f : Nat) : Bool :=
  mc.isFib f && mc.isWeq f

noncomputable def ModelCat.isTrivCof (mc : ModelCat) (f : Nat) : Bool :=
...
noncomputable def trivFib_is_fib_path (mc : ModelCat) (f : Nat) (h : mc.isTrivFib f = true) :
...
noncomputable def trivFib_is_weq_path (mc : ModelCat) (f : Nat) (h : mc.isTrivFib f = true) :
...
noncomputable def trivCof_is_cof_path (mc : ModelCat) (f : Nat) (h : mc.isTrivCof f = true) :
...
noncomputable def trivCof_is_weq_path (mc : ModelCat) (f : Nat) (h : mc.isTrivCof f = true) :
    Path (mc.isWeq f) true :=
...
noncomputable def fib_weq_trivFib_path (mc : ModelCat) (f : Nat)
    (hf : mc.isFib f = true) (hw : mc.isWeq f = true) :
...
noncomputable def cof_weq_trivCof_path (mc : ModelCat) (f : Nat)
    (hc : mc.isCof f = true) (hw : mc.isWeq f = true) :
...
noncomputable def cof_trivfib_sum_path (fact : CofTrivFibFact) :
...
noncomputable def cof_trivfib_zero_source_path (fact : CofTrivFibFact) (hs : fact.source = 0) :
...
noncomputable def fact_comm_path (a b : Nat) : Path (a + b) (b + a) :=
...
noncomputable def two_of_three_compose_path (tot : TwoOfThree) :
...
noncomputable def two_of_three_zero_f_path (tot : TwoOfThree) (hf : tot.fVal = 0) :
...
noncomputable def two_of_three_zero_g_path (tot : TwoOfThree) (hg : tot.gVal = 0) :
...
noncomputable def two_of_three_both_zero_path (tot : TwoOfThree)
    (hf : tot.fVal = 0) (hg : tot.gVal = 0) :
    Path tot.gfVal 0 :=
...
noncomputable def retract_sr_s_path (rd : RetractData) :
...
noncomputable def retract_sr_t_path (rd : RetractData) :
...
noncomputable def homotopy_sum_path (hr : HomotopyRel) :
...
noncomputable def quillen_adj_path (qp : QuillenPair) (n : Nat) :
...
noncomputable def quillen_adj_zero_path (qp : QuillenPair) :
...
noncomputable def derived_eq_orig_path (df : DerivedFunctor) (n : Nat) :
    Path (df.derived n) (df.original (df.cofibrant_replacement n)) :=
  Path.stepChain (df.derives_eq n)

-- 25. Constant derived functor
noncomputable def DerivedFunctor.const (c : Nat) : DerivedFunctor :=
...
noncomputable def derived_const_val_path (c n : Nat) :
...
noncomputable def derived_const_orig_path (c n : Nat) :
...
noncomputable def cofgen_total_path (cg : CofGenData) :
...
noncomputable def cofgen_zero_cof_path (cg : CofGenData) (hc : cg.genCofCard = 0) :
...
noncomputable def cofgen_zero_trivcof_path (cg : CofGenData) (ht : cg.genTrivCofCard = 0) :
...
noncomputable def cofgen_both_zero_path (cg : CofGenData)
    (hc : cg.genCofCard = 0) (ht : cg.genTrivCofCard = 0) :
    Path cg.totalCard 0 :=
...
noncomputable def SmallObjIteration.mk_simple (n : Nat) : SmallObjIteration :=
...
noncomputable def small_obj_simple_step_path (n : Nat) :
...
noncomputable def small_obj_cellcount_path (n k : Nat) :
...
noncomputable def small_obj_cumul_one_path (n : Nat) :
...
noncomputable def simplicial_adj_path (se : SimplicialEnrichment) (a b c : Nat) :
...
noncomputable def SimplicialEnrichment.const (v : Nat) : SimplicialEnrichment :=
...
noncomputable def simplicial_const_map_path (v a b : Nat) :
...
noncomputable def simplicial_const_tensor_path (v a b : Nat) :
...
noncomputable def simplicial_const_cotensor_path (v a b : Nat) :
...
noncomputable def cyl_proj_incl0_path (c : CylinderObj) :
...
noncomputable def cyl_proj_incl1_path (c : CylinderObj) :
...
noncomputable def cyl_incl_agree_path (c : CylinderObj) :
...
noncomputable def pathobj_ev0_diag_path (po : PathObj) :
...
noncomputable def pathobj_ev1_diag_path (po : PathObj) :
...
noncomputable def pathobj_ev_agree_path (po : PathObj) :
...
noncomputable def trivFib_fib_weq_chain (mc : ModelCat) (f : Nat) (h : mc.isTrivFib f = true) :
...
noncomputable def quillen_adj_sym_path (qp : QuillenPair) (n : Nat) :
    Path (n * 2 + n) (qp.leftVal n + qp.rightVal n) :=
  Path.symm (quillen_adj_path qp n)

-- 48. congrArg through Nat.succ on two-of-three
noncomputable def two_of_three_succ_path (tot : TwoOfThree) :
...
noncomputable def transport_derived (df : DerivedFunctor) (n : Nat)
...
noncomputable def homotopy_double_zero_path : Path (0 + 0) 0 :=
...
noncomputable def double_add_assoc_path (a b c : Nat) :
    Path ((a + b) + c) (a + (b + c)) :=
...
noncomputable def add_self_mul2_path (n : Nat) : Path (n + n) (2 * n) :=
...
noncomputable def lift_upper_lower_chain (ld : LiftData) :
...
noncomputable def cofgen_comm_path (cg : CofGenData) :
...
noncomputable def cofgen_total_comm_path (cg : CofGenData) :
...
noncomputable def homotopy_trans_arith_path (a b c : Nat) :
...
noncomputable def model_id_weq_path (mc : ModelCat) (n : Nat) (h : mc.isWeq n = true) :
