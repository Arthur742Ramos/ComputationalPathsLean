...
noncomputable def mkObj (l : String) (n : Nat) : TObj := ⟨l, n⟩

noncomputable def termObj  : TObj := mkObj "1" 1
noncomputable def omegaObj : TObj := mkObj "Ω" 2
noncomputable def initObj  : TObj := mkObj "0" 0

noncomputable def prodObj (A B : TObj) : TObj := mkObj (A.label ++ "×" ++ B.label) (A.uid * 10000 + B.uid + 100)
noncomputable def expObj  (A B : TObj) : TObj := mkObj (B.label ++ "^" ++ A.label) (A.uid * 10000 + B.uid + 200)
noncomputable def powObj  (A : TObj) : TObj := expObj A omegaObj
noncomputable def subObj  (A : TObj) : TObj := mkObj ("Sub(" ++ A.label ++ ")") (A.uid * 100 + 50)
noncomputable def pullObj (f g : String) (n : Nat) : TObj := mkObj ("Pb(" ++ f ++ "," ++ g ++ ")") (n + 300)
noncomputable def eqObj   (A : TObj) : TObj := mkObj ("Eq(" ++ A.label ++ ")") (A.uid * 100 + 60)
noncomputable def coprodObj (A B : TObj) : TObj := mkObj (A.label ++ "+" ++ B.label) (A.uid * 10000 + B.uid + 400)
noncomputable def sheafObj (j : String) (A : TObj) : TObj := mkObj ("Sh_" ++ j ++ "(" ++ A.label ++ ")") (A.uid * 100 + 70)
noncomputable def sliceObj (E : TObj) (B : TObj) : TObj := mkObj ("E/" ++ B.label) (E.uid * 10000 + B.uid + 500)
...
noncomputable def pair_both_proj (f g : MExpr) (A B : TObj) :
...
noncomputable def and_idemp_path : MPath (.comp (.pair .true_ .true_) .andM) .true_ :=
  Path.single (.rule "∧-idemp" _ _)

-- Theorem 14: ∨-absorption — true ∨ false = true
noncomputable def or_absorb_path : MPath (.comp (.pair .true_ .false_) .orM) .true_ :=
  Path.single (.rule "∨-absorb" _ _)

-- Theorem 15: ⇒-reflexivity — true ⇒ true = true
noncomputable def impl_refl_path : MPath (.comp (.pair .true_ .true_) .implM) .true_ :=
  Path.single (.rule "⇒-refl" _ _)

-- Theorem 16: ¬ true = false
noncomputable def neg_true_path : MPath (.comp .true_ .negM) .false_ :=
  Path.single (.rule "¬-true" _ _)

-- Theorem 17: Multi-step — (true ∧ true) then ¬ gives false via 3 steps
noncomputable def and_then_neg :
...
noncomputable def and_comm_path :
...
noncomputable def or_comm_path :
...
noncomputable def de_morgan_path :
...
noncomputable def power_transpose_universal (r : MExpr) (A : TObj) :
...
noncomputable def j_j_true_path (j : String) :
...
noncomputable def dense_topology_path :
...
noncomputable def id_jsheaf (A : TObj) : JSheaf where
  obj := A
  jName := "id"
  unitIso := Path.single (.rule "id-topology-unit-iso" _ _)

-- Theorem 33: Sheafification is idempotent
noncomputable def sheafification_idemp (j : String) (A : TObj) :
...
noncomputable def sheaf_reflection (j : String) (A : TObj) :
...
noncomputable def aJ_lex (j : String) (A B : TObj) :
...
noncomputable def id_geom_morph (E : TObj) : GeomMorph where
  name := "id"
  srcObj := E
  tgtObj := E
...
noncomputable def geom_triple_comp (f g h : String) (A B C D : TObj) :
...
noncomputable def classifying_path (T : String) :
...
noncomputable def classifying_obj_path :
...
noncomputable def pullback_pasting (A B C : TObj) :
...
noncomputable def beck_chevalley (f g : String) :
...
noncomputable def eq_diag_coherence (A : TObj) :
...
noncomputable def slice_omega (E B : TObj) :
...
noncomputable def pullback_lex (f : String) (A B E : TObj) :
...
noncomputable def disjoint_coprod (A B : TObj) :
...
noncomputable def char_pullback_symm (m : MExpr) :
