...
noncomputable def applyGate : Gate → QState → QState
  | Gate.id, s => s
...
/-- Evaluate a circuit -/
noncomputable def evalCircuit : Circuit → QState
  | Circuit.empty s => s
...
noncomputable def postMeasState : MeasOutcome → QState
...
noncomputable def measProb : QState → MeasOutcome → Probability
...
noncomputable def correctionGate : MeasOutcome → MeasOutcome → Gate
...
/-- Cloning map type -/
noncomputable def CloningMap := QState → QState × QState

/-- Theorem 33: The diagonal clones basis states -/
noncomputable def diagonal_clones_ket0 :
...
noncomputable def djOutcome : OracleType → QState
...
noncomputable def detectSyndrome : QError → Syndrome
...
/-- Recovery operation -/
noncomputable def recoverFromSyndrome : Syndrome → QError
...
/-- Phase addition -/
noncomputable def Phase.add : Phase → Phase → Phase
  | Phase.zero, p => p
  | p, Phase.zero => p
...
noncomputable def spiderFusion (s1 s2 : Spider) (_ : s1.color = s2.color) : Spider :=
  { color := s1.color
...
noncomputable def green_spider_fusion_zero :
...
noncomputable def gateToSpider : Gate → Spider
...
noncomputable def zxIdentitySpider (c : SpiderColor) : Spider :=
  ⟨c, Phase.zero, 1, 1⟩
...
noncomputable def hzh_path_trans_ket0 :
    Path (applyGate Gate.H (applyGate Gate.Z (applyGate Gate.H (QState.basis QubitBasis.ket0))))
         (QState.basis QubitBasis.ket1) :=
...
noncomputable def quantum_congrArg_trans
    (p : Path (QState.basis QubitBasis.ket0) (QState.basis QubitBasis.ket0))
...
noncomputable def quantum_congrArg_symm
    (p : Path (QState.basis QubitBasis.ket0) (QState.basis QubitBasis.ket0)) :
...
noncomputable def quantum_symm_trans
    (p : Path (QState.basis QubitBasis.ket0) QState.plus)
...
noncomputable def GateWord.flatten : GateWord → GateWord
...
noncomputable def dj_functorial
...
noncomputable def nested_congrArg_gate_meas
...
noncomputable def circuit_trans_chain :
    Path (QState.basis QubitBasis.ket0) (QState.basis QubitBasis.ket0) :=
...
noncomputable def zx_optimization_chain :
