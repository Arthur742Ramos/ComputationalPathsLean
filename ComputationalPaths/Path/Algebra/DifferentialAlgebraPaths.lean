...
def constantsClosedPath (DR : DiffRing R) (x y : R)
  ...
  Path.ofEq (constants_add DR x y hx hy)
...
def iterLeibniz2Path (DR : DiffRing R) (x y : R) :
  ...
  Path.ofEq (iter_leibniz_2 DR x y)
...
def leibnizTransPath (DR : DiffRing R) (x y z : R)
  ...
  Path.trans (Path.ofEq (_root_.congrArg DR.deriv hxy.symm)) (leibnizPath DR x y)
