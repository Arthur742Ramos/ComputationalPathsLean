...
@[simp] noncomputable def GrAbEl.add (a b : GrAbEl) : GrAbEl := ⟨a.deg, a.val + b.val⟩
@[simp] noncomputable def GrAbEl.zero (n : Nat) : GrAbEl := ⟨n, 0⟩
@[simp] noncomputable def GrAbEl.neg  (a : GrAbEl) : GrAbEl := ⟨a.deg, -a.val⟩
...
/-- The zero complex. -/
@[simp] noncomputable def ChainCpx.zeroCpx : ChainCpx := ⟨fun _ => 0, fun _ _ => 0⟩

/-- Chain map between complexes. -/
structure ChainMap (C D : ChainCpx) where
  map : Nat → Int → Int

@[simp] noncomputable def ChainMap.idMap (C : ChainCpx) : ChainMap C C := ⟨fun _ x => x⟩
@[simp] noncomputable def ChainMap.comp {C D E : ChainCpx}
    (f : ChainMap C D) (g : ChainMap D E) : ChainMap C E :=
  ⟨fun n x => g.map n (f.map n x)⟩

/-! ## Derived-functor model -/

@[simp] noncomputable def derivedZero (F : Int → Int) (x : Int) : Int := F x
@[simp] noncomputable def derivedHigher (_n : Nat) (_F : Int → Int) (_x : Int) : Int := 0

/-! ## Ext / Tor (vanishing model for projective/flat modules) -/

@[simp] noncomputable def ext (_n : Nat) (_a _b : Int) : Int := 0
@[simp] noncomputable def tor (_n : Nat) (_a _b : Int) : Int := 0

/-! ## Homology, mapping cone, connecting map -/

@[simp] noncomputable def homology (C : ChainCpx) (n : Nat) (x : Int) : Int := C.diff n x
@[simp] noncomputable def coneDiff (C D : ChainCpx) (n : Nat) (x : Int) : Int :=
  -(C.diff (n+1) x) + D.diff n x
@[simp] noncomputable def connecting (f : Int → Int) (x : Int) : Int := f x

/-! ## Spectral-sequence page entry (vanishing model) -/

@[simp] noncomputable def spectralEntry (_r _p _q : Nat) : Int := 0

/-! ## Dimensions -/

@[simp] noncomputable def projDim (_x : Int) : Nat := 0
@[simp] noncomputable def injDim (_x : Int) : Nat := 0
...
noncomputable def grabel_add_zero_path (a : GrAbEl) :
...
noncomputable def grabel_zero_add_path (a : GrAbEl) :
...
noncomputable def grabel_add_comm_path (a b : GrAbEl) (h : a.deg = b.deg) :
...
noncomputable def grabel_add_assoc_path (a b c : GrAbEl) :
...
noncomputable def grabel_add_neg_path (a : GrAbEl) :
...
noncomputable def comp_id_left_path {C D : ChainCpx} (f : ChainMap C D) :
...
noncomputable def comp_id_right_path {C D : ChainCpx} (f : ChainMap C D) :
...
noncomputable def comp_assoc_path {C D E F : ChainCpx}
    (f : ChainMap C D) (g : ChainMap D E) (h : ChainMap E F) :
...
noncomputable def comp_id_fg_path {C D E : ChainCpx}
...
noncomputable def comp_id_assoc_path {C D E : ChainCpx}
    (f : ChainMap C D) (g : ChainMap D E) :
...
noncomputable def ext_plus_tor_path (n : Nat) (a b : Int) :
...
noncomputable def derived_zero_path (F : Int → Int) (x : Int) :
    Path (derivedZero F x) (F x) :=
  Path.refl (F x)   -- definitional

-- 17. derivedHigher vanishes
noncomputable def derived_higher_path (n : Nat) (F : Int → Int) (x : Int) :
...
noncomputable def derived_grothendieck_path (n : Nat) (F : Int → Int) (x : Int) :
...
noncomputable def derived_higher_connected (n m : Nat) (F G : Int → Int) (x y : Int) :
...
noncomputable def derived_zero_comp_path (F G : Int → Int) (x : Int) :
...
noncomputable def spectral_vanish_path (r p q : Nat) : Path (spectralEntry r p q) 0 :=
  Path.refl 0

-- 22. **Multi-step** spectral degeneration:
--     E_r^{p,q} →[vanish] 0 →[symm vanish] E_s^{p,q}
noncomputable def spectral_degenerate_path (r s p q : Nat) :
...
--     E_r →[deg] E_s →[deg] E_t
noncomputable def spectral_three_page (r s t p q : Nat) :
    Path (spectralEntry r p q) (spectralEntry t p q) :=
  Path.trans (spectral_degenerate_path r s p q)
             (spectral_degenerate_path s t p q)

-- 24. projDim = 0
noncomputable def proj_dim_path (x : Int) : Path (projDim x) 0 := Path.refl 0
-- 25. injDim = 0
noncomputable def inj_dim_path (x : Int) : Path (injDim x) 0 := Path.refl 0

/-! ### 26-30 : Multi-step chain-map compositions -/

-- 26. f ∘ (g ∘ id) = f ∘ g  via congrArg
noncomputable def comp_right_id_path {C D E : ChainCpx}
    (f : ChainMap C D) (g : ChainMap D E) :
...
noncomputable def comp_fid_g_path {C D E : ChainCpx}
    (f : ChainMap C D) (g : ChainMap D E) :
...
noncomputable def full_simplify_path {C D E : ChainCpx}
    (f : ChainMap C D) (g : ChainMap D E) :
...
noncomputable def comp_id_left_symm_path {C D : ChainCpx} (f : ChainMap C D) :
...
-- 31. connecting id = id
noncomputable def connecting_id_path (x : Int) : Path (connecting id x) x :=
  Path.refl x

-- 32. connecting f ∘ connecting g = f ∘ g
noncomputable def connecting_comp_path (f g : Int → Int) (x : Int) :
...
noncomputable def connecting_comp_symm_path (f g : Int → Int) (x : Int) :
    Path (connecting (f ∘ g) x) (connecting f (connecting g x)) :=
  Path.refl (f (g x))

-- 34. cone of zero complexes: coneDiff 0 0 n x = 0
noncomputable def cone_zero_path (n : Nat) (x : Int) :
...
noncomputable def zero_homology_path (n : Nat) (x : Int) :
...
noncomputable def grabel_pentagon (a b c d : GrAbEl) :
...
noncomputable def grabel_cancel_right (a b : GrAbEl) :
...
noncomputable def grabel_val_add_zero (a : GrAbEl) :
...
noncomputable def ext_shift_path (n : Nat) (a b : Int) :
...
noncomputable def tor_shift_path (n : Nat) (a b : Int) :
