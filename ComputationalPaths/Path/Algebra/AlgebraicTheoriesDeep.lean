...
def fp_prod_comm (a b : FPObj) : Path FPObj (FPObj.prod a b) (FPObj.prod b a) :=
  Path.trans
...
def fp_terminal_unique (a : FPObj) :
    Path2 FPObj
      (fp_terminal_path a)
      (Path.single (Step.rule "bang'" a .term)) :=
...
def theoryOfMonoid : AlgTheory := ⟨1, 2, 3⟩
def theoryOfGroup  : AlgTheory := ⟨1, 3, 5⟩
def theoryOfRing   : AlgTheory := ⟨1, 4, 9⟩
def theoryOfComm   : AlgTheory := ⟨1, 2, 4⟩

/-- T11: Monoid → group embedding path. -/
def monoid_to_group_path : Path AlgTheory theoryOfMonoid theoryOfGroup :=
  Path.trans
...
def group_to_ring_path : Path AlgTheory theoryOfGroup theoryOfRing :=
  Path.trans
...
def monoid_to_ring_path : Path AlgTheory theoryOfMonoid theoryOfRing :=
...
def comm_monoid_quotient : Path AlgTheory theoryOfMonoid theoryOfComm :=
  Path.trans
...
def model_init (name : String) : Path ModelState (ModelState.mk name 0 0) (ModelState.mk name 0 0) :=
  .nil _

/-- T17: Operation evaluation path. -/
def model_eval_ops : (name : String) → (n : Nat) →
...
def model_check_eqs : (name : String) → (ops eqs : Nat) →
...
def model_validate (name : String) (ops eqs : Nat) :
...
def nat_trans_path : (src tgt : String) → (n : Nat) →
...
def naturality_square (f g : String) (n : Nat) :
    Path2 MorphState
      (Path.trans
...
def free_model_universal (gens : Nat) (target : String) :
...
def monadicity_path : Path MonadicSt (MonadicSt.mk 0 "theory") (MonadicSt.mk 5 "equivalence") :=
  Path.trans
...
def beck_criterion : Path MonadicSt (MonadicSt.mk 0 "functor") (MonadicSt.mk 3 "monadic") :=
  Path.trans
...
def fp_coherence (a b : FPObj) (p q : Path FPObj a b) : Path2 FPObj p q :=
  .step2 "fp_coherence" p q

/-- T46: Presentation path: adding relations. -/
def presentation_path : (gens rels : Nat) →
...
def variety_path : Path MonadicSt (MonadicSt.mk 0 "models") (MonadicSt.mk 3 "variety") :=
  Path.trans
...
def path_inv_cancel {α : Type} (a b : α) (p : Path α a b) :
...
def symm_symm_cancel {α : Type} {a b : α} (p : Path α a b) :
    Path2 α p.symm.symm p :=
  .step2 "symm_involution" _ _

/-- T57: Trans associativity as 2-cell. -/
def trans_assoc_2cell {α : Type} {a b c d : α}
