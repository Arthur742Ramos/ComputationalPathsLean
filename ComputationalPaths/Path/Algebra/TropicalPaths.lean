...
@[simp] noncomputable def tropInf : Nat := 1000000
@[simp] noncomputable def tropAdd (a b : Nat) : Nat := min a b
@[simp] noncomputable def tropMul (a b : Nat) : Nat := a + b
@[simp] noncomputable def tropZero : Nat := tropInf
@[simp] noncomputable def tropOne : Nat := 0
...
@[simp] noncomputable def evalMonomial (m : TropMonomial) (x : Nat) : Nat :=
  m.coeff + m.exp * x

structure TropPoly where
  terms : List TropMonomial

@[simp] noncomputable def evalPoly (p : TropPoly) (x : Nat) : Nat :=
  p.terms.foldl (fun acc m => min acc (evalMonomial m x)) tropInf

@[simp] noncomputable def valuation (n : Nat) : Nat := n
...
@[simp] noncomputable def tropGenus (c : TropCurve) : Nat :=
  if c.bends.length > 1 then c.bends.length - 1 else 0

@[simp] noncomputable def tropDegree (p : TropPoly) : Nat :=
  p.terms.foldl (fun acc m => max acc m.exp) 0

@[simp] noncomputable def tropPolyAdd (p q : TropPoly) : TropPoly :=
  ⟨p.terms ++ q.terms⟩
...
noncomputable def tropAdd_comm_step (a b : Nat) : Step Nat :=
  Step.mk (tropAdd a b) (tropAdd b a) (tropAdd_comm a b)

noncomputable def tropAdd_comm_path (a b : Nat) : Path (tropAdd a b) (tropAdd b a) :=
...
noncomputable def tropAdd_assoc_step (a b c : Nat) : Step Nat :=
  Step.mk (tropAdd (tropAdd a b) c) (tropAdd a (tropAdd b c)) (tropAdd_assoc a b c)

noncomputable def tropAdd_assoc_path (a b c : Nat) :
...
noncomputable def tropMul_comm_step (a b : Nat) : Step Nat :=
  Step.mk (tropMul a b) (tropMul b a) (tropMul_comm a b)

noncomputable def tropMul_comm_path (a b : Nat) : Path (tropMul a b) (tropMul b a) :=
...
noncomputable def tropMul_assoc_path (a b c : Nat) :
...
noncomputable def tropMul_one_left_path (a : Nat) : Path (tropMul tropOne a) a :=
...
noncomputable def tropMul_one_right_path (a : Nat) : Path (tropMul a tropOne) a :=
...
noncomputable def tropDistrib_step (a b c : Nat) : Step Nat :=
  Step.mk (tropMul a (tropAdd b c)) (tropAdd (tropMul a b) (tropMul a c)) (tropDistrib a b c)

noncomputable def tropDistrib_path (a b c : Nat) :
...
noncomputable def tropDistrib_right_path (a b c : Nat) :
...
noncomputable def tropAdd_idem_path (a : Nat) : Path (tropAdd a a) a :=
...
noncomputable def val_tropMul_path (a b : Nat) :
...
noncomputable def evalMonomial_at_zero_path (m : TropMonomial) :
...
noncomputable def evalMonomial_zero_exp_path (c x : Nat) :
...
noncomputable def evalMonomial_zero_coeff_path (e x : Nat) :
...
noncomputable def evalPoly_empty_path (x : Nat) : Path (evalPoly ⟨[]⟩ x) tropInf :=
...
noncomputable def val_idem_path (n : Nat) : Path (valuation (valuation n)) (valuation n) :=
...
noncomputable def tropMul_zero_absorb_path (a : Nat) :
...
noncomputable def tropPolyAdd_empty_left_path (p : TropPoly) :
...
noncomputable def tropAdd_comm_assoc_path (a b c : Nat) :
...
noncomputable def tropDistrib_comm_path (a b c : Nat) :
...
noncomputable def tropMul_comm_one_path (b : Nat) :
    Path (tropMul b tropOne) b :=
  tropMul_one_right_path b

-- 28. Left-right identity roundtrip
noncomputable def tropMul_identity_roundtrip (a : Nat) :
...
noncomputable def tropAdd_zero_left_path (a : Nat) (h : a ≤ tropInf) :
