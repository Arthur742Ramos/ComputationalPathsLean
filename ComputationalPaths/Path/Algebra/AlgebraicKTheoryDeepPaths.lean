...
@[simp] noncomputable def eval : KExpr → Int
  | zero       => 0
  | gen n      => n
...
@[simp] noncomputable def eval_eq : {a b : KExpr} → KStep a b → a.eval = b.eval
...
@[simp] noncomputable def eval_eq : {a b : KExpr} → KPath a b → a.eval = b.eval
  | _, _, refl _    => rfl
...
@[simp] noncomputable def toPath {a b : KExpr} (p : KPath a b) : ComputationalPaths.Path a.eval b.eval :=
  ComputationalPaths.Path.mk
    [ComputationalPaths.Step.mk a.eval b.eval (eval_eq p)]
    (eval_eq p)

/-- Length of a path. -/
@[simp] noncomputable def length : {a b : KExpr} → KPath a b → Nat
  | _, _, refl _    => 0
  | _, _, step _    => 1
...
-- 8
noncomputable def k0_add_zero (e : KExpr) : KPath (KExpr.add e KExpr.zero) e :=
  step (KStep.add_zero_right e)

-- 9
noncomputable def k0_zero_add (e : KExpr) : KPath (KExpr.add KExpr.zero e) e :=
  step (KStep.add_zero_left e)

-- 10
noncomputable def k0_add_inv (e : KExpr) : KPath (KExpr.add e (KExpr.neg e)) KExpr.zero :=
  step (KStep.add_neg_right e)

-- 11
noncomputable def k0_inv_add (e : KExpr) : KPath (KExpr.add (KExpr.neg e) e) KExpr.zero :=
  step (KStep.add_neg_left e)

-- 12
noncomputable def k0_comm (e₁ e₂ : KExpr) : KPath (KExpr.add e₁ e₂) (KExpr.add e₂ e₁) :=
  step (KStep.add_comm e₁ e₂)

-- 13
noncomputable def k0_assoc (e₁ e₂ e₃ : KExpr) :
    KPath (KExpr.add (KExpr.add e₁ e₂) e₃) (KExpr.add e₁ (KExpr.add e₂ e₃)) :=
  step (KStep.add_assoc e₁ e₂ e₃)

-- 14
noncomputable def k0_neg_neg (e : KExpr) : KPath (KExpr.neg (KExpr.neg e)) e :=
  step (KStep.neg_neg e)

/-! ## 3. Grothendieck Construction Paths (15–18) -/

-- 15
noncomputable def groth_pair_zero (e : KExpr) : KPath (KExpr.pair e KExpr.zero) e :=
  step (KStep.pair_zero_neg e)

-- 16
noncomputable def groth_pair_cancel (e : KExpr) : KPath (KExpr.pair e e) KExpr.zero :=
  step (KStep.pair_cancel e)

-- 17
noncomputable def groth_pair_neg (a b : KExpr) :
    KPath (KExpr.neg (KExpr.pair a b)) (KExpr.pair b a) :=
  step (KStep.pair_neg a b)

-- 18: neg of Grothendieck pair and then cancel
noncomputable def groth_neg_then_add (a b : KExpr) :
...
-- 19
noncomputable def k1_det_mul (a b : Int) :
    KPath (KExpr.add (KExpr.det a) (KExpr.det b)) (KExpr.det (a + b)) :=
  step (KStep.det_mul a b)

-- 20
noncomputable def k1_det_zero : KPath (KExpr.det 0) KExpr.zero :=
  step KStep.det_zero

-- 21
noncomputable def k1_det_neg (a : Int) :
    KPath (KExpr.neg (KExpr.det a)) (KExpr.det (-a)) :=
  step (KStep.det_neg a)

-- 22: det(a) + det(-a) → det(a + -a) → det(0) → 0
noncomputable def k1_det_cancel (a : Int) :
...
-- 27
noncomputable def bott_shift (n : Nat) : KPath (KExpr.bott (n + 2)) (KExpr.bott n) :=
  step (KStep.bott_period n)

-- 28
noncomputable def bott_K0 : KPath (KExpr.bott 0) KExpr.zero :=
  step KStep.bott_zero

-- 29
noncomputable def bott_K1 : KPath (KExpr.bott 1) (KExpr.gen 1) :=
  step KStep.bott_one

-- 30: Bott shift by 4 in 2 steps
noncomputable def bott_shift_4 (n : Nat) : KPath (KExpr.bott (n + 4)) (KExpr.bott n) :=
  trans (step (KStep.bott_period (n + 2))) (step (KStep.bott_period n))

-- 31: Bott shift by 6 in 3 steps
noncomputable def bott_shift_6 (n : Nat) : KPath (KExpr.bott (n + 6)) (KExpr.bott n) :=
  trans (step (KStep.bott_period (n + 4)))
        (trans (step (KStep.bott_period (n + 2))) (step (KStep.bott_period n)))

-- 32: bott(2) → bott(0) → 0 in 2 steps
noncomputable def bott_2_to_zero : KPath (KExpr.bott 2) KExpr.zero :=
  trans (step (KStep.bott_period 0)) (step KStep.bott_zero)

/-! ## 7. Composed Multi-Step Paths (33–40) -/

-- 33: (e + 0) + neg(e + 0) → 0
noncomputable def add_zero_then_cancel (e : KExpr) :
...
noncomputable def neg_neg_add_zero (e : KExpr) :
    KPath (KExpr.add (KExpr.neg (KExpr.neg e)) KExpr.zero) e :=
  trans (step (KStep.add_zero_right _)) (step (KStep.neg_neg e))

-- 35: neg(0) → 0
noncomputable def neg_zero_path : KPath (KExpr.neg KExpr.zero) KExpr.zero :=
  step KStep.neg_zero

-- 36: det(a) + det(b) commutes via add_comm
noncomputable def det_comm (a b : Int) :
...
noncomputable def assoc_then_comm (a b c : KExpr) :
...
noncomputable def zero_add_then_add_zero (e : KExpr) :
...
noncomputable def bott_3_to_gen : KPath (KExpr.bott 3) (KExpr.gen 1) :=
  trans (step (KStep.bott_period 1)) (step KStep.bott_one)

-- 40: neg(add(a,b)) → add(neg(a), neg(b)) → add(neg(b), neg(a))
noncomputable def neg_distrib_comm (a b : KExpr) :
