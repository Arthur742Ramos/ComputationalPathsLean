...
/-- Unit embeds a value. -/
noncomputable def punit (x : α) : PMon α := ⟨x⟩

/-- Join flattens nested PMon. -/
noncomputable def pjoin (x : PMon (PMon α)) : PMon α := ⟨x.val.val⟩

/-- Bind sequences. -/
noncomputable def pbind (x : PMon α) (f : α → PMon β) : PMon β := f x.val

/-- Map lifts a function. -/
noncomputable def pmap (f : α → β) (x : PMon α) : PMon β := ⟨f x.val⟩

/-! ## Monad laws as paths -/

/-- 1. Left unit law: bind (unit a) f = f a -/
noncomputable def left_unit (a : α) (f : α → PMon β) :
    Path (pbind (punit a) f) (f a) :=
  Path.refl (f a)

/-- 2. Right unit law: bind m unit = m -/
noncomputable def right_unit (m : PMon α) :
    Path (pbind m punit) m :=
  Path.refl m

/-- 3. Associativity: bind (bind m f) g = bind m (fun x => bind (f x) g) -/
noncomputable def assoc_law (m : PMon α) (f : α → PMon β) (g : β → PMon γ) :
...
noncomputable def kleisli (f : α → PMon β) (g : β → PMon γ) (x : α) : PMon γ :=
  pbind (f x) g

/-- 4. Kleisli left unit: kleisli punit f = f -/
noncomputable def kleisli_left_unit (f : α → PMon β) :
    Path (fun x => kleisli punit f x) f :=
  Path.refl f

/-- 5. Kleisli right unit: kleisli f punit = f -/
noncomputable def kleisli_right_unit (f : α → PMon β) :
    Path (fun x => kleisli f punit x) f :=
  Path.refl f

/-- 6. Kleisli associativity. -/
noncomputable def kleisli_assoc (f : α → PMon β) (g : β → PMon γ) (h : γ → PMon δ)
    (x : α) :
...
/-- 7. join ∘ unit = id. -/
noncomputable def join_unit (x : PMon α) :
    Path (pjoin (punit x)) x :=
  Path.refl x

/-- 8. join ∘ map unit = id. -/
noncomputable def join_map_unit (x : PMon α) :
    Path (pjoin (pmap punit x)) x :=
  Path.refl x

/-- 9. join ∘ join = join ∘ map join (associativity). -/
noncomputable def join_assoc (x : PMon (PMon (PMon α))) :
...
noncomputable def four_step_kleisli_coherence
...
noncomputable def unit_bind_unit_chain (a : α) :
...
noncomputable def congrArg_map_over_bind (f : α → β) (m : PMon α) :
...
noncomputable def whisker_bind_left (m : PMon α) (f f' : α → PMon β)
    (h : f = f') :
...
noncomputable def whisker_bind_right (m m' : PMon α) (f : α → PMon β)
    (h : m = m') :
    Path (pbind m f) (pbind m' f) :=
...
noncomputable def double_whisker (m m' : PMon α) (f f' : α → PMon β)
    (hm : m = m') (hf : f = f') :
    Path (pbind m f) (pbind m' f') :=
...
noncomputable def symm_trans_roundtrip (m : PMon α) :
...
noncomputable def deep_five_step (a : α) (f : α → PMon β) (g : β → PMon γ) :
