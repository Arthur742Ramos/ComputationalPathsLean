...
/-- Sum of two species -/
noncomputable def speciesSum (F G : Species) : Species where
...
noncomputable def speciesProd (F G : Species) : Species where
  apply := fun A => F.apply A × G.apply A
  transport := fun f ⟨x, y⟩ => (F.transport f x, G.transport f y)

/-- Hadamard (pointwise) product -/
noncomputable def speciesHadamard (F G : Species) : Species where
  apply := fun A => F.apply A × G.apply A
  transport := fun f ⟨x, y⟩ => (F.transport f x, G.transport f y)

/-- Pointing of a species: a structure with a distinguished element -/
noncomputable def speciesPointing (F : Species) : Species where
  apply := fun A => F.apply A × A
  transport := fun f ⟨s, a⟩ => (F.transport f s, f a)

/-- Cartesian product species -/
noncomputable def speciesCartesian (F G : Species) : Species where
  apply := fun A => F.apply A × G.apply A
  transport := fun f ⟨x, y⟩ => (F.transport f x, G.transport f y)
...
noncomputable def egf_prod_congrArg (f g : EGF) {n m : Nat} (p : Path n m) :
    Path (f.coeff n * g.coeff n) (f.coeff m * g.coeff m) :=
  Path.congrArg (fun k => f.coeff k * g.coeff k) p
...
noncomputable def speciesDerivative (F : Species) : Species where
  apply := fun A => F.apply (Option A)
  transport := fun f s => F.transport (Option.map f) s

-- Def 20: Derivative preserves transport
noncomputable def derivative_transport_path (F : Species) (A B : Type) (f : A → B)
    (s : F.apply (Option A)) :
...
noncomputable def speciesIsoRefl (F : Species) : SpeciesIso F F where
  forward := id
  backward := id

/-- Species iso composition -/
noncomputable def speciesIsoTrans (F G H : Species)
...
noncomputable def speciesIsoSymm (F G : Species) (iso : SpeciesIso F G) : SpeciesIso G F where
...
noncomputable def species_path_trans {F : Species} {A : Type}
    {x y z : F.apply A} (p : Path x y) (q : Path y z) : Path x z :=
  Path.trans p q

-- Theorem 30: Symmetry of species paths
noncomputable def species_path_symm {F : Species} {A : Type}
...
noncomputable def congrArg_sum_inl {F G : Species} {A : Type}
    {x y : F.apply A} (p : Path x y) :
...
noncomputable def congrArg_sum_inr {F G : Species} {A : Type}
...
noncomputable def speciesSubst (F G : Species) : Species where
...
noncomputable def joyalTransport (J : JoyalStructure) : JoyalStructure where
...
noncomputable def natTransId (F : Species) : SpeciesNatTrans F F where
  component := id

/-- Composition of natural transformations -/
noncomputable def natTransComp {F G H : Species}
...
noncomputable def species_hexagon {F G : Species} {A : Type}
    (x : F.apply A) (y : G.apply A) :
