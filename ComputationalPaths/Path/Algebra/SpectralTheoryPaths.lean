...
noncomputable def NMat2.mulVec (m : NMat2) (v : NVec2) : NVec2 :=
  ⟨m.a11 * v.x + m.a12 * v.y, m.a21 * v.x + m.a22 * v.y⟩

noncomputable def NMat2.add (m1 m2 : NMat2) : NMat2 :=
  ⟨m1.a11 + m2.a11, m1.a12 + m2.a12, m1.a21 + m2.a21, m1.a22 + m2.a22⟩

noncomputable def NMat2.smul (c : Nat) (m : NMat2) : NMat2 :=
  ⟨c * m.a11, c * m.a12, c * m.a21, c * m.a22⟩

noncomputable def NVec2.smul (c : Nat) (v : NVec2) : NVec2 :=
  ⟨c * v.x, c * v.y⟩

noncomputable def NVec2.add (v1 v2 : NVec2) : NVec2 :=
  ⟨v1.x + v2.x, v1.y + v2.y⟩

noncomputable def NVec2.zero : NVec2 := ⟨0, 0⟩
noncomputable def NMat2.id : NMat2 := ⟨1, 0, 0, 1⟩
noncomputable def NMat2.zero : NMat2 := ⟨0, 0, 0, 0⟩
noncomputable def NMat2.trace (m : NMat2) : Nat := m.a11 + m.a22

noncomputable def isNEigenvalue (m : NMat2) (lam : Nat) (v : NVec2) : Prop :=
...
noncomputable def nid_mulVec_step (v : NVec2) : Step NVec2 :=
  Step.mk (NMat2.id.mulVec v) v (nid_mulVec v)

noncomputable def nid_mulVec_path (v : NVec2) : Path (NMat2.id.mulVec v) v :=
...
noncomputable def nzero_mulVec_path (v : NVec2) : Path (NMat2.zero.mulVec v) NVec2.zero :=
...
noncomputable def ntrace_id_path : Path NMat2.id.trace 2 :=
...
noncomputable def ntrace_zero_path : Path NMat2.zero.trace 0 :=
...
noncomputable def ntrace_add_path (m1 m2 : NMat2) :
...
noncomputable def ntrace_smul_path (c : Nat) (m : NMat2) :
...
noncomputable def nmat_add_comm_step (m1 m2 : NMat2) : Step NMat2 :=
  Step.mk (NMat2.add m1 m2) (NMat2.add m2 m1) (nmat_add_comm m1 m2)

noncomputable def nmat_add_comm_path (m1 m2 : NMat2) :
...
noncomputable def nmat_add_zero_right_path (m : NMat2) :
...
noncomputable def nmat_add_zero_left_path (m : NMat2) :
...
noncomputable def nmat_add_assoc_path (m1 m2 m3 : NMat2) :
...
noncomputable def nsmul_one_path (m : NMat2) : Path (NMat2.smul 1 m) m :=
...
noncomputable def nsmul_zero_path (m : NMat2) : Path (NMat2.smul 0 m) NMat2.zero :=
...
noncomputable def nsmul_add_path (c : Nat) (m1 m2 : NMat2) :
...
noncomputable def nsmul_assoc_path (c1 c2 : Nat) (m : NMat2) :
...
noncomputable def diag_eigen_fst_path (a b : Nat) :
...
noncomputable def diag_eigen_snd_path (a b : Nat) :
...
noncomputable def nmulVec_add_path (m : NMat2) (v w : NVec2) :
...
noncomputable def nmulVec_smul_path (m : NMat2) (c : Nat) (v : NVec2) :
...
noncomputable def nvec_add_zero_path (v : NVec2) :
...
noncomputable def nvec_add_comm_path (v w : NVec2) :
...
noncomputable def spectral_roundtrip (a b : Nat) :
...
noncomputable def ntrace_add_comm_path (m1 m2 : NMat2) :
...
noncomputable def trace_mat_comm_path (m1 m2 : NMat2) :
...
noncomputable def mulVec_smul_one_path (m : NMat2) (v : NVec2) :
...
noncomputable def nmat_add_pentagon (m1 m2 m3 m4 : NMat2) :
...
noncomputable def smul_zero_add_path (m : NMat2) :
...
noncomputable def diag_eigen_chain (a b : Nat) :
    Path ((diagNMat a b).mulVec ⟨1, 0⟩) (NVec2.smul a ⟨1, 0⟩) :=
...
noncomputable def nsmul_one_roundtrip (m : NMat2) :
...
noncomputable def nid_add_zero_chain (v : NVec2) :
...
noncomputable def ntrace_smul_one_path (m : NMat2) :
