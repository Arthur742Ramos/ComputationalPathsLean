...
noncomputable def H0Rel {X : Type u} (x y : X) : Prop :=
  Nonempty (Path x y)

/-- Setoid of connected components. -/
noncomputable def H0Setoid (X : Type u) : Setoid X where
  r := H0Rel
  iseqv :=
    { refl := by
        intro x
...
noncomputable def mapH0 {X Y : Type u} (f : X → Y) : H0 X → H0 Y :=
  Quot.lift
...
noncomputable def mapH1 {X Y : Type u} (f : X → Y) (x₀ : X) : H1 X x₀ → H1 Y (f x₀) :=
  Quot.lift
...
noncomputable def baseU (M : CoverData) : M.U := M.iUVU M.baseUV

/-- Basepoint in `V` selected from overlap data. -/
noncomputable def baseV (M : CoverData) : M.V := M.iUVV M.baseUV

/-- Basepoint in `A`, taken from the `U`-side of the cover. -/
noncomputable def baseA (M : CoverData) : M.A := M.iU M.baseU

/-- Transport a loop in `V` to a loop at the `A` basepoint via the glue path. -/
noncomputable def loopFromV (M : CoverData) (q : Path M.baseV M.baseV) : Path M.baseA M.baseA :=
  Path.trans M.glue (Path.trans (Path.congrArg M.iV q) (Path.symm M.glue))

/-- Combine `U` and `V` loops into an ambient loop in `A`. -/
noncomputable def mergeLoops (M : CoverData)
...
noncomputable def loopFromV_respects {M : CoverData} {q q' : Path M.baseV M.baseV}
    (h : RwEq q q') :
...
noncomputable def mergeLoops_respects {M : CoverData}
...
noncomputable def LoopDecomposition.rweqWitness {M : CoverData} {p : Path M.baseA M.baseA}
...
noncomputable def iMap (M : CoverData) : H1 M.UV M.baseUV → H1Sum M :=
...
noncomputable def zeroH1 (M : CoverData) : H1 M.A M.baseA :=
  Quot.mk _ (Path.refl M.baseA)

/-- The zero class in `H₀(U∩V)`. -/
noncomputable def zeroH0 (M : CoverData) : H0 M.UV :=
  Quot.mk _ M.baseUV
...
noncomputable def mayerVietorisSequence (M : CoverData) (W : MayerVietorisWitness M) :
...
noncomputable def CircleArcCover.toCoverData (C : CircleArcCover) : CoverData where
  A := Circle
  U := C.U
  V := C.V
  UV := C.UV
...
/-- `H₁(S¹) ≅ ℤ` via Mayer-Vietoris — **counterfactually conditional** on a
    ...
    ⚠ **Impossible under the current definitions.**
    `pi1Bridge : SimpleEquiv (π₁(S¹)) circlePiOne`
    ...
    (`circlePiOneEquivInt`, as the first-isomorphism theorem for the synthetic
    winding map).  `MetadataRepair.no_circle_genuine_synthetic_bridge` proves
    that no such `SimpleEquiv` exists: the genuine quotient is contractible and
    the synthetic quotient is not.  This function is retained as a transparent
    conditional reduction, but its hypothesis is uninhabited; it is neither an
    open construction problem nor a closed proof of `H₁(S¹) ≅ ℤ` for the
    framework's own fundamental group. -/
