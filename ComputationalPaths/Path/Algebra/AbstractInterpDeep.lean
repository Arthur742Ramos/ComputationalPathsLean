...
def gc_extensive {C : Type u} {A : Type v}
...
def gc_reductive {C : Type u} {A : Type v}
    {cOrd : DomOrder C} {aOrd : DomOrder A}
    (gc : GaloisConn C A cOrd aOrd) :
...
def path_round_trip_ga {C : Type u} {A : Type v}
...
def transformer_soundness_path {C : Type u} {A : Type v}
...
def best_transformer_compose {C : Type u} {A : Type v}
...
def best_transformer_symm {C : Type u} {A : Type v}
    (alpha : C → A) (gamma : A → C) (f : C → C)
    {a1 a2 : A} (p : Path a1 a2) :
...
def widen_left_path {A : Type u} (w : Widening A)
...
def widen_right_path {A : Type u} (w : Widening A)
...
def widen_both_path {A : Type u} (w : Widening A)
...
def widen_chain_step {A : Type u} (w : Widening A) (f : A → A)
    {a1 a2 : A} (p : Path a1 a2) :
...
def iterN {A : Type u} (f : A → A) : Nat → A → A
  | 0, a => a
  | n + 1, a => f (iterN f n a)

/-- 21: iterN preserves paths on the seed -/
def iterN_path {A : Type u} (f : A → A) (n : Nat)
    {a1 a2 : A} (p : Path a1 a2) :
...
def narrowIterN {A : Type u} (nr : Narrowing A) (f : A → A) : Nat → A → A
  | 0, a => a
  | n + 1, a => nr.narrow (narrowIterN nr f n a) (f (narrowIterN nr f n a))

/-- 24: Narrowed iteration preserves seed paths -/
def narrowIterN_path {A : Type u} (nr : Narrowing A) (f : A → A)
    (n : Nat) {a1 a2 : A} (p : Path a1 a2) :
...
def reduction_path {A : Type u} {B : Type v}
...
def double_reduction_path {A : Type u} {B : Type v}
    (r : Reduction A B) {d1 d2 : ProdDom A B}
    (p : Path d1 d2) :
...
def reduced_product_transformer {A : Type u} {B : Type v}
...
def lifted_val_path {A : Type u} {a1 a2 : A}
...
def flat_val_path {A : Type u} {a1 a2 : A}
...
def intervalJoin (i1 i2 : IntervalDom) : IntervalDom :=
  { lo := min i1.lo i2.lo, hi := max i1.hi i2.hi }

/-- 32: Interval join preserves left-argument paths -/
def intervalJoin_left_path {i1 i2 : IntervalDom} (j : IntervalDom)
...
def intervalJoin_right_path (i : IntervalDom) {j1 j2 : IntervalDom}
    (p : Path j1 j2) : Path (intervalJoin i j1) (intervalJoin i j2) :=
  Path.congrArg (fun x => intervalJoin i x) p

/-- Interval widening. -/
def intervalWiden (i1 i2 : IntervalDom) : IntervalDom :=
...
def intervalWiden_left_path {i1 i2 : IntervalDom} (j : IntervalDom)
...
def powersetAlpha {C : Type u} {A : Type v} (alpha : C → A) :
    List C → List A :=
  List.map alpha

/-- 35: Powerset alpha preserves paths on equal sets -/
def powerset_alpha_path {C : Type u} {A : Type v}
...
def two_step_abstract {A : Type u} (f g : A → A) {a1 a2 : A}
    (p : Path a1 a2) : Path (f (g a1)) (f (g a2)) :=
  Path.congrArg (fun x => f (g x)) p

/-- 39: Three-step abstract computation path -/
def three_step_abstract {A : Type u} (f g h : A → A) {a1 a2 : A}
    (p : Path a1 a2) : Path (f (g (h a1))) (f (g (h a2))) :=
  Path.congrArg (fun x => f (g (h x))) p

/-- 40: Path chain from n-step widening -/
def widen_chain_compose {A : Type u} (w : Widening A) (f : A → A)
    (n : Nat) {a1 a2 : A} (p : Path a1 a2) :
...
def soundness_chain {C : Type u} {A : Type v}
    (alpha : C → A) {c1 c2 c3 : C}
...
def ag_coherence_path {C : Type u} {A : Type v}
    (alpha : C → A) (gamma : A → C)
    {a1 a2 : A} (p : Path a1 a2) :
...
def ga_coherence_path {C : Type u} {A : Type v}
    (alpha : C → A) (gamma : A → C)
...
def alpha_idempotence_path {C : Type u} {A : Type v}
    (alpha : C → A) (gamma : A → C)
    {c1 c2 : C} (p : Path c1 c2) :
...
def gamma_idempotence_path {C : Type u} {A : Type v}
    (alpha : C → A) (gamma : A → C)
    {a1 a2 : A} (p : Path a1 a2) :
...
def fixpoint_iter_invariant {A : Type u} (f : A → A)
...
def fixpoints_connected {A : Type u} (f : A → A)
...
def fixpoint_compose_path {A : Type u} (f g : A → A)
...
/-- Sign domain join. -/
def signJoin : SignDom → SignDom → SignDom
  | SignDom.sbot, x => x
  | x, SignDom.sbot => x
...
def signJoin_left_path {s1 s2 : SignDom} (t : SignDom)
...
def triple_compose_path {A : Type u}
...
def full_pipeline_path {C : Type u} {A : Type v}
    (alpha : C → A) (gamma : A → C)
