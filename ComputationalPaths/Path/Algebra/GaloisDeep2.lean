...
/-- Fixedness under two Galois automorphisms is witnessed by the two
corresponding action reductions. -/
noncomputable def fixed_compose_path
    (G : List FieldAut) (a : FieldElem) (σ τ : FieldAut)
    (hσ : σ ∈ G) (hτ : τ ∈ G) (hFixed : isFixed G a) :
    Path (σ.apply (τ.apply a)) a :=
  Path.trans
    (Path.congrArg σ.apply (Path.stepChain (hFixed τ hτ)))
    (Path.stepChain (hFixed σ hσ))

/-- The two-action fixed-field trace is coherently invertible. -/
noncomputable def fixed_compose_coherence
    (G : List FieldAut) (a : FieldElem) (σ τ : FieldAut)
    (hσ : σ ∈ G) (hτ : τ ∈ G) (hFixed : isFixed G a) :
    Path.RwEq
      (Path.trans (fixed_compose_path G a σ τ hσ hτ hFixed)
        (Path.symm (fixed_compose_path G a σ τ hσ hτ hFixed)))
      (Path.refl (σ.apply (τ.apply a))) :=
  Path.rweq_cmpA_inv_right
    (fixed_compose_path G a σ τ hσ hτ hFixed)
...
/-- Certificate that the quotient represented by `upper/lower` has a
commuting set of coset representatives. -/
structure AbelianFactorWitness
    (upper lower : List FieldAut) where
  representatives : List FieldAut
  covers :
    ∀ σ, σ ∈ upper →
      ∃ r, r ∈ representatives ∧
        ∃ l, l ∈ lower ∧ σ.apply = l.apply ∘ r.apply
  commutes :
    ∀ σ, σ ∈ representatives →
      ∀ τ, τ ∈ representatives → ∀ a,
        σ.apply (τ.apply a) = τ.apply (σ.apply a)

/-- A group is solvable if it has a subnormal series with certified abelian
successive factors. -/
structure SolvableSeries where
  ...
  abelianFactors :
    ∀ (i : Nat) (h : i + 1 < chain.length),
      AbelianFactorWitness
        (chain.get
          ⟨i, Nat.lt_trans (Nat.lt_succ_self i) h⟩)
        (chain.get ⟨i + 1, h⟩)
...
/-- Path from a generator to an element of a simple extension. -/
noncomputable def simpleExtPath (gen a : FieldElem) : ExtPath gen a :=
  ExtPath.cons (ExtStep.adjoin gen a) (ExtPath.nil a)

/-- Typed evidence that an element of an extension is reached from its
chosen generator without leaving the declared extension endpoints. -/
structure GenerationCertificate
    (ext : List FieldElem) (gen a : FieldElem) where
  generator_mem : gen ∈ ext
  target_mem : a ∈ ext
  route : ExtPath gen a

/-- Simple extension: the generator lies in the extension and every element
comes with explicit extension-path dependence on it. -/
noncomputable def IsSimple (ext : List FieldElem) (gen : FieldElem) : Prop :=
  gen ∈ ext ∧
    ∀ a, a ∈ ext → Nonempty (GenerationCertificate ext gen a)

/-- Primitive element theorem: finite separable extension is simple -/
theorem primitive_element (ext : List FieldElem) (hFin : ext ≠ [])
    : ∃ gen, IsSimple ext gen := by
  cases ext with
  | nil => exact False.elim (hFin rfl)
  | cons gen rest =>
      exact ⟨gen, List.mem_cons_self,
        fun a ha => ⟨
          { generator_mem := List.mem_cons_self
            target_mem := ha
            route := simpleExtPath gen a }⟩⟩
