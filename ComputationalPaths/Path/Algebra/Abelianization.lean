...
open BouquetN Hurewicz CompPath
...
/-- The commutator of the Klein bottle generators is the even loop. -/
theorem kleinBottle_commutator_generators :
    commutator kleinBottleMul kleinBottleInv (1, 0) (0, 1) = (0, -2) := by
  simp [commutator, kleinBottleMul, kleinBottleInv, kleinBottleAct,
    kleinBottleParity, natParity_zero, natParity_succ]
  decide
...
theorem kleinBottle_nonAbelian :
    -- ℤ ⋊ ℤ is not abelian
    kleinBottleMul (1, 0) (0, 1) ≠ kleinBottleMul (0, 1) (1, 0) := by
  intro h
  have h' := _root_.congrArg Prod.snd h
  have hParity : kleinBottleParity (1 : Int) = true := by
    simp [kleinBottleParity, natParity_zero, natParity_succ]
  have hAct : kleinBottleAct (1 : Int) (1 : Int) = -1 := by
    simp [kleinBottleAct, hParity]
  have h'' := h'
  simp [kleinBottleMul, hAct] at h''
