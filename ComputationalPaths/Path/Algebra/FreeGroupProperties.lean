...
def freeGroupOneCyclic : BouquetFreeGroup 1 → Prop :=
  fun x => ∃ k : Int, Nonempty (Path x (BouquetFreeGroup.genPow Fin'B.fzero k))
...
def TorsionFree (n : Nat) : Prop :=
  ∀ x : BouquetFreeGroup n, ∀ k : Nat,
    k > 0 →
      Nonempty (Path (BouquetFreeGroup.pow (n := n) x k) (BouquetFreeGroup.one (n := n))) →
      Nonempty (Path x (BouquetFreeGroup.one (n := n)))
