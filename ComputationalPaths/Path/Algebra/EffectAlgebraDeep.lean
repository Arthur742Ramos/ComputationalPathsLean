...
noncomputable def ea_elem_refl {A : Type u} (a : EAElem A) : Path a a :=
  Path.refl a

/-- Def 2: Symmetry of element paths -/
noncomputable def ea_elem_symm {A : Type u} {a b : EAElem A} (p : Path a b) : Path b a :=
  Path.symm p

/-- Def 3: Transitivity of element paths -/
noncomputable def ea_elem_trans {A : Type u} {a b c : EAElem A}
    (p : Path a b) (q : Path b c) : Path a c :=
  Path.trans p q

/-- Def 4: Summable witness is self-consistent -/
noncomputable def summable_refl {A : Type u} {a b r : A} (_s : Summable a b r)
    : Path r r :=
  Path.refl r

/-- Def 5: Orthocomplement witness reflexivity -/
noncomputable def ortho_refl {A : Type u} {a one : A} (oc : OrthoComplement a one)
...
noncomputable def comm_involution {A : Type u} {add : A → A → A} {a b : A}
...
noncomputable def assoc_chain {A : Type u} {add : A → A → A} {a b c d : A}
...
noncomputable def handler_op_consistent {E A B : Type u}
...
/-- Map over free terms -/
noncomputable def FreeTerm.map {E A B : Type u} (f : A → B) : FreeTerm E A → FreeTerm E B
  | .pure a => .pure (f a)
  | .op e t => .op e (FreeTerm.map f t)

/-- Bind for free terms -/
noncomputable def FreeTerm.bind {E A B : Type u} (t : FreeTerm E A) (f : A → FreeTerm E B)
    : FreeTerm E B :=
  match t with
  | .pure a => f a
...
noncomputable def free_bind_pure_left {E A B : Type u} (a : A) (f : A → FreeTerm E B)
    : Path (FreeTerm.bind (FreeTerm.pure a) f) (f a) :=
  Path.refl (f a)

/-- Def 15: Pure is right identity for bind -/
noncomputable def free_bind_pure_right {E A : Type u} : (t : FreeTerm E A) →
...
noncomputable def free_bind_assoc {E A B C : Type u} (f : A → FreeTerm E B) (g : B → FreeTerm E C)
    : (t : FreeTerm E A) →
...
noncomputable def free_map_id {E A : Type u} : (t : FreeTerm E A) →
...
noncomputable def free_map_comp {E A B C : Type u} (f : A → B) (g : B → C)
    : (t : FreeTerm E A) →
...
noncomputable def le_refl_witness {A : Type u} (join : A → A → A) (a : A)
    (idem : Path (join a a) a) : LEWitness join a a :=
  ⟨idem⟩

/-- Def 20: LE from path to known LE -/
noncomputable def le_compose {A : Type u} {join : A → A → A} {a b : A}
...
noncomputable def orthomod_symm {A : Type u} {join : A → A → A} {compl : A → A} {a b : A}
...
noncomputable def mba_meet_comm_invol {A : Type u} {m : MBA A} (mc : MBACommMeet m)
    (a b : A) : Path (m.meet a b) (m.meet a b) :=
  Path.trans (mc.commMeet a b) (mc.commMeet b a)

/-- Def 23: Join commutativity is involutive -/
noncomputable def mba_join_comm_invol {A : Type u} {m : MBA A} (mc : MBACommJoin m)
...
noncomputable def mba_meet_assoc4 {A : Type u} {m : MBA A}
...
noncomputable def mba_join_assoc4 {A : Type u} {m : MBA A}
...
noncomputable def composeHandlers {A B C : Type u}
...
noncomputable def interpretPure {E A B : Type u} (h : Handler E A B) (a : A) : B :=
  h.retH.ret a

/-- Def 34: Interpreting pure is just return handler -/
noncomputable def interpret_pure {E A B : Type u} (h : Handler E A B) (a : A)
    : Path (interpretPure h a) (h.retH.ret a) :=
  Path.refl (h.retH.ret a)

/-- Def 35: congrArg lifts path through interpretation -/
noncomputable def interpret_congr {E A B : Type u} (h : Handler E A B)
    {a1 a2 : A} (p : Path a1 a2)
...
noncomputable def effect_eq_symm {E A : Type u} {l r : FreeTerm E A} (eq : EffectEq l r)
    : Path r l :=
  Path.symm eq.proof

/-- Def 37: Effect equations are transitive -/
noncomputable def effect_eq_trans {E A : Type u} {l m r : FreeTerm E A}
    (eq1 : EffectEq l m) (eq2 : EffectEq m r) : Path l r :=
  Path.trans eq1.proof eq2.proof

/-- Def 38: Effect equations under op context -/
noncomputable def effect_eq_under_op {E A : Type u} (e : E)
...
noncomputable def reduce_to_path {E A : Type u} {t1 t2 : FreeTerm E A}
...
noncomputable def church_rosser {E A : Type u} {t u v : FreeTerm E A}
    (r1 : Path t u) (r2 : Path t v) : Path u v :=
  Path.trans (Path.symm r1) r2

/-- Def 41: Diamond property via path composition -/
noncomputable def confluence_diamond {E A : Type u} {t u v : FreeTerm E A}
...
noncomputable def morphism_comp_preserves {A B C : Type u}
...
noncomputable def morphism_comp_zero {A B C : Type u}
    {eaA : EACoherence A} {eaB : EACoherence B} {eaC : EACoherence C}
...
noncomputable def effect_eq_bind_congr {E A B : Type u}
...
noncomputable def ea_comm_double {A : Type u} (ea : EACoherence A) (a b : A)
    : Path (ea.add a b) (ea.add a b) :=
  Path.trans (ea.comm a b) (ea.comm b a)

/-- Def 57: Four-fold associativity via two steps -/
noncomputable def four_assoc {A : Type u} (ea : EACoherence A) (a b c d : A)
    : Path (ea.add (ea.add a b) (ea.add c d))
           (ea.add a (ea.add b (ea.add c d))) :=
  ea.assoc a b (ea.add c d)

/-- Def 58: Handler naturality -/
noncomputable def handler_naturality {A B C : Type u}
