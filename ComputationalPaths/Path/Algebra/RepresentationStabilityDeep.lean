...
noncomputable def FIMorphism.id (X : FIObject) : FIMorphism where
  src := X
  tgt := X
  map := fun n => n

noncomputable def FIMorphism.comp (g f : FIMorphism) : FIMorphism where
  src := f.src
  tgt := g.tgt
...
noncomputable def CharacterPolynomial.shift (P : CharacterPolynomial) : CharacterPolynomial where
  degree := P.degree + 1
...
noncomputable def fi_object_refl (X : FIObject) : Path X X :=
  Path.refl X

noncomputable def fi_morphism_refl (f : FIMorphism) : Path f f :=
  Path.refl f

noncomputable def fi_id_refl (X : FIObject) : Path (FIMorphism.id X) (FIMorphism.id X) :=
  Path.refl _

noncomputable def fi_comp_refl (f g : FIMorphism) :
    Path (FIMorphism.comp g f) (FIMorphism.comp g f) :=
  Path.refl _

noncomputable def fi_comp_congr_left (f : FIMorphism) {g1 g2 : FIMorphism}
    (p : Path g1 g2) :
    Path (FIMorphism.comp g1 f) (FIMorphism.comp g2 f) :=
  Path.congrArg (fun g => FIMorphism.comp g f) p

noncomputable def fi_comp_congr_right (g : FIMorphism) {f1 f2 : FIMorphism}
    (p : Path f1 f2) :
    Path (FIMorphism.comp g f1) (FIMorphism.comp g f2) :=
  Path.congrArg (fun f => FIMorphism.comp g f) p

noncomputable def fi_comp_congr_both {f1 f2 g1 g2 : FIMorphism}
...
noncomputable def fi_src_congr {f g : FIMorphism} (p : Path f g) :
    Path f.src g.src :=
  Path.congrArg FIMorphism.src p

noncomputable def fi_tgt_congr {f g : FIMorphism} (p : Path f g) :
    Path f.tgt g.tgt :=
  Path.congrArg FIMorphism.tgt p

noncomputable def fi_size_congr {X Y : FIObject} (p : Path X Y) :
    Path X.size Y.size :=
  Path.congrArg FIObject.size p

noncomputable def fi_map_congr {f g : FIMorphism} (p : Path f g) (n : Nat) :
    Path (f.map n) (g.map n) :=
  Path.congrArg (fun h => h.map n) p

noncomputable def fi_comp_roundtrip {f g : FIMorphism}
    (p : Path (FIMorphism.comp g f) (FIMorphism.comp g f)) :
    Path (FIMorphism.comp g f) (FIMorphism.comp g f) :=
  Path.trans p (Path.symm p)

noncomputable def fi_comp_assoc_hyp (f g h : FIMorphism)
...
noncomputable def fi_comp_assoc_chain (f g h : FIMorphism)
...
noncomputable def fi_id_roundtrip (X : FIObject) :
    Path (FIMorphism.id X) (FIMorphism.id X) :=
  Path.trans (Path.refl _) (Path.refl _)

/-! ## FI-module action theorems -/

noncomputable def fimodule_refl (M : FIModule) : Path M M :=
  Path.refl M

noncomputable def fimodule_action_refl (M : FIModule) (f : FIMorphism) (x : M.carrier) :
    Path (M.action f x) (M.action f x) :=
  Path.refl _

noncomputable def fimodule_action_congr_morphism (M : FIModule) {f g : FIMorphism}
    (p : Path f g) (x : M.carrier) :
    Path (M.action f x) (M.action g x) :=
  Path.congrArg (fun h => M.action h x) p

noncomputable def fimodule_action_congr_element (M : FIModule) (f : FIMorphism)
    {x y : M.carrier} (p : Path x y) :
    Path (M.action f x) (M.action f y) :=
  Path.congrArg (M.action f) p

noncomputable def fimodule_action_trans (M : FIModule) (f : FIMorphism) {x y z : M.carrier}
    (p1 : Path (M.action f x) (M.action f y))
    (p2 : Path (M.action f y) (M.action f z)) :
    Path (M.action f x) (M.action f z) :=
  Path.trans p1 p2

noncomputable def fimodule_action_symm (M : FIModule) (f : FIMorphism) {x y : M.carrier}
    (p : Path (M.action f x) (M.action f y)) :
    Path (M.action f y) (M.action f x) :=
  Path.symm p

noncomputable def fimodule_action_roundtrip (M : FIModule) (f : FIMorphism) {x y : M.carrier}
    (p : Path (M.action f x) (M.action f y)) :
    Path (M.action f x) (M.action f x) :=
  Path.trans p (Path.symm p)

noncomputable def fimodule_action_three_step (M : FIModule) (f : FIMorphism)
    {x y z w : M.carrier}
    (p1 : Path (M.action f x) (M.action f y))
    (p2 : Path (M.action f y) (M.action f z))
...
noncomputable def fimodule_carrier_path (M : FIModule) :
    Path M.carrier M.carrier :=
  Path.refl _

noncomputable def fimodule_action_id_hyp (M : FIModule) (X : FIObject) (x : M.carrier)
    (p : Path (M.action (FIMorphism.id X) x) x) :
    Path (M.action (FIMorphism.id X) x) x :=
  p

noncomputable def fimodule_action_comp_hyp (M : FIModule) (f g : FIMorphism) (x : M.carrier)
    (p : Path (M.action (FIMorphism.comp g f) x) (M.action g (M.action f x))) :
    Path (M.action (FIMorphism.comp g f) x) (M.action g (M.action f x)) :=
  p

noncomputable def fimodule_action_comp_chain (M : FIModule) (f g : FIMorphism) (x : M.carrier)
    (p1 : Path (M.action (FIMorphism.comp g f) x) (M.action g (M.action f x)))
    (p2 : Path (M.action g (M.action f x)) (M.action g (M.action f x))) :
    Path (M.action (FIMorphism.comp g f) x) (M.action g (M.action f x)) :=
  Path.trans p1 p2

noncomputable def fimodule_action_congr_both (M : FIModule) {f g : FIMorphism}
...
noncomputable def fimodule_action_diamond (M : FIModule) (f : FIMorphism) {x y : M.carrier}
    (p : Path (M.action f x) (M.action f y))
    (q : Path (M.action f x) (M.action f y)) :
    Path (Path.trans p (Path.symm q)) (Path.trans p (Path.symm q)) :=
  Path.refl _

noncomputable def fimodule_action_self_trans (M : FIModule) (f : FIMorphism) (x : M.carrier) :
    Path (M.action f x) (M.action f x) :=
  Path.trans (Path.refl _) (Path.refl _)

/-! ## Consistent sequence and eventual behavior -/

noncomputable def consistent_term_refl (S : ConsistentSequence) (n : Nat) :
    Path (S.term n) (S.term n) :=
  Path.refl _

noncomputable def consistent_step_refl (S : ConsistentSequence) (n : Nat) (x : S.term n) :
    Path (S.step n x) (S.step n x) :=
  Path.refl _

noncomputable def consistent_step_congr (S : ConsistentSequence) (n : Nat)
    {x y : S.term n} (p : Path x y) :
    Path (S.step n x) (S.step n y) :=
  Path.congrArg (S.step n) p

noncomputable def consistent_step_trans (S : ConsistentSequence) (n : Nat)
    {x y z : S.term n}
...
noncomputable def consistent_step_symm (S : ConsistentSequence) (n : Nat)
    {x y : S.term n} (p : Path (S.step n x) (S.step n y)) :
    Path (S.step n y) (S.step n x) :=
  Path.symm p

noncomputable def consistent_step_roundtrip (S : ConsistentSequence) (n : Nat)
    {x y : S.term n} (p : Path (S.step n x) (S.step n y)) :
    Path (S.step n x) (S.step n x) :=
  Path.trans p (Path.symm p)

noncomputable def consistent_next_step_refl (S : ConsistentSequence) (n : Nat)
    (x : S.term (n + 1)) :
    Path (S.step (n + 1) x) (S.step (n + 1) x) :=
  Path.refl _

noncomputable def consistent_next_step_congr (S : ConsistentSequence) (n : Nat)
    {x y : S.term (n + 1)} (p : Path x y) :
    Path (S.step (n + 1) x) (S.step (n + 1) y) :=
  Path.congrArg (S.step (n + 1)) p

noncomputable def consistent_next_step_trans (S : ConsistentSequence) (n : Nat)
    {x y z : S.term (n + 1)}
...
noncomputable def eventual_surjective_roundtrip (S : ConsistentSequence) :
    eventual_surjective_zero S = eventual_surjective_zero S :=
  rfl

/-! ## Representation stability for symmetric groups -/

noncomputable def sym_rep_refl (R : SymRepSequence) : Path R R :=
  Path.refl R

noncomputable def sym_action_refl (R : SymRepSequence) (n : Nat) (s : Sym) (x : R.space) :
    Path (R.act n s x) (R.act n s x) :=
  Path.refl _

noncomputable def sym_action_congr_element (R : SymRepSequence) (n : Nat) (s : Sym)
    {x y : R.space} (p : Path x y) :
    Path (R.act n s x) (R.act n s y) :=
  Path.congrArg (R.act n s) p

noncomputable def sym_action_congr_sym (R : SymRepSequence) (n : Nat) (x : R.space)
    {s1 s2 : Sym} (p : Path s1 s2) :
    Path (R.act n s1 x) (R.act n s2 x) :=
  Path.congrArg (fun s => R.act n s x) p

noncomputable def sym_action_trans (R : SymRepSequence) (n : Nat) (s : Sym)
    {x y z : R.space}
...
noncomputable def sym_action_symm (R : SymRepSequence) (n : Nat) (s : Sym)
    {x y : R.space} (p : Path (R.act n s x) (R.act n s y)) :
    Path (R.act n s y) (R.act n s x) :=
  Path.symm p

noncomputable def sym_structure_refl (R : SymRepSequence) (n : Nat) (x : R.space) :
    Path (R.structureMap n x) (R.structureMap n x) :=
  Path.refl _

noncomputable def sym_structure_congr (R : SymRepSequence) (n : Nat)
    {x y : R.space} (p : Path x y) :
    Path (R.structureMap n x) (R.structureMap n y) :=
  Path.congrArg (R.structureMap n) p

noncomputable def sym_structure_trans (R : SymRepSequence) (n : Nat)
    {x y z : R.space}
...
noncomputable def sym_structure_roundtrip (R : SymRepSequence) (n : Nat)
...
noncomputable def cef_refl (F : CEFFramework) : Path F F :=
  Path.refl F

noncomputable def cef_generation_refl (F : CEFFramework) :
    Path F.generationDegree F.generationDegree :=
  Path.refl _

noncomputable def cef_relation_refl (F : CEFFramework) :
    Path F.relationDegree F.relationDegree :=
  Path.refl _

noncomputable def cef_generation_congr {F G : CEFFramework} (p : Path F G) :
    Path F.generationDegree G.generationDegree :=
  Path.congrArg CEFFramework.generationDegree p

noncomputable def cef_relation_congr {F G : CEFFramework} (p : Path F G) :
...
noncomputable def church_ellenberg_farb_chain (F : CEFFramework)
...
noncomputable def charpoly_refl (P : CharacterPolynomial) : Path P P :=
  Path.refl P

noncomputable def char_eval_refl (P : CharacterPolynomial) (n : Nat) :
    Path (P.eval n) (P.eval n) :=
  Path.refl _

noncomputable def char_eval_congr_index (P : CharacterPolynomial) {n m : Nat}
    (p : Path n m) :
    Path (P.eval n) (P.eval m) :=
  Path.congrArg P.eval p

noncomputable def char_eval_trans (P : CharacterPolynomial) {n m k : Nat}
    (p1 : Path (P.eval n) (P.eval m))
    (p2 : Path (P.eval m) (P.eval k)) :
    Path (P.eval n) (P.eval k) :=
  Path.trans p1 p2

noncomputable def char_eval_symm (P : CharacterPolynomial) {n m : Nat}
    (p : Path (P.eval n) (P.eval m)) :
    Path (P.eval m) (P.eval n) :=
  Path.symm p

noncomputable def char_degree_refl (P : CharacterPolynomial) :
    Path P.degree P.degree :=
  Path.refl _

noncomputable def char_degree_congr {P Q : CharacterPolynomial} (p : Path P Q) :
    Path P.degree Q.degree :=
  Path.congrArg CharacterPolynomial.degree p

noncomputable def char_shift_refl (P : CharacterPolynomial) :
    Path (CharacterPolynomial.shift P) (CharacterPolynomial.shift P) :=
  Path.refl _

noncomputable def char_shift_eval_refl (P : CharacterPolynomial) (n : Nat) :
    Path ((CharacterPolynomial.shift P).eval n) ((CharacterPolynomial.shift P).eval n) :=
  Path.refl _

noncomputable def char_eval_roundtrip (P : CharacterPolynomial) (n : Nat) :
    Path (P.eval n) (P.eval n) :=
  Path.trans (Path.refl _) (Path.refl _)

noncomputable def character_stability_hyp (P : CharacterPolynomial) (n : Nat)
    (p : Path (P.eval n) (P.eval n)) :
    Path (P.eval n) (P.eval n) :=
  p

noncomputable def character_stability_chain (P : CharacterPolynomial) (n : Nat)
...
noncomputable def central_refl (C : CentralStabilityDatum) : Path C C :=
  Path.refl C

noncomputable def central_radius_refl (C : CentralStabilityDatum) :
    Path C.radius C.radius :=
  Path.refl _

noncomputable def central_map_refl (C : CentralStabilityDatum) (n : Nat) :
    Path (C.centerMap n) (C.centerMap n) :=
  Path.refl _

noncomputable def central_map_congr (C : CentralStabilityDatum) {n m : Nat}
    (p : Path n m) :
    Path (C.centerMap n) (C.centerMap m) :=
  Path.congrArg C.centerMap p

noncomputable def central_map_trans (C : CentralStabilityDatum) {a b c : Nat}
    (p1 : Path (C.centerMap a) (C.centerMap b))
    (p2 : Path (C.centerMap b) (C.centerMap c)) :
    Path (C.centerMap a) (C.centerMap c) :=
  Path.trans p1 p2

noncomputable def central_map_symm (C : CentralStabilityDatum) {a b : Nat}
    (p : Path (C.centerMap a) (C.centerMap b)) :
    Path (C.centerMap b) (C.centerMap a) :=
  Path.symm p

noncomputable def central_map_roundtrip (C : CentralStabilityDatum) {a b : Nat}
    (p : Path (C.centerMap a) (C.centerMap b)) :
    Path (C.centerMap a) (C.centerMap a) :=
  Path.trans p (Path.symm p)

noncomputable def central_radius_congr {C D : CentralStabilityDatum} (p : Path C D) :
    Path C.radius D.radius :=
  Path.congrArg CentralStabilityDatum.radius p

noncomputable def central_stability_hyp (C : CentralStabilityDatum)
    (p : Path C.radius C.radius) :
    Path C.radius C.radius :=
  p

noncomputable def central_stability_chain (C : CentralStabilityDatum)
...
noncomputable def twisted_refl (T : TwistedStabilityDatum) : Path T T :=
  Path.refl T

noncomputable def twisted_boundary_refl (T : TwistedStabilityDatum) (n : Nat) (x : T.chain) :
    Path (T.boundary n x) (T.boundary n x) :=
  Path.refl _

noncomputable def twisted_boundary_congr_element (T : TwistedStabilityDatum) (n : Nat)
    {x y : T.chain} (p : Path x y) :
    Path (T.boundary n x) (T.boundary n y) :=
  Path.congrArg (T.boundary n) p

noncomputable def twisted_boundary_congr_index (T : TwistedStabilityDatum) (x : T.chain)
    {n m : Nat} (p : Path n m) :
    Path (T.boundary n x) (T.boundary m x) :=
  Path.congrArg (fun k => T.boundary k x) p

noncomputable def twisted_boundary_trans (T : TwistedStabilityDatum) (n : Nat)
    {x y z : T.chain}
...
noncomputable def twisted_boundary_symm (T : TwistedStabilityDatum) (n : Nat)
    {x y : T.chain} (p : Path (T.boundary n x) (T.boundary n y)) :
    Path (T.boundary n y) (T.boundary n x) :=
  Path.symm p

noncomputable def twisted_boundary_roundtrip (T : TwistedStabilityDatum) (n : Nat)
    {x y : T.chain} (p : Path (T.boundary n x) (T.boundary n y)) :
    Path (T.boundary n x) (T.boundary n x) :=
  Path.trans p (Path.symm p)

noncomputable def twisted_structure_refl (T : TwistedStabilityDatum) (n : Nat) (x : T.chain) :
    Path (T.structureMap n x) (T.structureMap n x) :=
  Path.refl _

noncomputable def twisted_structure_congr (T : TwistedStabilityDatum) (n : Nat)
    {x y : T.chain} (p : Path x y) :
    Path (T.structureMap n x) (T.structureMap n y) :=
  Path.congrArg (T.structureMap n) p

noncomputable def twisted_structure_trans (T : TwistedStabilityDatum) (n : Nat)
    {x y z : T.chain}
...
noncomputable def twisted_twist_refl (T : TwistedStabilityDatum) (n : Nat) :
    Path (T.twist n) (T.twist n) :=
  Path.refl _

noncomputable def twisted_twist_congr (T : TwistedStabilityDatum) {n m : Nat}
    (p : Path n m) :
...
noncomputable def stability_final_chain {A : Type u} {a b c d : A}
    (p1 : Path a b) (p2 : Path b c) (p3 : Path c d) :
    Path a d :=
  Path.trans (Path.trans p1 p2) p3

noncomputable def stability_final_diamond {A : Type u} {a b : A}
    (p q : Path a b) :
    Path (Path.trans p (Path.symm q)) (Path.trans p (Path.symm q)) :=
  Path.refl _

noncomputable def stability_final_symm_symm {A : Type u} {a b : A} (p : Path a b) :
    Path a b :=
  Path.symm (Path.symm p)

noncomputable def stability_final_congr {A : Type u} {B : Type v}
    (f : A → B) {x y : A} (p : Path x y) :
    Path (f x) (f y) :=
  Path.congrArg f p

noncomputable def stability_final_trans {A : Type u} {a b c : A}
