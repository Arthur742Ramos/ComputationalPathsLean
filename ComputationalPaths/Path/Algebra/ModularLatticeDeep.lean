...
noncomputable def meet_idem_coherence (lat : PathLattice L) (a : L) :
    Path (lat.meet a a) a :=
  lat.meet_idem a

-- 2: Join idempotency coherence
noncomputable def join_idem_coherence (lat : PathLattice L) (a : L) :
    Path (lat.join a a) a :=
  lat.join_idem a

-- 3: Double meet commutativity is a path loop
noncomputable def meet_comm_involutive (lat : PathLattice L) (a b : L) :
    Path (lat.meet a b) (lat.meet a b) :=
  Path.trans (lat.meet_comm a b) (lat.meet_comm b a)

-- 4: Double join commutativity is a path loop
noncomputable def join_comm_involutive (lat : PathLattice L) (a b : L) :
...
noncomputable def latLeq (lat : PathLattice L) (a b : L) : Prop :=
...
noncomputable def latLeqJoin (lat : PathLattice L) (a b : L) : Prop :=
...
noncomputable def meet_absorb_self (lat : PathLattice L) (a b : L) :
    Path (lat.meet a (lat.join a b)) a :=
  lat.meet_join_absorb a b
...
noncomputable def modular_apply (ml : ModularLattice L) (a b c : L)
...
noncomputable def modular_comm_meet (ml : ModularLattice L) (a b c : L)
    (h : Path (ml.meet c a) c) :
...
noncomputable def modular_self (ml : ModularLattice L) (a b : L) :
    Path (ml.meet a (ml.join b a)) (ml.join (ml.meet a b) a) :=
  ml.modular_law a b a (ml.meet_idem a)

-- 18: Modular law symmetric form
noncomputable def modular_symmetric (ml : ModularLattice L) (a b c : L)
    (h : Path (ml.meet c a) c) :
...
noncomputable def modular_self_absorption (ml : ModularLattice L) (a b : L) :
...
noncomputable def distrib_coherence (dl : DistributiveLattice L) (a b c : L) :
    Path (dl.meet a (dl.join b c)) (dl.join (dl.meet a b) (dl.meet a c)) :=
  dl.meet_join_distrib a b c

-- 21: Dual distributivity coherence
noncomputable def dual_distrib_coherence (dl : DistributiveLattice L) (a b c : L) :
...
noncomputable def distrib_comm (dl : DistributiveLattice L) (a b c : L) :
...
noncomputable def distributive_is_modular (dl : DistributiveLattice L) :
    ModularLattice L :=
...
noncomputable def distrib_symm (dl : DistributiveLattice L) (a b c : L) :
...
noncomputable def distrib_comm_input (dl : DistributiveLattice L) (a b c : L) :
...
noncomputable def bounded_absorption_bot (bl : BoundedLattice L) (a : L) :
...
noncomputable def compl_involutive_path (ba : BooleanAlgebra L) (a : L) :
...
noncomputable def boolean_compl_bot (ba : BooleanAlgebra L) (a : L) :
...
noncomputable def boolean_compl_symm_meet (ba : BooleanAlgebra L) (a : L) :
...
noncomputable def boolean_compl_symm_join (ba : BooleanAlgebra L) (a : L) :
...
noncomputable def boolean_compl_meet_join (ba : BooleanAlgebra L) (a : L) :
...
noncomputable def boolean_top_from_compl (ba : BooleanAlgebra L) (a : L) :
    Path (ba.join a (ba.complement a)) ba.top :=
  ba.compl_join a
...
noncomputable def id_lattice_hom (lat : PathLattice L) : LatticeHom lat lat where
  func := id
...
noncomputable def hom_preserves_meet_comm (lat1 lat2 : PathLattice L)
...
noncomputable def hom_compose (lat1 lat2 lat3 : PathLattice L)
...
noncomputable def hom_preserves_meet_idem (lat1 lat2 : PathLattice L)
...
noncomputable def hom_preserves_join_idem (lat1 lat2 : PathLattice L)
    (h : LatticeHom lat1 lat2) (a : L) :
...
noncomputable def hom_preserves_join_comm (lat1 lat2 : PathLattice L)
    (h : LatticeHom lat1 lat2) (a b : L) :
...
/- 55: Operation-level congruence data.  Each related pair carries an
   actual path between the pair together with local meet/join loop and
   commutativity witnesses. -/
structure OperationPairWitness (lat : PathLattice L) (a b : L) : Prop where
  pair_path : Nonempty (Path a b)
...
theorem of_path (lat : PathLattice L) {a b : L} (p : Path a b) :
    OperationPairWitness lat a b where
  pair_path := ⟨p⟩
  meet_loop := ⟨meet_comm_involutive lat a b⟩
  join_loop := ⟨join_comm_involutive lat a b⟩
  meet_swap := ⟨lat.meet_comm a b⟩
  join_swap := ⟨lat.join_comm a b⟩

theorem symm (lat : PathLattice L) {a b : L} (h : OperationPairWitness lat a b) :
    OperationPairWitness lat b a :=
  match h.pair_path with
  | ⟨p⟩ => of_path lat (Path.symm p)

theorem trans (lat : PathLattice L) {a b c : L}
    (h1 : OperationPairWitness lat a b) (h2 : OperationPairWitness lat b c) :
    OperationPairWitness lat a c :=
  match h1.pair_path, h2.pair_path with
  | ⟨p⟩, ⟨q⟩ => of_path lat (Path.trans p q)

theorem compat_meet (lat : PathLattice L) {a b c d : L}
    (h1 : OperationPairWitness lat a b) (h2 : OperationPairWitness lat c d) :
    OperationPairWitness lat (lat.meet a c) (lat.meet b d) :=
  match h1.pair_path, h2.pair_path with
  | ⟨p⟩, ⟨q⟩ =>
      of_path lat (Path.trans
        (Path.congrArg (fun x => lat.meet x c) p)
        (Path.congrArg (fun x => lat.meet b x) q))

theorem compat_join (lat : PathLattice L) {a b c d : L}
    (h1 : OperationPairWitness lat a b) (h2 : OperationPairWitness lat c d) :
    OperationPairWitness lat (lat.join a c) (lat.join b d) :=
  match h1.pair_path, h2.pair_path with
  | ⟨p⟩, ⟨q⟩ =>
      of_path lat (Path.trans
        (Path.congrArg (fun x => lat.join x c) p)
        (Path.congrArg (fun x => lat.join b x) q))
...
noncomputable def operation_witness_congruence (lat : PathLattice L) : LatticeCongruence lat where
  ...
  rel_refl := fun a => OperationPairWitness.of_path lat (Path.refl a)
  rel_symm := fun _ _ h => OperationPairWitness.symm lat h
  rel_trans := fun _ _ _ h1 h2 => OperationPairWitness.trans lat h1 h2
  compat_meet := fun _ _ _ _ h1 h2 => OperationPairWitness.compat_meet lat h1 h2
  compat_join := fun _ _ _ _ h1 h2 => OperationPairWitness.compat_join lat h1 h2
...
noncomputable def dedekind_phi_coherence (ml : ModularLattice L) (a b : L)
    (dt : DedekindTransposition ml a b) (x : L) :
    Path (dt.phi x) (ml.join a x) :=
  dt.phi_def x

-- 62: Dedekind transposition psi coherence
noncomputable def dedekind_psi_coherence (ml : ModularLattice L) (a b : L)
    (dt : DedekindTransposition ml a b) (x : L) :
    Path (dt.psi x) (ml.meet b x) :=
  dt.psi_def x

-- 63: Round-trip path
noncomputable def dedekind_roundtrip (ml : ModularLattice L) (a b : L)
    (dt : DedekindTransposition ml a b) (x : L) :
    Path (dt.phi (dt.psi x)) (ml.join a (ml.meet b x)) :=
  dt.phi_psi x

-- 64: Reverse round-trip path
noncomputable def dedekind_roundtrip_rev (ml : ModularLattice L) (a b : L)
    (dt : DedekindTransposition ml a b) (x : L) :
...
noncomputable def trivial_chain (lat : PathLattice L) (a : L) :
    LatticeChain lat 0 where
  elements := fun _ => a
  ordered := fun i => Fin.elim0 i

-- 66: Chain length extraction
noncomputable def chain_length (_lat : PathLattice L) (n : Nat)
...
noncomputable def double_absorption (lat : PathLattice L) (a b : L) :
...
noncomputable def triple_meet_assoc (lat : PathLattice L) (a b c d : L) :
...
noncomputable def triple_join_assoc (lat : PathLattice L) (a b c d : L) :
...
noncomputable def meet_join_meet_chain (lat : PathLattice L) (a b : L) :
    Path (lat.meet a (lat.join a (lat.meet a b))) a :=
  Path.trans
    (Path.congrArg (lat.meet a ·) (lat.join_meet_absorb a b))
    (lat.meet_idem a)
...
noncomputable def join_meet_join_chain (lat : PathLattice L) (a b : L) :
...
noncomputable def meet_comm_assoc (lat : PathLattice L) (a b c : L) :
...
noncomputable def join_comm_assoc (lat : PathLattice L) (a b c : L) :
...
noncomputable def bounded_double_bot (bl : BoundedLattice L) (a : L) :
...
noncomputable def meet_comm_symm (lat : PathLattice L) (a b : L) :
    Path (lat.meet b a) (lat.meet a b) :=
  lat.meet_comm b a

-- 83: Symm of join commutativity
noncomputable def join_comm_symm (lat : PathLattice L) (a b : L) :
    Path (lat.join b a) (lat.join a b) :=
  lat.join_comm b a

-- 84: Meet assoc inverse
noncomputable def meet_assoc_inv (lat : PathLattice L) (a b c : L) :
    Path (lat.meet a (lat.meet b c)) (lat.meet (lat.meet a b) c) :=
  Path.symm (lat.meet_assoc a b c)

-- 85: Join assoc inverse
noncomputable def join_assoc_inv (lat : PathLattice L) (a b c : L) :
