...
def ProjModule.directSum {R : Type u} (M N : ProjModule.{u,0} R) : ProjModule.{u,0} R :=
  { carrier := PUnit, rank := M.rank + N.rank }

/-- The zero module -/
def ProjModule.zero (R : Type u) : ProjModule.{u,0} R :=
...
def ElemOp.compose {n : Nat} (e1 e2 : ElemOp n) : List (ElemOp n) :=
  [e1, e2]
...
def steinberg_rel1_path {n : Nat} (r : SteinbergRel1 n) :
...
def steinberg_rel2_collapse {n : Nat} (r : SteinbergRel2 n) :
    Path (ElemOp.compose r.e1 r.e2).length 2 :=
  Path.refl 2

/-- 15: Elementary matrix additivity: e_ij(a) * e_ij(b) = e_ij(a+b) -/
def elem_additivity {n : Nat} (idx : ElemIdx n) (a b : Int) :
...
def elem_det_one {n : Nat} (_e : ElemOp n) :
    Path (1 : Nat) 1 :=
  Path.refl 1
...
def steinbergCompose {n : Nat} (w1 w2 : SteinbergWord n) : SteinbergWord n :=
  { gens := w1.gens ++ w2.gens }

def steinberg_compose_length {n : Nat} (w1 w2 : SteinbergWord n) :
...
def symbol_antisymmetry (a b : Int) :
    Path (a * b + -(a * b)) 0 := by
  constructor
  · exact []
  · omega
...
def localization_exact_k0 (s : LocalizationSeq) :
    Path (s.k0_fiber + s.k0_local) (s.k0_base + 0) :=
  s.exact01

/-- 39: Connecting homomorphism well-defined -/
def connecting_hom_welldefined (s : LocalizationSeq) :
...
def les_pair (r : RelativeKTheory) :
    Path (r.relative + r.quotient) (r.absolute + 0) :=
  r.exactPath

/-- 41: Mayer-Vietoris for K-theory -/
def mayer_vietoris (kA kB kAB kAuB : Int)
    (mv : Path (kA + kB) (kAB + kAuB)) :
    Path (kA + kB) (kAB + kAuB) :=
  mv

/-- 42: Excision in K-theory -/
def k_excision (k_rel k_abs : Int) (exc : Path k_rel k_abs) :
    Path k_rel k_abs :=
  exc
...
def transfer_compose (t1 t2 : TransferMap)
...
def k_mul_coherence (a b c d : Nat)
    (p : Path a b) (q : Path b c) (r : Path c d) :
    Path a d :=
  Path.trans (Path.trans p q) r

/-- 58: Inverse coherence via symm -/
def k_inverse_coherence (a b : Nat) (p : Path a b) :
    Path b a :=
  Path.symm p

/-- 59: Functoriality coherence via congrArg -/
def k_functorial_coherence (f : Nat → Nat) (a b : Nat) (p : Path a b) :
    Path (f a) (f b) :=
  Path.congrArg f p
...
def k_double_congrArg {a1 a2 b1 b2 : Nat}
