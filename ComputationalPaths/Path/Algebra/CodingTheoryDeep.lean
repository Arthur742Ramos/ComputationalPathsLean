...
noncomputable def Codeword (A : Type u) (n : Nat) := Fin n → A

/-- Hamming distance: counts positions where two codewords differ -/
noncomputable def hammingDistAux [DecidableEq A] (w1 w2 : Codeword A n) : Fin n → Nat :=
...
noncomputable def encodeDecode_path {A : Type u} {n k : Nat}
...
noncomputable def doubleRoundtrip_path {A : Type u} {n k : Nat}
    (code : LinearCode A n k) (w : Codeword A k)
...
noncomputable def encode_congrArg {A : Type u} {n k : Nat}
...
noncomputable def decode_congrArg {A : Type u} {n k : Nat}
...
noncomputable def hammingDistAux_self [DecidableEq A] (w : Codeword A n) (i : Fin n)
...
noncomputable def hammingDistAux_symm [DecidableEq A] (w1 w2 : Codeword A n) (i : Fin n)
...
noncomputable def hammingDistAux_self_symm [DecidableEq A] (w : Codeword A n) (i : Fin n)
    : Path 0 (hammingDistAux w w i) :=
  Path.symm (hammingDistAux_self w i)

/-- Theorem 10: Hamming distance auxiliary composites via trans -/
noncomputable def hammingDistAux_trans [DecidableEq A] (w1 w2 : Codeword A n) (i : Fin n)
...
/-- Syndrome type -/
noncomputable def Syndrome (A : Type u) (r : Nat) := Fin r → A
...
noncomputable def validCodeword_path {A : Type u} {n k : Nat}
...
noncomputable def syndrome_correction_path {A : Type u} {n k : Nat}
    (sd : SyndromeDecoder A n k) (c : Codeword A n)
    (h : sd.computeSyndrome c = sd.syndromeZero)
    : Path (sd.correctError (sd.computeSyndrome c) c) c :=
...
noncomputable def syndrome_congrArg {A : Type u} {n k : Nat}
...
noncomputable def hammingParams (r : Nat) : CodeParams where
  n := 2^r - 1
  k := 2^r - 1 - r
  d := 3
  q := 2
...
noncomputable def hamming_min_dist (r : Nat) : Path (hammingParams r).d 3 :=
  Path.refl 3

/-- Theorem 20: Hamming code alphabet size is 2 -/
noncomputable def hamming_alphabet (r : Nat) : Path (hammingParams r).q 2 :=
  Path.refl 2

/-- Theorem 21: Hamming params n accessor -/
noncomputable def hamming_n (r : Nat) : Path (hammingParams r).n (2^r - 1) :=
  Path.refl (2^r - 1)

/-- Theorem 22: Hamming params k accessor -/
noncomputable def hamming_k (r : Nat) : Path (hammingParams r).k (2^r - 1 - r) :=
  Path.refl (2^r - 1 - r)
...
noncomputable def reedSolomonParams (n k q : Nat) : CodeParams where
  n := n
  k := k
  d := n - k + 1
  q := q
...
noncomputable def primal_dual_symm {A : Type u} {n k : Nat}
...
noncomputable def WeightEnumerator (n : Nat) := Fin (n + 1) → Nat

/-- Theorem 31: Weight enumerator reflexivity -/
noncomputable def weight_enum_refl {n : Nat} (we : WeightEnumerator n)
    : Path we we :=
  Path.refl we
...
noncomputable def macwilliams_roundtrip {n : Nat} (mt : MacWilliamsTransform n)
...
noncomputable def macwilliams_double {n : Nat} (mt : MacWilliamsTransform n)
    (we : WeightEnumerator n)
...
noncomputable def macwilliams_symm {n : Nat} (mt : MacWilliamsTransform n)
    (we : WeightEnumerator n)
...
noncomputable def detectsErrors (d : Nat) (t : Nat) : Prop := t ≤ d - 1

/-- Error correction capability -/
noncomputable def correctsErrors (d : Nat) (t : Nat) : Prop := 2 * t + 1 ≤ d
...
noncomputable def cyclicShift {A : Type u} {n : Nat} (w : Codeword A (n + 1)) : Codeword A (n + 1) :=
...
noncomputable def cyclic_membership_path {A : Type u} {n : Nat}
...
noncomputable def cyclic_shift_congrArg {A : Type u} {n : Nat}
...
noncomputable def cyclic_shift_identity {A : Type u} {n : Nat}
    (w : Codeword A (n + 1))
    : Path w w :=
  Path.refl w
...
noncomputable def concat_roundtrip_path {A : Type u} {n1 n2 k1 k2 : Nat}
...
noncomputable def concat_encode_congrArg {A : Type u} {n1 n2 k1 k2 : Nat}
...
noncomputable def concat_distance_bound (d1 d2 d_concat : Nat) (h : d_concat = d1 * d2)
    : Path d_concat (d1 * d2) :=
  Path.mk [Step.mk _ _ h] h

/-- Theorem 48: Concatenated code double roundtrip -/
noncomputable def concat_double_roundtrip {A : Type u} {n1 n2 k1 k2 : Nat}
    (cc : ConcatenatedCode A n1 n2 k1 k2) (w : Codeword A k1)
...
noncomputable def morphism_commutes_path {A : Type u} {n1 k1 n2 k2 : Nat}
...
noncomputable def morphism_encode_path {A : Type u} {n1 k1 n2 k2 : Nat}
...
noncomputable def morphism_roundtrip {A : Type u} {n1 k1 n2 k2 : Nat}
    (cm : CodeMorphism A n1 k1 n2 k2) (w : Codeword A k1)
...
noncomputable def morphism_pipeline {A : Type u} {n1 k1 n2 k2 : Nat}
    (cm : CodeMorphism A n1 k1 n2 k2) (w : Codeword A k1)
...
noncomputable def gilbertVarshamovBound (p : CodeParams) : Prop :=
  p.q ≥ 1 → p.k ≤ p.n
...
noncomputable def triple_encode_chain {A : Type u} {n1 k1 : Nat}
...
noncomputable def decode_chain {A : Type u} {n k : Nat}
    (code : LinearCode A n k)
...
noncomputable def encode_decode_naturality {A : Type u} {n k : Nat}
    (code : LinearCode A n k)
...
noncomputable def encode_decode_naturality_symm {A : Type u} {n k : Nat}
    (code : LinearCode A n k)
    (w1 w2 : Codeword A k) (p : Path w1 w2)
...
noncomputable def code_path_iso {A : Type u} {n k : Nat}
    (code : LinearCode A n k) (w : Codeword A k)
    : Path (code.decode (code.encode w)) w :=
  encodeDecode_path code w

/-- Theorem 61: Function application preserves path -/
noncomputable def func_preserves_path {A : Type u} {B : Type u} {n : Nat}
    (f : Codeword A n → B)
    (w1 w2 : Codeword A n)
    (p : Path w1 w2)
    : Path (f w1) (f w2) :=
...
noncomputable def code_map_compose {A : Type u} {n m l : Nat}
...
noncomputable def product_min_dist (d1 d2 dprod : Nat) (h : dprod = d1 * d2)
    : Path dprod (d1 * d2) :=
  Path.mk [Step.mk _ _ h] h

/-- Theorem 64: Product code dimension -/
noncomputable def product_dimension (k1 k2 kprod : Nat) (h : kprod = k1 * k2)
    : Path kprod (k1 * k2) :=
  Path.mk [Step.mk _ _ h] h

/-- Theorem 65: Product code symmetry (row-col vs col-row) -/
noncomputable def product_symmetry (d1 d2 : Nat)
    : Path (d1 * d2) (d2 * d1) :=
...
noncomputable def puncture {A : Type u} {n : Nat} (w : Codeword A (n + 1)) (pos : Fin (n + 1))
    : Codeword A n :=
  fun i =>
...
noncomputable def puncture_congrArg {A : Type u} {n : Nat}
...
noncomputable def punctured_distance (d dpunct : Nat) (h : dpunct = d - 1)
    : Path dpunct (d - 1) :=
  Path.mk [Step.mk _ _ h] h
...
noncomputable def trivial_roundtrip {A : Type u} {n : Nat} (w : Codeword A n)
...
noncomputable def encode_injective_path {A : Type u} {n k : Nat}
    (code : LinearCode A n k)
    (w1 w2 : Codeword A k)
...
noncomputable def decode_encode_injective {A : Type u} {n k : Nat}
    (code : LinearCode A n k)
    (w1 w2 : Codeword A k)
    (p : Path (code.encode w1) (code.encode w2))
    : Path w1 w2 :=
...
noncomputable def grand_pipeline {A : Type u} {n k : Nat}
    (code : LinearCode A n k)
...
noncomputable def grand_decode_pipeline {A : Type u} {n k : Nat}
    (code : LinearCode A n k)
    (sd : SyndromeDecoder A n k)
    (w : Codeword A k)
    (hsyn : sd.computeSyndrome (code.encode w) = sd.syndromeZero)
