...
/-- The abelianization relation: x ~ y iff they differ by commutators.

We define this as the equivalence relation generated by ab ~ ba. -/
inductive AbelianizationRel (G : Type u) (mul : G → G → G) : G → G → Prop where
  | refl : ∀ (x : G), AbelianizationRel G mul x x
  | symm : ∀ {x y : G}, AbelianizationRel G mul x y → AbelianizationRel G mul y x
  | trans : ∀ {x y z : G}, AbelianizationRel G mul x y → AbelianizationRel G mul y z →
      AbelianizationRel G mul x z
  | comm : ∀ (a b : G), AbelianizationRel G mul (mul a b) (mul b a)
  | congr_left : ∀ {x y : G} (z : G), AbelianizationRel G mul x y →
      AbelianizationRel G mul (mul z x) (mul z y)
  | congr_right : ∀ {x y : G} (z : G), AbelianizationRel G mul x y →
      AbelianizationRel G mul (mul x z) (mul y z)

/-- The abelianization relation is an equivalence. -/
theorem abelianizationRel_equiv (G : Type u) (mul : G → G → G) :
    Equivalence (AbelianizationRel G mul) where
  refl := AbelianizationRel.refl
  symm := AbelianizationRel.symm
  trans := AbelianizationRel.trans

/-- Setoid for abelianization. -/
def abelianizationSetoid (G : Type u) (mul : G → G → G) : Setoid G where
  r := AbelianizationRel G mul
  iseqv := abelianizationRel_equiv G mul
...
def Abelianization (G : Type u) (mul : G → G → G) (_inv : G → G) (_e : G) : Type u :=
  Quot (AbelianizationRel G mul)

/-- The quotient map G → G^ab. -/
def abelianization_mk {G : Type u} (mul : G → G → G) (inv : G → G) (e : G) :
    G → Abelianization G mul inv e :=
  Quot.mk (AbelianizationRel G mul)

/-- Commutators become trivial in the abelianization.

[a, b] = aba⁻¹b⁻¹ ~ e because:
  aba⁻¹b⁻¹ ~ baa⁻¹b⁻¹ = bb⁻¹ = e (using ab ~ ba)

This requires the full group laws to prove, so we axiomatize the
AbelianizationRel instance for commutators. -/
axiom abelianizationRel_commutator {G : Type u} (mul : G → G → G) (inv : G → G) (e : G)
    (a b : G) : AbelianizationRel G mul (commutator mul inv a b) e
