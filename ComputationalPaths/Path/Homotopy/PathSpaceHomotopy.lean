...
noncomputable def pathSpaceAllPaths (A : Type u) (a : A)
...
noncomputable def pathSpaceEval_base_path (A : Type u) (a : A) :
...
noncomputable def basedPathSpaceDeformationRetract (A : Type u) (a : A) :
...
noncomputable def FreePathSpace (A : Type u) : Type u :=
  Σ (x y : A), Path x y

/-- Source projection from the free path space. -/
noncomputable def freePathSpaceSource {A : Type u} : FreePathSpace A → A :=
  fun ⟨x, _, _⟩ => x

/-- Target projection from the free path space. -/
noncomputable def freePathSpaceTarget {A : Type u} : FreePathSpace A → A :=
  fun ⟨_, y, _⟩ => y

/-- The free path space basepoint at a : the constant path at a. -/
noncomputable def freePathSpaceRefl {A : Type u} (a : A) : FreePathSpace A :=
  ⟨a, a, Path.refl a⟩

/-- The constant section: sends a to (a, a, refl a). -/
noncomputable def freePathSpaceConstSection {A : Type u} : A → FreePathSpace A :=
...
noncomputable def freePathSpace_source_section_path (a : A) :
    Path (freePathSpaceSource (freePathSpaceConstSection a)) a :=
  Path.refl a

/-- Path witness for target-section roundtrip. -/
noncomputable def freePathSpace_target_section_path (a : A) :
...
noncomputable def freePathSpaceReverse {A : Type u} : FreePathSpace A → FreePathSpace A :=
...
noncomputable def freePathSpace_reverse_source_path {A : Type u} (p : FreePathSpace A) :
    Path
      (freePathSpaceSource (freePathSpaceReverse (freePathSpaceReverse p)))
...
noncomputable def basedPathSpace_as_fiber (A : Type u) (a : A) :
...
noncomputable def basedPathSpace_fiber_source (A : Type u) (a : A) :
    Path
...
noncomputable def pathSpaceConcat_base_left {A : Type u} {a : A}
    (p : PathSpace A a) :
...
noncomputable def pathSpaceUnit {A : Type u} (a : A) : PathSpace A a :=
  pathSpaceBase A a

/-- The multiplication (join) of the path space monad: flatten nested
    path spaces. -/
noncomputable def pathSpaceJoin {A : Type u} {a : A}
...
noncomputable def pathSpace_eval_coherence {A : Type u} {a : A}
    (x : PathSpace A a) :
...
/-- Path witness: the deformation retract is compatible with
    the evaluation map. -/
noncomputable def deformationRetract_eval_coherence {A : Type u} {a : A}
    (_x : PathSpace A a) :
    Path
...
noncomputable def constSection_source_coherence {A : Type u} (a : A) :
    Path (freePathSpaceSource (freePathSpaceConstSection a)) a :=
  Path.refl a

/-- Path witness: the free path space constant section is
    compatible with target projection. -/
noncomputable def constSection_target_coherence {A : Type u} (a : A) :
    Path (freePathSpaceTarget (freePathSpaceConstSection a)) a :=
  Path.refl a
