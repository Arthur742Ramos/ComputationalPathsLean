...
theorem canonicalSeq_exact {P : B → Type u} (b : B) (x₀ : P b) :
...
theorem fiber_incl_proj {P : B → Type u} (b : B) (x₀ : P b) (f : P b) :
    (canonicalFiberSeq b x₀).proj ((canonicalFiberSeq b x₀).incl f) = b :=
