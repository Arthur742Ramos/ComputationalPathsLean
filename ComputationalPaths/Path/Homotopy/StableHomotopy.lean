...
/-- Certificate for a sampled stabilized suspension-loop adjunction round trip. -/
structure StableAdjunctionCertificate (n : Nat) (X Y : Pointed) where
  /-- Sampled suspended pointed map. -/
  suspendedMap : PointedMap (iteratedSigmaPointed n X) Y
  /-- Its adjunct. -/
  adjunctMap : PointedMap X (iteratedOmegaEqPointed n Y)
  /-- Decoding the adjunct back to a suspended map. -/
  decodedMap : PointedMap (iteratedSigmaPointed n X) Y
  /-- The recorded adjunct is the actual forward adjunction image. -/
  adjunctPath : Path adjunctMap ((stableAdjunction n X Y).toFun suspendedMap)
  /-- The recorded decoding is the actual inverse adjunction image. -/
  decodedPath : Path decodedMap ((stableAdjunction n X Y).invFun adjunctMap)
  /-- Left round-trip witness at the sampled map. -/
  leftRoundTrip : Path decodedMap suspendedMap
  /-- Path-algebra cancellation for the left round-trip witness. -/
  leftCancel :
    RwEq (Path.trans leftRoundTrip (Path.symm leftRoundTrip)) (Path.refl decodedMap)

/-- Build a stabilized adjunction certificate for one pointed map. -/
noncomputable def stableAdjunction_certificate (n : Nat) (X Y : Pointed)
    (f : PointedMap (iteratedSigmaPointed n X) Y) :
    StableAdjunctionCertificate n X Y where
  suspendedMap := f
  adjunctMap := (stableAdjunction n X Y).toFun f
  decodedMap := (stableAdjunction n X Y).invFun ((stableAdjunction n X Y).toFun f)
  adjunctPath := Path.refl _
  decodedPath := Path.refl _
  leftRoundTrip := stableAdjunction_left_inverse n X Y f
  leftCancel := rweq_cmpA_inv_right (stableAdjunction_left_inverse n X Y f)
...
/-- Certificate for a named stable-stem torsion calculation. -/
structure StableStemTorsionCertificate where
  /-- Stable stem index. -/
  stemIndex : Nat
  /-- Claimed order/multiple. -/
  order : Nat
  /-- Carrier in which the torsion calculation is checked. -/
  carrier : Type
  /-- Named element of the carrier. -/
  generator : carrier
  /-- Zero element of the carrier. -/
  zeroElement : carrier
  /-- The computed multiple of the named element. -/
  multiple : carrier
  /-- Computational path witnessing the torsion relation. -/
  torsionPath : Path multiple zeroElement

/-- Torsion certificate for eta in the first stable stem. -/
noncomputable def etaTorsionCertificate : StableStemTorsionCertificate where
  stemIndex := 1
  order := 2
  carrier := StablePi 1
  generator := StableStems.eta
  zeroElement := (0 : StableStems.StableStem1)
  multiple := (2 : Nat) • StableStems.eta
  torsionPath := stablePi_one_two_torsion

/-- Torsion certificate for eta squared in the second stable stem. -/
noncomputable def etaSquaredTorsionCertificate : StableStemTorsionCertificate where
  stemIndex := 2
  order := 2
  carrier := StablePi 2
  generator := StableStems.etaSquared
  zeroElement := (0 : StableStems.StableStem2)
  multiple := (2 : Nat) • StableStems.etaSquared
  torsionPath := stablePi_two_two_torsion

/-- Torsion certificate for nu in the third stable stem. -/
noncomputable def nuTorsionCertificate : StableStemTorsionCertificate where
  stemIndex := 3
  order := 24
  carrier := StablePi 3
  generator := StableStems.nu
  zeroElement := (0 : StableStems.StableStem3)
  multiple := (24 : Nat) • StableStems.nu
  torsionPath := stablePi_three_twentyfour_torsion

/-- Torsion certificate for sigma in the seventh stable stem. -/
noncomputable def sigmaTorsionCertificate : StableStemTorsionCertificate where
  stemIndex := 7
  order := 240
  carrier := StablePi 7
  generator := StableStems.sigma
  zeroElement := (0 : StableStems.StableStem7)
  multiple := (240 : Nat) • StableStems.sigma
  torsionPath := stablePi_seven_twohundredforty_torsion
...
/-- Concrete coordinate certificate for an element of the eighth stable stem. -/
structure StablePiEightCertificate where
  /-- The sampled element. -/
  element : StablePi 8
  /-- First Z₂ coordinate. -/
  first : StableStems.Z2
  /-- Second Z₂ coordinate. -/
  second : StableStems.Z2
  /-- Computational path from the element to its recorded coordinates. -/
  coordinatePath : Path element (first, second)

/-- Build the coordinate certificate for `π₈^s`. -/
noncomputable def stablePi_eight_certificate (x : StablePi 8) :
    StablePiEightCertificate :=
  match stablePi_eight_generators x with
  | ⟨a, b, p⟩ =>
      { element := x, first := a, second := b, coordinatePath := p }
...
/-- Concrete coordinate certificate for an element of the ninth stable stem. -/
structure StablePiNineCertificate where
  /-- The sampled element. -/
  element : StablePi 9
  /-- First Z₂ coordinate. -/
  first : StableStems.Z2
  ...
  second : StableStems.Z2
  /-- Third Z₂ coordinate. -/
  third : StableStems.Z2
  /-- Computational path from the element to its recorded coordinates. -/
  coordinatePath : Path element (first, second, third)

/-- Build the coordinate certificate for `π₉^s`. -/
noncomputable def stablePi_nine_certificate (x : StablePi 9) :
    StablePiNineCertificate :=
  match stablePi_nine_generators x with
  | ⟨a, b, c, p⟩ =>
      { element := x, first := a, second := b, third := c, coordinatePath := p }
