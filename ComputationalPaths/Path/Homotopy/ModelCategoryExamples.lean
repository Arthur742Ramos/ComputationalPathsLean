...
noncomputable def dMap_refl_path {C : Nat → Type u} (cx : ChainComplex C)
    {n : Nat} (x : C (n + 1)) :
    Path (dMap cx (Path.refl x)) (Path.refl (cx.d x)) :=
  Path.stepChain (dMap_refl cx x)
...
noncomputable def dMap_trans_path {C : Nat → Type u} (cx : ChainComplex C)
    {n : Nat} {x y z : C (n + 1)} (p : Path x y) (q : Path y z) :
    Path (dMap cx (Path.trans p q)) (Path.trans (dMap cx p) (dMap cx q)) :=
  Path.stepChain (dMap_trans cx p q)
...
noncomputable def chainMapPath_symm_path {C D : Nat → Type u}
    {cC : ChainComplex C} {cD : ChainComplex D}
    (φ : ChainMap cC cD) {n : Nat} {x y : C n} (p : Path x y) :
    Path (chainMapPath φ (Path.symm p)) (Path.symm (chainMapPath φ p)) :=
  Path.stepChain (chainMapPath_symm φ p)
...
noncomputable def roundTripRL_refl_path {A B : Type u} (Q : QuillenAdjData A B) (a : A) :
    Path (roundTripRL Q (Path.refl a)) (Path.refl (Q.R (Q.L a))) :=
  Path.stepChain (roundTripRL_refl Q a)
...
noncomputable def roundTripRL_trans_path {A B : Type u} (Q : QuillenAdjData A B)
    {a₁ a₂ a₃ : A} (p : Path a₁ a₂) (q : Path a₂ a₃) :
    Path (roundTripRL Q (Path.trans p q))
      (Path.trans (roundTripRL Q p) (roundTripRL Q q)) :=
  Path.stepChain (roundTripRL_trans Q p q)
...
noncomputable def cylI₀Path_trans_path {A : Type u} (C : CylinderObj A)
    {a₁ a₂ a₃ : A} (p : Path a₁ a₂) (q : Path a₂ a₃) :
    Path (cylI₀Path C (Path.trans p q))
      (Path.trans (cylI₀Path C p) (cylI₀Path C q)) :=
  Path.stepChain (cylI₀Path_trans C p q)
...
noncomputable def wfsComposite_refl_path {A B : Type u} (w : WFS A B) (a : A) :
    Path (wfsComposite w (Path.refl a)) (Path.refl (w.rightMap (w.leftMap a))) :=
  Path.stepChain (wfsComposite_refl w a)
...
noncomputable def wfsComposite_trans_path {A B : Type u} (w : WFS A B)
    {a₁ a₂ a₃ : A} (p : Path a₁ a₂) (q : Path a₂ a₃) :
    Path (wfsComposite w (Path.trans p q))
      (Path.trans (wfsComposite w p) (wfsComposite w q)) :=
  Path.stepChain (wfsComposite_trans w p q)
...
noncomputable def leftDerivedPath_refl_path {A B : Type u} (D : LeftDerived A B) (a : A) :
    Path (leftDerivedPath D (Path.refl a)) (Path.refl (D.F a)) :=
  Path.stepChain (leftDerivedPath_refl D a)
