...
theorem pointedMap_subsingleton_of_subsingleton {X Y : Pointed}
  ...
  apply funext
  intro x
  have hx : x = X.pt := proof_irrel _ _
  calc
...
noncomputable def unit_naturality {X Y : Pointed} (f : PointedMap X Y) :
  ...
  apply proof_irrel _ (instSubsingleton_loopSpaceEq _)
...
theorem triangle_sigma (X : Pointed) :
  ...
  apply proof_irrel _ (instSubsingleton_pointedMap_sigma _ _)
...
theorem triangle_omega (Y : Pointed) :
  ...
  apply proof_irrel _ (instSubsingleton_pointedMap_omegaEq _ _)
