...
def natPathToEq {m n : Nat} (p : Path m n) : m = n :=
  p.toEq
...
/-- Path from 0 to 0. -/
def zero_path : Path (0 : Nat) 0 := Path.refl 0

/-- Path from succ n to succ n. -/
def succ_refl (n : Nat) : Path (Nat.succ n) (Nat.succ n) :=
  Path.refl (Nat.succ n)

/-- congrArg Nat.succ on a path. -/
def succ_congrArg {m n : Nat} (p : Path m n) :
...
/-- Path witness: 2 + 3 = 5. -/
def two_plus_three : Path (2 + 3) 5 := Path.stepChain rfl

/-- Path witness: 3 * 4 = 12. -/
def three_times_four : Path (3 * 4) 12 := Path.stepChain rfl

/-- Path witness: 0 + 0 = 0. -/
def zero_plus_zero : Path (0 + 0) 0 := Path.stepChain rfl

/-- Path witness: 1 * 1 = 1. -/
def one_times_one : Path (1 * 1) 1 := Path.stepChain rfl
...
def nat_transport {P : Nat → Type u} {m n : Nat}
...
def nat_transport_add_zero {P : Nat → Type u} (n : Nat) (x : P (n + 0)) :
    P n :=
  nat_transport (add_zero_path n) x

/-- Transport along zero_add_path. -/
def nat_transport_zero_add {P : Nat → Type u} (n : Nat) (x : P (0 + n)) :
    P n :=
...
def induction_base {P : Nat → Type u} (p0 : P 0) :
    P 0 := p0

/-- Path witness for the inductive step: P n → P (succ n). -/
def induction_step {P : Nat → Type u}
