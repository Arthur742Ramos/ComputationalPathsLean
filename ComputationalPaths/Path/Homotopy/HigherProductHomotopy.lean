  ...
  right_inv := by
    ...
    | mk y₁ y₂ =>
      ...
      | up x₁ =>
        ...
        | up x₂ =>
          change
            (ULift.up (((prodPiOneEquiv a b).toFun ((prodPiOneEquiv a b).invFun (x₁, x₂))).1),
             ULift.up (((prodPiOneEquiv a b).toFun ((prodPiOneEquiv a b).invFun (x₁, x₂))).2)) =
            (ULift.up x₁, ULift.up x₂)
          rw [(prodPiOneEquiv a b).right_inv (x₁, x₂)]
...
noncomputable def higherPiN_inv {X : Type u} (x : X) (n : Nat)
  ...
  | 1 => fun ⟨p⟩ => ULift.up (LoopQuot.inv (A := X) (a := x) p)
  | 2 => PiTwo.inv (A := X) (a := x)
...
noncomputable def homotopyHigherProductHomotopyAssoc (a b c : Nat) : Path ((a + b) + c) (a + (b + c)) :=
  Path.ofEq (Nat.add_assoc a b c)

/-- Commutativity rewrite `a + b ⤳ b + a`: one genuine step. -/
noncomputable def homotopyHigherProductHomotopyComm (a b : Nat) : Path (a + b) (b + a) :=
  Path.ofEq (Nat.add_comm a b)

/-- Inner commutativity `a + (b + c) ⤳ a + (c + b)` via congruence in the
    right argument (`_root_.congrArg`, since `congrArg` here is `Path.congrArg`). -/
noncomputable def homotopyHigherProductHomotopyInner (a b c : Nat) : Path (a + (b + c)) (a + (c + b)) :=
  Path.ofEq (_root_.congrArg (fun t => a + t) (Nat.add_comm b c))

/-- A genuine **two-step** path: reassociate, then commute the inner pair.
    Its trace has length two — this is not a reflexive path. -/
noncomputable def homotopyHigherProductHomotopyTwoStep (a b c : Nat) : Path ((a + b) + c) (a + (c + b)) :=
  Path.trans (homotopyHigherProductHomotopyAssoc a b c) (homotopyHigherProductHomotopyInner a b c)

/-- The two-step path composed with its inverse cancels to the reflexive path —
    a non-decorative `RwEq` (the `trans_symm` rule on a length-two trace). -/
noncomputable def homotopyHigherProductHomotopyCancel (a b c : Nat) :
    Path.RwEq (Path.trans (homotopyHigherProductHomotopyTwoStep a b c) (Path.symm (homotopyHigherProductHomotopyTwoStep a b c)))
      (Path.refl ((a + b) + c)) :=
  Path.rweq_cmpA_inv_right (homotopyHigherProductHomotopyTwoStep a b c)

/-- Associativity-of-composition (`trans_assoc`, the `tt` rewrite) on any three
    composable paths — a genuine `RwEq` between distinct bracketings. -/
noncomputable def homotopyHigherProductHomotopyAssocCoh {α : Type} {a b c d : α}
