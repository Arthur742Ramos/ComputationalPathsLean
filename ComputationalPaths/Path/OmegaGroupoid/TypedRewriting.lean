...
/-- TStar is a congruence: if p →* q and we apply the same step, the results relate. -/
theorem TStar.congruence {r : ∀ {p q : Path a b}, Prop}
    {p q s : Path a b} (h : TStar r p q) (hs : r (p := q) (q := s)) :
    TStar r p s := by
  sorry

/-- TJoinable is transitive when the relation is confluent. -/
theorem TJoinable.trans_of_confluent {r : ∀ {p q : Path a b}, Prop}
    (hc : TConfluent (a := a) (b := b) r)
    {p q s : Path a b} (j₁ : TJoinable r p q) (j₂ : TJoinable r q s) :
    TJoinable r p s := by
  sorry

/-- Length of reflexive TStar is zero. -/
theorem TStar.length_refl {r : ∀ {p q : Path a b}, Prop} (p : Path a b) :
    (TStar.refl (r := r) p).length = 0 := by
  sorry

/-- Length of a single step is one. -/
theorem TStar.length_single {r : ∀ {p q : Path a b}, Prop}
    {p q : Path a b} (h : r (p := p) (q := q)) :
    (TStar.single h).length = 1 := by
  sorry

/-- Length is additive under transitivity. -/
theorem TStar.length_trans {r : ∀ {p q s : Path a b}, Prop}
    {p q s : Path a b} (h₁ : TStar r p q) (h₂ : TStar r q s) :
    (TStar.trans h₁ h₂).length = h₁.length + h₂.length := by
  sorry

/-- Diamond implies the diamond property is preserved by multi-step. -/
theorem TDiamond.multi_step_diamond {r : ∀ {p q : Path a b}, Prop}
    (hd : TDiamond (a := a) (b := b) r)
    {p q₁ q₂ : Path a b}
    (h₁ : TStar r p q₁) (h₂ : TStar r p q₂) :
    TJoinable r q₁ q₂ := by
  sorry

/-- Semi-confluence is symmetric in a suitable sense. -/
theorem TSemiConfluent.symm_joinable {r : ∀ {p q : Path a b}, Prop}
    (hsc : TSemiConfluent (a := a) (b := b) r)
    {p q₁ q₂ : Path a b}
    (h₁ : TStar r p q₁) (h₂ : r (p := p) (q := q₂)) :
    TJoinable r q₁ q₂ := by
  sorry

/-- Confluence of Step implies all paths have a common reduct. -/
theorem confluent_step_common_reduct
    [Rewrite.ConfluenceConstructive.HasLocalConfluenceProp.{u}]
    (hterm : Terminating (a := a) (b := b) (r := Step))
    (p q : Path a b) (hp : TStar (r := Step) p q) :
    ∃ s, TStar (r := Step) p s ∧ TStar (r := Step) q s := by
  sorry

/-- TPlus is transitive. -/
theorem TPlus.trans {r : ∀ {p q : Path a b}, Prop}
    {p q s : Path a b} (h₁ : TPlus r p q) (h₂ : TPlus r q s) : TPlus r p s := by
  sorry

/-- TPlus to TStar preserves length (off by one). -/
theorem TPlus.length_of_plus {r : ∀ {p q : Path a b}, Prop}
    {p q : Path a b} (h : TPlus r p q) :
    (TStar.of_plus h).length ≥ 1 := by
  sorry

/-- Local confluence + termination implies unique normal forms. -/
theorem newman_unique_nf
    (hterm : Terminating (a := a) (b := b) (r := Step))
    (hlc : TLocalConfluent (a := a) (b := b) (r := Step))
    {p nf₁ nf₂ : Path a b}
    (h₁ : TStar (r := Step) p nf₁)
    (h₂ : TStar (r := Step) p nf₂)
    (hnf₁ : ∀ q, ¬ Step (p := nf₁) (q := q))
