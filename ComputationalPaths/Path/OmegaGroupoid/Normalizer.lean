...
This module develops a normalizer for `Derivation₂` using the groupoid-law
constructors of `MetaStep₃`, with the remaining residual transport boundary
isolated in the core `OmegaGroupoid` strict connector.  The exported
contractibility witness now really routes through flat-chain normalization and
then hands the resulting strict forms to `OmegaGroupoid.connect_strict`.
...
explicit groupoid-law 3-cells.  Remaining hard strict-shape comparisons are
first rerouted through normalized inverses, so `OmegaGroupoid.strict_transport₃`
now appears only as the final zero-fuel safety fallback.
...
strict-form comparison: mixed-polarity and uniqueness-style cases still funnel
through the core `OmegaGroupoid.connect_strict` machinery, whose last-resort
zero-fuel branch is `strict_transport₃`.
...
/-- Export the level-3 connector through the normalizer interface.
    ...
    The only remaining non-structural boundary is therefore the one already
    isolated inside the core strict connector (`strict_transport₃` at zero fuel). -/
