...
- Generators: `Step p q` (atomic rewrite steps)
...
- `step_eq` — coherence between parallel atomic steps
...
The sign is explicit; endpoint coherence is handled by `step_eq` and
`contractibility₃` when needed. -/
...
underlying step witnesses is handled by higher cells (`step_eq`/`contractibility₃`). -/
...
inductive Cancels {A : Type u} {a b : A} :
    {p q : Path a b} → SignedStep p q → {r : Path a b} → SignedStep q r → Type (u + 1) where
...
noncomputable def cancel_witness {p q : Path a b}
  ...
  cases hc with
  | fwd_bwd s₁ s₂ =>
    -- Align the right factor by step coherence, then cancel.
    exact .vcomp
      (Derivation₃.whiskerLeft₃ (.step s₁) (Derivation₃.inv_congr₃ (.step (.step_eq s₂ s₁))))
      (.step (.vcomp_inv_right (.step s₁)))
  | bwd_fwd s₁ s₂ =>
    -- Align the right factor by step coherence, then cancel.
    exact .vcomp
      (Derivation₃.whiskerLeft₃ (.inv (.step s₁)) (.step (.step_eq s₂ s₁)))
...
`Step p q` (or equivalently, `Nonempty (Step p q)` when a Prop relation
is needed).
...
Any two steps between the same endpoints are connected by `step_eq`.
Therefore, two reduced chains between
