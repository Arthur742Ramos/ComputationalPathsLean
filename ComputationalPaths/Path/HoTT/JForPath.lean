...
theorem level_12_proof_irrelevant {a b : A} (p q : Path a b)
    (h₁ h₂ : p = q) : h₁ = h₂ := subsingleton_eq_by_cases h₁ h₂
...
theorem semantic_projection_wd {a b : A} (p q : Path a b) :
    p.toEq = q.toEq := subsingleton_eq_by_cases _ _
...
theorem path_refines_eq {a b : A} (p q : Path a b) :
  ...
  ⟨subsingleton_eq_by_cases _ _, fun hne heq => hne (_root_.congrArg Path.steps heq)⟩
...
theorem J_left_unit {a b : A} (p : Path a b) :
    (trans (refl a) p).proof = p.proof := subsingleton_eq_by_cases _ _

theorem J_right_unit {a b : A} (p : Path a b) :
    (trans p (refl b)).proof = p.proof := subsingleton_eq_by_cases _ _

theorem J_assoc {a b c d : A} (p : Path a b) (q : Path b c) (r : Path c d) :
    (trans (trans p q) r).proof = (trans p (trans q r)).proof := subsingleton_eq_by_cases _ _

theorem J_left_inv {a b : A} (p : Path a b) :
    (trans (symm p) p).proof = (refl b).proof := subsingleton_eq_by_cases _ _

theorem J_right_inv {a b : A} (p : Path a b) :
    (trans p (symm p)).proof = (refl a).proof := subsingleton_eq_by_cases _ _
...
theorem path_encode_decode {a y : A} (c : PathCode a y) :
    path_encode (path_decode c) = c := subsingleton_eq_by_cases _ _
...
def K_for_Eq (a : A) (C : a = a → Prop) (c : C rfl) (h : a = a) : C h := by
  have : h = rfl := subsingleton_eq_by_cases h rfl; rw [this]; exact c
...
theorem proof_factoring_motive {a b : A}
  ...
  have : p.proof = q.proof := subsingleton_eq_by_cases _ _; rw [this]
