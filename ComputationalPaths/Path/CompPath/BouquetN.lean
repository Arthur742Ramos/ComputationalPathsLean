...
- `BouquetN n`: The bouquet of n circles as a computational-path construction
...
private noncomputable def bouquetNData : Nat → Σ A : Type u, A
  | 0 => ⟨CompPath.PUnit'.{u}, CompPath.PUnit'.unit⟩
  | Nat.succ n =>
...
noncomputable def bouquetLoop : {n : Nat} → Fin'B n →
  ...
  | Nat.succ n, .fzero =>
      ...
      Path.trans glue (Path.trans (Pushout.inrPath (A := prev.1) (B := Circle.{u}) (C := CompPath.PUnit') (f := fun _ => prev.2) (g := fun _ => circleBase) circleLoop) (Path.symm glue))
  | Nat.succ n, .fsucc i => Pushout.inlPath (A := (bouquetNData n).1) (B := Circle.{u}) (C := CompPath.PUnit') (f := fun _ => (bouquetNData n).2) (g := fun _ => circleBase) (bouquetLoop (n := n) i)
