...
/-- The 2-sphere as suspension of the computational circle. -/
noncomputable def Sphere2CompPath : Type u :=
...
/-- North pole of S². -/
noncomputable def north : Sphere2CompPath :=
  PushoutCompPath.inl PUnit'.unit

/-- South pole of S². -/
noncomputable def south : Sphere2CompPath :=
  PushoutCompPath.inr PUnit'.unit

/-- Meridian from north to south. -/
noncomputable def merid (x : CircleCompPath) : Path (north : Sphere2CompPath) south :=
  PushoutCompPath.glue (A := PUnit') (B := PUnit') (C := CircleCompPath)
    (f := fun _ => PUnit'.unit) (g := fun _ => PUnit'.unit) x

/-- Basepoint of S² (north pole). -/
noncomputable def basepoint : Sphere2CompPath := north

noncomputable instance : Nonempty Sphere2CompPath := ⟨basepoint⟩

/-! ## Subsingleton and π₁ triviality -/

noncomputable instance : Subsingleton Sphere2CompPath where
  allEq x y := by
...
noncomputable def sphere2CompPath_path (x y : Sphere2CompPath) : Path x y :=
