/-
# Topological fundamental-group comparison for the presented circle

This module proves the fundamental-group comparison between the proof-relevant circle
presentation and Mathlib's topological fundamental group of the additive
circle `ℝ ⧸ ℤ`.
...
namespace Path
namespace CompPath
namespace CircleTopologicalRealization
...
theorem circleCover_liftLoopPath_extend
  ...
  rw [_root_.Path.extend_extends _ ht,
    _root_.Path.extend_extends _ ht]
...
theorem liftLoop_trans
  ...
  unfold liftLoop
  rw [isCoveringMap_circleCover.eq_liftPath_iff']
  exact ⟨combinedLiftLoopPath_lifts γ δ, by simp [combinedLiftLoopPath]⟩
...
theorem topologicalWinding_comp
    ...
    topologicalWinding
        (_root_.Path.Homotopic.Quotient.comp x y) =
...
theorem standardLoop_homotopic
  ...
  have hprojected :
    ...
    simpa [projectedStraightWindingPath, projectedLiftLoopPath,
      straightWindingPath, liftLoopPath] using hmap
...
noncomputable def topologicalLoopQuotEquivInt :
    SimpleEquiv TopologicalLoopQuot ℤ where
...
/-! ## Comparison with Mathlib's fundamental group -/

/-- Mathlib's topological fundamental group of the unit additive circle. -/
abbrev TopologicalPiOne : Type :=
  FundamentalGroup TopologicalCircle (0 : TopologicalCircle)

/-- Mathlib packages the fundamental group as automorphisms in the fundamental
groupoid; its underlying arrows are exactly loop homotopy classes. -/
noncomputable def fundamentalGroupLoopQuotEquiv :
    SimpleEquiv TopologicalPiOne TopologicalLoopQuot where
  toFun := fun x =>
    FundamentalGroup.toPath
      (X := TopCat.of TopologicalCircle) x
  invFun := fun p =>
    FundamentalGroup.fromPath
      (X := TopCat.of TopologicalCircle) p
  left_inv := by
    intro x
...
/-- Mathlib's topological fundamental group of `AddCircle 1` is classified by
integer winding number. -/
noncomputable def topologicalPiOneEquivInt :
    SimpleEquiv TopologicalPiOne ℤ :=
  fundamentalGroupLoopQuotEquiv.trans topologicalLoopQuotEquivInt

/-- Realization comparison for the circle: the proof-relevant presented
fundamental group and Mathlib's topological fundamental group of `ℝ ⧸ ℤ` have
the same winding classification. -/
noncomputable def presentedCircleTopologicalPiOneEquiv :
    SimpleEquiv CirclePresented.PiOne TopologicalPiOne :=
  CirclePresented.piOneEquivInt.trans topologicalPiOneEquivInt.symm
...
@[simp] theorem topologicalPiOneEquivInt_one :
  ...
  change
    topologicalWinding
        (Quotient.mk' (_root_.Path.refl (0 : TopologicalCircle))) = 0
  exact windingPath_refl

theorem topologicalPiOneEquivInt_mul (x y : TopologicalPiOne) :
    topologicalPiOneEquivInt.toFun (x * y) =
      topologicalPiOneEquivInt.toFun x +
        topologicalPiOneEquivInt.toFun y := by
  change
    topologicalWinding
        (FundamentalGroup.toPath
          (X := TopCat.of TopologicalCircle) (x * y)) =
      topologicalWinding
          (FundamentalGroup.toPath
            (X := TopCat.of TopologicalCircle) x) +
        topologicalWinding
          (FundamentalGroup.toPath
            (X := TopCat.of TopologicalCircle) y)
  rw [show
    FundamentalGroup.toPath
        (X := TopCat.of TopologicalCircle) (x * y) =
      _root_.Path.Homotopic.Quotient.comp
        (FundamentalGroup.toPath
          (X := TopCat.of TopologicalCircle) y)
        (FundamentalGroup.toPath
          (X := TopCat.of TopologicalCircle) x) from rfl]
  rw [topologicalWinding_comp, Int.add_comm]

theorem topologicalPiOneEquivInt_inv (x : TopologicalPiOne) :
    topologicalPiOneEquivInt.toFun x⁻¹ =
      -(topologicalPiOneEquivInt.toFun x) := by
  have h := topologicalPiOneEquivInt_mul x x⁻¹
  simp only [mul_inv_cancel, topologicalPiOneEquivInt_one] at h
  omega

private theorem simpleEquiv_toFun_injective
    {α : Type*} {β : Type*} (e : SimpleEquiv α β) :
    Function.Injective e.toFun := by
  intro x y h
  calc
    x = e.invFun (e.toFun x) := (e.left_inv x).symm
    _ = e.invFun (e.toFun y) := _root_.congrArg e.invFun h
    _ = y := e.left_inv y

@[simp] theorem topologicalPiOneEquivInt_presentedBridge
    (x : CirclePresented.PiOne) :
    topologicalPiOneEquivInt.toFun
        (presentedCircleTopologicalPiOneEquiv.toFun x) =
      CirclePresented.encode x :=
  topologicalPiOneEquivInt.right_inv _

/-- The realization comparison as a group equivalence. -/
noncomputable def presentedCircleTopologicalPiOneGroupEquiv :
    Presented.GroupEquiv
      CirclePresented.PiOne TopologicalPiOne
      Presented.PiOne.mul Presented.PiOne.id Presented.PiOne.inv
      (fun x y : TopologicalPiOne => x * y) 1 Inv.inv where
  equiv := presentedCircleTopologicalPiOneEquiv
  map_mul := by
    intro x y
    apply simpleEquiv_toFun_injective topologicalPiOneEquivInt
    rw [topologicalPiOneEquivInt_presentedBridge,
      topologicalPiOneEquivInt_mul,
      topologicalPiOneEquivInt_presentedBridge,
      topologicalPiOneEquivInt_presentedBridge,
      CirclePresented.encode_mul]
  map_one := by
    apply simpleEquiv_toFun_injective topologicalPiOneEquivInt
    rw [topologicalPiOneEquivInt_presentedBridge,
      topologicalPiOneEquivInt_one,
      CirclePresented.encode_id]
  map_inv := by
    intro x
    apply simpleEquiv_toFun_injective topologicalPiOneEquivInt
    rw [topologicalPiOneEquivInt_presentedBridge,
      topologicalPiOneEquivInt_inv,
      topologicalPiOneEquivInt_presentedBridge,
      CirclePresented.encode_inv]

/-- Concrete computational-path certificate that both sides of the realization
comparison carry the same winding number. -/
noncomputable def realizationWindingPath
    (x : CirclePresented.PiOne) :
    Path
      (CirclePresented.encode x)
      (topologicalPiOneEquivInt.toFun
        (presentedCircleTopologicalPiOneGroupEquiv.equiv.toFun x)) :=
  Path.stepChain
    (topologicalPiOneEquivInt_presentedBridge x).symm

/-- The realization winding certificate is coherent with right-unit
composition in the core computational-path calculus. -/
noncomputable def realizationWindingCoherence
    (x : CirclePresented.PiOne) :
    RwEq
      (Path.trans (realizationWindingPath x)
        (Path.refl
          (topologicalPiOneEquivInt.toFun
            (presentedCircleTopologicalPiOneGroupEquiv.equiv.toFun x))))
      (realizationWindingPath x) :=
  rweq_cmpA_refl_right (realizationWindingPath x)

end
end CircleTopologicalRealization
end CompPath
end Path
