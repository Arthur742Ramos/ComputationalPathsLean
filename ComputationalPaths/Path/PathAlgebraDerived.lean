...
theorem rweq_inner_left_unit (p : Path a b) (q : Path b c) :
...
theorem rweq_inner_right_unit (p : Path a b) (q : Path b c) :
...
theorem rweq_symm_trans3 (p : Path a b) (q : Path b c) (r : Path c d) :
...
theorem rweq_cancel_left (r : Path a b) {p q : Path b c}
...
theorem rweq_whisker_inv_left (p : Path a b) (q : Path b c) :
...
theorem rweq_whisker_inv_right (p : Path a b) (q : Path b c) :
...
theorem rweq_conjugate_refl_left (q : Path a a) :
...
theorem rweq_conjugate_refl_inner (p : Path a b) :
...
theorem rweq_ofEq_assoc {a b c d : A} (h₁ : a = b) (h₂ : b = c) (h₃ : c = d) :
