...
noncomputable def PathEquiv.comp {A B C : Type u} (e₁ : PathEquiv A B) (e₂ : PathEquiv B C) :
    PathEquiv A C where
...
noncomputable def equivReflPath (A : Type u) :
...
noncomputable def equivCompForwardPath {A B C : Type u} (e₁ : PathEquiv A B) (e₂ : PathEquiv B C)
    (a : A) :
...
noncomputable def equivRightInvPath {A B : Type u} (e : PathEquiv A B) (b : B) :
    Path (e.toFun (e.invFun b)) b := e.right_inv_path b

/-- Left-inverse path from the equivalence structure. -/
noncomputable def equivLeftInvPath {A B : Type u} (e : PathEquiv A B) (a : A) :
...
noncomputable def inv_inv_path {A B : Type u} (e : PathEquiv A B) (a : A) :
    Path ((PathEquiv.inv (PathEquiv.inv e)).toFun a) (e.toFun a) := Path.refl _

/-! ## Transport along equivalences -/

/-- Forward transport. -/
noncomputable def equivTransport {A B : Type u} (e : PathEquiv A B) (a : A) : B :=
  e.toFun a

/-- Backward transport. -/
noncomputable def equivTransportBack {A B : Type u} (e : PathEquiv A B) (b : B) : A :=
  e.invFun b

/-- Roundtrip forward-back: path from left inverse. -/
noncomputable def equivTransportRoundtrip {A B : Type u} (e : PathEquiv A B) (a : A) :
    Path (equivTransportBack e (equivTransport e a)) a :=
  e.left_inv_path a

/-- Roundtrip back-forward: path from right inverse. -/
noncomputable def equivTransportRoundtrip' {A B : Type u} (e : PathEquiv A B) (b : B) :
    Path (equivTransport e (equivTransportBack e b)) b :=
  e.right_inv_path b

/-- Double roundtrip: forward, back, forward recovers forward. -/
noncomputable def equivDoubleRoundtrip {A B : Type u} (e : PathEquiv A B) (a : A) :
...
noncomputable def HalfAdjEquiv.idHAE (A : Type u) : HalfAdjEquiv A A where
  toFun := id
  invFun := id
  right_inv_path _ := Path.refl _
  left_inv_path _ := Path.refl _
...
noncomputable def halfAdjRightPath {A B : Type u} (e : HalfAdjEquiv A B) (b : B) :
    Path (e.toFun (e.invFun b)) b :=
  e.right_inv_path b

/-- Half-adjoint left-inverse path. -/
noncomputable def halfAdjLeftPath {A B : Type u} (e : HalfAdjEquiv A B) (a : A) :
...
noncomputable def equivCongrArg {A B : Type u} (e : PathEquiv A B) {a₁ a₂ : A}
    (p : Path a₁ a₂) : Path (e.toFun a₁) (e.toFun a₂) :=
  Path.congrArg e.toFun p

/-- CongrArg through inverse map. -/
noncomputable def equivInvCongrArg {A B : Type u} (e : PathEquiv A B) {b₁ b₂ : B}
...
noncomputable def pathToEquiv {A B : Type u} (h : A = B) : PathEquiv A B := by
  cases h; exact PathEquiv.idEquiv A

theorem pathToEquiv_refl (A : Type u) :
    (pathToEquiv (rfl : A = A)).toFun = id := rfl

noncomputable def pathToEquiv_refl_path (A : Type u) :
...
noncomputable def pathToEquiv_rfl_apply (A : Type u) (a : A) :
...
noncomputable def equivComp_assoc_path {A B C D : Type u}
    (e₁ : PathEquiv A B) (e₂ : PathEquiv B C) (e₃ : PathEquiv C D)
    (a : A) :
...
noncomputable def equivComp_id_left_path {A B : Type u} (e : PathEquiv A B) (a : A) :
    Path ((PathEquiv.comp (PathEquiv.idEquiv A) e).toFun a) (e.toFun a) := Path.refl _

noncomputable def equivComp_id_right_path {A B : Type u} (e : PathEquiv A B) (a : A) :
...
noncomputable def equivComp_inv_right_path {A B : Type u} (e : PathEquiv A B) (a : A) :
    Path ((PathEquiv.comp e (PathEquiv.inv e)).toFun a) a :=
  e.left_inv_path a

/-- e⁻¹ composed with e gives identity (via right inverse path). -/
noncomputable def equivComp_inv_left_path {A B : Type u} (e : PathEquiv A B) (b : B) :
...
noncomputable def idFiber (A : Type u) (a : A) : Fiber (fun x : A => x) a where
  point := a
  path := Path.refl _

/-- The fiber point for id is the point itself. -/
noncomputable def idFiber_path (A : Type u) (a : A) : Path (idFiber A a).point a := Path.refl _
...
noncomputable def equivFiber {A B : Type u} (e : PathEquiv A B) (b : B) :
    Fiber e.toFun b where
  point := e.invFun b
  path := e.right_inv_path b

/-- Two fiber points related via congruence of the inverse. -/
noncomputable def fiberPointPath {A B : Type u} (e : PathEquiv A B) {b₁ b₂ : B}
    (p : Path b₁ b₂) :
...
noncomputable def transportEquiv (A B : Type u) (h : A = B) : PathEquiv A B := by
  cases h; exact PathEquiv.idEquiv A

theorem transportEquiv_refl (A : Type u) :
    (transportEquiv A A rfl).toFun = id := rfl

noncomputable def transportEquiv_refl_path (A : Type u) :
