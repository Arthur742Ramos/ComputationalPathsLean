...
noncomputable example (p : Path a b) : RwEq p p := by path_rfl

/-- `path_symm` applies symmetry to RwEq goals. -/
noncomputable example (p q : Path a b) (h : RwEq p q) : RwEq q p := by
  path_symm
  exact h

/-- `path_simp` simplifies using groupoid laws. -/
noncomputable example (p : Path a b) : RwEq (trans (refl a) p) p := by path_simp

noncomputable example (p : Path a b) : RwEq (trans p (refl b)) p := by exact rweq_cmpA_refl_right p

noncomputable example (p : Path a b) : RwEq (symm (symm p)) p := by exact rweq_ss p

noncomputable example : RwEq (symm (refl a)) (refl a) := by exact rweq_sr a

/-! ## 2. Transitivity Examples -/

/-- `path_trans h` uses hypothesis h for transitivity. -/
noncomputable example (p q r : Path a b) (h1 : RwEq p q) (h2 : RwEq q r) : RwEq p r := by
  exact rweq_trans h1 h2

/-- `path_trans_via` specifies an explicit intermediate path. -/
noncomputable example (p : Path a b) : RwEq (trans (refl a) p) p := by
...
noncomputable example (p p' : Path a b) (q : Path b c) (h : RwEq p p') :
    RwEq (trans p q) (trans p' q) := by
  path_congr_left h

/-- `path_congr_right` applies congruence on the right of trans. -/
noncomputable example (p : Path a b) (q q' : Path b c) (h : RwEq q q') :
    RwEq (trans p q) (trans p q') := by
  path_congr_right h

/-- Congruence on both sides via rweq_trans_congr. -/
noncomputable example (p p' : Path a b) (q q' : Path b c) (hp : RwEq p p') (hq : RwEq q q') :
    RwEq (trans p q) (trans p' q') := by
...
noncomputable example (p : Path a b) (q : Path b c) (r : Path c d) :
...
noncomputable example (p : Path a b) : RwEq (trans p (refl b)) p := by
  exact rweq_cmpA_refl_right p

/-- `path_cancel_left` cancels inverse on left. -/
noncomputable example (p : Path a b) : RwEq (trans (symm p) p) (refl b) := by
  path_cancel_left

/-- `path_cancel_right` cancels inverse on right. -/
noncomputable example (p : Path a b) : RwEq (trans p (symm p)) (refl a) := by
  path_cancel_right
...
noncomputable example (p : Path a b) : RwEq (symm (symm p)) p := by
  exact rweq_ss p
...
noncomputable example (p : Path a b) :
...
/-- Product fst beta. -/
noncomputable example {α β : Type} {a₁ a₂ : α} {b₁ b₂ : β}
...
noncomputable example (p : Path a b) (q : Path b c) :
...
noncomputable example {B : Type} (f : A → B) (p q : Path a b) (h : RwEq p q) :
    congrArg f p ≈ congrArg f q := by
  exact rweq_congrArg_of_rweq f h

/-- CongrArg of refl is refl. -/
noncomputable example {B : Type} (f : A → B) (a : A) :
...
noncomputable example (p : Path a b) : symm (symm p) ≈ p := by
  path_inv_inv

/-- `path_symm_refl` handles symm of refl: σ(ρ) ≈ ρ. -/
noncomputable example : symm (refl a) ≈ refl a := by
  path_symm_refl
...
noncomputable example (p q : Path a b) (h : RwEq p q) :
    symm p ≈ symm q := by
  path_congr_symm h
...
noncomputable example (p q r : Path a b) (hp : RwEq p r) (hq : RwEq q r) :
    RwEq p q := by
  path_both_eq hp, hq
...
noncomputable example (p q r s : Path a b) (h1 : RwEq p q) (h2 : RwEq q r) (h3 : RwEq r s) :
    RwEq p s := by
  path_chain3 h1, h2, h3

/-- `path_chain4 h1, h2, h3, h4` chains four hypotheses. -/
noncomputable example (p q r s t : Path a b)
...
noncomputable example (p : Path a b) (q : Path b a) :
