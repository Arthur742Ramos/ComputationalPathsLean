...
/-- Two joined expressions are equal (in the minimal core). -/
theorem join_eq {A : Type u} {a b : A}
    {p q : PathExpr (A := A) (a := a) (b := b)}
    (J : Join (A := A) (a := a) (b := b) p q) :
    p = q := by
  exact ((rw_eq_source J.left).symm).trans (rw_eq_source J.right)
...
def join_atom_atom {A : Type u} {a b : A} (p : Path a b) :
    Join (A := A) (a := a) (b := b) (PathExpr.atom p) (PathExpr.atom p) :=
...
def join_refl_refl {A : Type u} (a : A) :
    Join (A := A) (a := a) (b := a) (PathExpr.refl a) (PathExpr.refl a) :=
...
def join_trans_trans {A : Type u} {a b c : A}
    {p₁ q₁ : PathExpr (A := A) (a := a) (b := b)}
    {p₂ q₂ : PathExpr (A := A) (a := b) (b := c)}
    (J₁ : Join (A := A) (a := a) (b := b) p₁ q₁)
    (J₂ : Join (A := A) (a := b) (b := c) p₂ q₂) :
    Join (A := A) (a := a) (b := c)
...
theorem church_rosser
    ...
    {p q r : PathExpr (A := A) (a := a) (b := b)}
    (hq : Rw p q) (hr : Rw p r) :
    ∃ s : PathExpr (A := A) (a := a) (b := b), Rw q s ∧ Rw r s := by
...
theorem church_rosser_steps
    ...
    ∃ s : PathExpr (A := A) (a := a) (b := b), Rw q s ∧ Rw r s :=
