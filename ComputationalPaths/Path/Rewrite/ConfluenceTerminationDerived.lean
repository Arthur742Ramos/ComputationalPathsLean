...
@[simp] noncomputable def join_symm {p q : Path a b} (j : Join (A := A) (a := a) (b := b) p q) :
    Join (A := A) (a := a) (b := b) q p :=
  { meet := j.meet, left := j.right, right := j.left }

/-- Join composition with `Rw` on the left. -/
@[simp] noncomputable def join_of_rw_left {p q r : Path a b} (h : Rw p q)
...
@[simp] noncomputable def join_of_rw_right {p q r : Path a b} (h : Rw r q)
...
noncomputable def join_congr_left_rw {p p' q : Path a b}
    (h : Rw p' p)
    (j : Join (A := A) (a := a) (b := b) p q) :
...
noncomputable def join_congr_right_rw {p q q' : Path a b}
    (h : Rw q' q)
    (j : Join (A := A) (a := a) (b := b) p q) :
    Join (A := A) (a := a) (b := b) p q' :=
...
@[simp] noncomputable def join_of_rw [Confluence.HasJoinOfRw.{u}]
    {p q r : Path a b} (hq : Rw p q) (hr : Rw p r) :
