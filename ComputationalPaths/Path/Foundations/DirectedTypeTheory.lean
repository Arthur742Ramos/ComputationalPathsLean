...
noncomputable def dpathRefl {α : Type u} (x : α) : DPath α := ⟨x, x⟩
noncomputable def dpathRev {α : Type u} (p : DPath α) : DPath α := ⟨p.tgt, p.src⟩
noncomputable def dpathComp {α : Type u} (p q : DPath α) : DPath α := ⟨p.src, q.tgt⟩
noncomputable def dpathStart {α : Type u} (p : DPath α) : α := p.src
noncomputable def dpathEnd {α : Type u} (p : DPath α) : α := p.tgt
noncomputable def dpathBoundary {α : Type u} (p : DPath α) : α × α := (dpathStart p, dpathEnd p)
noncomputable def dpathToPath {α : Type u} (x : α) : Path x x := Path.refl x
noncomputable def directedIdentityPath {α : Type u} (x : α) : Path x x :=
...
noncomputable def semiObj0 {α : Type u} (S : SemiSimplicialType α) : Type u := S.obj 0
noncomputable def semiObj1 {α : Type u} (S : SemiSimplicialType α) : Type u := S.obj 1
noncomputable def semiEdge {α : Type u} (S : SemiSimplicialType α) (a b : α) : Type u := S.edge a b

structure SegalType (α : Type u) where
  hom : α → α → Type u
  comp : {a b c : α} → hom a b → hom b c → hom a c

noncomputable def segalHom {α : Type u} (S : SegalType α) (a b : α) : Type u := S.hom a b
noncomputable def segalComp {α : Type u} (S : SegalType α) {a b c : α}
...
noncomputable def rezkCarrier {α : Type u} (R : RezkCompletion α) : Type u := R.carrier
noncomputable def rezkEmbed {α : Type u} (R : RezkCompletion α) : α → rezkCarrier R := R.embed
noncomputable def rezkComplete {α : Type u} (R : RezkCompletion α) : Prop := R.complete

structure DirectedUnivalence (α : Type u) where
  code : α → α → Type u
  decode : {a b : α} → code a b → DPath α

noncomputable def directedCode {α : Type u} (U : DirectedUnivalence α) (a b : α) : Type u := U.code a b
noncomputable def directedDecode {α : Type u} (U : DirectedUnivalence α) {a b : α}
...
noncomputable def i1Hom {α : Type u} (C : InfinityOneCategory α) (a b : α) : Type u := C.hom a b
noncomputable def i1Id {α : Type u} (C : InfinityOneCategory α) (a : α) : C.hom a a := C.id a
noncomputable def i1Comp {α : Type u} (C : InfinityOneCategory α) {a b c : α}
