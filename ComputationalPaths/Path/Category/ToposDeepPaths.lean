...
@[simp] noncomputable def tvMeet (a b : Bool) : Bool := a && b

/-- Join (disjunction) of truth values. -/
@[simp] noncomputable def tvJoin (a b : Bool) : Bool := a || b

/-- Implication in the Heyting algebra. -/
@[simp] noncomputable def tvImpl (a b : Bool) : Bool := !a || b

/-- Negation in the Heyting algebra. -/
@[simp] noncomputable def tvNeg (a : Bool) : Bool := !a
...
noncomputable def meetTrueLeftPath (a : Bool) : Path (tvMeet true a) a :=
  Path.refl a

/-- Path: meet distributes then simplifies — 2-step chain. -/
noncomputable def meetDistribPath (a b : Bool) :
...
noncomputable def joinDistribPath (a b : Bool) :
...
noncomputable def doubleNegPath (a : Bool) : Path (tvNeg (tvNeg a)) a :=
  Path.mk [Step.mk (tvNeg (tvNeg a)) a (tvNeg_invol a)] (tvNeg_invol a)

/-- Path: De Morgan for meet, 3-step chain. -/
noncomputable def deMorganMeetPath (a b : Bool) :
...
noncomputable def deMorganJoinPath (a b : Bool) :
...
noncomputable def deMorganCommPath (a b : Bool) :
...
noncomputable def meetAssocSymmPath (a b c : Bool) :
...
noncomputable def meetReassocPath (a b c : Bool) :
...
noncomputable def doubleNegMeetPath (a b : Bool) :
    Path (tvNeg (tvNeg (tvMeet a b))) (tvMeet a b) :=
  doubleNegPath (tvMeet a b)

/-- Path: (a → a) = true for any a. -/
noncomputable def implReflPath (a : Bool) : Path (tvImpl a a) true :=
...
noncomputable def idLT : LTTop where
  j := id
  j_true := rfl
  j_idem := fun _ => rfl
  j_meet := fun _ _ => rfl
...
noncomputable def denseLT : LTTop where
  j := fun _ => true
  j_true := rfl
  j_idem := fun _ => rfl
  j_meet := fun _ _ => rfl
...
noncomputable def ltIdemPath (top : LTTop) (a : Bool) : Path (top.j (top.j a)) (top.j a) :=
  Path.mk [Step.mk _ _ (top.j_idem a)] (top.j_idem a)

/-- Path: triple application = single application — 2-step trans chain. -/
noncomputable def ltTriplePath (top : LTTop) (a : Bool) :
...
noncomputable def ltMeetPath (top : LTTop) (a b : Bool) :
...
noncomputable def ltMeetCommPath (top : LTTop) (a b : Bool) :
    Path (top.j (tvMeet a b)) (top.j (tvMeet b a)) :=
  Path.congrArg top.j
    (Path.mk [Step.mk _ _ (tvMeet_comm a b)] (tvMeet_comm a b))

/-- Path: j(a) ∧ j(b) = j(b) ∧ j(a) — via meet commutativity. -/
noncomputable def ltResultCommPath (top : LTTop) (a b : Bool) :
...
noncomputable def ltMeetFlipPath (top : LTTop) (a b : Bool) :
...
noncomputable def idGeom : GeomMorph where
  direct := id
  inverse := id
  unit := fun _ => rfl
  counit := fun _ => rfl
...
noncomputable def negGeom : GeomMorph where
  direct := tvNeg
  inverse := tvNeg
...
noncomputable def geomUnitPath (gm : GeomMorph) (x : Bool) :
    Path (gm.inverse (gm.direct x)) x :=
  Path.mk [Step.mk _ _ (gm.unit x)] (gm.unit x)

/-- Path: counit of adjunction. -/
noncomputable def geomCounitPath (gm : GeomMorph) (x : Bool) :
...
noncomputable def geomRoundTripPath (gm : GeomMorph) (x : Bool) :
...
noncomputable def geomComp (f g : GeomMorph) : GeomMorph where
...
noncomputable def geomCompUnitPath (f g : GeomMorph) (x : Bool) :
...
noncomputable def idSheafify : Sheafify where
  sh := id
  unit := fun _ => Nat.le_refl _
  idem := fun _ => rfl

/-- Constant sheafification (everything maps to a fixed value c ≥ all inputs). -/
noncomputable def constSheafify : Sheafify where
  sh := id
  unit := fun _ => Nat.le_refl _
  idem := fun _ => rfl

/-- Path: sheafification is idempotent. -/
noncomputable def sheafIdemPath (s : Sheafify) (x : Nat) : Path (s.sh (s.sh x)) (s.sh x) :=
  Path.mk [Step.mk _ _ (s.idem x)] (s.idem x)

/-- Path: triple sheafification = single — 2-step chain. -/
noncomputable def sheafTriplePath (s : Sheafify) (x : Nat) :
...
noncomputable def sheafQuadPath (s : Sheafify) (x : Nat) :
...
noncomputable def modusPonensPath (a b : Bool) (himp : tvImpl a b = true) (ha : a = true) :
    Path b true :=
...
noncomputable def contrapositivePath (a b : Bool) (h : tvImpl a b = true) :
...
noncomputable def excludedMiddlePath (a : Bool) : Path (tvJoin a (tvNeg a)) true :=
...
noncomputable def classStablePath (cm : ClassMap) (x : Nat) : Path (cm.cl (cm.cl x)) (cm.cl x) :=
  Path.mk [Step.mk _ _ (cm.idem x)] (cm.idem x)

/-- Path: triple classification — 2-step chain. -/
noncomputable def classTriplePath (cm : ClassMap) (x : Nat) :
...
noncomputable def classFunctorialPath (cm : ClassMap) {x y : Nat} (p : Path x y) :
...
noncomputable def tvTransport {D : Bool → Type u} {a b : Bool}
...
noncomputable def restrictIdPath (p : SmallPresheaf) (i : Nat) :
    Path (p.restrict i i) 0 :=
  Path.mk [Step.mk _ _ (p.restrict_id i)] (p.restrict_id i)

/-- Path: restriction composition — single step. -/
noncomputable def restrictCompPath (p : SmallPresheaf) (i j k : Nat) :
...
noncomputable def restrictDoubleIdPath (p : SmallPresheaf) (i : Nat) :
