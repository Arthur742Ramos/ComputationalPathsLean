...
noncomputable def commIdemPath (a b : Nat) :
...
noncomputable def joinReassocPath (a b c : Nat) :
...
noncomputable def meetAssoc4Path (a b c d : Nat) :
...
/-- Identity frame hom. -/
noncomputable def idFrameHom : FrameHom where
  map := id
  pres_meet := fun _ _ => rfl
  pres_join := fun _ _ => rfl

/-- Composition of frame homs. -/
noncomputable def compFrameHom (f g : FrameHom) : FrameHom where
  map := f.map ∘ g.map
...
noncomputable def homMeetPath (f : FrameHom) (a b : Nat) :
...
noncomputable def homCongrPath (f : FrameHom) {a b : Nat} (p : Path a b) :
    Path (f.map a) (f.map b) :=
  Path.congrArg f.map p

/-- Path: frame hom preserves absorption — 3-step chain. -/
noncomputable def homAbsorpPath (f : FrameHom) (a b : Nat) :
...
/-- Identity nucleus. -/
noncomputable def idNucleus : Nucleus where
  j := id
  idem := fun _ => rfl
...
noncomputable def nucleusIdemPath (n : Nucleus) (a : Nat) : Path (n.j (n.j a)) (n.j a) :=
  Path.mk [Step.mk _ _ (n.idem a)] (n.idem a)

/-- Path: triple nucleus = single — 2-step chain. -/
noncomputable def nucleusTriplePath (n : Nucleus) (a : Nat) :
...
noncomputable def nucleusQuadPath (n : Nucleus) (a : Nat) :
...
noncomputable def nucleusMeetPath (n : Nucleus) (a b : Nat) :
...
noncomputable def nucleusMeetCommPath (n : Nucleus) (a b : Nat) :
...
noncomputable def idNucleusPath (a : Nat) : Path (idNucleus.j a) a := Path.refl a

/-- Path: nucleus functoriality. -/
noncomputable def nucleusCongrPath (n : Nucleus) {a b : Nat} (p : Path a b) :
    Path (n.j a) (n.j b) :=
  Path.congrArg n.j p
...
@[simp] noncomputable def openNuc (a : Nat) (x : Nat) : Nat := fJoin a x

/-- Closed nucleus: c_a(x) = min x a. -/
@[simp] noncomputable def closedNuc (a : Nat) (x : Nat) : Nat := fMeet x a
...
noncomputable def openNucIdemPath (a x : Nat) : Path (openNuc a (openNuc a x)) (openNuc a x) :=
  Path.mk [Step.mk _ _ (openNuc_idem a x)] (openNuc_idem a x)

/-- Path: closed nucleus idempotence. -/
noncomputable def closedNucIdemPath (a x : Nat) : Path (closedNuc a (closedNuc a x)) (closedNuc a x) :=
  Path.mk [Step.mk _ _ (closedNuc_idem a x)] (closedNuc_idem a x)

/-- Path: triple open nucleus = single — 2-step chain. -/
noncomputable def openNucTriplePath (a x : Nat) :
...
noncomputable def closedNucTriplePath (a x : Nat) :
...
/-- Identity locale map. -/
noncomputable def idLocMap : LocMap where
  pullback := idFrameHom

/-- Composition of locale maps (reversed). -/
noncomputable def compLocMap (f g : LocMap) : LocMap where
  pullback := compFrameHom g.pullback f.pullback

/-- Path: id locale map is identity. -/
noncomputable def idLocMapPath (a : Nat) : Path (idLocMap.pullback.map a) a := Path.refl a

/-- Path: composition is associative at the map level. -/
noncomputable def compLocMapAssocPath (f g h : LocMap) (a : Nat) :
...
noncomputable def compLocMapIdLeftPath (f : LocMap) (a : Nat) :
    Path ((compLocMap idLocMap f).pullback.map a) (f.pullback.map a) :=
  Path.refl _

/-- Path: comp with id on right is identity. -/
noncomputable def compLocMapIdRightPath (f : LocMap) (a : Nat) :
...
noncomputable def trivialPoint : LocPoint where
  isMember := fun _ => true
  pres_meet := fun _ _ => rfl
  pres_join := fun _ _ => rfl

/-- Path: point preserves meet. -/
noncomputable def pointMeetPath (pt : LocPoint) (a b : Nat) :
...
noncomputable def pointJoinPath (pt : LocPoint) (a b : Nat) :
...
noncomputable def pointTransportPath (pt : LocPoint) {a b : Nat} (p : Path a b) :
    Path (pt.isMember a) (pt.isMember b) :=
  Path.congrArg pt.isMember p

/-- Path: trivial point is always true. -/
noncomputable def trivialPointPath (a : Nat) : Path (trivialPoint.isMember a) true := Path.refl true
...
noncomputable def meetCongrLeft {a₁ a₂ : Nat} (p : Path a₁ a₂) (b : Nat) :
    Path (fMeet a₁ b) (fMeet a₂ b) :=
  Path.congrArg (fun x => min x b) p

/-- CongrArg of meet on the right. -/
noncomputable def meetCongrRight (a : Nat) {b₁ b₂ : Nat} (p : Path b₁ b₂) :
    Path (fMeet a b₁) (fMeet a b₂) :=
  Path.congrArg (fun x => min a x) p

/-- CongrArg of join along a path. -/
noncomputable def joinCongrLeft {a₁ a₂ : Nat} (p : Path a₁ a₂) (b : Nat) :
    Path (fJoin a₁ b) (fJoin a₂ b) :=
  Path.congrArg (fun x => max x b) p

/-- CongrArg of join on the right. -/
noncomputable def joinCongrRight (a : Nat) {b₁ b₂ : Nat} (p : Path b₁ b₂) :
    Path (fJoin a b₁) (fJoin a b₂) :=
