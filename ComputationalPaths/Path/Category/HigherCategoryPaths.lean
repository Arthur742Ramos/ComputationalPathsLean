...
theorem congrArg_comp₁_left (C : TwoCat) {a b c : C.Obj}
    ...
    Path.congrArg (fun f => C.comp₁ f g) (Path.ofEq h) =
      Path.ofEq (_root_.congrArg (fun f => C.comp₁ f g) h) := by
  subst h; simp
...
theorem congrArg_comp₁_right (C : TwoCat) {a b c : C.Obj}
    ...
    Path.congrArg (fun g => C.comp₁ f g) (Path.ofEq h) =
      Path.ofEq (_root_.congrArg (fun g => C.comp₁ f g) h) := by
  subst h; simp
