...
def hAssoc_path (D : StrictDblCat) {a b c d : D.Obj}
...
def hLeftUnit_path (D : StrictDblCat) {a b : D.Obj} (f : D.HMor a b) :
    Path (D.hComp (D.hId a) f) f :=
  D.hLeftId f

/-- Horizontal right unit witnessed by Path -/
def hRightUnit_path (D : StrictDblCat) {a b : D.Obj} (f : D.HMor a b) :
    Path (D.hComp f (D.hId b)) f :=
  D.hRightId f

/-- Vertical associativity witnessed by Path -/
def vAssoc_path (D : StrictDblCat) {a b c d : D.Obj}
...
def vLeftUnit_path (D : StrictDblCat) {a b : D.Obj} (f : D.VMor a b) :
    Path (D.vComp (D.vId a) f) f :=
  D.vLeftId f

/-- Vertical right unit witnessed by Path -/
def vRightUnit_path (D : StrictDblCat) {a b : D.Obj} (f : D.VMor a b) :
...
def companion_id_path (D : DblCat) (a : D.Obj) :
    Path (D.vId a) (D.vId a) :=
  Path.refl (D.vId a)

/-- Path witness that conjoint of identity is identity -/
def conjoint_id_path (D : DblCat) (a : D.Obj) :
    Path (D.vId a) (D.vId a) :=
  Path.refl (D.vId a)
...
def connection_preserves_id (D : DblCat) (a : D.Obj) :
...
def strict_hfunctor_comp_path {D E : StrictDblCat}
...
def strict_hfunctor_assoc_coherence {D E : StrictDblCat}
    (F : StrictHFunctor D E) {a b c d : D.Obj}
    (f : D.HMor a b) (g : D.HMor b c) (h : D.HMor c d) :
...
def vfunctor_id_coherence (D E : DblCat) (F : VFunctor D E) (a : D.Obj) :
    Path (F.vMap (D.vId a)) (E.vId (F.objMap a)) :=
  F.presVId a

/-- Composing vertical functor preservation paths -/
def vfunctor_comp_coherence (D E : DblCat) (F : VFunctor D E)
...
def hnat_id_coherence (D E : DblCat) (F : HFunctor D E) (a : D.Obj) :
...
def vnat_id_coherence (D E : DblCat) (F : HFunctor D E) (a : D.Obj) :
...
def fibrant_id_companion (D : FibrantDblCat) (a : D.Obj) :
...
def hcomp_path_left {D : DblCat} {a b c : D.Obj}
...
def vcomp_path_right {D : DblCat} {a b c : D.Obj}
...
def hcomp_path_both {D : DblCat} {a b c : D.Obj}
...
def vcomp_path_both {D : DblCat} {a b c : D.Obj}
...
def hfunctorComp (D E F : DblCat) (G : HFunctor D E) (H : HFunctor E F) : HFunctor D F where
...
def hfunctor_comp_assoc_obj (D E F G : DblCat)
...
def hfunctor_comp_hmap (D E F : DblCat) (G : HFunctor D E) (H : HFunctor E F)
    {a b : D.Obj} (f : D.HMor a b) :
    Path ((hfunctorComp D E F G H).hMap f) (H.hMap (G.hMap f)) :=
  Path.refl _

/-- Identity horizontal functor -/
def hfunctorId (D : DblCat) : HFunctor D D where
  objMap := id
  hMap := id
  vMap := id
  sqMap := id
...
def hfunctor_comp_left_unit (D E : DblCat) (F : HFunctor D E) (a : D.Obj) :
    Path ((hfunctorComp D D E (hfunctorId D) F).objMap a) (F.objMap a) :=
  Path.refl _

/-- Right unit for functor composition -/
def hfunctor_comp_right_unit (D E : DblCat) (F : HFunctor D E) (a : D.Obj) :
...
def hfunctor_hcomp_map {D E : DblCat} (F : HFunctor D E)
...
def symm_trans_vcomp {D : StrictDblCat} {a b c : D.Obj}
...
def path_trans_assoc_hmor {D : DblCat} {a b : D.Obj}
    {f g h k : D.HMor a b}
...
def path_trans_refl_left_vmor {D : DblCat} {a b : D.Obj}
    {f g : D.VMor a b} (p : Path f g) :
    Path.trans (Path.refl f) p = p :=
  trans_refl_left p

/-- Path.trans right unit for vertical morphisms -/
def path_trans_refl_right_vmor {D : DblCat} {a b : D.Obj}
    {f g : D.VMor a b} (p : Path f g) :
    Path.trans p (Path.refl g) = p :=
  trans_refl_right p
...
def congrArg_trans_hcomp {D : DblCat} {a b c : D.Obj}
...
def congrArg_symm_hcomp {D : DblCat} {a b c : D.Obj}
...
def congrArg_trans_vcomp {D : DblCat} {a b c : D.Obj}
...
def congrArg_symm_vcomp {D : DblCat} {a b c : D.Obj}
...
def hcomp_left_right_id (D : StrictDblCat) {a b : D.Obj} (f : D.HMor a b) :
    Path (D.hComp (D.hId a) (D.hComp f (D.hId b))) (D.hComp (D.hId a) f) :=
  hcomp_path_right (D.hId a) (D.hRightId f)

/-- Left identity then right identity, full chain -/
def hcomp_lr_id_chain (D : StrictDblCat) {a b : D.Obj} (f : D.HMor a b) :
...
def vcomp_lr_id_chain (D : StrictDblCat) {a b : D.Obj} (f : D.VMor a b) :
...
def hcomp_rl_id_chain (D : StrictDblCat) {a b : D.Obj} (f : D.HMor a b) :
...
def pentagon_path (D : StrictDblCat) {a b c d e : D.Obj}
...
/-- Vertical triangle -/
def vtriangle_path (D : StrictDblCat) {a b c : D.Obj}
...
def vfunctorComp (D E F : DblCat) (G : VFunctor D E) (H : VFunctor E F) : VFunctor D F where
  objMap := fun a => H.objMap (G.objMap a)
  hMap := fun f => H.hMap (G.hMap f)
  vMap := fun f => H.vMap (G.vMap f)
  sqMap := fun s => H.sqMap (G.sqMap s)
...
def vfunctor_comp_pres_id (D E F : DblCat)
...
def vfunctor_comp_pres_comp (D E F : DblCat)
...
def vfunctorId (D : DblCat) : VFunctor D D where
  objMap := id
  hMap := id
  vMap := id
  sqMap := id
...
def vfunctor_comp_left_unit_obj (D E : DblCat) (F : VFunctor D E) (a : D.Obj) :
    Path ((vfunctorComp D D E (vfunctorId D) F).objMap a) (F.objMap a) :=
  Path.refl _

/-- Right unit for vertical functor composition -/
def vfunctor_comp_right_unit_obj (D E : DblCat) (F : VFunctor D E) (a : D.Obj) :
