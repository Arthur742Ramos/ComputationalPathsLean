...
/-- Meet (∧). -/
noncomputable def HeytingSub.meet {A : Type u} (s₁ s₂ : HeytingSub A) : HeytingSub A :=
  { predicate := fun x => s₁.predicate x ∧ s₂.predicate x }

/-- Join (∨). -/
noncomputable def HeytingSub.join {A : Type u} (s₁ s₂ : HeytingSub A) : HeytingSub A :=
  { predicate := fun x => s₁.predicate x ∨ s₂.predicate x }

/-- Implication (⇒). -/
noncomputable def HeytingSub.imp {A : Type u} (s₁ s₂ : HeytingSub A) : HeytingSub A :=
  { predicate := fun x => s₁.predicate x → s₂.predicate x }

/-- Negation (¬). -/
noncomputable def HeytingSub.neg {A : Type u} (s : HeytingSub A) : HeytingSub A :=
  { predicate := fun x => ¬ s.predicate x }

/-- Top (⊤). -/
noncomputable def HeytingSub.top (A : Type u) : HeytingSub A :=
  { predicate := fun _ => True }

/-- Bottom (⊥). -/
noncomputable def HeytingSub.bot (A : Type u) : HeytingSub A :=
  { predicate := fun _ => False }

/-- 1. Meet is idempotent. -/
noncomputable def heyting_meet_idem {A : Type u} (s : HeytingSub A) :
    Path (s.meet s).predicate s.predicate :=
  Path.mk [] (by ext x; exact ⟨fun ⟨h, _⟩ => h, fun h => ⟨h, h⟩⟩)

/-- 2. Meet is commutative. -/
noncomputable def heyting_meet_comm {A : Type u} (s₁ s₂ : HeytingSub A) :
    Path (s₁.meet s₂).predicate (s₂.meet s₁).predicate :=
  Path.mk [] (by ext x; exact ⟨fun ⟨a, b⟩ => ⟨b, a⟩, fun ⟨a, b⟩ => ⟨b, a⟩⟩)

/-- 3. Join is commutative. -/
noncomputable def heyting_join_comm {A : Type u} (s₁ s₂ : HeytingSub A) :
...
noncomputable def heyting_meet_distrib_join {A : Type u} (s₁ s₂ s₃ : HeytingSub A) :
...
noncomputable def heyting_mp {A : Type u} (s t : HeytingSub A) (x : A)
...
noncomputable def heyting_double_neg {A : Type u} (s : HeytingSub A) (x : A)
    (h : s.predicate x) : (s.neg.neg).predicate x :=
  fun hn => hn h

/-- 8. ¬¬¬s = ¬s (triple negation). -/
noncomputable def heyting_triple_neg {A : Type u} (s : HeytingSub A) :
...
noncomputable def heyting_meet_top {A : Type u} (s : HeytingSub A) :
...
noncomputable def heyting_join_bot {A : Type u} (s : HeytingSub A) :
    Path (s.join (HeytingSub.bot A)).predicate s.predicate :=
  Path.mk [] (by ext x; exact ⟨fun h => h.elim id False.elim, fun h => Or.inl h⟩)

/-- 11. ⊥ ⇒ s = ⊤ (ex falso). -/
noncomputable def heyting_ex_falso {A : Type u} (s : HeytingSub A) :
...
/-- Meet of forcings. -/
noncomputable def Forcing.meet {W : Type u} {kj : KripkeJoyal W} (f₁ f₂ : Forcing W kj) : Forcing W kj :=
  { forces := fun w => f₁.forces w ∧ f₂.forces w
    monotone := fun w₁ w₂ h ⟨h₁, h₂⟩ => ⟨f₁.monotone w₁ w₂ h h₁, f₂.monotone w₁ w₂ h h₂⟩ }

/-- Join of forcings. -/
noncomputable def Forcing.join {W : Type u} {kj : KripkeJoyal W} (f₁ f₂ : Forcing W kj) : Forcing W kj :=
  { forces := fun w => f₁.forces w ∨ f₂.forces w
...
noncomputable def Forcing.kimp {W : Type u} {kj : KripkeJoyal W} (f₁ f₂ : Forcing W kj) : Forcing W kj :=
...
noncomputable def forcing_meet_comm {W : Type u} {kj : KripkeJoyal W} (f₁ f₂ : Forcing W kj) :
    Path (f₁.meet f₂).forces (f₂.meet f₁).forces :=
  Path.mk [] (by ext w; exact ⟨fun ⟨a, b⟩ => ⟨b, a⟩, fun ⟨a, b⟩ => ⟨b, a⟩⟩)

/-- 13. Forcing meet with self. -/
noncomputable def forcing_meet_idem {W : Type u} {kj : KripkeJoyal W} (f : Forcing W kj) :
...
noncomputable def kripke_monotone {W : Type u} {kj : KripkeJoyal W} (f : Forcing W kj)
    (w w' : W) (hle : kj.le w w') (hw : f.forces w) : f.forces w' :=
  f.monotone w w' hle hw

/-- 15. Kripke forcing of ⊤. -/
noncomputable def forcing_top (W : Type u) (kj : KripkeJoyal W) : Forcing W kj :=
  { forces := fun _ => True, monotone := fun _ _ _ _ => trivial }

/-- 16. Top is forced everywhere. -/
noncomputable def forcing_top_everywhere {W : Type u} {kj : KripkeJoyal W} (w : W) :
...
noncomputable def lt_inflationary {A : Type u} (top : LTTopology A) (φ : A → Prop) (x : A)
    (h : φ x) : top.j φ x :=
  top.j_ge φ x h

/-- 18. j is idempotent as a path. -/
noncomputable def lt_idempotent {A : Type u} (top : LTTopology A) (φ : A → Prop) :
    Path (top.j (top.j φ)) (top.j φ) :=
  Path.mk [] (by rw [top.j_idem])

/-- 19. j preserves meet as a path. -/
noncomputable def lt_preserves_meet {A : Type u} (top : LTTopology A) (φ ψ : A → Prop) :
...
noncomputable def isJClosed {A : Type u} (top : LTTopology A) (φ : A → Prop) : Prop :=
  top.j φ = φ

/-- 20. Top predicate is j-closed. -/
noncomputable def top_is_j_closed {A : Type u} (top : LTTopology A)
...
noncomputable def sheafify {A : Type u} (top : LTTopology A) (φ : A → Prop) : A → Prop :=
  top.j φ

/-- 23. Sheafification is j-closed. -/
noncomputable def sheafify_closed {A : Type u} (top : LTTopology A) (φ : A → Prop) :
    isJClosed top (sheafify top φ) :=
  j_closure_is_closed top φ

/-- 24. Sheafification is inflationary. -/
noncomputable def sheafify_ge {A : Type u} (top : LTTopology A) (φ : A → Prop) (x : A)
    (h : φ x) : sheafify top φ x :=
  top.j_ge φ x h

/-- 25. Sheafification is idempotent. -/
noncomputable def sheafify_idem {A : Type u} (top : LTTopology A) (φ : A → Prop) :
...
noncomputable def heyting_contrapositive {A : Type u} (s t : HeytingSub A) :
...
noncomputable def heyting_trans_example {A : Type u} (s : HeytingSub A) :
...
noncomputable def heyting_symm_example {A : Type u} (s₁ s₂ : HeytingSub A) :
    Path (s₂.meet s₁).predicate (s₁.meet s₂).predicate :=
  Path.symm (heyting_meet_comm s₁ s₂)

/-- 30. Forcing join is commutative. -/
noncomputable def forcing_join_comm {W : Type u} {kj : KripkeJoyal W} (f₁ f₂ : Forcing W kj) :
...
noncomputable def lt_trans_example {A : Type u} (top : LTTopology A) (φ : A → Prop) :
