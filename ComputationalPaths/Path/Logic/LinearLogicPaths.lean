...
noncomputable def LProp.neg {n : Nat} : LProp n → LProp n
  | var i => negVar i
  | negVar i => var i
...
noncomputable def LNegStep.toPath : LNegStep n a b → Path a b
...
noncomputable def par_to_neg_tensor_path {n : Nat} (A B : LProp n) :
    Path (LProp.par A.neg B.neg) (LProp.neg (LProp.tensor A B)) :=
  Path.symm (tensor_demorgan_path A B)

-- 11. Reverse: tensor to neg-par
noncomputable def tensor_to_neg_par_path {n : Nat} (A B : LProp n) :
...
noncomputable def neg_tensor_chain {n : Nat} (A B : LProp n) :
...
noncomputable def demorgan_roundtrip {n : Nat} (A B : LProp n) :
...
noncomputable def PhaseStep.toPath : PhaseStep P a b → Path a b
...
noncomputable def PhaseSpace.orth (P : PhaseSpace) (S : P.carrier → Prop) : P.carrier → Prop :=
  fun a => ∀ b, S b → P.pole (P.mul a b)

/-- Double orthogonal closure. -/
noncomputable def PhaseSpace.biorth (P : PhaseSpace) (S : P.carrier → Prop) : P.carrier → Prop :=
  P.orth (P.orth S)
...
noncomputable def MLLProof.size {n : Nat} : MLLProof n → Nat
  | ax _ => 1
...
noncomputable def MLLProof.cutFree {n : Nat} : MLLProof n → Prop
  | ax _ => True
  | cut _ _ => False
...
noncomputable def proof_size_congrArg {n : Nat} {p₁ p₂ : MLLProof n} (h : Path p₁ p₂) :
    Path p₁.size p₂.size :=
  Path.congrArg MLLProof.size h

-- 35. Trans path for proof composition
noncomputable def proof_trans_path {n : Nat} {p₁ p₂ p₃ : MLLProof n}
    (h₁ : Path p₁ p₂) (h₂ : Path p₂ p₃) : Path p₁ p₃ :=
  Path.trans h₁ h₂

-- 36. Symm path for proof reversal
noncomputable def proof_symm_path {n : Nat} {p₁ p₂ : MLLProof n} (h : Path p₁ p₂) :
    Path p₂ p₁ :=
  Path.symm h
...
noncomputable def demorgan_then_neg {n : Nat} (A B : LProp n) :
...
noncomputable def tensorR_size_congrArg {n : Nat} {p₁ p₂ q : MLLProof n} (h : Path p₁ p₂) :
...
noncomputable def neg_neg_tensor_chain {n : Nat} (A B : LProp n) :
