...
inductive Step {A : Sort u} : {a b : A} → Path a b → Path a b → Prop
  | symm_refl (a : A) :
...
inductive Rw {A : Sort u} {a b : A} : Path a b → Path a b → Prop
  | refl (p : Path a b) : Rw p p
  | tail {p q r : Path a b} : Rw p q → Step (a := a) (b := b) q r → Rw p r

variable {A : Sort u} {a b : A}
...
inductive RwEq {A : Sort u} {a b : A} : Path a b → Path a b → Prop
