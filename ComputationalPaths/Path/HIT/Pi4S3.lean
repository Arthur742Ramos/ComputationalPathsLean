...
abbrev S3FourLoop : Type := Z2

/-- The trivial 4-loop (constant map). -/
def s3FourLoop_refl : S3FourLoop := Z2.zero

/-- The generator: the suspended Hopf map Ση : S⁴ → S³.

This is the suspension of η : S³ → S². It generates π₄(S³) ≃ ℤ/2ℤ. -/
def s3FourLoop_suspEta : S3FourLoop := Z2.one

/-- Composition of 4-loops. -/
def s3FourLoop_comp : S3FourLoop → S3FourLoop → S3FourLoop := Z2.add

/-- Inverse of a 4-loop. -/
def s3FourLoop_inv : S3FourLoop → S3FourLoop := id
...
def s3FourLoop_parity : S3FourLoop → Z2 := id

/-- Construct a 4-loop from its parity. -/
def s3FourLoop_of_parity : Z2 → S3FourLoop := id

/-- The generator Ση has parity 1. -/
theorem s3FourLoop_suspEta_parity : s3FourLoop_parity s3FourLoop_suspEta = Z2.one := rfl

/-- The trivial loop has parity 0. -/
theorem s3FourLoop_refl_parity : s3FourLoop_parity s3FourLoop_refl = Z2.zero := rfl

/-- Composition adds parities (mod 2). -/
theorem s3FourLoop_comp_parity (α β : S3FourLoop) :
...
abbrev SpherePiNPlus1 (n : Nat) (_hn : n ≥ 3) : Type := Z2

/-- π_{n+1}(Sⁿ) ≃ ℤ/2ℤ for n ≥ 3 (stable range). -/
noncomputable def sphereNPlus1_equiv_Z2 (n : Nat) (hn : n ≥ 3) :
    SimpleEquiv (SpherePiNPlus1 n hn) Z2 where
  toFun := id
  invFun := id
...
def sphereNPlus1_generator (n : Nat) (hn : n ≥ 3) : SpherePiNPlus1 n hn := Z2.one

/-- The generator maps to 1 ∈ ℤ/2ℤ. -/
theorem sphereNPlus1_generator_is_one (n : Nat) (hn : n ≥ 3) :
...
abbrev S2FourLoop : Type := Z2

/-- The trivial 4-loop in S². -/
def s2FourLoop_refl : S2FourLoop := Z2.zero

/-- The generator of π₄(S²): η ∘ Ση (η composed with suspended η).

This is sometimes written as η² in the literature. -/
def s2FourLoop_etaSquared : S2FourLoop := Z2.one

/-- The parity map for π₄(S²). -/
def s2FourLoop_parity : S2FourLoop → Z2 := id

/-- Round-trip axioms for π₄(S²). -/
def s2FourLoop_of_parity : Z2 → S2FourLoop := id
theorem s2FourLoop_parity_of_parity (p : Z2) :
    s2FourLoop_parity (s2FourLoop_of_parity p) = p
  := rfl
theorem s2FourLoop_eq_of_parity_eq (α β : S2FourLoop) :
...
def hopf_pi4_map : S3FourLoop → S2FourLoop := id

/-- The map is a group homomorphism. -/
theorem hopf_pi4_map_hom (α β : S3FourLoop) :
