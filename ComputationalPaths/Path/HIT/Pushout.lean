...
noncomputable def Pushout (A : Type u) (B : Type u) (C : Type u)
    (f : C → A) (g : C → B) : Type u :=
...
noncomputable def rec {D : Type v} (data : RecData (f := f) (g := g) D) :
    Pushout A B C f g → D :=
  Quot.lift
    (fun s : Sum A B =>
      match s with
...
noncomputable instance instSubsingleton_of_subsingleton_of_nonempty
...
noncomputable def Wedge (A : Type uu) (B : Type uu) (a₀ : A) (b₀ : B) : Type uu :=
...
noncomputable def Suspension (A : Type uu) : Type uu :=
...
noncomputable def IsPathConnected (A : Type u) : Prop :=
