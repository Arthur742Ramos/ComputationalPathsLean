/--
# π₅(S³) ≃ ℤ/2ℤ: The Hopf Square

This module proves π₅(S³) ≃ ℤ/2ℤ. The generator is η², the composition
of Hopf maps, often called the Hopf square.
...
We reuse the ℤ/2ℤ definition from Pi4S3.
-/
...
def s3FiveLoop_refl : S3FiveLoop := Z2.zero

/-- The generator: η² : S⁵ → S³.

This is the Hopf square, the composition of Hopf maps. -/
def s3FiveLoop_etaSquared : S3FiveLoop := Z2.one

/-- Composition of 5-loops. -/
def s3FiveLoop_comp : S3FiveLoop → S3FiveLoop → S3FiveLoop := Z2.add
...
theorem s3FiveLoop_etaSquared_parity :
    s3FiveLoop_parity s3FiveLoop_etaSquared = Z2.one := rfl

/-- The trivial loop has parity 0. -/
theorem s3FiveLoop_refl_parity : s3FiveLoop_parity s3FiveLoop_refl = Z2.zero := rfl

/-- Composition adds parities (mod 2). -/
theorem s3FiveLoop_comp_parity (α β : S3FiveLoop) :
    s3FiveLoop_parity (s3FiveLoop_comp α β) =
    Z2.add (s3FiveLoop_parity α) (s3FiveLoop_parity β)
  := rfl
...
theorem s3FiveLoop_eq_of_parity_eq (α β : S3FiveLoop) :
    s3FiveLoop_parity α = s3FiveLoop_parity β → α = β
  := fun h => h
