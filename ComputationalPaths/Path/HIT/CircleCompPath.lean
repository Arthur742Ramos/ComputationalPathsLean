...
@[simp] theorem circleCompPathEncodeExpr_pow (n : Nat) :
  ...
  | succ n ih =>
      simp [circleCompPathLoopExprPow, ih, Int.natCast_succ, Int.add_comm]
...
@[simp] theorem circleCompPathEncodeExpr_zpow (z : Int) :
  ...
  cases z with
  | ofNat n =>
      simpa [circleCompPathLoopExprZPow] using circleCompPathEncodeExpr_pow n
  | negSucc n =>
...
@[simp] def circleLoop : Path circleBase circleBase :=
