...
noncomputable def trans : EffPath A B → EffPath B C → EffPath A C
  | .nil, q => q
  | .cons s p, q => .cons s (trans p q)

noncomputable def symm : EffPath A B → EffPath B A
  | .nil => .nil
  | .cons s p => trans (symm p) (.cons (.symm s) .nil)

noncomputable def congrArg (f : EffTy → EffTy) : EffPath A B → EffPath (f A) (f B)
  | .nil => .nil
...
noncomputable def state_except_compose (s e : Nat) (α : Nat) :
...
noncomputable def row_swap_swap (s1 s2 : EffSig) (sigs : List EffSig) (α : Nat) :
    EffPath (EffTy.row (s1 :: s2 :: sigs) α) (EffTy.row (s1 :: s2 :: sigs) α) :=
  .cons .rowSwap (.cons .rowSwap .nil)

-- 42. Continuation: shift/reset coherence
noncomputable def cont_shift_reset_coherence (a : Nat) :
    EffPath (EffTy.eff (EffSig.cont a a) a) (EffTy.pure a) :=
  .cons .contReset .nil

-- 43. Deep congruence: exception inside state
noncomputable def deep_effect_congruence (s e α : Nat) :
    EffPath (EffTy.handled (EffSig.state s) (EffTy.handled (EffSig.except e) (EffTy.pure α)))
...
noncomputable def except_nested_flatten (e : Nat) (A : EffTy) :
    EffPath (EffTy.handled (EffSig.except e) (EffTy.handled (EffSig.except e) A))
            (EffTy.handled (EffSig.except e) A) :=
  .cons .exceptCatchCatch .nil

-- 49. Row swap under congruence
noncomputable def row_swap_congruence (s1 s2 : EffSig) (sigs : List EffSig) (α : Nat) (sig : EffSig) :
...
noncomputable def free_then_handle (sig : EffSig) (α : Nat) :
    EffPath (EffTy.eff sig α) (EffTy.free sig α) :=
  .cons .freeImpure .nil
