...
@[simp] noncomputable def ket0 : QState := ⟨1, 0⟩
@[simp] noncomputable def ket1 : QState := ⟨0, 1⟩
@[simp] noncomputable def ketPlus : QState := ⟨1, 1⟩
@[simp] noncomputable def ketMinus : QState := ⟨1, 1⟩

/-! ## Quantum gates as functions on QState -/

@[simp] noncomputable def pauliX (q : QState) : QState := ⟨q.ampOne, q.ampZero⟩
@[simp] noncomputable def pauliZ (q : QState) : QState := q
@[simp] noncomputable def gateId (q : QState) : QState := q
@[simp] noncomputable def hadamard (q : QState) : QState :=
  ⟨q.ampZero + q.ampOne, q.ampZero + q.ampOne⟩
@[simp] noncomputable def measure (q : QState) : QState :=
...
@[simp] noncomputable def tensor (a b : QState) : QState2 :=
  ⟨a.ampZero * b.ampZero, a.ampZero * b.ampOne,
   a.ampOne * b.ampZero, a.ampOne * b.ampOne⟩

@[simp] noncomputable def bellPhiPlus : QState2 := ⟨1, 0, 0, 1⟩

@[simp] noncomputable def cnot (q : QState2) : QState2 :=
  ⟨q.amp00, q.amp01, q.amp11, q.amp10⟩

@[simp] noncomputable def swap (q : QState2) : QState2 :=
  ⟨q.amp00, q.amp10, q.amp01, q.amp11⟩

@[simp] noncomputable def pauliX_first (q : QState2) : QState2 :=
  ⟨q.amp10, q.amp11, q.amp00, q.amp01⟩

@[simp] noncomputable def pauliX_second (q : QState2) : QState2 :=
  ⟨q.amp01, q.amp00, q.amp11, q.amp10⟩

@[simp] noncomputable def hadamard_first (q : QState2) : QState2 :=
  ⟨q.amp00 + q.amp10, q.amp01 + q.amp11,
   q.amp00 + q.amp10, q.amp01 + q.amp11⟩
...
noncomputable def QGateRewrite.toPath : QGateRewrite a b → Path a b
...
noncomputable def QGateRewrite2.toPath : QGateRewrite2 a b → Path a b
...
noncomputable def pauliX_involution_path (q : QState) : Path (pauliX (pauliX q)) q :=
...
noncomputable def gateId_path (q : QState) : Path (gateId q) q :=
  (QGateRewrite.gateId_id q).toPath

-- 4. PauliZ via rewrite step
noncomputable def pauliZ_path (q : QState) : Path (pauliZ q) q :=
  (QGateRewrite.pauliZ_id q).toPath

-- 5. PauliX ket0→ket1 via rewrite step
noncomputable def pauliX_ket0_path : Path (pauliX ket0) ket1 :=
  QGateRewrite.pauliX_ket0.toPath

-- 6. PauliX ket1→ket0 via rewrite step
noncomputable def pauliX_ket1_path : Path (pauliX ket1) ket0 :=
  QGateRewrite.pauliX_ket1.toPath

-- 7. Hadamard of ket0 via rewrite step
noncomputable def hadamard_ket0_path : Path (hadamard ket0) ketPlus :=
  QGateRewrite.hadamard_basis_0.toPath

-- 8. Measurement idempotence via rewrite step
noncomputable def measure_idem_path (q : QState) : Path (measure (measure q)) (measure q) :=
  (QGateRewrite.measure_idem q).toPath

-- 9. Measurement of basis states via rewrite steps
noncomputable def measure_ket0_path : Path (measure ket0) ket0 :=
  QGateRewrite.measure_basis_0.toPath

noncomputable def measure_ket1_path : Path (measure ket1) ket1 :=
  QGateRewrite.measure_basis_1.toPath

-- 10. CNOT involution via two-qubit rewrite step
noncomputable def cnot_involution_path (q : QState2) : Path (cnot (cnot q)) q :=
  (QGateRewrite2.cnot_cancel q).toPath

-- 11. SWAP involution via two-qubit rewrite step
noncomputable def swap_involution_path (q : QState2) : Path (swap (swap q)) q :=
  (QGateRewrite2.swap_cancel q).toPath

-- 12. pauliX_first involution
noncomputable def pauliX_first_involution_path (q : QState2) : Path (pauliX_first (pauliX_first q)) q :=
...
noncomputable def pauliX_quad_path (q : QState) : Path (pauliX (pauliX (pauliX (pauliX q)))) q :=
  Path.trans
...
noncomputable def pauliX_hadamard_ket0_path : Path (pauliX (hadamard ket0)) (pauliX ketPlus) :=
  Path.congrArg pauliX hadamard_ket0_path

-- 15. Chain: H|0⟩ then X then X cancels back
noncomputable def hadamard_pauliX_roundtrip : Path (pauliX (pauliX (hadamard ket0))) (hadamard ket0) :=
  Path.trans
...
noncomputable def pauliX_congrArg (p : Path a b) : Path (pauliX a) (pauliX b) :=
  Path.congrArg pauliX p

-- 17. CongrArg through cnot
noncomputable def cnot_congrArg {a b : QState2} (p : Path a b) : Path (cnot a) (cnot b) :=
  Path.congrArg cnot p

-- 18. CongrArg through measure
noncomputable def measure_congrArg (p : Path a b) : Path (measure a) (measure b) :=
  Path.congrArg measure p

-- 19. CongrArg through hadamard
noncomputable def hadamard_congrArg (p : Path a b) : Path (hadamard a) (hadamard b) :=
...
noncomputable def pauliX_comp_self_path : Path (pauliX ∘ pauliX) gateId :=
...
noncomputable def bell_creation_path : Path (cnot (hadamard_first ⟨1, 0, 0, 0⟩)) bellPhiPlus :=
...
@[simp] noncomputable def teleport_correct (measurement : Fin 4) (q : QState) : QState :=
...
noncomputable def teleport_id_path (q : QState) :
...
noncomputable def teleport_double_X_path (q : QState) :
...
noncomputable def teleport_double_Z_path (q : QState) :
...
noncomputable def isBasis (q : QState) : Prop := q = ket0 ∨ q = ket1
...
noncomputable def measure_pauliX_ket0_path : Path (measure (pauliX ket0)) ket1 :=
  Path.trans (measure_congrArg pauliX_ket0_path) measure_ket1_path

-- 30. Measure after pauliX on ket1
noncomputable def measure_pauliX_ket1_path : Path (measure (pauliX ket1)) ket0 :=
...
noncomputable def cnot_swap_path : Path (cnot (swap (cnot ⟨1, 0, 0, 0⟩))) (swap ⟨1, 0, 0, 0⟩) :=
...
noncomputable def pauliX_undo_path (q : QState) : Path q (pauliX (pauliX q)) :=
  Path.symm (pauliX_involution_path q)

noncomputable def cnot_undo_path (q : QState2) : Path q (cnot (cnot q)) :=
...
noncomputable def gate_comp_assoc_path (f g h : QState → QState) :
    Path ((f ∘ g) ∘ h) (f ∘ (g ∘ h)) :=
  Path.refl _
...
noncomputable def pauliX_pauliZ_comm_path (q : QState) :
...
noncomputable def hadamard_X_hadamard_path :
...
noncomputable def cnot_double_roundtrip (q : QState2) : Path q q :=
...
noncomputable def measure_triple_path (q : QState) :
