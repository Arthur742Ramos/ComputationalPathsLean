...
def trans : ProcPath a b → ProcPath b c → ProcPath a c
  | nil _, q => q
  | cons s p, q => cons s (trans p q)

def symm : ProcPath a b → ProcPath b a
  | nil _ => nil _
  | cons s p => trans (symm p) (cons (ProcStep.symm s) (nil _))

def congrArg (f : Proc → Proc) : ProcPath a b → ProcPath (f a) (f b)
  | nil _ => nil _
  | cons s p => cons (ProcStep.congrArg f s) (congrArg f p)

def single (s : ProcStep a b) : ProcPath a b := cons s (nil _)
...
def choice_comm_bisim (p q : Proc) :
...
def bisim_refines_failure (p : Proc) :
...
def trace_refines_failure (p : Proc) :
...
def sync_then_tau (n : Nat) (p q : Proc) :
    ProcPath (Proc.par (Proc.prefix (PAction.send n) p)
                        (Proc.prefix (PAction.recv n) q))
             (Proc.par p q) :=
...
def restrict_par_distrib (n : Nat) (p q : Proc) :
...
def bisim_par_congruence (p r : Proc) :
...
def scope_ext_then_comm (n ch v : Nat) (p q : Proc) :
...
def expand_applied (a b : PAction) (p q : Proc) :
...
def decompose_prime_round (p : Proc) :
