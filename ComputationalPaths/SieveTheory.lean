...
noncomputable instance : CoeFun Sieve (fun _ => Nat → Prop) where
  coe := Sieve.carrier

/-- The full sieve (all natural numbers). -/
noncomputable def univ : Sieve := ⟨fun _ => True⟩

/-- The empty sieve. -/
noncomputable def empty : Sieve := ⟨fun _ => False⟩

/-- Singleton sieve at a fixed natural number. -/
noncomputable def singleton (n : Nat) : Sieve := ⟨fun m => m = n⟩

/-- Union of two sieves. -/
noncomputable def union (S T : Sieve) : Sieve := ⟨fun n => S n ∨ T n⟩

/-- Intersection of two sieves. -/
noncomputable def inter (S T : Sieve) : Sieve := ⟨fun n => S n ∧ T n⟩

/-- Complement of a sieve. -/
noncomputable def compl (S : Sieve) : Sieve := ⟨fun n => ¬ S n⟩

/-- Predicate for multiples of a modulus. -/
noncomputable def isMultiple (p n : Nat) : Prop := ∃ k, n = p * k

/-- Remove multiples of `p` from a sieve. -/
noncomputable def removeMultiples (p : Nat) (S : Sieve) : Sieve :=
...
noncomputable def eval : SieveStep → Sieve
  | base S => S
...
noncomputable def eratosthenesSteps (primes : List Nat) (base : Sieve) : SieveStep :=
...
noncomputable def eratosthenesSieve (primes : List Nat) (base : Sieve) : Sieve :=
...
noncomputable def sift (S : CombinatorialSieve) (step : SieveStep) : Sieve :=
  Sieve.inter S.universeSieve (SieveStep.eval step)

/-- Formal weight of a sieve step using the attached weights. -/
noncomputable def weightOf (S : CombinatorialSieve) (step : SieveStep) : Int :=
...
noncomputable def inclusion_exclusion_path (w : SieveWeight) (s t : SieveStep) :
...
noncomputable def inclusion_exclusion_path_symm (w : SieveWeight) (s t : SieveStep) :
