...
- Canonical normal forms are now unique: every path rewrites to `ofEq (toEq p)`, and the rewrite relation is confluent, so any two reductions from the same origin meet again at that canonical witness.  This matches the paper's discussion of canonical proofs while staying constructively computational.
- Definition 3.5's contextual substitution rules (`sub L`/`sub R`) are available through the new `Context`/`BiContext` abstractions (`ComputationalPaths/Path/Basic/Context.lean`).  These package unary and binary holes so that plugging a computational path through any context automatically produces the induced path, together with proofs that the operation respects `refl`, `symm`, and `trans`.
...
- `ComputationalPaths/Path/Basic/Globular.lean` - iterated identity tower yielding the globular set generated by computational paths, closing it under `refl`/`symm`/`trans` dimension-wise, and providing `toPath` to recover the underlying computational witness.
- `ComputationalPaths/Path/Basic/Context.lean` - Definition 3.5 contexts with unary/binary substitution operators, capturing the labelled deduction `sub L`/`sub R` rules.
