...
- Extend the new normalization theorems to the remaining LNDEQ rules (notably the full context/`map2` hierarchy) and push them deeper into downstream constructions (transport, functors, higher coherences) so that every operation on `PathRwQuot` automatically exposes its canonical witness.
