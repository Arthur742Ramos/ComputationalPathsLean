...
This is the small statement surface for the accepted paper
*Computational Paths Form a Weak omega-Groupoid: A Constructive Proof*.
The definitions deliberately expose the paper's three ingredients:

* a path carries an equality proof and an observable rewrite trace;
* 2-cells are proof-relevant symmetric rewrite derivations; and
* 3-cells are generated by equality of the mere propositions saying that two
  paths are rewrite-related.

The selected results formalize the paper's proof-irrelevance contraction and
its explicit unit, inverse, pentagon, triangle, interchange, and
Eckmann--Hilton witnesses.  The higher tail is represented by the same
proof-irrelevance construction for an arbitrary cell carrier.  This boundary
does not claim to formalize every application or every axiom schema of a
general Batanin--Leinster definition.
-/
...
def contractibility3 {A : Type u} {a b : A} {p q : Path A a b}
    (d e : RwEq p q) : Derivation3 d e := by sorry

def contractibilityHigher {T : Type u} (x y : T) : HigherCell x y := by sorry

theorem pentagon_route_counts {A : Type u} {a b c d e : A}
    (f : Path A a b) (g : Path A b c) (h : Path A c d) (k : Path A d e) :
    RwEq.stepCount (pentagonRight f g h k) = 2 ∧
      RwEq.stepCount (pentagonLeft f g h k) = 3 := by sorry

def pentagon_coherence {A : Type u} {a b c d e : A}
    (f : Path A a b) (g : Path A b c) (h : Path A c d) (k : Path A d e) :
    Derivation3 (pentagonRight f g h k) (pentagonLeft f g h k) := by sorry
...
def interchange_coherence {A : Type u} {a b c : A}
    ...
    Derivation3 (hcomp α β) (hcompAlt α β) := by sorry
...
theorem computational_paths_form_weak_omega_groupoid (A : Type u) :
    Nonempty (WeakOmegaGroupoidCertificate A) := by sorry
